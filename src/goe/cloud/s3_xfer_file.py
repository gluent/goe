...
S3_RETRIABLE_ERRORS = (socket.timeout, ReadTimeoutError, IncompleteReadError)
...
class S3XferFile(XferFile):
    ...
    def _get_stats(self):
        ...
        try:
            ...
            response = self._client.head_object(
                Bucket=self.bucket, Key=self.prefix, **kwargs
            )
        except ClientError as e:
            logger.debug(
                "Exception: %s on HEAD response from %s. Assuming, object does not exist"
                % (e, self)
            )
            return {}
        ...
        if not response:
            logger.debug(
                "No HEAD response from %s. Assuming, object does not exist" % self
            )
            return {}
        if response.get("DeleteMarker", False):
            logger.debug(
                "Object: %s is deleted. Treating as though it does not exist" % self
            )
            return {}
        ...
        stats = {
            k: cast_to_proper_type(k, metadata.get(k, None))
            for k in metadata
            if k in S3_METADATA_KEYS
        }
    ...
    def _get_extra_s3_options(self, restrict_options=None):
        ...
        for opt in options:
            if ("*" != SUPPORTED_S3_OPTION_VALUES[opt]) and (
                options[opt] not in SUPPORTED_S3_OPTION_VALUES[opt]
            ):
                raise XferFileException(
                    "Value: %s is invalid for option: %s" % (options[opt], opt)
                )
    ...
    def copy_s3(self, dst_s3):
        args = {
            ...
            "CopySource": {"Bucket": self.bucket, "Key": self.prefix},
        }
    ...
    def copy(self, other, force=False):
        ...
        if DST_HDFS == other.filetype:
            ...
            raise NotImplementedError(
                "Copy from S3 to: %s is not supported yet" % other.filetype
            )
