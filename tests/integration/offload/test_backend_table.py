    ...
    def _run_call_sql_expression_from_sql(
        ...
        if db and table:
            ...
            sql = "SELECT %s, %s FROM %s%s LIMIT 1" % (
                ...
                self.test_api.enclose_object_reference(self.db, self.table),
                where_clause,
            )
        else:
        ...
        if self.api.synthetic_partitioning_supported():
            ...
            if date_columns:
                for granularity, expected_length in [("Y", 4), ("M", 7), ("D", 10)]:
                    ...
                    cast = self.api._gen_synthetic_part_date_as_string_sql_expr(
                        date_columns[0].name,
                        granularity,
    ...
    def _test__gen_synthetic_part_number_sql_expr(self):
        ...
        for granularity, digits, expected_trailing_zeros in [
            ...
            if self.api.synthetic_partition_numbers_are_string():
                ...
                self.assertEqual(
                    synthetic_value[-expected_trailing_zeros:],
                    "0" * expected_trailing_zeros,
                )
                # Check that the value from SQL matches output of our Python logic
                python_value = SyntheticPartitionLiteral._gen_number_string_literal(
                    raw_column_value, granularity, digits
                )
            else:
