...
# EXCEPTIONS
###############################################################################
class RetryLogicException(Exception): pass
            ...
            for i, delay in enumerate(retry_generator):
                ...
                try:
                    ...
                    if any(isinstance(e, _) for _ in retriable_exceptions):
                        logger.debug("Try: %d raised a re-triable EXCEPTION: %s" % (i, e), exc_info=True)
                    else:
                        logger.debug("Try: %d raised a critical EXCEPTION: %s" % (i, e), exc_info=True)
                        raise
...
if __name__ == "__main__":
    ...
    class b:
        def __init__(self):
            setattr(self, 'f', retry_call_base(exponential_backoff(3))(getattr(self, 'f')))
        def f(self, p):
            return a(p)
