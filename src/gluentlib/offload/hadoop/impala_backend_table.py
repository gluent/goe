...
from gluentlib.offload.column_metadata import get_column_names, valid_column_list
from gluentlib.offload.hadoop.hadoop_backend_table import BackendHadoopTable
from gluentlib.offload.hadoop.hadoop_column import HADOOP_TYPE_BIGINT, HADOOP_TYPE_INT, HADOOP_TYPE_SMALLINT,\
    HADOOP_TYPE_TINYINT
from gluentlib.offload.offload_constants import FILE_STORAGE_FORMAT_PARQUET, FILE_STORAGE_FORMAT_PARQUET_IMPALA

###############################################################################
# CONSTANTS
###############################################################################

FNV_HASH_BUCKET_EXPRESSION = 'CAST(COALESCE(PMOD(FNV_HASH(%(dividend)s),%(divisor)s),0) AS SMALLINT)'
...
class BackendImpalaTable(BackendHadoopTable):
    ...
    def __init__(self, db_name, table_name, backend_type, orchestration_options, messages, orchestration_operation=None,
        ...
        """ CONSTRUCTOR
        """
        super(BackendImpalaTable, self).__init__(db_name, table_name, backend_type, orchestration_options, messages,
    ...
    def _create_new_backend_table(self, sort_column_names=None):
        ...
        return self._db_api.create_table(self.db_name, self.table_name, self.get_columns(), partition_column_names,
                                         storage_format=storage_format, location=location, external=external_table,
                                         table_properties=table_properties, sort_column_names=sort_column_names, sync=True)

    def _compute_impala_table_statistics(self, incremental_stats):
        self._db_api.compute_stats(self.db_name, self._base_table_name, incremental=incremental_stats)
    ...
    def _final_insert_format_sql(self, select_expr_tuples, filter_clauses, sort_expr_list, for_materialized_join=False):
        ...
        partition_expr_tuples = [(self.get_final_table_cast(_), _) for _ in get_column_names(self.get_partition_columns())]
        ...
        return self._db_api.gen_insert_select_sql_text(self.db_name, self._base_table_name,
                                                       ...
                                                       filter_clauses=filter_clauses, insert_hint=impala_hint)

    def _final_insert_query_options(self):
        """ Returns a dict of Impala query options to be executed before the final insert """
        query_options = {}
        if self._user_requested_storage_format == FILE_STORAGE_FORMAT_PARQUET and self._user_requested_storage_compression:
    ...
    def _gen_synthetic_part_number_granularity_sql_expr(self, column_expr, backend_col, granularity,
        ...
        if backend_col.data_type in (HADOOP_TYPE_BIGINT, HADOOP_TYPE_INT, HADOOP_TYPE_SMALLINT, HADOOP_TYPE_TINYINT):
