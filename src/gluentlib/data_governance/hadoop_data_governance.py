...
from gluentlib.data_governance.cloudera_navigator import ClouderaNavigator, ClouderaNavigatorNoMetadataException
...
def is_valid_data_governance_dynamic_token(tag, list_of_valid_tokens, custom_strings_allowed=False):
    if tag is None:
        return False
    if type(tag) is not str:
        return False
    if is_dynamic_tag(tag) and stripped_dynamic_tag_name(tag) not in list_of_valid_tokens:
        return False
...
def filter_tags_by_gluent_object_type(tag_list, gluent_object_type, messages):
    ...
    filtered_tags = [_ for _ in tag_list if not is_dynamic_tag(_) or stripped_dynamic_tag_name(_) not in mute_list]
    if tag_list != filtered_tags:
        messages.debug('Tags filtered by object type to: %s' % filtered_tags, detail=SUPPRESS_STDOUT)
...
def expand_dynamic_data_governance_tag(tag, data_gov_client):
    ...
    if is_dynamic_tag(tag):
        ...
        if dyn_tag == DATA_GOVERNANCE_DYNAMIC_TAG_RDBMS_NAME:
            assert data_gov_client.rdbms_name, 'Rdbms name must be cached for dynamic tag'
...
def filter_properties_by_gluent_object_type(prop_dict, gluent_object_type, messages=None):
    ...
    filtered_props = {_: prop_dict[_] for _ in list(prop_dict.keys()) if _ not in mute_list}
    if list(prop_dict.keys()) != list(filtered_props.keys()) and messages:
        messages.debug('Properties filtered by object type to: %s' % list(filtered_props.keys()), detail=SUPPRESS_STDOUT)
...
def expand_data_governance_props(data_gov_client, messages, gluent_object_type=None):
    ...
    if data_gov_client.custom_properties:
        if type(data_gov_client.custom_properties) is dict:
            ...
            try:
                data_gov_custom_prop_dict = json.loads(data_gov_client.custom_properties)
            except ValueError as ve:
                messages.log(traceback.format_exc(), detail=VVERBOSE)
                raise Exception('Invalid JSON value for custom data governance properties: %s' % str(ve))
...
def data_governance_register_new_object(hadoop_db, object_name, data_gov_client, messages, gluent_object_type, renaming_from_db=None, renaming_from_object_name=None, dg_object_type=None):
    ...
    if dg_object_type == DATA_GOVERNANCE_OBJECT_TYPE_TABLE:
        dg_metadata = data_gov_client.get_hive_table_metadata(hadoop_db, object_name, ignore_missing_metadata=True)
    elif dg_object_type == DATA_GOVERNANCE_OBJECT_TYPE_VIEW:
        dg_metadata = data_gov_client.get_hive_view_metadata(hadoop_db, object_name, ignore_missing_metadata=True)
    else:
        dg_metadata = data_gov_client.get_hive_object_metadata(hadoop_db, object_name, ignore_missing_metadata=True)

    messages.log('Registering data governance metadata: %s' % list(custom_metadata.keys()), detail=VERBOSE)
    if dg_metadata:
        ...
        messages.warning('New object %s.%s already has data governance information, metadata will be merged' % (hadoop_db, object_name))
        metadata_id = data_gov_client.get_metadata_id_from_full_metadata(dg_metadata)
        _ = data_gov_client.update_custom_metadata(metadata_id, custom_metadata, reset=True, renaming_from_db=renaming_from_db, renaming_from_object_name=renaming_from_object_name)
    else:
        metadata_id = data_gov_client.register_custom_object_metadata(hadoop_db, object_name, custom_metadata, renaming_from_db=renaming_from_db, renaming_from_object_name=renaming_from_object_name)
...
def data_governance_update_metadata(hadoop_db, object_name, data_gov_client, messages):
    ...
    metadata_changes = data_gov_client.new_custom_metadata(tags=data_gov_tags, properties=data_gov_props)
    try:
        dg_metadata = data_gov_client.get_hive_object_metadata(hadoop_db, object_name)
        metadata_id = data_gov_client.get_metadata_id_from_full_metadata(dg_metadata)
        messages.log('Registering data governance metadata: %s' % list(metadata_changes.keys()), detail=VERBOSE)
        updated_metadata = data_gov_client.update_custom_metadata(metadata_id, metadata_changes)
        if updated_metadata:
        ...
        messages.warning('Missing data governance metadata for %s.%s, registering fresh metadata' % (hadoop_db, object_name))
...
def data_governance_register_new_db(hadoop_db, gluent_object_type, data_gov_client, messages):
    ...
    if dg_metadata:
        # this "new" object already has metadata, data_gov_client.update_custom_metadata() will merge with it just to be sure we don't do any damage
        # the reset means we'll overwrite "INITIAL_" properties
        messages.warning('DB %s already has data governance information, metadata will be merged' % hadoop_db)
        metadata_id = data_gov_client.get_metadata_id_from_full_metadata(dg_metadata)
        _ = data_gov_client.update_custom_metadata(metadata_id, custom_metadata)
    else:
        metadata_id = data_gov_client.register_custom_db_metadata(hadoop_db, custom_metadata)
    messages.log('Metadata id: %s' % metadata_id, detail=VVERBOSE)
...
def data_governance_register_new_object_step(hadoop_db, object_name, data_gov_client, messages, gluent_object_type, options=None, renaming_from_db=None, renaming_from_object_name=None, dg_object_type=None):
  ...
  if data_gov_client:
    def step_fn():
      data_governance_register_new_object(hadoop_db,
                                          object_name,
                                          data_gov_client,
                                          messages,
                                          gluent_object_type,
...
def data_governance_update_metadata_step(hadoop_db, hadoop_object_name, data_gov_client, messages, options=None):
    opts_execute = options.execute if options else True
    if data_gov_client:
        def step_fn():
            data_governance_update_metadata(hadoop_db, hadoop_object_name, data_gov_client, messages)
        return messages.offload_step(command_steps.STEP_REGISTER_DATA_GOV, step_fn,
                                     execute=opts_execute, optional=True)
...
if __name__ == "__main__":
    ...
    class SomeOpts(object):
        def __init__(self):
            self.data_governance_api_url = 'http://ec2-54-162-125-123.compute-1.amazonaws.com:7187/api'
