...
class FrontendApiInterface(metaclass=ABCMeta):
    ...
    def __init__(self, connection_options, frontend_type, messages, conn_user_override=None,
    ...
    @abstractmethod
    def _create_table_sql_text(self, schema, table_name, column_list, partition_column_names,
    ...
    @abstractmethod
    def _execute_ddl_or_dml(self, sql, query_options=None, log_level=VERBOSE, trace_action=None, query_params=None):
    ...
    def _execute_query_fetch_x(self, sql, fetch_action=FETCH_ACTION_ALL, query_options=None, log_level=None,
                               time_sql=False, not_when_dry_running=False, query_params=None, profile=False,
                               trace_action=None, as_dict=False):
    ...
    @abstractmethod
    def _executemany_dml(self, sql, query_options=None, log_level=VERBOSE, trace_action=None, query_params=None,
                         batch_size=None):
    ...
    @abstractmethod
    def _fast_executemany_dml(self, sql, query_options=None, log_level=VERBOSE, trace_action=None, query_params=None,
                              batch_size=None, param_inputsizes=None):
    ...
    def _create_table_columns_clause_common(self, column_list):
        ...
        return '    \n,   '.join(['%-32s %-20s' % (self.enclose_identifier(_.name), _.format_data_type())
    ...
    @abstractmethod
    def execute_function(self, sql_fn, return_type=None, return_type_name=None, arg_list=None, log_level=VERBOSE,
    ...
    @abstractmethod
    def get_object_ddl(self, schema, object_name, object_type, as_list=False, terminate_sql=False, remap_schema=None):
    ...
    @abstractmethod
    def get_table_row_count(self, schema, table_name, filter_clause=None, filter_clause_params=None, hint_block=None,
    ...
    def create_table(self, schema, table_name, column_list, partition_column_names=None,
                     table_properties=None, log_level=VERBOSE) -> list:
        ...
        sql = self._create_table_sql_text(schema, table_name, column_list, partition_column_names,
    ...
    def enclose_object_reference(self, schema, object_name):
        ...
        """
        return '.'.join([self.enclose_identifier(schema),
    ...
    def execute_ddl(self, sql, query_options=None, log_level=VERBOSE, trace_action=None) -> list:
        ...
        return_sqls = self._execute_ddl_or_dml(sql, query_options=query_options, log_level=log_level,
                                               trace_action=trace_action)
        assert return_sqls is not None
        return return_sqls

    def execute_dml(self, sql, query_options=None, log_level=VERBOSE, trace_action=None, query_params=None) -> list:
        return_sqls = self._execute_ddl_or_dml(sql, query_options=query_options, log_level=log_level,
                                               trace_action=trace_action, query_params=query_params)
        assert return_sqls is not None
        return return_sqls

    def execute_query_fetch_all(self, sql, query_options=None, log_level=None, time_sql=False,
                                not_when_dry_running=False, query_params=None, profile=False, trace_action=None,
                                as_dict=False) -> list:
        """ For parameter descriptions see: _execute_query_fetch_x """
        rows = self._execute_query_fetch_x(
            sql, fetch_action=FETCH_ACTION_ALL, query_options=query_options, log_level=log_level,
            time_sql=time_sql, not_when_dry_running=not_when_dry_running, query_params=query_params,
            profile=profile, trace_action=trace_action, as_dict=as_dict
        )
        return rows
    ...
    def execute_query_fetch_one(self, sql, query_options=None, log_level=None, time_sql=False,
        ...
        row = self._execute_query_fetch_x(
            sql, fetch_action=FETCH_ACTION_ONE, query_options=query_options, log_level=log_level,
            time_sql=time_sql, not_when_dry_running=not_when_dry_running, query_params=query_params,
            profile=profile, trace_action=trace_action, as_dict=as_dict
        )
        return row
    ...
    def execute_query_get_cursor(self, sql, query_options=None, log_level=None, time_sql=False,
                                 not_when_dry_running=False, query_params=None, trace_action=None):
        ...
        return self._execute_query_fetch_x(
            sql, fetch_action=FETCH_ACTION_CURSOR, query_options=query_options, log_level=log_level,
            time_sql=time_sql, not_when_dry_running=not_when_dry_running, query_params=query_params,
            trace_action=trace_action
        )

    def executemany_dml(self, sql, query_options=None, log_level=VERBOSE, trace_action=None, query_params=None,
                        batch_size=None) -> list:
        """ Run SQL that has no result set with multiple input records.
        ...
        return_sqls = self._executemany_dml(sql, query_options=query_options, log_level=log_level,
                                            trace_action=trace_action, query_params=query_params,
                                            batch_size=batch_size)
        assert return_sqls is not None
        return return_sqls

    def fast_executemany_dml(self, sql, query_options=None, log_level=VERBOSE, trace_action=None, query_params=None,
                             batch_size=None, param_inputsizes=None) -> list:
        """ Run SQL that has no result set with multiple input records using the fast_executemany option.
            For executemany we need query_params to be populated and a list of lists.
            sql is a single SQL statement. We trust the developer to add parameter markers matching query_params.
        ...
        return_sqls = self._fast_executemany_dml(sql, query_options=query_options, log_level=log_level,
                                                 trace_action=trace_action, query_params=query_params,
                                                 batch_size=batch_size, param_inputsizes=param_inputsizes)
        assert return_sqls is not None
        return return_sqls
    ...
    def get_table_ddl(self, schema, table_name, as_list=False, terminate_sql=False, remap_schema=None):
        """ Return CREATE TABLE DDL as a string (or a list of strings split on CR if as_list=True)
            ...
            remap_schema: Replaces the actual schema (schema) with a different name (remap_schema)
        """
        return self.get_object_ddl(schema, table_name, GET_DDL_TYPE_TABLE, as_list=as_list,
                                   terminate_sql=terminate_sql, remap_schema=remap_schema)

    def get_view_ddl(self, schema, view_name, as_list=False, terminate_sql=False, remap_schema=None):
        """ Return CREATE VIEW DDL as a string (or a list of strings split on CR if as_list=True)
        """
        return self.get_object_ddl(schema, view_name, GET_DDL_TYPE_VIEW, as_list=as_list,
                                   terminate_sql=terminate_sql, remap_schema=remap_schema)
