...
from gluentlib.config.orchestration_defaults import backend_distribution_default
...
from gluentlib.offload.offload_constants import BACKEND_DISTRO_MAPR
from gluentlib.util.misc_functions import is_number
...
class CliHdfs(GluentDfs):
    ...
    def __init__(self, hdfs_host, ssh_user, tmp_dir='/tmp', dry_run=False, messages=None, do_not_connect=False,
                 db_path_suffix=None, hdfs_data=None):
    ...
    def stat(self, dfs_path):
        ...
        if returncode == 0:
            ...
            if tokens:
                ...
                return {'permission': self.convert_rwx_perms_to_oct_str(tokens[0]), 'type': file_type,
                        'length': file_length}
        else:
    ...
    def copy_from_local(self, local_path, dfs_path, overwrite=False):
        ...
        try:
            ...
            rm_cmd = self._ssh(self._ssh_user, self._hdfs_host) + ['rm', '-f', hadoop_temp_file]
    ...
    def list_snapshottable_dirs(self):
        ...
        cmd = self._ssh(self._ssh_user, self._hdfs_host) + ['hdfs', 'lsSnapshottableDir']
        returncode, output = self._run_cmd(cmd, force_run=True)
        self._check_returncode(returncode, output)
        return [_[_.find(b'/'):].strip().decode() for _ in output if self._valid_ls_line(_)]
