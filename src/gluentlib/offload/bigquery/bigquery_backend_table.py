...
from gluentlib.data_governance.hadoop_data_governance_constants import DATA_GOVERNANCE_GLUENT_OBJECT_TYPE_LOAD_TABLE
...
from gluentlib.offload.backend_table import BackendTableInterface,\
    PARTITION_KEY_OUT_OF_RANGE, TYPICAL_DATE_GRANULARITY_TERMS
from gluentlib.offload.offload_messages import VERBOSE, VVERBOSE
from gluentlib.offload.staging.avro.avro_staging_file import AVRO_TYPE_DOUBLE, AVRO_TYPE_LONG
from gluentlib.offload.staging.parquet.parquet_column import PARQUET_TYPE_DOUBLE, PARQUET_TYPE_INT64
...
class BackendBigQueryTable(BackendTableInterface):
    ...
    def _final_insert_query_options(self):
        if self._kms_key_name:
            return {'destination_encryption_configuration': bigquery.EncryptionConfiguration(kms_key_name=self._kms_key_name)}
        else:
            return None
    ...
    def _gen_synthetic_partition_column_object(self, synthetic_name, canonical_column):
        ...
        if canonical_column.is_date_based():
            return self._db_api.gen_column_object(synthetic_name, data_type=BIGQUERY_TYPE_DATE,
                                                  partition_info=canonical_column.partition_info)
        elif canonical_column.is_number_based():
            return self._db_api.gen_column_object(synthetic_name, data_type=BIGQUERY_TYPE_INT64,
                                                  partition_info=canonical_column.partition_info)
        else:
            raise NotImplementedError('BigQuery synthetic partitioning not implemented for column type: %s'
    ...
    def _gen_synthetic_partition_date_as_string_sql_expr(self, extract_name, pad_size, source_column_cast):
        ...
        """
        return "LPAD(CAST(EXTRACT(%s FROM %s) AS STRING), %s, '0')" % (extract_name, source_column_cast, pad_size)
    ...
    def _gen_synthetic_partition_date_truncated_sql_expr(self, synthetic_name, column_name, granularity, source_column_cast):
        ...
        sql_trunc = 'SAFE.%s_TRUNC(%s, %s)' % (backend_column.data_type, source_column_cast,
    ...
    def _gen_synthetic_part_number_granularity_sql_expr(self, column_expr, backend_col, granularity,
        ...
        to_synth_expr = 'CAST(FLOOR(%s / %s) * %s AS %s)'\
                        % (to_synth_expr, granularity, granularity, BIGQUERY_TYPE_INT64)
    ...
    def _staging_to_backend_cast(self, rdbms_column, backend_column, staging_column) -> tuple:
        ...
        def date_to_string_format(expr, has_time_element):
            if has_time_element:
                ...
                return "FORMAT_DATETIME('%E4Y-%m-%d %H:%M:%E6S', PARSE_DATETIME('%F %H:%M:%E*S', {col}))".format(col=expr)
            else:
                return "FORMAT_DATE('%E4Y-%m-%d', PARSE_DATE('%F', {col}))".format(col=expr)

        def staging_file_int64_match(backend_column, staging_column):
            return bool(backend_column.data_type == BIGQUERY_TYPE_INT64
        ...
        def staging_file_float64_match(backend_column, staging_column):
            return bool(backend_column.data_type == BIGQUERY_TYPE_FLOAT64
        ...
        if isinstance(staging_column, BigQueryColumn) and staging_column.data_type == backend_column.data_type:
            ...
            if (staging_file_int64_match(backend_column, staging_column)
            ...
            if backend_column.is_string_based():
                ...
                ELSE
                    {date_format}
                END""".format(col=self._format_staging_column_name(staging_column),
                              date_format=date_to_string_format(self._format_staging_column_name(staging_column),
    ...
    def create_backend_table(self):
        ...
        cmds = self._db_api.create_table(self.db_name, self.table_name, self.get_columns(),
                                         partition_column_names, sort_column_names=self._sort_columns)
        if not self._dry_run:
            self._drop_state()
        return cmds

    def create_db(self):
        return self._create_final_db(location=self._orchestration_config.bigquery_dataset_location)
