...
class GluentAzure(GluentDfs):
    ...
    def __init__(self, azure_account_name, azure_account_key, azure_account_domain, messages, dry_run=False,
                 do_not_connect=False, db_path_suffix=None):
        assert azure_account_name
    ...
    def delete(self, dfs_path, recursive=False):
        ...
        def pragmatic_delete(container, path):
            ...
            try:
                ...
                self.debug('delete_blob(%s, %s) returned 404 (AzureMissingResourceHttpError), accepting as deleted'
                           % (container, path))
