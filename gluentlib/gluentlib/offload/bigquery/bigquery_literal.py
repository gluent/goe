...
from gluentlib.offload.bigquery.bigquery_column import BIGQUERY_TYPE_BIGNUMERIC, BIGQUERY_TYPE_BYTES,\
    BIGQUERY_TYPE_DATE, BIGQUERY_TYPE_DATETIME, BIGQUERY_TYPE_NUMERIC, BIGQUERY_TYPE_TIME, BIGQUERY_TYPE_TIMESTAMP
    ...
    def format_literal(cls, python_value, data_type=None):
        ...
        if isinstance(python_value, datetime64):
            ...
            if data_type == BIGQUERY_TYPE_DATE:
                ...
                new_py_val = cls._format_data_type_with_prefix(python_value.strftime("%Y-%m-%d %H:%M:%S.%f%z"),
                                                               data_type)
            elif data_type == BIGQUERY_TYPE_TIME:
