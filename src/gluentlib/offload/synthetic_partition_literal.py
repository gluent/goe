...
from gluentlib.offload.column_metadata import ColumnMetadataInterface, match_table_column
from gluentlib.offload.offload_constants import PART_COL_DATE_GRANULARITIES, PART_COL_GRANULARITY_MONTH,\
    PART_COL_GRANULARITY_YEAR
from gluentlib.util.misc_functions import MAX_SUPPORTED_PRECISION


class SyntheticPartitionLiteralException(Exception): pass
...
class SyntheticPartitionLiteral(object):
    ...
    def _gen_date_as_string_literal(date_value, granularity):
        ...
        assert re.match('^\d{4}-\d\d?-\d\d?$', date_string)
    ...
    def _gen_date_truncated_literal(date_value, granularity):
        ...
        if isinstance(date_value, date):
            ...
            date_truncated = date_value.astype('datetime64[{}]'.format(granularity)).astype('datetime64[D]')
            return date_truncated
    ...
    def _gen_synthetic_literal_function(partition_column, source_column):
        ...
        if source_column.name.upper() != (source_column_name or '').upper():
            raise SyntheticPartitionLiteralException('Source column mismatch: %s != %s'
