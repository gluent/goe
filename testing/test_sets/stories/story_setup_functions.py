...
from test_sets.stories.story_globals import STORY_SETUP_TYPE_HYBRID, STORY_SETUP_TYPE_PYTHON
...
def dbms_stats_gather_string(schema, table, frontend_api=None) -> str:
    ...
    if not frontend_api:
        return "BEGIN DBMS_STATS.GATHER_TABLE_STATS('%s','%s'); END;" % (schema.upper(), table.upper())
...
def dbms_stats_delete_string(schema, table, frontend_api=None):
    # frontend_api optional to ease implementation
    if not frontend_api:
        return "BEGIN DBMS_STATS.DELETE_TABLE_STATS('%s','%s'); END;" % (schema.upper(), table.upper())
    return frontend_api.remove_table_stats_sql_text(schema.upper(), table.upper())


def gen_drop_fap_ddl(options, frontend_api, hybrid_schema, rule_name, max_hybrid_length_override=None):
    if not options:
        return []
    max_table_name_length = max_hybrid_length_override or frontend_api.max_table_name_length()
    trunc_rule = trunc_with_hash(rule_name, options.hash_chars, max_table_name_length).upper()
    params = {'schema': hybrid_schema,
              'rule': trunc_rule,
              'ext': trunc_with_hash(('%s_ext' % trunc_rule).upper(), options.hash_chars,
                                     max_table_name_length).upper()}
    return ['DROP TABLE %(schema)s.%(ext)s' % params,
            'DROP VIEW %(schema)s.%(rule)s' % params,
            """BEGIN SYS.DBMS_ADVANCED_REWRITE.DROP_REWRITE_EQUIVALENCE('%(schema)s.%(rule)s'); END;""" % params]


def gen_hybrid_drop_ddl(options, frontend_api, hybrid_schema, drop_name, drop_aggs=True, drop_incr=False,
                        drop_rewrite=False, max_hybrid_length_override=None) -> list:
    ...
    for table_name in drop_name:
        ...
        params = {'schema': hybrid_schema,
                  'table': table_name,
                  'uv': trunc_with_hash(('%s_uv' % table_name).upper(), options.hash_chars, max_table_name_length).upper(),
                  'log': trunc_with_hash(('%s_log' % table_name).upper(), options.hash_chars, max_table_name_length).upper(),
                  'ext': trunc_with_hash(('%s_ext' % table_name).upper(), options.hash_chars, max_table_name_length).upper()}
        if drop_rewrite:
            dar_ddl = ["""BEGIN SYS.DBMS_ADVANCED_REWRITE.DROP_REWRITE_EQUIVALENCE('%(schema)s.%(table)s'); END;""" % params]
        if drop_incr:
            incr_ddl = ['DROP TABLE %(schema)s.%(log)s PURGE' % params,
                        'DROP TRIGGER %(schema)s.%(table)s' % params,
                        'DROP TRIGGER %(schema)s.%(uv)s' % params,
                        'DROP VIEW %(schema)s.%(uv)s' % params]
        if drop_aggs:
            agg_ddl = gen_drop_fap_ddl(options, frontend_api, hybrid_schema, table_name.upper() + '_agg',
                                       max_hybrid_length_override=max_table_name_length) +\
                      gen_drop_fap_ddl(options, frontend_api, hybrid_schema, table_name.upper() + '_cnt_agg',
                                       max_hybrid_length_override=max_table_name_length)
        all_ddl.extend(['DROP TABLE %(schema)s.%(ext)s' % params,
                        'DROP VIEW %(schema)s.%(table)s' % params] + dar_ddl + agg_ddl + incr_ddl)
    return all_ddl


def gen_rdbms_dim_create_ddl(frontend_api, schema, table_name, source='products', pk_col=None,
                             degree=None, filter_clause=None, extra_col_tuples=None) -> list:
    if not frontend_api:
        return []
    if extra_col_tuples:
    ...
    params = {'schema': schema, 'source': source, 'pk_col': pk_col,
              'where': where_clause, 'extra_cols': extra_cols}
    ...
    if source == 'suppliers':
        ...
        subquery = """SELECT t.*%(extra_cols)s FROM %(schema)s.%(source)s t%(where)s""" % params

    ddl = []
    ddl.extend(frontend_api.gen_ctas_from_subquery(schema, table_name, subquery, pk_col_name=pk_col,
                                                   table_parallelism=degree, with_drop=True))
    ddl.append(frontend_api.collect_table_stats_sql_text(schema, table_name))
    return ddl


def gen_sales_based_fact_create_ddl(frontend_api, schema, table_name, maxval_partition=False, extra_pred=None,
                                    degree=None, subpartitions=0, enable_row_movement=False, noseg_partition=True,
                                    part_key_type=None, time_id_column_name=None,
                                    extra_col_tuples=None, simple_partition_names=False,
                                    range_start_literal_override=None) -> list:
    if not frontend_api:
        return []
    return frontend_api.sales_based_fact_create_ddl(
        schema, table_name.upper(), maxval_partition=maxval_partition, extra_pred=extra_pred,
        degree=degree, subpartitions=subpartitions, enable_row_movement=enable_row_movement,
        noseg_partition=noseg_partition, part_key_type=part_key_type, time_id_column_name=time_id_column_name,
        extra_col_tuples=extra_col_tuples, simple_partition_names=simple_partition_names, with_drop=True,
...
def gen_add_sales_based_fact_partition_ddl(schema, table_name, options, frontend_api) -> list:
    if not options:
        return []
    return frontend_api.sales_based_fact_add_partition_ddl(schema, table_name)


def gen_insert_late_arriving_sales_based_data(frontend_api, schema, table_name, new_time_id_string) -> list:
    if not frontend_api:
        return []
    return frontend_api.sales_based_fact_late_arriving_data_sql(schema, table_name, new_time_id_string)


def gen_insert_late_arriving_sales_based_list_data(frontend_api, schema, table_name,
                                                   new_time_id_string, yrmon_string) -> list:
    if not frontend_api:
        return []
    return frontend_api.sales_based_list_fact_late_arriving_data_sql(schema, table_name, new_time_id_string,
                                                                     yrmon_string)


def get_sales_based_fact_partition_list(schema, table_name, hv_string_list, frontend_api) -> list:
...
def gen_drop_sales_based_fact_partition_ddls(schema, table_name, hv_string_list, frontend_api,
                                             truncate_instead_of_drop=False, dropping_oldest=None):
    """ hv_string_list in format YYYY-MM-DD
    ...
    if truncate_instead_of_drop:
        ...
        return frontend_api.sales_based_fact_drop_partition_ddl(schema, table_name, hv_string_list=hv_string_list,
                                                                dropping_oldest=dropping_oldest)


def gen_truncate_sales_based_fact_partition_ddls(schema, table_name, hv_string_list, frontend_api):
    """ hv_string_list in format YYYY-MM-DD
    """
    return gen_drop_sales_based_fact_partition_ddls(schema, table_name, hv_string_list, frontend_api,
                                                    truncate_instead_of_drop=True)
...
def get_oracle_hv_for_partition_name(schema, table_name, pname, test_cursor, part_type='RANGE'):
    ...
    """
    log('get_partition_hv_for_sales_based_hv_constant: %s.%s %s' % (schema, table_name, pname), detail=verbose)
    q = dedent("""\
        SELECT high_value
...
def gen_sales_based_list_create_ddl(frontend_api, schema, table_name, default_partition=False, extra_pred=None,
                                    part_key_type=None, out_of_sequence=False, include_older_partition=False,
                                    yrmon_column_name=None, extra_col_tuples=None):
    ...
    return frontend_api.sales_based_list_fact_create_ddl(
        schema, table_name, default_partition=default_partition, extra_pred=extra_pred,
        part_key_type=part_key_type, out_of_sequence=out_of_sequence, include_older_partition=include_older_partition,
        yrmon_column_name=yrmon_column_name, extra_col_tuples=extra_col_tuples, with_drop=True)


def get_max_sales_based_list_hv(schema, table_name, frontend_api, has_default_partition=False):
...
def gen_add_sales_based_list_partition_ddl(schema, table_name, options, frontend_api, next_ym_override=None):
    ...
    if not options:
        return []
    log('gen_add_sales_based_list_partition_ddl: %s.%s' % (schema, table_name.upper()), detail=verbose)
    return frontend_api.sales_based_list_fact_add_partition_ddl(schema, table_name.upper(),
                                                                next_ym_override=next_ym_override)


def gen_add_specific_sales_based_list_partition_ddl(schema, table_name, options, test_cursor, specific_yyyy, specific_mm):
    ...
    return gen_add_sales_based_list_partition_ddl(schema, table_name, options, test_cursor,
...
def gen_split_sales_based_list_partition_ddl(schema, table_name, options, frontend_api):
    ...
    if not options:
        return []
    log('gen_split_sales_based_list_partition_ddl: %s.%s' % (schema, table_name), detail=verbose)
    hv = get_max_sales_based_list_hv(schema, table_name, frontend_api, has_default_partition=True)
    to_split = get_max_partition_name(schema, table_name, frontend_api)
    q = "SELECT TO_CHAR(next_date,'YYYY'), TO_CHAR(next_date,'MM') FROM (SELECT ADD_MONTHS(TO_DATE(%(hv)s,'YYYYMM'),1) next_date FROM dual)" % {'hv': hv}
    next_y, next_m = frontend_api.execute_query_fetch_one(q)
    log('gen_split_sales_based_list_partition_ddl adding partition: %s%s' % (next_y, next_m), detail=verbose)
    alt = """ALTER TABLE %(schema)s.%(table)s SPLIT PARTITION %(to_split)s VALUES (%(next_y)s%(next_m)s)
        ...
        % {'schema': schema, 'table': table_name, 'to_split': to_split, 'next_y': next_y, 'next_m': next_m}
...
def gen_list_multi_part_value_create_ddl(schema, table_name, part_key_type, part_key_chars):
    ...
    """
    part_key_type_mappings = {'NUMBER': 'NUMBER(3)', 'VARCHAR2': 'VARCHAR2(3)', 'CHAR': 'CHAR(3)',
                              'NVARCHAR2': 'NVARCHAR2(3)', 'NCHAR': 'NCHAR(3)',
    ...
    part_key_type_defaults = {'NUMBER': '-1', 'VARCHAR2': 'X', 'CHAR': 'X', 'NVARCHAR2': 'X', 'NCHAR': 'X',
                              'DATE': '1970-01-01', 'TIMESTAMP': '1970-01-01'}
    ...
    params = {'schema': schema,
              ...
              'fn_mask': fn_mask}
    return ["""DROP TABLE %(schema)s.%(table)s""" % params,
            """CREATE TABLE %(schema)s.%(table)s (id NUMBER(8),data NVARCHAR2(30),cat %(part_key_type)s)
               ...
               , PARTITION p_def VALUES (DEFAULT) STORAGE (INITIAL 16k))""" % params,
            """INSERT INTO %(schema)s.%(table)s (id, data, cat)
               ...
               ,      CASE MOD(ROWNUM,3)
                      ...
                      END AS cat
               FROM   dual
               CONNECT BY ROWNUM <= 1000""" % params,
            dbms_stats_gather_string(schema, table_name)]


def drop_backend_test_table(options, backend_api, db, table_name, drop_any=False, view=False):
    ...
    if not backend_api.database_exists(db):
        log('drop_backend_test_table(%s, %s) DB does not exist' % (db, table_name), detail=verbose)
        return
    if drop_any:
        log('drop_backend_test_table(%s, %s, drop_any=True)' % (db, table_name), detail=verbose)
        backend_api.drop(db, table_name, sync=True)
    elif view:
        log('drop_backend_test_table(%s, %s, view=True)' % (db, table_name), detail=verbose)
        backend_api.drop_view(db, table_name, sync=True)
    else:
        log('drop_backend_test_table(%s, %s, table=True)' % (db, table_name), detail=verbose)
...
def delete_hybrid_metadata_for_backend_test_table(options, frontend_api, hybrid_schema,
                                                  db, backend_table_name, repo_client):
    if not options:
        return
    ...
    if options.backend_distribution in HADOOP_BASED_BACKEND_DISTRIBUTIONS:
        binds = [QueryParameter('1', hybrid_schema),
    ...
    for row in frontend_api.execute_query_fetch_all(q, query_params=binds):
        log('calling metadata drop_offload_metadata(%s, %s)' % (hybrid_schema, row[0]), detail=verbose)
        repo_client.drop_offload_metadata(hybrid_schema, row[0])


def setup_backend_only(backend_api, frontend_api, hybrid_schema, data_db, table, options, repo_client,
                       source_table='CUSTOMERS', hyb_table=None, extra_cols=None, row_limit=None, no_stats=False):
    ...
    assert source_metadata, f'Missing hybrid metadata for {hybrid_schema}.{source_table}'
    source_db, source_table = source_metadata.backend_owner, source_metadata.backend_table

    table = convert_backend_identifier_case(options, table)
    source_cols = [(col.name, col.name) for col in backend_api.get_non_synthetic_columns(data_db, source_table)]
    if extra_cols:
        if extra_cols == 'AUTO':
    ...
    python_fns = [lambda: backend_api.drop_table(data_db, table, sync=True),
                  # In case there was a conversion view
                  lambda: backend_api.drop_view(data_db, add_suffix_in_same_case(table, '_conv'), sync=True),
                  lambda: backend_api.create_table_as_select(data_db, table, backend_api.default_storage_format(),
                                                             source_cols,
                                                             from_db_name=data_db, from_table_name=source_table,
                                                             row_limit=row_limit, compute_stats=bool(not no_stats))]
...
def setup_partitioned_backend_table(backend_api, frontend_api, hybrid_schema, data_db, table, source_table, options,
                                    no_stats=False, filter_clauses=None):
    """ Setup a table which does not exist in the RDBMS, used for present testing
    """
    if not options:
        return {}
    ...
    python_fns = [lambda: backend_api.drop_table(data_db, table, sync=True),
                  # In case there was a conversion view
                  lambda: backend_api.drop_view(data_db, add_suffix_in_same_case(table, '_conv'), sync=True),
                  lambda: backend_api.create_partitioned_test_table(data_db, table, source_table,
...
def gen_sales_based_multi_pcol_fact_create_ddl(schema, table_name, maxval_partition=False):
    ...
    if maxval_partition:
        ...
        params.update({'hv2': "TO_DATE('2013-01-01','YYYY-MM-DD')", 'hv4': "TO_DATE('2014-01-01','YYYY-MM-DD')"})
    return ['DROP TABLE %(schema)s.%(table)s' % params,
...
def gen_insert_late_arriving_sales_based_multi_pcol_data(schema, table_name, new_time_id_string):
    ins = """INSERT INTO %(schema)s.%(table)s
             ...
             FROM   %(schema)s.sales
             WHERE  ROWNUM = 1""" \
          % {'schema': schema, 'table': table_name, 'time_id': new_time_id_string}
    return [ins]


def gen_sales_based_subpartitioned_fact_ddl(schema, table_name, top_level='LIST', rowdependencies=False):
    ...
    params = {'schema': schema, 'table': table_name, 'partition_scheme': partition_scheme, 'rowdependencies': rowdeps}
    return ['DROP TABLE %(schema)s.%(table)s' % params,
...
def gen_variable_columns_dim_ddl(schema, table_name, num_cols):
    ...
    return ['DROP TABLE %(schema)s.%(table)s PURGE' % params,
            """CREATE TABLE %(schema)s.%(table)s
               (id NUMBER PRIMARY KEY, %(columns)s)""" % params,
            """INSERT INTO %(schema)s.%(table)s (id, %(column_names)s) VALUES (%(values)s)""" % params,
            dbms_stats_gather_string(schema, table_name)]


def gen_incr_update_part_ddl(schema, table, rowdependencies=False, list_table=False) -> list:
    rowdeps = '%sROWDEPENDENCIES' % ('' if rowdependencies else 'NO')
    ...
    if list_table:
        ...
        params['partitions'] = """PARTITION P2010 VALUES (DATE '2010-01-01') STORAGE (INITIAL 64K),
        ...
        params['dt_gen_expr'] = "TRUNC(ADD_MONTHS(DATE '2010-01-01', MOD(ROWNUM, 60)), 'Y')"
    else:
        params['part_by_clause'] = "RANGE (dt) INTERVAL (NUMTODSINTERVAL(1,'day'))"
        params['partitions'] = """PARTITION P2010 VALUES LESS THAN (DATE '2011-01-01') STORAGE (INITIAL 64K),
        ...
        params['dt_gen_expr'] = "ADD_MONTHS(DATE '2010-01-01', MOD(ROWNUM, 48)) + MOD(ROWNUM, 365)"
    ...
    ddl += ["""CREATE TABLE %(schema)s.%(table)s
               ...
               ( %(partitions)s )
               ENABLE ROW MOVEMENT %(rowdependencies)s""" % params,
            """INSERT INTO %(schema)s.%(table)s (id, data, dt, dt2, num)
               ...
               FROM DUAL CONNECT BY ROWNUM <= 5 * 1e3""" % params,
...
def gen_max_length_identifier(short_name, max_length, pad_with=None):
    ...
    if pad_with:
        ...
        padded_name = prefix + ''.join(str(_ % 10) for _ in range(1, max_length - len(prefix) + 1))
