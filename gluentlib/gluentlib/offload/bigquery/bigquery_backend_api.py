...
from gluentlib.offload.column_metadata import CanonicalColumn, ColumnMetadataInterface, ColumnPartitionInfo, \
...
from gluentlib.offload.backend_api import BackendApiInterface, BackendApiException, BackendStatsException, \
...
from gluentlib.offload.offload_constants import EMPTY_BACKEND_TABLE_STATS_DICT, EMPTY_BACKEND_COLUMN_STATS_LIST, \
    EMPTY_BACKEND_COLUMN_STATS_DICT, FILE_STORAGE_FORMAT_AVRO, FILE_STORAGE_FORMAT_BIGTABLE, \
    BIGQUERY_BACKEND_CAPABILITIES, PART_COL_GRANULARITY_DAY, PART_COL_GRANULARITY_MONTH, PART_COL_GRANULARITY_YEAR, \
...
from gluentlib.util.hive_table_stats import parse_stats_into_tab_col, transform_stats_as_tuples
from gluentlib.util.misc_functions import backtick_sandwich, bytes_to_human_size, format_list_for_logging
...
class BackendBigQueryApi(BackendApiInterface):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False,
                 do_not_connect=False):
        """ CONSTRUCTOR
        """
        super(BackendBigQueryApi, self).__init__(connection_options, backend_type, messages, dry_run=dry_run,
        ...
        if self._google_kms_key_ring_location and self._google_kms_key_ring_name and self._google_kms_key_name:
    ...
    def _add_kms_key_to_job_config(self, job_config):
        ...
        if self._kms_key_name:
            self._log('Setting job session option: kms_key_name=%s' % self._kms_key_name, detail=VVERBOSE)
    ...
    def _check_kms_key_name(self, kms_key_name, key_type='job'):
        ...
        if self._kms_key_name and not (kms_key_name or '').startswith(self._kms_key_name):
            self._warning('BigQuery %s KMS key: %s\n != expected KMS key: %s'
                      % (key_type, kms_key_name, self._kms_key_name))

    def _create_external_table(self, db_name, table_name, column_list, storage_format, location=None):
        """ Create a BigQuery external table using the API
        """
        assert db_name
        assert table_name
        if storage_format not in [FILE_STORAGE_FORMAT_AVRO, FILE_STORAGE_FORMAT_PARQUET]:
            assert column_list
        assert valid_column_list(column_list), 'Incorrectly formed column_list: %s' % column_list
        assert storage_format in (FILE_STORAGE_FORMAT_AVRO, FILE_STORAGE_FORMAT_BIGTABLE, FILE_STORAGE_FORMAT_PARQUET)
        assert location
        ...
        if storage_format in [FILE_STORAGE_FORMAT_AVRO, FILE_STORAGE_FORMAT_PARQUET]:
            ...
            column_spec = [bigquery.SchemaField(_.name, _.format_data_type(),
                                                mode='NULLABLE' if _.nullable else 'REQUIRED')
                           for _ in column_list]

        new_table = bigquery.Table(self._bq_table_id(db_name, table_name), schema=column_spec)
    ...
    def _create_table_properties_clause(self, table_properties):
        ...
        if table_properties:
            ...
            table_prop_clause = '\nOPTIONS (%s)' \
                                % ', '.join('%s=%s' % (k, v) for k, v in table_properties.items())
        else:
            table_prop_clause = ''
        return table_prop_clause

    def _execute_ddl_or_dml(self, sql, sync=None, query_options=None, log_level=VERBOSE, profile=None,
                            no_log_items=None):
        ...
        for i, run_sql in enumerate(sqls):
            self._log_or_not('%s SQL: %s' % (self._sql_engine_name, run_sql),
                             log_level=log_level, no_log_items=no_log_items)
        ...
        if self._global_session_parameters:
            ...
            for k, v in [(str(k).lower(), v) for k, v in self._global_session_parameters.items()]:
                if k not in default_config_white_list:
                    raise BackendApiException('Modification of job configuration %s is not permitted, valid values: %s'
    ...
    def _execute_query_fetch_x(self, sql, fetch_action=FETCH_ACTION_ALL, query_options=None, log_level=None,
                               time_sql=False, not_when_dry_running=False, query_params=None, profile=None,
                               as_dict=False):
    ...
    def _get_bq_client(self):
        ...
        return bigquery.Client(default_query_job_config=self._execute_global_session_parameters(),
    ...
    def _gen_bigquery_create_table_sql_text(self, db_name, table_name, column_list, partition_column_names,
                                            table_properties=None, sort_column_names=None, for_replace=False):
        ...
        if partition_column_names:
            ...
            if not real_col:
                self._log('Proposed table columns: %s' % str_list_of_columns(column_list), detail=VERBOSE)
...
%(col_projection)s
    ...
    def _get_query_profile(self, query_identifier=None):
        ...
        try:
            ...
            self._log('Exception formatting BigQuery profile (non-fatal): %s' % str(exc), detail=VERBOSE)
    ...
    def _object_exists(self, db_name, table_name, table_type=None):
        ...
        try:
            ...
            if not table_type:
                return True
            elif table_type.upper() == 'TABLE' and table.table_type.upper() in ['TABLE', 'EXTERNAL']:
                return True
    ...
    def _table_is_partitioned(self, db_name, table_name):
        ...
        assert db_name and table_name
        table = self._get_bq_table(db_name, table_name)
        return bool(self._get_bq_table_partition_type(table) in (SUPPORTED_BQ_DATE_PARTITIONING_TYPES +
    ...
    def add_columns(self, db_name, table_name, column_tuples, sync=None):
        ...
        submittable_lines = ", ".join(["ADD COLUMN IF NOT EXISTS " + " ".join([val for val in column])
                                       for column in column_tuples])
        sql = ("ALTER TABLE %s %s" % (self.enclose_object_reference(db_name, table_name),
                                      submittable_lines))
        if not self._dry_run:
    ...
    def alter_sort_columns(self, db_name, table_name, sort_column_names, sync=None):
        ...
        log_cmd = 'update_table(%s, %s, clustering_fields=%s)' % (db_name, table_name, str(table.clustering_fields))
        self._log('BigQuery call: %s' % log_cmd, detail=VERBOSE)
        if not self._dry_run:
    ...
    def backend_report_info(self):
        ...
        """
        return {REPORT_ATTR_BACKEND_CLASS: 'Cloud Warehouse',
                        ...
                        results.append(
                            {CONNECT_TEST: 'Test KMS key',
                             ...
                             CONNECT_DETAIL: 'KMS key: %s is an %s key and is %s' % (
                                 key.name, kms.CryptoKey.CryptoKeyPurpose.ENCRYPT_DECRYPT.name,
                        ...
                        results.append(
                            {CONNECT_TEST: 'Test KMS key',
                             ...
                             CONNECT_DETAIL: 'KMS key: %s has incorrect state: %s, expected key state is: %s' % (
                                 key.name, key.primary.state.name,
                                 kms.CryptoKeyVersion.CryptoKeyVersionState.ENABLED.name)}
                        )
                else:
                    results.append(
                        {CONNECT_TEST: 'Test KMS key',
                         ...
                         CONNECT_DETAIL: 'KMS key: %s has incorrect purpose: %s, expected key purpose is: %s' % (
                             key.name, key.purpose.name, kms.CryptoKey.CryptoKeyPurpose.ENCRYPT_DECRYPT.name)}
                    )
            except Exception:
                self._log(traceback.format_exc(), detail=VVERBOSE)
                results.append(
    ...
    def compute_stats(self, db_name, table_name, incremental=None, for_columns=False, partition_tuples=None, sync=None):
    ...
    def create_database(self, db_name, comment=None, properties=None):
        ...
        if self.database_exists(db_name):
            self._log('Dataset already exists, not attempting to create: %s' % db_name, detail=VVERBOSE)
    ...
    def create_table(self, db_name, table_name, column_list, partition_column_names, storage_format=None, location=None,
                     external=False, table_properties=None, sort_column_names=None, without_db_name=False, sync=None):
        ...
        """
        if external:
            return self._create_external_table(db_name, table_name, column_list, storage_format, location=location)

        # Normal (non-external) table
        sql = self._gen_bigquery_create_table_sql_text(db_name, table_name, column_list, partition_column_names,
        ...
        if self._kms_key_name and not self._dry_run:
            table_id = "{}.{}.{}".format(self._backend_project_name(), db_name, table_name)
            table = self._client.get_table(table_id)
            self._check_kms_key_name(table.encryption_configuration.kms_key_name, key_type='table')
        return cmds

    def create_view(self, db_name, view_name, column_tuples, ansi_joined_tables, filter_clauses=None, sync=None):
...
FROM   %(from_tables)s%(where_clause)s""" \
    ...
    def create_udf(self, db_name, udf_name, return_data_type, parameter_tuples, udf_body, or_replace=False,
        ...
        sql = 'CREATE%s FUNCTION %s(%s) RETURNS %s AS (%s)' % (or_replace_clause,
                                                               self.enclose_object_reference(db_name, udf_name),
                                                               udf_param_clause, return_data_type, udf_body)
    ...
    def derive_native_partition_info(self, db_name, table_name, column, position):
        ...
        if self._get_bq_table_partition_type(table) in SUPPORTED_BQ_DATE_PARTITIONING_TYPES:
            ...
            return ColumnPartitionInfo(position=position, granularity=granularity, digits=1,
                                       source_column_name=column_name)
        elif self._get_bq_table_partition_type(table) == SUPPORTED_BQ_RANGE_PARTITIONING_TYPE:
            ...
            if table_json.get('rangePartitioning'):
                ...
                try:
                    ...
                    return ColumnPartitionInfo(position=position, source_column_name=column_name,
                                               granularity=range_interval, range_start=range_start, range_end=range_end)
                except ValueError:
                    self._log('Incorrectly formatted rangePartitioning document: %s'
...
LIMIT 1""" % {'db_table': self.enclose_object_reference(db_name, table_name),
    ...
    def diagnose_backend_config(self):
        raise NotImplementedError('Diagnose backend configuration not implemented for BigQuery')

    def diagnose_query_log(self, **kwargs):

        def header(header_character='='):
            return header_character * 100

        def blank_line():
            return ''

        def worker_header():
            return '{0:>34s} {1:>17s} {2:>17s} {3:>17s}'.format('Wait', 'Read', 'Compute', 'Write')

        def worker_line(title, wait, read, compute, write):
            return '{0:>15s}: {1:>15}ms {2:>15}ms {3:>15}ms {4:>15}ms'.format(title, wait, read, compute, write)

        assert 'query_id' in kwargs
        job_details = self.get_job_details(kwargs['query_id'])

        job_information = [
            self._diagnose_key_value_line('Job ID', '%s:%s.%s' % (job_details.project, job_details.location, job_details.job_id)),
            self._diagnose_key_value_line('User', job_details.user_email),
            self._diagnose_key_value_line('Location', job_details.location),
            self._diagnose_key_value_line('Creation time', job_details.created),
            self._diagnose_key_value_line('Start time', job_details.started),
            self._diagnose_key_value_line('End time', job_details.ended),
            self._diagnose_key_value_line('Duration', (job_details.ended - job_details.started)),
            self._diagnose_key_value_line('Bytes processed',
                                          bytes_to_human_size(job_details.total_bytes_processed, 2) if job_details.total_bytes_processed else '0B'),
            self._diagnose_key_value_line('Bytes billed',
                                          bytes_to_human_size(job_details.total_bytes_billed, 2) if job_details.total_bytes_billed else '0B'),
            self._diagnose_key_value_line('Job priority', job_details.priority),
            self._diagnose_key_value_line('Destination table', job_details.destination),
            self._diagnose_key_value_line('Use legacy SQL', job_details.use_legacy_sql),
            self._diagnose_key_value_line('Query text', job_details.query, wrap=False),
        ]

        bytes_shuffled = sum(j.shuffle_output_bytes for j in job_details.query_plan[:-1])
        bytes_shuffled_spilled = sum(j.shuffle_output_bytes_spilled for j in job_details.query_plan[:-1])
        execution_details = [
            self._diagnose_key_value_line('Elapsed time', (job_details.ended - job_details.started)),
            self._diagnose_key_value_line('Slot time consumed',
                                          '{0:.3f} sec'.format(float(job_details.timeline[-1].slot_millis) / 1000)),
            self._diagnose_key_value_line('Bytes shuffled',
                                          bytes_to_human_size(bytes_shuffled, 2) if bytes_shuffled else '0B'),
            self._diagnose_key_value_line('Bytes spilled to disk',
                                          bytes_to_human_size(bytes_shuffled_spilled, 2) if bytes_shuffled_spilled else '0B'),
        ]

        tuples = [('Job Information', '\n'.join(job_information)),
                  ('Execution Details', '\n'.join(execution_details))]

        for plan_line in job_details.query_plan:
            plan_line_detail = [
                self._diagnose_key_value_line('Status', plan_line.status),
                self._diagnose_key_value_line('Input rows', '{:,}'.format(plan_line.records_read)),
                self._diagnose_key_value_line('Output rows', '{:,}'.format(plan_line.records_written)),
                header('~'),
                'Worker timing',
                header('~'),
                worker_header(),
                '{0:>88s}'.format('_' * 69),
                worker_line('Avg', plan_line.wait_ms_avg, plan_line.read_ms_avg,
                            plan_line.compute_ms_avg, plan_line.write_ms_avg),
                worker_line('Max', plan_line.wait_ms_max, plan_line.read_ms_max,
                            plan_line.compute_ms_max, plan_line.write_ms_max),
            ]
            if plan_line.steps:
                plan_line_detail.append(blank_line())
                plan_line_detail.append(header('-'))
                plan_line_detail.append('Steps')
                plan_line_detail.append(header('-'))
                for step in plan_line.steps:
                    plan_line_detail.append(self._diagnose_key_value_line(step.kind, ' '.join(step.substeps)))
            tuples.append(('Stage %s' % plan_line.name, '\n'.join(plan_line_detail)))
    ...
    def execute_query_fetch_all(self, sql, query_options=None, log_level=None, time_sql=False,
                                not_when_dry_running=False, query_params=None, profile=None, as_dict=False):
        """ For parameter descriptions see: _execute_query_fetch_x
        """
        rows = self._execute_query_fetch_x(sql, fetch_action=FETCH_ACTION_ALL, query_options=query_options,
                                           log_level=log_level, time_sql=time_sql, profile=profile,
                                           not_when_dry_running=not_when_dry_running, query_params=query_params,
                                           as_dict=as_dict)
        return rows
    ...
    def execute_query_fetch_one(self, sql, query_options=None, log_level=None, time_sql=False,
        ...
        """ For parameter descriptions see: _execute_query_fetch_x
        """
        row = self._execute_query_fetch_x(sql, fetch_action=FETCH_ACTION_ONE, query_options=query_options,
                                          log_level=log_level, time_sql=time_sql, profile=profile,
                                          not_when_dry_running=not_when_dry_running, query_params=query_params,
                                          as_dict=as_dict)
        return row

    def execute_query_get_cursor(self, sql, query_options=None, log_level=None, time_sql=False,
                                 not_when_dry_running=False, query_params=None):
    ...
    def gen_copy_into_sql_text(self, db_name, table_name, from_object_clause, select_expr_tuples, filter_clauses=None):
    ...
    def gen_default_numeric_column(self, column_name, data_scale=None):
        ...
        """
        if data_scale == 0:
            return BigQueryColumn(column_name, data_type=BIGQUERY_TYPE_INT64, nullable=True, safe_mapping=False)
        elif data_scale and data_scale <= 9:
            return BigQueryColumn(column_name, data_type=BIGQUERY_TYPE_NUMERIC,
                                  data_precision=38, data_scale=9,
                                  nullable=True, safe_mapping=False)
        else:
            return BigQueryColumn(column_name, data_type=BIGQUERY_TYPE_BIGNUMERIC,
                                  data_precision=self.max_decimal_precision(), data_scale=self.max_decimal_scale(),
                                  nullable=True, safe_mapping=False)

    def gen_ctas_sql_text(self, db_name, table_name, storage_format, column_tuples, from_db_name=None,
                          from_table_name=None, row_limit=None, external=False, table_properties=None):
        ...
        projection = self._format_select_projection(column_tuples)
        from_clause = '\nFROM   {}'.format(
            self.enclose_object_reference(from_db_name, from_table_name)) if from_db_name and from_table_name else ''
    ...
    def gen_insert_select_sql_text(self, db_name, table_name, from_db_name, from_table_name, select_expr_tuples,
                                   partition_expr_tuples=None, filter_clauses=None, sort_expr_list=None,
                                   distribute_columns=None, insert_hint=None, from_object_override=None):
        ...
        """
        self._gen_insert_select_sql_assertions(db_name, table_name, from_db_name, from_table_name, select_expr_tuples,
                                               partition_expr_tuples, filter_clauses, from_object_override)
...
SELECT %(proj)s
FROM   %(from_db_table)s%(where)s""" % {'db_table': self.enclose_object_reference(db_name, table_name),
                                        'proj': projection,
    ...
    def gen_native_range_partition_key_cast(self, partition_column):
        ...
        if partition_column.data_type in [BIGQUERY_TYPE_DATE, BIGQUERY_TYPE_DATETIME, BIGQUERY_TYPE_TIMESTAMP]:
            if partition_column.partition_info.granularity == PART_COL_GRANULARITY_YEAR:
                part_col_expr = '%s_TRUNC(%s,YEAR)' % (partition_column.data_type.upper(),
                                                       self.enclose_identifier(partition_column.name))
            elif partition_column.partition_info.granularity == PART_COL_GRANULARITY_MONTH:
                part_col_expr = '%s_TRUNC(%s,MONTH)' % (partition_column.data_type.upper(),
                                                        self.enclose_identifier(partition_column.name))
                ...
                if partition_column.data_type == BIGQUERY_TYPE_DATE:
                    ...
                    part_col_expr = '%s_TRUNC(%s,DAY)' % (partition_column.data_type.upper(),
                                                          self.enclose_identifier(partition_column.name))
            else:
                raise NotImplementedError('BigQuery table partitioning not implemented for data type/granularity: %s/%s'
                                          % (partition_column.data_type, partition_column.partition_info.granularity))
            ...
            raise NotImplementedError('BigQuery table partitioning not implemented for data type:'
                                      '%s' % partition_column.data_type)
        return part_col_expr

    def gen_sql_text(self, db_name, table_name, column_names=None, filter_clauses=None, measures=None, agg_fns=None):
        return self._gen_sql_text_common(db_name, table_name, column_names=column_names,
                                         filter_clauses=filter_clauses, measures=measures, agg_fns=agg_fns)
    ...
    def get_columns(self, db_name, table_name):
        ...
        for col in table.schema:
            ...
            backend_columns.append(BigQueryColumn(str(col.name), data_type=str(col.field_type),
                                                  nullable=str(col.is_nullable)))
        return backend_columns

    def get_distinct_column_values(self, db_name, table_name, column_name_list, columns_to_cast_to_string=None,
                                   order_results=False, not_when_dry_running=False):
        ...
        return self.get_distinct_expressions(db_name, table_name, expression_list,
                                             order_results=order_results, not_when_dry_running=not_when_dry_running)
    ...
    def get_max_column_values(self, db_name, table_name, column_name_list, columns_to_cast_to_string=None,
                              optimistic_prune_clause=None, not_when_dry_running=False):
        """ Run SQL to get max value for a set of columns.
            See interface description for parameter details.
        """
        return self._get_max_column_values_common(db_name, table_name, column_name_list,
    ...
    def get_partition_columns(self, db_name, table_name):
        ...
        if self._get_bq_table_partition_type(table) in (SUPPORTED_BQ_DATE_PARTITIONING_TYPES +
                                                        [SUPPORTED_BQ_RANGE_PARTITIONING_TYPE]):
            part_col = match_table_column(self._get_bq_table_partition_field(table),
    ...
    def get_table_location(self, db_name, table_name):
        ...
        if self._is_external_table(db_name, table_name):
            table = self._get_bq_table(db_name, table_name)
            return table.external_data_configuration.source_uris[
                0] if table.external_data_configuration.source_uris else None
        else:
            return None
    ...
    def get_table_row_count(self, db_name, table_name, filter_clause=None,
                            not_when_dry_running=False, log_level=VVERBOSE):
        if self._dry_run and not_when_dry_running:
            return None
        if (filter_clause or
                self.is_view(db_name, table_name) or
                self._is_external_table(db_name, table_name) or
                self._has_outstanding_streaming_inserts(db_name, table_name)):
            sql = self._gen_select_count_sql_text_common(db_name, table_name, filter_clause=filter_clause)
            row = self.execute_query_fetch_one(sql, log_level=log_level, time_sql=True,
                                               not_when_dry_running=not_when_dry_running)
            return row[0] if row else None
        else:
            # Forget the table before getting a count because metadata is cached
            self._forget_bq_table(db_name, table_name)
            self._log_or_not('%s call: %s.num_rows' % (self._sql_engine_name, self._bq_table_id(db_name, table_name)),
                             log_level=log_level)
    ...
    def get_table_stats(self, db_name, table_name, as_dict=False):
        ...
        if as_dict:
            ...
            stats_tuple = (tab_stats['num_rows'], tab_stats['num_bytes'], tab_stats['avg_row_len'])
    ...
    def get_view_ddl(self, db_name, view_name, as_list=False, terminate_sql=False, for_replace=False):
    ...
    def insert_literal_values(self, db_name, table_name, literal_list, column_list=None, max_rows_per_insert=250,
                              split_by_cr=True):
        ...
        while remaining_rows:
            ...
            for row in this_chunk:
                formatted_rows.append(','.join(gen_literal(py_val, data_type)
                                               for py_val, data_type in zip(row, data_type_strs)))
            sql = self._insert_literals_using_insert_values_sql_text(db_name, table_name, column_names,
                                                                     formatted_rows, split_by_cr=split_by_cr)
    ...
    def is_valid_partitioning_data_type(self, data_type):
        if not data_type:
            return False
        return bool(data_type.upper() in [BIGQUERY_TYPE_DATE, BIGQUERY_TYPE_DATETIME, BIGQUERY_TYPE_INT64,
                                          BIGQUERY_TYPE_BIGNUMERIC, BIGQUERY_TYPE_NUMERIC, BIGQUERY_TYPE_TIMESTAMP])
    ...
    def populate_sequence_table(self, db_name, table_name, starting_seq, target_seq, split_by_cr=False):
        raise NotImplementedError('Sequence table does not apply for BigQuery')
    ...
    def rename_table(self, from_db_name, from_table_name, to_db_name, to_table_name, sync=None):
        ...
        if not self._dry_run and not self.table_exists(from_db_name, from_table_name):
            raise BackendApiException('Source table does not exist, cannot rename table: %s.%s'
                                      % (from_db_name, from_table_name))

        if not self._dry_run and self.exists(to_db_name, to_table_name):
            raise BackendApiException('Target table already exists, cannot rename table to: %s.%s'
                                      % (to_db_name, to_table_name))

        sql = ("ALTER TABLE IF EXISTS %s RENAME TO %s" % (self.enclose_object_reference(from_db_name, from_table_name),
    ...
    def supported_backend_data_types(self):
        return [BIGQUERY_TYPE_BIGNUMERIC, BIGQUERY_TYPE_BOOLEAN, BIGQUERY_TYPE_BYTES, BIGQUERY_TYPE_DATE,
                BIGQUERY_TYPE_DATETIME, BIGQUERY_TYPE_FLOAT64, BIGQUERY_TYPE_INT64, BIGQUERY_TYPE_NUMERIC,
                BIGQUERY_TYPE_STRING, BIGQUERY_TYPE_TIME, BIGQUERY_TYPE_TIMESTAMP]

    def supported_date_based_partition_granularities(self):
        return [PART_COL_GRANULARITY_DAY, PART_COL_GRANULARITY_MONTH, PART_COL_GRANULARITY_YEAR]

    def supported_incremental_update_extraction_methods(self):
        return []

    def supported_partition_function_parameter_data_types(self):
        return [BIGQUERY_TYPE_BIGNUMERIC, BIGQUERY_TYPE_INT64, BIGQUERY_TYPE_NUMERIC, BIGQUERY_TYPE_STRING]
    ...
    def transform_regexp_replace_expression(self, backend_column, regexp_replace_pattern, regexp_replace_string):
        return 'REGEXP_REPLACE(%s, %s, %s)' % (self.enclose_identifier(backend_column.name),
                                               regexp_replace_pattern, regexp_replace_string)
    ...
    def to_canonical_column(self, column):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, safe_mapping=None):
            ...
            return CanonicalColumn(name=col.name, data_type=data_type, data_length=data_length,
                                   ...
                                   char_length=data_length, char_semantics=CANONICAL_CHAR_SEMANTICS_CHAR)
