...
from gluentlib.schema_sync.schema_sync_analyzer import SchemaSyncAnalyzer
from gluentlib.schema_sync.schema_sync_processor import SchemaSyncProcessor
from gluentlib.schema_sync.schema_sync_command_file import SchemaSyncCommandFile
...
def get_schema_sync_opts():
    ...
    opt.add_option('--include', dest='include',
    ...
    opt.add_option('--command-file', dest='command_file',
...
def schema_sync(options, messages: "OffloadMessages", execution_id: "ExecutionId",
                repo_client: "OrchestrationRepoClientInterface"):

    messages.setup_offload_step(error_before_step=options.error_before_step,
    ...
    def display_exceptions(exceptions):
        ...
        for table, exception in exceptions:
            source_table = '%s.%s' % (double_quote_sandwich(table['offloaded_owner']),
                                      double_quote_sandwich(table['offloaded_table']))
            log('{0:70s} {1:70s}'.format(source_table, exception))

    tables_in_scope = messages.offload_step(command_steps.STEP_NORMALIZE_INCLUDES,
    ...
    for table in tables_in_scope:
        ...
        if 'vectors' in table_changes:
            table_owner, table_name = table_changes['offloaded_owner'], table_changes['offloaded_table']
            try:
                ...
                messages.notice('%s %s.%s, skipping' % (SCHEMA_SYNC_LOCKED_MESSAGE_TEXT,
    ...
    if exceptions:
        messages.offload_step(command_steps.STEP_REPORT_EXCEPTIONS,
                              lambda: display_exceptions(exceptions), execute=options.execute)
        return 1
    else:
        return 0
