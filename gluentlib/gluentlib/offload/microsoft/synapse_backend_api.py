...
from gluentlib.offload.backend_api import BackendApiInterface, BackendApiException, BackendStatsException,\
    FETCH_ACTION_ALL, FETCH_ACTION_CURSOR, FETCH_ACTION_ONE, REPORT_ATTR_BACKEND_CLASS, REPORT_ATTR_BACKEND_TYPE,\
    REPORT_ATTR_BACKEND_DISPLAY_NAME, REPORT_ATTR_BACKEND_HOST_INFO_TYPE, REPORT_ATTR_BACKEND_HOST_INFO,\
    SORT_COLUMNS_UNLIMITED
from gluentlib.offload.column_metadata import CanonicalColumn, is_safe_mapping, valid_column_list, \
    ...
    GLUENT_TYPE_TIMESTAMP_TZ, GLUENT_TYPE_VARIABLE_STRING, \
...
from gluentlib.offload.offload_constants import SYNAPSE_BACKEND_CAPABILITIES, FILE_STORAGE_FORMAT_PARQUET,\
    EMPTY_BACKEND_TABLE_STATS_DICT, EMPTY_BACKEND_TABLE_STATS_LIST, EMPTY_BACKEND_COLUMN_STATS_DICT,\
    EMPTY_BACKEND_COLUMN_STATS_LIST, DBTYPE_SYNAPSE
...
from gluentlib.offload.microsoft.synapse_column import SynapseColumn, \
    ...
    SYNAPSE_TYPE_VARBINARY, SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_MAX_TOKEN
from gluentlib.offload.microsoft.synapse_literal import SynapseLiteral
from gluentlib.util.misc_functions import add_prefix_in_same_case, format_list_for_logging,\
    human_size_to_bytes, id_generator
...
SYNAPSE_VALID_AUTH_MECHANISMS = [SYNAPSE_AUTH_MECHANISM_AD_PASSWORD, SYNAPSE_AUTH_MECHANISM_AD_MSI,
                                 SYNAPSE_AUTH_MECHANISM_AD_SERVICE_PRINCIPAL, SYNAPSE_AUTH_MECHANISM_SQL_PASSWORD]
SYNAPSE_USER_PASS_AUTH_MECHANISMS = [SYNAPSE_AUTH_MECHANISM_AD_PASSWORD, SYNAPSE_AUTH_MECHANISM_SQL_PASSWORD]
...
def synapse_collation_clause(column_or_collation):
    ...
    if isinstance(column_or_collation, SynapseColumn):
        return ' COLLATE {}'.format(column_or_collation.collation) if column_or_collation.collation else ''
    else:
...
class BackendSynapseApi(BackendApiInterface):
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False,
        ...
        super().__init__(connection_options, backend_type, messages, dry_run=dry_run,
        ...
        if not do_not_connect:
            url = ['Driver=' + '{%s}' % connection_options.backend_odbc_driver_name,
                   'Server=tcp:%s,%s' % (connection_options.synapse_server,
                                         connection_options.synapse_port),
            ...
            url.extend(['Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30',
    ...
    def _create_table_columns_clause(self, column_list, external=False):
        ...
        """
        sql_cols = [(self.enclose_identifier(_.name),
                     _.format_data_type(),
    ...
    def _create_table_sql_text(self, db_name, table_name, column_list, partition_column_names, location=None,
                               external=False, table_properties=None, sort_column_names=None):
        ...
        def cci_incompatible(column_list):
            ...
            return bool(any(SYNAPSE_TYPE_MAX_TOKEN in _
                            for _ in [col.format_data_type()
                                      for col in column_list
                                      if col.data_type in (SYNAPSE_TYPE_VARCHAR,
        ...
        if sort_column_names:
            if cci_incompatible(column_list):
                self._messages.notice("Ignoring sort columns due to the presence of datatype(s) with %s precision"
        ...
        sql = dedent("""\
                    ...
                    %(col_projection)s
                    )%(with_clause)s""") % {'external': external_clause,
    ...
    def _execute_ddl_or_dml(self, sql, sync=None, query_options=None, log_level=VERBOSE, profile=None,
        ...
        run_sqls = self._execute_sqls(sql, query_options, log_level=log_level,
                                      profile=profile, no_log_items=no_log_items)
        return run_sqls
    ...
    def _add_option_clause_to_sql_text(self, sql_text: str, option_dict: dict):
        ...
        if not self._option_clause_valid_for_sql_text(sql_text):
            ...
            self._log(f'Cannot add OPTION clause to incompatible SQL statement: {option_clause}', detail=VVERBOSE)
            return sql_text
    ...
    def _execute_query_fetch_x(self, sql, fetch_action=FETCH_ACTION_ALL, query_options=None, log_level=None,
        ...
        try:
            ...
            self._log_or_not('%s SQL: %s' % (self._sql_engine_name, sql), log_level=log_level)
            if query_params:
                self._log_or_not('%s SQL parameters: %s' % (self._sql_engine_name, query_params), log_level=log_level)
            ...
            if profile and self._option_clause_valid_for_sql_text(sql):
                self._log(self._get_query_profile(query_identifier=query_option_label), detail=VVERBOSE)
        finally:
    ...
    def _execute_sqls(self, sql, query_options=None, log_level=VERBOSE, profile=None, no_log_items=None):
        return_list = []
    ...
    def _gen_max_column_values_sql(self, db_name, table_name, column_name_list, filter_clause=None,
        ...
        def add_sql_cast(col_name, col_expr=None, add_alias=False):
            ...
            if col_name in (columns_to_cast_to_string or []):
                cast_str = 'CAST(%s AS %s)' % (col_expr, self.generic_string_data_type())
            else:
                cast_str = col_expr
            if add_alias:
        ...
        if len(column_name_list) == 1:
            ...
            proj_col = add_sql_cast(column_name_list[0],
                                    col_expr='MAX(%s)' % self.enclose_identifier(column_name_list[0]))
            sql = 'SELECT %s FROM %s' % (proj_col, self.enclose_object_reference(db_name, table_name))
            if filter_clause:
            ...
            sql = 'SELECT %s FROM (SELECT TOP(1) %s FROM %s%s GROUP BY %s ORDER BY %s) v' \
                % (proj_cols, cols, self.enclose_object_reference(db_name, table_name), where, cols, order_by)
        return sql

    def _gen_sample_stats_sql_sample_clause(self, db_name, table_name, sample_perc=None):
        assert db_name and table_name
        if (sample_perc is not None and sample_perc >= 0
    ...
    def add_columns(self, db_name, table_name, column_tuples, sync=None):
        ...
        column_clause = ', '.join('{} {}'.format(self.enclose_identifier(col_name),
                                                 col_type) for col_name, col_type in column_tuples)
        sqls = ['ALTER TABLE %s ADD %s' % (self.enclose_object_reference(db_name, table_name),
                                           column_clause)]
    ...
    def backend_report_info(self):
        ...
        """
        return {REPORT_ATTR_BACKEND_CLASS: 'Azure',
    ...
    def compute_stats(self, db_name, table_name, incremental=None, for_columns=False, partition_tuples=None, sync=None):
        ...
        if for_columns:
            ...
            sqls.append('UPDATE STATISTICS %s WITH SAMPLE %s PERCENT' % (
    ...
    def create_database(self, db_name, comment=None, properties=None):
        ...
        if self.database_exists(db_name):
            self._log('Schema already exists, not attempting to create: %s' % db_name, detail=VVERBOSE)
    ...
    def create_table(self, db_name, table_name, column_list, partition_column_names, storage_format=None, location=None,
        ...
        sql = self._create_table_sql_text(db_name, table_name, column_list_with_collations, partition_column_names,
                                          location=location, external=external, table_properties=table_properties,
    ...
    def data_type_accepts_length(self, data_type):
        return bool(data_type in [SYNAPSE_TYPE_DECIMAL, SYNAPSE_TYPE_NUMERIC,
                                  SYNAPSE_TYPE_CHAR, SYNAPSE_TYPE_NCHAR,
                                  SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_NVARCHAR,
                                  SYNAPSE_TYPE_BINARY, SYNAPSE_TYPE_VARBINARY])
    ...
    def detect_column_has_fractional_seconds(self, db_name, table_name, column):
        ...
        sql = dedent("""\
                SELECT TOP(1) %(col)s
                FROM %(db_table)s
                WHERE DATEPART(ns, %(col)s) != 0""") % {'db_table': self.enclose_object_reference(db_name, table_name),
    ...
    def drop_view(self, db_name, view_name, if_exists=True, sync=None):
        ...
        drop_sql = 'DROP VIEW %s%s.%s' % (if_exists_clause, self.enclose_identifier(db_name),
    ...
    def enclose_object_reference(self, db_name, object_name):
        ...
        return '.'.join([self.enclose_identifier(self._synapse_database),
    ...
    def execute_query_fetch_all(self, sql, query_options=None, log_level=None, time_sql=False,
        ...
        return self._execute_query_fetch_x(sql, fetch_action=FETCH_ACTION_ALL, query_options=query_options,
    ...
    def execute_query_fetch_one(self, sql, query_options=None, log_level=None, time_sql=False,
                                not_when_dry_running=False, query_params=None, profile=None, as_dict=False):
        """ For parameter descriptions see: _execute_query_fetch_x """
        return self._execute_query_fetch_x(sql, fetch_action=FETCH_ACTION_ONE, query_options=query_options,
                                           log_level=log_level, time_sql=time_sql, profile=profile,
                                           not_when_dry_running=not_when_dry_running, query_params=query_params,
                                           as_dict=as_dict)
    ...
    def execute_query_get_cursor(self, sql, query_options=None, log_level=None, time_sql=False,
        ...
        return self._execute_query_fetch_x(sql, fetch_action=FETCH_ACTION_CURSOR, query_options=query_options,
                                           log_level=log_level, time_sql=time_sql,
                                           not_when_dry_running=not_when_dry_running, query_params=query_params)
    ...
    def format_column_comparison(self, left_col, operator, right_col, left_alias=None, right_alias=None):
        ...
        if left_alias:
            left_identifier = '{}.{}'.format(self.enclose_identifier(left_alias), left_identifier)
        right_identifier = self.enclose_identifier(right_col.name)
        if right_alias:
            right_identifier = '{}.{}'.format(self.enclose_identifier(right_alias), right_identifier)
        if left_col.collation or right_col.collation:
            ...
            return '{}{} {} {}{}'.format(
                left_identifier, self._collation_clause(self._connection_options.synapse_collation), operator,
                right_identifier, self._collation_clause(self._connection_options.synapse_collation)
            )
        else:
    ...
    def gen_default_numeric_column(self, column_name, data_scale=18):
        ...
        """
        return SynapseColumn(column_name, data_type=SYNAPSE_TYPE_NUMERIC,
                             data_precision=self.max_decimal_precision(),
                             data_scale=data_scale, nullable=True, safe_mapping=False)
...
FROM   %(from_db_table)s%(where)s""" % {'db_table': self.enclose_object_reference(db_name, table_name),
    ...
    def get_columns(self, db_name, table_name):
        ...
        col_rows = self.execute_query_fetch_all(sql, log_level=VVERBOSE,
                                                query_params=[self._synapse_database, db_name, table_name])
        for col in col_rows:
            (col_name, data_type, is_nullable, char_length, byte_length, num_precision,
             num_scale, time_scale, collation) = col
            # Date is stored as 0
            ...
            if byte_length == -1:
                byte_length = None
            backend_columns.append(SynapseColumn(col_name, data_type, data_length=byte_length,
                                                 char_length=char_length, data_precision=num_precision,
                                                 data_scale=data_scale, nullable=is_nullable, collation=collation))
    ...
    def get_table_ddl(self, db_name, table_name, as_list=False, terminate_sql=False, for_replace=False):
        ...
        def check_external():
            ...
            row = self.execute_query_fetch_one(sql, log_level=VVERBOSE,
                                               query_params=[f'{db_name}.{table_name}'])
        ...
        def get_external_table_options():
            ...
            if row:
                external_table_options = {'DATA_SOURCE': self.enclose_identifier(row[0]),
        ...
        ddl_str = self._create_table_sql_text(db_name, table_name, column_list, None,
    ...
    def get_table_row_count(self, db_name, table_name, filter_clause=None,
        ...
        row = self.execute_query_fetch_one(sql, log_level=VVERBOSE, time_sql=True,
                                           not_when_dry_running=not_when_dry_running)
        return row[0] if row else None
    ...
    def get_table_size_and_row_count(self, db_name, table_name):
        ...
        row = self.execute_query_fetch_one(sql, query_params=[self.enclose_object_reference(db_name, table_name)],
                                           log_level=VVERBOSE)
        if row:
    ...
    def get_table_sort_columns(self, db_name, table_name, as_csv=True):
        ...
        rows = self.execute_query_fetch_all(sql, log_level=VVERBOSE,
                                            query_params=[db_name, table_name])
        if not rows:
            return []
        ...
        if all([_[1] for _ in column_stats]):
            ...
            for x, col_stat_name in enumerate(column_stats):
                ...
                if x == 0:
                    ...
                    stats.append(stats_row[dbcc_showstatistics_stat_header.rows.value] if stats_row[
                            dbcc_showstatistics_stat_header.rows.value] else 0)
                if stats_row:
                    stats.extend([
                        col_stat_name[0],                                                       # name
                        int(1 / stats_row[dbcc_showstatistics_stat_header.density.value]) if stats_row[
                            dbcc_showstatistics_stat_header.density.value] else 0,              # ndv
                        None,                                                                   # num_nulls
                        int(stats_row[dbcc_showstatistics_stat_header.average_key_length.value]) if stats_row[
                            dbcc_showstatistics_stat_header.average_key_length.value] else 0,   # avg_col_len
                        None,                                                                   # low_value
                        None,                                                                   # high_value
    ...
    def get_view_ddl(self, db_name, view_name, as_list=False, terminate_sql=False, for_replace=False):
        ...
        row = self.execute_query_fetch_one(sql, query_params=[f'{db_name}.{view_name}'], log_level=VVERBOSE)
        if not row:
            raise BackendApiException('View does not exist for DDL retrieval: %s.%s' % (db_name, view_name))
        ddl = row[0]
    ...
    def insert_literal_values(self, db_name, table_name, literal_list, column_list=None, max_rows_per_insert=250,
        ...
        def insert_from_union_all_select(literal_csv_list):
            ...
            insert_template = 'INSERT INTO %s ' % (self.enclose_object_reference(db_name, table_name))
        ...
        while remaining_rows:
            ...
            for row in this_chunk:
                formatted_rows.append(','.join(gen_literal(py_val, col.data_type)
    ...
    def is_valid_sort_data_type(self, data_type):
        return bool(data_type not in [SYNAPSE_TYPE_CHAR, SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_NCHAR,
                                      SYNAPSE_TYPE_NVARCHAR])
    ...
    def native_integer_types(self):
        return [SYNAPSE_TYPE_BIGINT, SYNAPSE_TYPE_INT, SYNAPSE_TYPE_SMALLINT, SYNAPSE_TYPE_TINYINT]
    ...
    def _option_clause_valid_for_sql_text(self, sql_text):
        ...
        if not sql_text:
            return False
        valid_sql_start_tokens = ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'CREATE TABLE']
    ...
    def decimal_scale_validation_expression(self, column_name, column_scale):
        ...
        """
        return ' AND '.join([f"CHARINDEX('.',{column_name}) > 0",
    ...
    def rename_table(self, from_db_name, from_table_name, to_db_name, to_table_name, sync=None):
        ...
        rename_sql = 'RENAME OBJECT {} TO {}'.format(self.enclose_object_reference(from_db_name, from_table_name),
    ...
    def sample_table_stats_partitionwise(self, db_name, table_name, sample_stats_perc, num_bytes_fudge, as_dict=False):
        raise NotImplementedError('sample_table_stats_partitionwise not supported for Synapse')
    ...
    def supported_backend_data_types(self):
        return [SYNAPSE_TYPE_BIGINT, SYNAPSE_TYPE_DECIMAL, SYNAPSE_TYPE_FLOAT, SYNAPSE_TYPE_INT, SYNAPSE_TYPE_MONEY,
                SYNAPSE_TYPE_NUMERIC, SYNAPSE_TYPE_REAL, SYNAPSE_TYPE_SMALLINT, SYNAPSE_TYPE_SMALLMONEY,
                SYNAPSE_TYPE_TINYINT,
                SYNAPSE_TYPE_DATE, SYNAPSE_TYPE_DATETIME, SYNAPSE_TYPE_DATETIME2, SYNAPSE_TYPE_SMALLDATETIME,
                SYNAPSE_TYPE_DATETIMEOFFSET, SYNAPSE_TYPE_TIME,
                SYNAPSE_TYPE_CHAR, SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_NCHAR, SYNAPSE_TYPE_NVARCHAR,
                SYNAPSE_TYPE_BINARY, SYNAPSE_TYPE_UNIQUEIDENTIFIER, SYNAPSE_TYPE_VARBINARY]
    ...
    def synapse_external_data_source_exists(self, external_data_source_name):
        ...
        sql = dedent("""\
                    ...
                    WHERE  name = ?""")
        row = self.execute_query_fetch_one(sql, log_level=VVERBOSE,
                                           query_params=[external_data_source_name])
        return bool(row)
    ...
    def transform_translate_expression(self, backend_column, from_string, to_string):
        return 'TRANSLATE(%s, %s, %s)' % (self.enclose_identifier(backend_column.name), from_string, to_string)
    ...
    def valid_canonical_override(self, column, canonical_override):
        ...
        if column.is_number_based():
            ...
            if column.data_type in [SYNAPSE_TYPE_CHAR, SYNAPSE_TYPE_NCHAR]:
                return bool(target_type in STRING_CANONICAL_TYPES
    ...
    def to_canonical_column(self, column):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, safe_mapping=None,
                       char_length=None, char_semantics=None):
            ...
            return CanonicalColumn(col.name, data_type, data_length=data_length, data_precision=data_precision,
                                   ...
                                   safe_mapping=safe_mapping, char_length=char_length, partition_info=None,
                                   bucket_info=None, char_semantics=char_semantics)

        assert column
        assert isinstance(column, SynapseColumn)

        if column.data_type == SYNAPSE_TYPE_CHAR:
            return new_column(column, GLUENT_TYPE_FIXED_STRING, data_length=column.data_length,
                              char_length=column.char_length, char_semantics=CANONICAL_CHAR_SEMANTICS_BYTE,
                              safe_mapping=True)
        elif column.data_type == SYNAPSE_TYPE_NCHAR:
            return new_column(column, GLUENT_TYPE_FIXED_STRING, data_length=column.data_length,
                              char_length=column.char_length, char_semantics=CANONICAL_CHAR_SEMANTICS_CHAR,
                              safe_mapping=True)
            ...
            return new_column(column, GLUENT_TYPE_DECIMAL, data_precision=column.data_precision or 19,
                              data_scale=column.data_scale or 4)
        elif column.data_type == SYNAPSE_TYPE_SMALLMONEY:
            return new_column(column, GLUENT_TYPE_DECIMAL, data_precision=column.data_precision or 10,
                              data_scale=column.data_scale or 4)
    ...
    def from_canonical_column(self, column, decimal_padding_digits=0):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None,
                       char_length=None, safe_mapping=None):
            safe_mapping = is_safe_mapping(col.safe_mapping, safe_mapping)
            return SynapseColumn(col.name, data_type=data_type, data_length=data_length, char_length=char_length,
                                 data_precision=data_precision, data_scale=data_scale, nullable=col.nullable,
                                 data_default=col.data_default, char_semantics=col.char_semantics,
                                 safe_mapping=safe_mapping)
        ...
        def nchar_or_char(data_type, char_semantics):
            ...
            elif data_type == SYNAPSE_TYPE_VARCHAR and char_semantics == CANONICAL_CHAR_SEMANTICS_UNICODE:
        ...
        if column.data_type == GLUENT_TYPE_FIXED_STRING:
            return new_column(column,
                              ...
                              char_length=column.char_length, safe_mapping=True)
        ...
        # Synapse tinyint is unsigned, so cannot cater for all INTEGER_1 values; map to smallint
            ...
            return new_column(column, SYNAPSE_TYPE_NUMERIC, data_precision=38, data_scale=0, safe_mapping=True)
            ...
            if column.data_precision is None and column.data_scale is None:
                ...
                return new_column(column, SYNAPSE_TYPE_NUMERIC, data_precision=data_precision,
            ...
            data_scale = (column.data_scale if (column.data_scale or 0) < self.max_datetime_scale()
                          else self.max_datetime_scale())
            return new_column(column, SYNAPSE_TYPE_TIME, data_scale=data_scale, safe_mapping=safe_mapping)
        elif column.data_type == GLUENT_TYPE_TIMESTAMP:
            safe_mapping = bool(column.data_scale is None or column.data_scale <= self.max_datetime_scale())
            data_scale = (column.data_scale if (column.data_scale or 0) < self.max_datetime_scale()
                          else self.max_datetime_scale())
            return new_column(column, SYNAPSE_TYPE_DATETIME2, data_scale=data_scale, safe_mapping=safe_mapping)
        elif column.data_type == GLUENT_TYPE_TIMESTAMP_TZ:
            safe_mapping = bool(column.data_scale is None or column.data_scale <= self.max_datetime_scale())
            data_scale = (column.data_scale if (column.data_scale or 0) < self.max_datetime_scale()
                          else self.max_datetime_scale())
            return new_column(column, SYNAPSE_TYPE_DATETIMEOFFSET, data_scale=data_scale, safe_mapping=safe_mapping)
