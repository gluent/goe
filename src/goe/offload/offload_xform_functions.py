...
def apply_transformation(column_transformations, column, db_api, messages):
    ...
    if column.name.lower() in column_transformations:
        ...
        messages.log('Found transformation for %s: %s' % (column.name, col_trans), detail=VVERBOSE)
        if col_trans['transformation'] == 'null':
            return_expr = db_api.transform_null_cast(column)
        elif col_trans['transformation'] == 'suppress':
            messages.log('Transformation applied: %s' % col_trans['transformation'], detail=VVERBOSE)
            return None
            ...
            return_expr = db_api.transform_regexp_replace_expression(column, col_trans['params'][0], col_trans['params'][1])
...
def transformations_to_metadata(column_transformations):
    ...
    for colname in column_transformations:
        if column_transformations[colname]['params']:
            xform = '%s(%s)' % (column_transformations[colname]['transformation'], ','.join(column_transformations[colname]['params']))
        else:
