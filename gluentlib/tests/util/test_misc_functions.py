...
from gluentlib.util.misc_functions import add_prefix_in_same_case, add_suffix_in_same_case, \
    case_insensitive_in, substitute_in_same_case, backtick_sandwich, bytes_to_human_size, dict_to_namespace,\
    format_list_for_logging, get_integral_part_magnitude, get_temp_path, human_size_to_bytes,\
    is_number, is_pos_int, is_power_of_10, remove_chars, split_not_in_quotes, standard_file_name,\
    standard_log_name, trunc_with_hash, truncate_number, wildcard_matches_in_list
...
class TestMiscFunctions(TestCase):
    ...
    def test_truncate_number(self):
        for n, digits, expected_result in [
            ...
            (int('9' * 38), 0, int('9' * 38)),
            (decimal.Decimal('1998849133104929799110640.00301078'), 0, decimal.Decimal('1998849133104929799110640')),
            (decimal.Decimal('-1998849133104929799110640.00301078'), 0, decimal.Decimal('-1998849133104929799110640')),
            ...
            (decimal.Decimal('1998849133104929799110640.00301078'), -2, decimal.Decimal('1998849133104929799110600')),
            (decimal.Decimal('-1998849133104929799110640.00301078'), -2, decimal.Decimal('-1998849133104929799110600')),
            (decimal.Decimal('1998849133104929799110640.00301078'), -10, decimal.Decimal('1998849133104920000000000')),
            (decimal.Decimal('-1998849133104929799110640.00301078'), -10, decimal.Decimal('-1998849133104920000000000')),
            ...
            self.assertEqual(truncate_number(n, digits), expected_result,
            ...
            if not isinstance(n, decimal.Decimal):
                ...
                self.assertEqual(truncate_number(dn, digits), dexpected,
                                 f'Test input: truncate_number({dn}, {digits})')
            if digits == 0:
                self.assertEqual(truncate_number(n), expected_result,
                                 f'Test input: truncate_number({n}, {digits})')
    ...
    def test_split_not_in_quotes(self):
        ...
        for to_split, sep, exclude_empty_tokens, should_match in assert_tuples:
            self.assertEqual(split_not_in_quotes(to_split, sep, exclude_empty_tokens=exclude_empty_tokens),
                             should_match)
    ...
    def test_format_list_for_logging(self):
        test_input = [('Header1', 'LongerHeader'),
                      ('Blah', 123)]
    ...
    def test_remove_chars(self):
        self.assertIsNone(remove_chars(None, 'ace'), )
    ...
    def test_standard_file_name(self):
        ...
        self.assertEqual(standard_file_name(std_prefix, name_suffix='more', extension='.txt'), std_prefix + '_more.txt')
        # The timestamp will add 22 characters, plus 4 for the extension and 5 for the hash, therefore trim above 31.
        self.assertEqual(len(standard_file_name(std_prefix, max_name_length=45, with_datetime=True)), 45)
        self.assertEqual(len(standard_file_name(std_prefix, max_name_length=45, with_datetime=True,
                                                max_name_length_extra_slack=5)), 40)
        # Length 50 output will have at least 10 characters of the original name
        self.assertTrue(standard_file_name(std_prefix, max_name_length=45, with_datetime=True).startswith(std_prefix[:10]))
        # Length 50 output will NOT have 20 characters of the original name
        self.assertFalse(standard_file_name(std_prefix, max_name_length=45, with_datetime=True).startswith(std_prefix[:20]))
    ...
    def test_wildcard_matches_in_list(self):
        ...
        self.assertListEqual(wildcard_matches_in_list('*_id', list_of_names, case_sensitive=False),
                             ['UPPER_ID', 'lower_id'])
        self.assertListEqual(wildcard_matches_in_list('*_year', list_of_names), ['lower_year', 'lower2_year'])
        self.assertEqual(len(wildcard_matches_in_list('*_year', list_of_names, case_sensitive=False)), 4)
        self.assertEqual(len(wildcard_matches_in_list('*_*', list_of_names)), len(list_of_names))
        self.assertEqual(len(wildcard_matches_in_list('*_*', list_of_names, case_sensitive=False)), len(list_of_names))
