...
TERADATA_STRING_DATA_TYPES = [TERADATA_TYPE_CHAR, TERADATA_TYPE_CLOB, TERADATA_TYPE_VARCHAR]

TERADATA_INTERVAL_DATA_TYPES = [
    TERADATA_TYPE_INTERVAL_YR, TERADATA_TYPE_INTERVAL_YM, TERADATA_TYPE_INTERVAL_MO,
    TERADATA_TYPE_INTERVAL_DY, TERADATA_TYPE_INTERVAL_DH, TERADATA_TYPE_INTERVAL_DM,
    TERADATA_TYPE_INTERVAL_DS, TERADATA_TYPE_INTERVAL_HR, TERADATA_TYPE_INTERVAL_HM,
    TERADATA_TYPE_INTERVAL_HS, TERADATA_TYPE_INTERVAL_MI, TERADATA_TYPE_INTERVAL_MS,
...
class TeradataColumn(ColumnMetadataInterface):
    ...
    def __init__(self, name, data_type, data_length=None, data_precision=None, data_scale=None,
                 nullable=None, data_default=None, char_length=None, safe_mapping=True,
                 partition_info=None, char_semantics=None):
        super().__init__(name, data_type, data_length, data_precision, data_scale, nullable,
                         data_default, safe_mapping=safe_mapping, partition_info=partition_info,
                         char_length=char_length, char_semantics=char_semantics)
    ...
    def from_teradata(attribute_list):
        ...
        """
        _, name, nullable, data_type, data_precision, data_scale, data_length, data_default = attribute_list
        ...
        col = TeradataColumn(
            name, data_type, data_length=data_length, data_precision=data_precision, data_scale=data_scale,
            nullable=nullable, data_default=data_default, char_length=char_length)
        return col
    ...
    def format_data_type(self):
        ...
        if self.data_type in [TERADATA_TYPE_DECIMAL, TERADATA_TYPE_NUMBER]:
            if self.data_precision and self.data_scale is not None:
                return '%s(%s,%s)' % (data_type_name, self.data_precision, self.data_scale)
            ...
            TERADATA_TYPE_BLOB, TERADATA_TYPE_BYTE, TERADATA_TYPE_VARBYTE, TERADATA_TYPE_CHAR, TERADATA_TYPE_VARCHAR,
        ]:
        ...
        elif self.data_type == TERADATA_TYPE_INTERVAL_DS and self.data_precision is not None and self.data_scale is not None:
            return TERADATA_INTERVAL_DS_SUB_PATTERN % (self.data_precision or 2, self.data_scale or 6)
        elif self.data_type == TERADATA_TYPE_INTERVAL_YM and self.data_precision is not None:
    ...
    def has_time_element(self):
        ...
        return bool(self.data_type in [TERADATA_TYPE_TIME, TERADATA_TYPE_TIME_TZ,
                                       TERADATA_TYPE_TIMESTAMP, TERADATA_TYPE_TIMESTAMP_TZ])
    ...
    def is_number_based(self):
        ...
        """
        return bool(self.data_type in [TERADATA_TYPE_DECIMAL, TERADATA_TYPE_NUMBER, TERADATA_TYPE_BIGINT,
                                       TERADATA_TYPE_INTEGER, TERADATA_TYPE_SMALLINT,
                                       TERADATA_TYPE_BYTEINT, TERADATA_TYPE_DOUBLE])
