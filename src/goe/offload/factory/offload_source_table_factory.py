...
from goe.offload.offload_constants import DBTYPE_MSSQL, DBTYPE_NETEZZA, DBTYPE_ORACLE, DBTYPE_TERADATA
...
class OffloadSourceTable(object):
    ...
    """
    @staticmethod
    def create(schema_name, table_name, connection_options, messages, dry_run=False, offload_by_subpartition=False,
               conn=None, do_not_connect=False):
        ...
        if connection_options.db_type == DBTYPE_ORACLE:
            ...
            rdbms_table = OracleSourceTable(schema_name, table_name, connection_options, messages,
                                            dry_run=dry_run, conn=conn, do_not_connect=do_not_connect)
            ...
            return MSSQLSourceTable(schema_name, table_name, connection_options, messages,
            ...
            return NetezzaSourceTable(schema_name, table_name, connection_options, messages,
                                      dry_run=dry_run, do_not_connect=do_not_connect)
        elif connection_options.db_type == DBTYPE_TERADATA:
            from goe.offload.teradata.teradata_offload_source_table import TeradataSourceTable
            return TeradataSourceTable(schema_name, table_name, connection_options, messages,
                                       dry_run=dry_run, do_not_connect=do_not_connect)
