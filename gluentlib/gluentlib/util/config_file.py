...
class GluentConfig(SafeConfigParser):
    ...
    def extract_options(self, section, required_options, error_if_not_set=False):
        ...
        for option in required_options:
            ...
            if not option_value and error_if_not_set:
                raise ConfigException("Please, define %s.%s configuration parameter" % (section, option))
            else:
    ...
    def get(self, section, option, missing_replacement=None, **kwargs):
        ...
        if not self.has_option(section, option):
            logger.debug("Config Option: %s.%s does NOT exist. get() returns 'replacement': %s" % \
    ...
    def getint(self, section, option, missing_replacement=None):
        ...
        if not self.has_option(section, option):
            logger.debug("Config Option: %s.%s does NOT exist. getint() returns 'replacement': %s" % \
                (section, option, missing_replacement))
            return missing_replacement
...
class GluentRemoteConfig(GluentConfig):
    ...
    def default_config(cls):
        ...
        if DEFAULT_REMOTE_CONFIG_OSVAR not in os.environ:
            raise ConfigException("%s environment variable is NOT set" % DEFAULT_REMOTE_CONFIG_OSVAR)
    ...
    def section_type(self, section):
        ...
        elif all([self.get(section, _) for _ in ('hdfs_root', 'hdfs_url', 'webhdfs_url')]):
    ...
    def base_url(self, section):
        ...
        if SECTION_TYPE_S3 == s_type:
            ...
            raise ConfigException("Base URL for section type: %s is NOT supported" % s_type)
    ...
    def base_root(self, section):
        ...
        if SECTION_TYPE_S3 == s_type:
            ...
            raise ConfigException("Base ROOT for section type: %s is NOT supported" % s_type)
