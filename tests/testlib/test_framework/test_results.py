...
# EXCEPTIONS
###############################################################################
class TestResultsException(Exception): pass
...
class TestResults(object):
    ...
    def __init__(self, root_cause_file, db_type=None):
        ...
        self._agg_results = {STATE_PASS: {}, STATE_FAIL: {}, STATE_ERROR: {}} # Aggregated results
        self._agg_stats = {}                                                  # Aggregated statistics

        self._root_cause_config = root_cause_file # (YAML) configuration file with root cause definition
        self._db_type = db_type.upper()           # 'Db type' marker (to upload additional section from config)

        # Load root causes from config file specific to 'db type'
        self._root_causes = self._load_root_causes(self._root_cause_config, self._db_type)
    ...
    def _calculate_statistics(self, agg_results):
        ...
        for _ in allowed_states():
            agg_stats[_] = sum(1 for rc in agg_results[_] for test in agg_results[_][rc])
    ...
    def _make_state_report(self, tests, report_level):
        ...
        for rc, tts in sorted(list(tests.items()), key=lambda x: len(x[1]), reverse=True):
    ...
    def _report_state_details(self, state, report_level, rc_filter, name_filter):
            ...
            for root_cause in results:
                for test_name in results[root_cause]:
                    if regex_name.search(test_name):
                        ...
                        new_results[root_cause][test_name] = results[root_cause][test_name]
        ...
        printable_headers = {STATE_PASS: "PASSED", STATE_FAIL: "FAILED", STATE_ERROR: "EXCEPTION"}

        agg_results = self._agg_results[state]
        if rc_filter:
            agg_results = filter_by_rc(agg_results, rc_filter)
        logger.debug("Root causes left after applying rc filter: %s for state: %s" % (list(agg_results.keys()), state))
        if name_filter:
    ...
    def aggregate(self):
        ...
        for test in self._raw_results:
            ...
            logger.debug("Aggregating test: %s to state: %s, root cause: %s" % \
            ...
            if STATE_PASS == state:
                root_cause = 'OK' # Default (and the only) 'root cause' for 'good' tests
    ...
    def report_details(self, states, report_level, rc_filter=None, name_filter=None):
        ...
        for state in states:
            ...
            if state not in allowed_states():
                ...
                report_text += self._report_state_details(state, report_level, rc_filter, name_filter)
...
def normalize_state(raw_state):
    ...
    if state.startswith('pass'):
        ...
        logger.warn("Unrecognized state: %s. Returning: %s" % (raw_state, STATE_UNKNOWN))
