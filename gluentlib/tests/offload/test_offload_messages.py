...
from gluentlib.offload.offload_messages import OffloadMessages, OffloadMessagesForcedException,\
    VERBOSE, VVERBOSE
...
class TestOffloadMessages(TestCase):
    ...
    def test_offload_step(self):
        ...
        with open('/dev/null', 'a') as fh:
            ...
            messages = OffloadMessages.from_options(FakeOpts(), log_fh=fh)
            messages.offload_step(command_steps.STEP_MESSAGES, do_nothing,
                                  command_type=orchestration_constants.COMMAND_TEST)
            messages.offload_step(command_steps.STEP_UNITTEST_SKIP, do_nothing,
                                  command_type=orchestration_constants.COMMAND_TEST)
            messages.offload_step(command_steps.STEP_UNITTEST_ERROR_BEFORE, do_nothing,
                                  command_type=orchestration_constants.COMMAND_TEST)
            messages.offload_step(command_steps.STEP_UNITTEST_ERROR_AFTER, do_nothing,
                                  command_type=orchestration_constants.COMMAND_TEST)
            ...
            messages = OffloadMessages.from_options(FakeOpts(), log_fh=fh)
            messages.setup_offload_step(skip=[skip_title.replace(' ', '_').lower()],
            ...
            self.assertRaises(OffloadMessagesForcedException,
                              lambda: messages.offload_step(command_steps.STEP_UNITTEST_ERROR_BEFORE, do_nothing,
                                                            command_type=orchestration_constants.COMMAND_TEST))
            self.assertNotIn(error_before_title, messages.steps)
            self.assertRaises(OffloadMessagesForcedException,
                              lambda: messages.offload_step(command_steps.STEP_UNITTEST_ERROR_AFTER, do_nothing,
                                                            command_type=orchestration_constants.COMMAND_TEST))
            ...
            self.assertRaises(ZeroDivisionError,
                              lambda: messages.offload_step(command_steps.STEP_MESSAGES, do_divzero,
                                                            command_type=orchestration_constants.COMMAND_TEST))
