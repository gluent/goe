...
from gluentlib.data_governance.hadoop_data_governance_constants import SUPPORTED_DATA_GOVERNANCE_BACKENDS
from gluentlib.data_governance.hadoop_data_governance import is_valid_data_governance_tag, \
    is_valid_data_governance_auto_property
from gluentlib.filesystem.gluent_dfs import AZURE_OFFLOAD_FS_SCHEMES, OFFLOAD_FS_SCHEME_MAPRFS, \
    VALID_OFFLOAD_FS_SCHEMES, OFFLOAD_FS_SCHEMES_REQUIRING_CONTAINER
from gluentlib.offload.microsoft.synapse_backend_api import SYNAPSE_AUTH_MECHANISM_AD_SERVICE_PRINCIPAL, \
    SYNAPSE_USER_PASS_AUTH_MECHANISMS, SYNAPSE_VALID_AUTH_MECHANISMS
from gluentlib.offload.offload_constants import BACKEND_DISTRO_GCP, BACKEND_DISTRO_MAPR, \
    BACKEND_DISTRO_SNOWFLAKE, BACKEND_DISTRO_MSAZURE,\
    DBTYPE_HIVE, DBTYPE_IMPALA, DBTYPE_MSSQL, DBTYPE_NETEZZA,\
    DBTYPE_ORACLE, DBTYPE_TERADATA, HADOOP_BASED_BACKEND_DISTRIBUTIONS
from gluentlib.offload.offload_transport import derive_rest_api_verify_value_from_url, \
    SPARK_SUBMIT_SPARK_YARN_MASTER, VALID_OFFLOAD_TRANSPORTS, VALID_SPARK_SUBMIT_EXECUTABLES
...
def verify_json_option(option_name, option_value):
    if option_value:
        try:
            ...
            if invalid_props:
                prop_details = '\n'.join(f'Invalid property value for key/value pair: {k}: {properties[k]}'
                                         for k in invalid_props)
                raise OrchestrationConfigException(
                    'Invalid property value in {} for keys: {}\n{}'.format(option_name, str(invalid_props),
                                                                           prop_details)
                )
...
def normalise_bigquery_options(options, exc_cls=OrchestrationConfigException):
    ...
    if options.google_dataproc_batches_version:
        if not re.match(r'^[1-9]\.[0-9](\.[0-9])?$', options.google_dataproc_batches_version):
            raise exc_cls(f'Invalid value for GOOGLE_DATAPROC_BATCHES_VERSION: {options.google_dataproc_batches_version}')
...
def normalise_data_governance_config(options):
    if options.data_governance_api_url:
        ...
        if options.data_governance_backend not in SUPPORTED_DATA_GOVERNANCE_BACKENDS:
            raise OrchestrationConfigException('Invalid DATA_GOVERNANCE_BACKEND %s. Valid values are: %s'
                                               % (options.data_governance_backend, SUPPORTED_DATA_GOVERNANCE_BACKENDS))
        if options.data_governance_backend == 'navigator' and not options.cloudera_navigator_hive_source_id:
            raise OrchestrationConfigException(
                '--cloudera-navigator-hive-source-id is required for Cloudera Navigator integration')

        tag_list = options.data_governance_auto_tags_csv.split(',') if options.data_governance_auto_tags_csv else []
        ...
        if invalid_auto_tags:
            ...
            raise OrchestrationConfigException('Invalid values for DATA_GOVERNANCE_AUTO_TAGS: %s' % invalid_auto_tags)

        auto_properties = options.data_governance_auto_properties_csv.split(
            ',') if options.data_governance_auto_properties_csv else []
        invalid_auto_props = [_ for _ in auto_properties if not is_valid_data_governance_auto_property(_)]
...
def normalise_filesystem_options(options, exc_cls=OrchestrationConfigException):
    options.offload_fs_scheme = options.offload_fs_scheme.lower() if options.offload_fs_scheme else options.offload_fs_scheme
    ...
    if options.offload_fs_scheme == OFFLOAD_FS_SCHEME_MAPRFS and options.backend_distribution != BACKEND_DISTRO_MAPR:
        raise exc_cls('OFFLOAD_FS_SCHEME/--offload-fs-scheme %s not valid for this backend system'
...
def normalise_listener_options(options):
    ...
    if options.listener_heartbeat_interval:
        options.listener_heartbeat_interval = orchestration_defaults.posint_option_from_string(
            'OFFLOAD_LISTENER_HEARTBEAT_INTERVAL', options.listener_heartbeat_interval)
    if options.password_key_file:
        ...
        if options.listener_shared_token:
            options.listener_shared_token = pass_tool.b64decrypt(options.listener_shared_token, gl_key)
        if options.listener_redis_password:
            options.listener_redis_password = pass_tool.b64decrypt(options.listener_redis_password, gl_key)
...
def normalise_offload_transport_config(options, exc_cls=OrchestrationConfigException):
    def simple_file_csv(var_val, var_name):
        ...
        if not re.match(r'^[a-z0-9\/_\-:.,]+$', var_val):
            raise exc_cls(
                f'Invalid characters found in {var_name}: {var_val}'
            )

    # Spark config
    verify_json_option('OFFLOAD_TRANSPORT_RDBMS_SESSION_PARAMETERS',
                       options.offload_transport_rdbms_session_parameters)

    if options.offload_transport_spark_submit_executable and options.offload_transport_spark_submit_executable not in VALID_SPARK_SUBMIT_EXECUTABLES:
        raise exc_cls('Invalid value for OFFLOAD_TRANSPORT_SPARK_SUBMIT_EXECUTABLE: %s'
    ...
    # Livy config
    if type(options.offload_transport_livy_api_verify_ssl) is not bool and options.offload_transport_livy_api_verify_ssl is not None:
...
def normalise_rdbms_oracle_options(options, exc_cls=OrchestrationConfigException):
    ...
    elif not options.use_oracle_wallet and (not options.ora_adm_user or not options.ora_adm_pass
                                            or not options.ora_app_user or not options.ora_app_pass):
        raise exc_cls(
    ...
    pass_tool = PasswordTools()
    gl_key = pass_tool.get_password_key_from_key_file(options.password_key_file)
    options.rdbms_app_pass = options.ora_app_pass = pass_tool.b64decrypt(options.rdbms_app_pass, gl_key)
...
def normalise_netezza_options(options, exc_cls=OrchestrationConfigException):
    if not options.netezza_app_user or not options.netezza_app_pass or not options.netezza_dsn:
...
def normalise_teradata_options(options, exc_cls=OrchestrationConfigException):
    ...
    pass_tool = PasswordTools()
    gl_key = pass_tool.get_password_key_from_key_file(options.password_key_file)
    options.rdbms_app_pass = options.teradata_app_pass = pass_tool.b64decrypt(options.teradata_app_pass, gl_key)
...
def normalise_webhdfs(options):
    ...
    options.webhdfs_port = (options.webhdfs_port
...
def check_offload_fs_scheme_supported_in_backend(offload_fs_scheme, backend_api):
    if offload_fs_scheme not in backend_api.valid_offload_fs_schemes():
        ...
        raise OrchestrationConfigException('Offload to %s%s does not support filesystem scheme: %s'
