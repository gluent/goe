...
class SynapseColumn(ColumnMetadataInterface):
    ...
    def __init__(self, name, data_type, data_length=None, data_precision=None, data_scale=None, nullable=None,
                 ...
                 char_semantics=None, collation=None):

        super(SynapseColumn, self).__init__(name, data_type, data_length=data_length, data_precision=data_precision,
    ...
    def format_data_type(self):
        ...
        elif self.data_type in [SYNAPSE_TYPE_CHAR, SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_BINARY,
                                SYNAPSE_TYPE_VARBINARY, SYNAPSE_TYPE_NCHAR, SYNAPSE_TYPE_NVARCHAR]:
            if self.data_type in [SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_VARBINARY, SYNAPSE_TYPE_NVARCHAR] and not self.data_length:
                return '%s(%s)' % (self.data_type, SYNAPSE_TYPE_MAX_TOKEN)
            elif self.char_length and self.char_semantics in [CANONICAL_CHAR_SEMANTICS_CHAR,
                ...
                return self.data_type
        elif (self.is_date_based() or self.data_type == SYNAPSE_TYPE_TIME) and self.data_scale is not None:
    ...
    def has_time_element(self):
        ...
        return bool(self.data_type in [SYNAPSE_TYPE_DATETIME, SYNAPSE_TYPE_DATETIME2, SYNAPSE_TYPE_DATETIMEOFFSET,
                                       SYNAPSE_TYPE_SMALLDATETIME, SYNAPSE_TYPE_TIME])
    ...
    def is_number_based(self):
        ...
        return bool(self.data_type in [SYNAPSE_TYPE_BIT, SYNAPSE_TYPE_DECIMAL, SYNAPSE_TYPE_NUMERIC, SYNAPSE_TYPE_INT,
                                       SYNAPSE_TYPE_BIGINT, SYNAPSE_TYPE_SMALLINT, SYNAPSE_TYPE_TINYINT,
                                       SYNAPSE_TYPE_MONEY,
                                       SYNAPSE_TYPE_SMALLMONEY, SYNAPSE_TYPE_FLOAT, SYNAPSE_TYPE_REAL])

    def is_date_based(self):
        """ Is the column date in class """
        return bool(self.data_type in [SYNAPSE_TYPE_DATE, SYNAPSE_TYPE_DATETIME, SYNAPSE_TYPE_DATETIME2,
                                       SYNAPSE_TYPE_DATETIMEOFFSET, SYNAPSE_TYPE_SMALLDATETIME])
    ...
    def is_string_based(self):
        ...
        return bool(self.data_type in [SYNAPSE_TYPE_CHAR, SYNAPSE_TYPE_VARCHAR, SYNAPSE_TYPE_NCHAR,
