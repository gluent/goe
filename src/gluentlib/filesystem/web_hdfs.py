...
from gluentlib.filesystem.gluent_dfs import GluentDfs, GluentDfsDeleteNotComplete, GluentDfsException,\
    gen_fs_uri,\
...
class WebHdfs(GluentDfs):
    ...
    def __init__(self, hdfs_namenode, webhdfs_port, hdfs_user=None, use_kerberos=False, verify_ssl_cert=None,
                 dry_run=False, messages=None, do_not_connect=False, db_path_suffix=None, hdfs_data=None, **kwargs):
        assert hdfs_namenode
        assert webhdfs_port
        assert (hdfs_user or use_kerberos or verify_ssl_cert is not None)

        logger.info('Client setup: (%s, %s, %s, %s, %s)' % (hdfs_namenode, webhdfs_port, hdfs_user,
                                                            use_kerberos, verify_ssl_cert))
    ...
    def _hdfs_client(self, hdfs_namenode, webhdfs_port, hdfs_user, use_kerberos, verify_ssl_cert=None, **kwargs):
        ...
        if len(namenodes) > 1:
            ...
            for namenode in namenodes:
                ...
                namenode_url = '{protocol}://{name_node}:{port}/jmx?qry=Hadoop:service=NameNode,name=NameNodeStatus'.format(protocol=http_protocol(verify_ssl_cert),
                                                                                                                            name_node=namenode,
                                                                                                                            port=webhdfs_port)
                try:
                        ...
                        try:
                            ...
                            if beans:
                                if beans[0]['State'].lower() == 'active':
                                    self.debug('Namenode {name_node}, status: active'.format(name_node=namenode))
                                    self._url = '%s://%s:%s' % (http_protocol(verify_ssl_cert), namenode, webhdfs_port)
                                    break
                            else:
                                self.notice('WebHDFS active namenode check failed for {name_node} with empty [beans]'.format(name_node=namenode))
                                self.debug('Namenode {name_node} response: {json}'.format(name_node=namenode, json=response.json()))
                        except KeyError:
                            self.notice('WebHDFS active namenode check failed for {name_node} with unknown response'.format(name_node=namenode))
                            self.debug('Namenode {name_node} status unknown: KeyError: {json}'.format(name_node=namenode, json=response.json()))
                    else:
                        self.debug('Namenode {name_node} status code: {code}'.format(name_node=namenode, code=response.status_code))
                except (ConnectionError, MutualAuthenticationError) as exc:
                    self.notice('WebHDFS active namenode check failed for {name_node} with connection error'.format(name_node=namenode))
                    self.debug('Namenode {name_node} connection error: {message}'.format(name_node=namenode, message=str(exc)))
            if getattr(self, "_url", None) is None:
                self.warning('Multiple WebHDFS namenodes detected, unable to determine active node, defaulting to {name_node}'.format(name_node=namenodes[0]))
                self._url = '%s://%s:%s' % (http_protocol(verify_ssl_cert), namenodes[0], webhdfs_port)
        else:
            self._url = '%s://%s:%s' % (http_protocol(verify_ssl_cert), hdfs_namenode, webhdfs_port)
    ...
    def gen_uri(self, scheme, container, path_prefix, backend_db=None, table_name=None, container_override=None):
        if not scheme or scheme == OFFLOAD_FS_SCHEME_INHERIT:
            # Fall back to hdfs_data for backward compatibility
            uri = gen_fs_uri(self._hdfs_data,
