...
from gluentlib.offload.format_literal import FormatLiteralInterface, RE_TIMEZONE_NO_COLON
from gluentlib.offload.microsoft.synapse_column import SYNAPSE_TYPE_BINARY, \
    SYNAPSE_TYPE_DATE, SYNAPSE_TYPE_DATETIME, SYNAPSE_TYPE_DATETIME2, SYNAPSE_TYPE_DATETIMEOFFSET,\
    SYNAPSE_TYPE_NCHAR, SYNAPSE_TYPE_NVARCHAR, SYNAPSE_TYPE_SMALLDATETIME, SYNAPSE_TYPE_TIME,\
    ...
    def format_literal(cls, python_value, data_type=None):
        ...
        def format_date_for_data_type(str_val, data_type):
            if data_type == SYNAPSE_TYPE_DATE:
                return "'%s'" % str_val[:10]
            elif data_type in [SYNAPSE_TYPE_DATETIME, SYNAPSE_TYPE_DATETIME2, SYNAPSE_TYPE_SMALLDATETIME]:
                return "'%s'" % cls._strip_unused_time_scale(str_val)
            elif data_type == SYNAPSE_TYPE_DATETIMEOFFSET and re.match(RE_TIMEZONE_NO_COLON, str_val):
        ...
        if isinstance(python_value, datetime64):
            ...
            if data_type == SYNAPSE_TYPE_DATE:
                ...
                if not python_value.tzinfo:
                    ...
                    new_py_val = format_date_for_data_type(python_value.strftime("%Y-%m-%d %H:%M:%S.%f +00:00"),
                                                           data_type)
                else:
                    # Synapse only understands HH:MM time zone offset, not named time zones
                    new_py_val = format_date_for_data_type(python_value.strftime("%Y-%m-%d %H:%M:%S.%f %z"),
                                                           data_type)
                ...
                # Assuming DATETIME if no data_type specified
                new_py_val = format_date_for_data_type(python_value.strftime("%Y-%m-%d %H:%M:%S.%f"),
