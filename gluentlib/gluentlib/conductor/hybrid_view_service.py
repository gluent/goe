...
from gluentlib.offload.offload_constants import LOG_LEVEL_DEBUG
from gluentlib.offload.offload_messages import OffloadMessages, VVERBOSE
from gluentlib.offload.offload_validation import CrossDbValidator, HybridCountValidator,\
...
class HybridViewServiceException(Exception): pass
...
JSON_ALL_BACKEND_KEYS = [JSON_KEY_BACKEND_NUM_ROWS,
...
class HybridViewService(object):
    ...
    def __init__(self, hybrid_schema, hybrid_view, dry_run=False, existing_log_fh=None):
        ...
        self._offload_metadata = OrchestrationMetadata.from_name(hybrid_schema, hybrid_view,
    ...
    def _backend_table_does_not_exist_exception(self):
        raise HybridViewServiceException('Backend table does not exist: %s.%s'
                                         % (self._backend_table_owner, self._backend_table_name))

    def _bind_in_predicates(self):
        return bool(self._ipa_predicate_type in [INCREMENTAL_PREDICATE_TYPE_RANGE,
    ...
    def _get_backend_api(self):
        ...
        return backend_api_factory(self._connection_options.target, self._connection_options,
    ...
    def _get_backend_table(self):
        ...
        return backend_table_factory(self._backend_table_owner, self._backend_table_name,
                                     self._connection_options.target, self._connection_options,
    ...
    def _get_backend_detail(self, attribute_name=None):
        ...
        if not self._backend_info:
            ...
            self._backend_info = {'columns': backend_table.get_columns(),
                                  JSON_KEY_BACKEND_SIZE: size_in_bytes,
                                  JSON_KEY_BACKEND_NUM_ROWS: num_rows,
    ...
    def _get_frontend_table(self):
        ...
        return OffloadSourceTable.create(self._frontend_table_owner, self._frontend_table_name,
    ...
    def _get_frontend_detail(self, attribute_name):
        ...
        if not self._frontend_info:
            ...
            frontend_table = OffloadSourceTable.create(
                self._frontend_table_owner, self._frontend_table_name, self._connection_options,
                self._messages, offload_by_subpartition=self._offload_metadata.is_subpartition_offload()
            )
    ...
    def validate_by_aggregation(self, lower_hv=None, upper_hv=None, as_json=True):
        ...
        if not self._frontend_table_owner or not self._frontend_table_name:
            raise HybridViewServiceException('Hybrid view is not for an offloaded table: %s.%s'
        ...
        if self._ipa_predicate_type == INCREMENTAL_PREDICATE_TYPE_LIST and upper_hvs:
            ...
            upper_hvs = [upper_hvs]
        frontend_filters, query_binds = build_verification_clauses(frontend_table, self._ipa_predicate_type,
                                                                   upper_hvs, prior_hvs,
                                                                   with_binds=self._bind_in_predicates())
        self._debug('Generated frontend filters:\n%r' % frontend_filters)
        backend_filters, _ = build_verification_clauses(frontend_table, self._ipa_predicate_type,
                                                        upper_hvs, prior_hvs,
                                                        with_binds=False, backend_table=backend_table)
        self._debug('Generated backend filters:\n%r' % backend_filters)

        validator = CrossDbValidator(self._frontend_table_owner, self._frontend_table_name, self._connection_options,
                                     backend_table.get_backend_api(), messages=self._messages,
                                     backend_db=self._backend_table_owner, backend_table=self._backend_table_name)
        status, agg_msg = validator.validate(safe=False, filters=backend_filters, execute=bool(not self._dry_run),
    ...
    def validate_by_count(self, lower_hv=None, upper_hv=None, as_json=True):
        """ Return response from HybridCountValidator as JSON: {'success': True/False, 'message': "blah"}
            lower_hv, upper_hv: DBA_TAB_PARTITIONS.HIGH_VALUE based. Can have both empty, upper_hv only or both.
        """
        if lower_hv:
            assert isinstance(lower_hv, str)
        ...
        if self._ipa_predicate_type == INCREMENTAL_PREDICATE_TYPE_LIST and upper_hvs:
            # HVs for LIST are lists of tuples, not just a tuple as per RANGE
            upper_hvs = [upper_hvs]
        threshold_clauses, query_binds = build_verification_clauses(frontend_table, self._ipa_predicate_type,
                                                                    upper_hvs, prior_hvs,
                                                                    with_binds=self._bind_in_predicates())
        self._debug('Generated frontend filters:\n%r' % threshold_clauses)

        validator = HybridCountValidator(self._frontend_table_owner, self._frontend_table_name,
                                         self.hybrid_schema, self.hybrid_view,
                                         self._connection_options, messages=self._messages, dry_run=self._dry_run)
        num_diff, source_rows, hybrid_rows = validator.validate(threshold_clauses, query_binds)
        self._debug('validate_by_count returning: %s, %s, %s' % (num_diff, source_rows, hybrid_rows))
        self._debug(t.show())
        validation_msg = 'Source and Hybrid mismatch: %s differences, %s origin rows, %s hybrid rows'\
                         % (num_diff, source_rows, hybrid_rows)
        payload = {JSON_KEY_VALIDATE_STATUS: bool(num_diff == 0), JSON_KEY_VALIDATE_MESSAGE: validation_msg}
        return self._format_payload(payload, as_json)
