...
# EXCEPTIONS
############################################################
class OracleQueryException(Exception): pass
...
def get_oracle_connection(ora_user, ora_pass, ora_dsn, ora_wallet=False, ora_proxy_user=None, ora_trace_id='GLUENT'):
    if ora_wallet:
        ...
        if ora_proxy_user:
            ora_conn = cx_Oracle.connect('%s[%s]' % (ora_user, ora_proxy_user), ora_pass, ora_dsn)
        else:
...
class OracleQuery(OffloadMessagesMixin, object):
    ...
    def __init__(self, user, password, dsn, **kwargs):
        ...
        self._my_connection = False # Marker: "this object created cx_Oracle connection"
    ...
    def fromcursor(cls, cursor, **kwargs):
        ...
        logger.debug("Constructed OracleQuery object from existing cursor for user: %s, dsn: %s" % \
            (obj._user, obj._dsn))
        return obj
    ...
    def _execute(self, sql, binds, bulk, ignore_errors, detail):
        ...
        self.log_vverbose("\nBinds: %s\n" % binds)
        logger.debug("Executing %s ORACLE SQL [%s]: %s in db: %s with binds: %s" % \
        ...
        try:
            ...
            logger.debug("Executing %s ORACLE SQL [%s]: %s in db: %s - SUCCESS" % \
                (bulk_type, sql_command, sql, self._dsn))
        except cx_Oracle.Error as e:
            logger.debug("Executing %s ORACLE SQL [%s]: %s in db: %s - EXCEPTION: %s" % \
            ...
            if ignore_errors:
                logger.warn("ORACLE execution problem: [%s] %s" % (self._retcode, self._err))
                return False
            else:
    ...
    def arraysize(self, val):
        ...
        if not isinstance(val, int) or val <= 0 or val > MAX_ARRAYSIZE:
            raise OracleQueryException("Invalid arraysize: %s Needs to be a number within range: 1..%d" % \
    ...
    def execute(self, sql, binds=None, cursor_fn=None, bulk=False, as_dict=False, ignore_errors=False, detail=VERBOSE):
        """ Execute ORACLE SQL
    ...
    def disconnect(self):
        ...
        if self._my_connection:
            ...
            logger.debug("We re-used an external cx_Oracle connection, no need to close")
