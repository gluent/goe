...
from gluentlib.offload.column_metadata import ColumnPartitionInfo,\
    is_synthetic_partition_column, match_table_column
from gluentlib.offload.offload_constants import OFFLOAD_BUCKET_NAME, PART_COL_GRANULARITY_DAY
...
from gluentlib.offload.offload_source_table import OFFLOAD_PARTITION_TYPE_HASH, OFFLOAD_PARTITION_TYPE_LIST
from gluentlib.offload.oracle.oracle_column import ORACLE_TYPE_INTERVAL_DS, ORACLE_TYPE_INTERVAL_YM
from gluentlib.util.misc_functions import case_insensitive_in, csv_split
...
MISSING_PARTITION_GRANULARITY_EXCEPTION_TEXT = 'Partition granularity (--partition-granularity) mandatory for column/data type'
...
def default_offload_partition_columns_from_rdbms(rdbms_partition_columns, rdbms_partition_type,
                                                 offload_partition_functions, backend_table, messages):
    ...
    if rdbms_partition_columns and rdbms_partition_type == OFFLOAD_PARTITION_TYPE_HASH:
        messages.notice('Backend partition columns not seeded by RDBMS partition columns for %s partitioned table'
                        % OFFLOAD_PARTITION_TYPE_HASH, detail=VERBOSE)
        return []
    return_cols = [_.name for _ in rdbms_partition_columns
                   if _.data_type not in (ORACLE_TYPE_INTERVAL_DS, ORACLE_TYPE_INTERVAL_YM)]
    if rdbms_partition_columns and [_ for _ in rdbms_partition_columns
...
def default_granularity_for_matching_rdbms_list_partition_key(rdbms_col, rdbms_partition_columns, rdbms_partition_type,
                                                              messages, date_as_string):
    """ RDBMS list partition keys should be safe to have a granularity resulting in value parity """
    new_granularity = None
    if rdbms_partition_type == OFFLOAD_PARTITION_TYPE_LIST and match_table_column(rdbms_col.name,
    ...
    if new_granularity:
        messages.notice('Defaulting granularity to %s for %s partition column: %s'
                        % (new_granularity, OFFLOAD_PARTITION_TYPE_LIST, rdbms_col.name), detail=VERBOSE)
...
def derive_partition_digits(backend_table):
    ...
    for partition_column in backend_table.get_partition_columns():
        if partition_column.partition_info:
            source_column = match_table_column(partition_column.partition_info.source_column_name,
...
def offload_options_to_partition_info(offload_partition_columns, offload_partition_functions,
                                      offload_partition_granularity, offload_partition_lower_value,
                                      offload_partition_upper_value, synthetic_partition_digits,
    ...
    if partition_column_match and backend_table.partition_by_column_supported():
        ...
        partition_info = ColumnPartitionInfo(position=position,
    ...
    if rdbms_column.is_date_based():
        if backend_column:
            if backend_column.is_string_based():
                return True
        elif (variable_string_columns_csv
              and case_insensitive_in(rdbms_column.name, csv_split(variable_string_columns_csv))):
            return True
    return False


def validate_offload_partition_columns(offload_partition_columns, rdbms_columns, rdbms_partition_columns,
                                       rdbms_partition_type, offload_partition_functions, backend_table,
                                       messages, offload_chunk_column):
    """ Default and validate requested backend partition columns """
    if offload_partition_columns:
        # User defined partition columns
        new_partition_columns = expand_columns_csv(offload_partition_columns.upper(), rdbms_columns,
                                                   retain_non_matching_names=True)
    else:
        new_partition_columns = default_offload_partition_columns_from_rdbms(rdbms_partition_columns,
                                                                             rdbms_partition_type,
                                                                             offload_partition_functions,
                                                                             backend_table, messages)

    if offload_chunk_column:
        if not [pcol for pcol in (new_partition_columns or []) + [OFFLOAD_BUCKET_NAME.upper()]
    ...
    if len(new_partition_columns) > backend_table.max_partition_columns():
        raise OffloadPartitionControlsException(
            '%s: %s' % (TOO_MANY_BACKEND_PART_COLS_EXCEPTION_TEXT, ','.join(new_partition_columns))
        )
    return new_partition_columns


def validate_offload_partition_functions(offload_partition_functions, offload_partition_columns,
                                         backend_table, messages):
    ...
    if not offload_partition_columns:
        if offload_partition_functions:
            messages.log('Ignoring partition functions because there are no partition columns', detail=VVERBOSE)
        return []
    ...
    if len(offload_partition_columns) != len(offload_partition_functions):
        raise OffloadPartitionControlsException(
            ...
            % (','.join(offload_partition_functions), len(offload_partition_columns),
               PARTITION_FUNCTIONS_ELEMENT_EXCEPTION_TEXT, ','.join(offload_partition_columns))
        )
...
def validate_offload_partition_granularity(offload_partition_granularity, offload_partition_columns,
                                           offload_partition_functions, rdbms_columns, rdbms_partition_columns,
                                           rdbms_partition_type, backend_table, messages,
                                           variable_string_columns_csv, backend_columns=None):
    if offload_partition_columns:
        assert isinstance(offload_partition_columns, list)
        assert isinstance(offload_partition_functions, list)
    else:
    ...
    if offload_partition_granularity:
        ...
        if len(offload_partition_columns) != len(granularity_list):
            raise OffloadPartitionControlsException(
                ...
                % (offload_partition_granularity, len(offload_partition_columns), ','.join(offload_partition_columns))
            )
    ...
    for col_name, granularity, part_fn in zip(offload_partition_columns, granularity_list, offload_partition_functions):
        ...
        if not part_fn:
            ...
            if backend_column and backend_column.is_string_based() and not backend_table.partition_by_string_supported():
                raise OffloadPartitionControlsException(
                    '%s%s: %s' % (PARTITION_BY_STRING_NOT_SUPPORTED_EXCEPTION_TEXT, pf_text, col_name)
                )

        if not granularity:
            granularity = default_granularity_for_matching_rdbms_list_partition_key(rdbms_col, rdbms_partition_columns,
                                                                                    rdbms_partition_type, messages,
                                                                                    date_as_string)
        ...
        if not granularity and (backend_table.partition_function_requires_granularity() or not part_fn):
            raise OffloadPartitionControlsException(
                '%s: %s/%s%s' % (MISSING_PARTITION_GRANULARITY_EXCEPTION_TEXT, rdbms_col.name, rdbms_col.data_type,
                                 ' (as variable string)' if date_as_string else '')
            )

        # Check that specified granularity matches the column data type
        if rdbms_col.is_date_based() and granularity not in backend_table.supported_date_based_partition_granularities() and not date_as_string:
            raise OffloadPartitionControlsException(
                'Partition granularity %s not valid for column/data type: %s/%s'
                % (granularity, rdbms_col.name, rdbms_col.data_type)
            )
        elif ((rdbms_col.is_number_based() or rdbms_col.is_string_based() or date_as_string)
