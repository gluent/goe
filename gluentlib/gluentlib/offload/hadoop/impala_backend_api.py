...
from gluentlib.offload.backend_api import BackendApiException, MissingSequenceTableException, UdfDetails, UdfParameter
from gluentlib.offload.column_metadata import is_safe_mapping, match_table_column,\
    str_list_of_columns, valid_column_list, \
    GLUENT_TYPE_BINARY, GLUENT_TYPE_LARGE_BINARY
from gluentlib.offload.offload_constants import CAPABILITY_CANONICAL_DATE, CAPABILITY_COLUMN_STATS_SET,\
    CAPABILITY_DROP_COLUMN, CAPABILITY_FS_SCHEME_ABFS, CAPABILITY_FS_SCHEME_ADL, CAPABILITY_FS_SCHEME_S3A,\
    CAPABILITY_INCREMENTAL_UPDATE, CAPABILITY_RANGER, CAPABILITY_REFRESH_FUNCTIONS, CAPABILITY_SENTRY,\
    CAPABILITY_SORTED_TABLE, FILE_STORAGE_FORMAT_PARQUET, IMPALA_BACKEND_CAPABILITIES
from gluentlib.offload.offload_messages import VERBOSE, VVERBOSE
from gluentlib.offload.hadoop.hadoop_backend_api import BackendHadoopApi, \
    HADOOP_DECIMAL_MAX_PRECISION, IMPALA_UDF_LIB
...
from gluentlib.util.better_impyla import BetterImpylaException, \
    ...
    HADOOP_TYPE_INT, HADOOP_TYPE_BIGINT, HADOOP_TYPE_DATE, HADOOP_TYPE_DECIMAL, HADOOP_TYPE_FLOAT, HADOOP_TYPE_DOUBLE, \
    HADOOP_TYPE_REAL, HADOOP_TYPE_TIMESTAMP, HADOOP_TYPE_BOOLEAN
...
def gen_decimal_impala_functions_in_constants(fpq=False):
    ...
    if fpq:
        ...
        ps_list = [(18, s * 2) for s in range(9)] + [(HADOOP_DECIMAL_MAX_PRECISION, s * 2) for s in range(20)]
    else:
        ps_list = [(18, _) for _ in range(18)] + [(HADOOP_DECIMAL_MAX_PRECISION, _) for _ in range(39)]
    for precision, scale in ps_list:
        int_input = 1 if precision != scale else 0.1
        if fpq:
            yield ('GLUENT_TO_FPQ', 'STRING', 'DECIMAL(%s,%s)' % (precision, scale),
                   'CAST(%s AS DECIMAL(%s,%s))' % (int_input, precision, scale), 'gluentToFpqNumber')
        else:
            yield ('GLUENT_TO_INTERNAL_NUMBER', 'STRING', 'DECIMAL(%s,%s)' % (precision, scale),
                   'CAST(%s AS DECIMAL(%s,%s))' % (int_input, precision, scale), 'toInternal')


IMPALA_UDF_SPECS = list(gen_decimal_impala_functions_in_constants()) + [
    ('GLUENT_TO_INTERNAL_DOUBLE', 'STRING', 'DOUBLE', 'CAST(1.1 AS DOUBLE)', 'toInternal'),
    ('GLUENT_TO_INTERNAL_FLOAT', 'STRING', 'FLOAT', 'CAST(1.1 AS FLOAT)', 'toInternalFloat'),
    ('GLUENT_TO_INTERNAL_FLOAT', 'STRING', 'DOUBLE', 'CAST(1.1 AS FLOAT)', 'toInternalFloat'),
    ('GLUENT_TO_INTERNAL_NUMBER', 'STRING', 'TINYINT', 'CAST(1 AS TINYINT)', 'toInternal'),
    ('GLUENT_TO_INTERNAL_NUMBER', 'STRING', 'SMALLINT', 'CAST(1 AS SMALLINT)', 'toInternal'),
    ('GLUENT_TO_INTERNAL_NUMBER', 'STRING', 'INT', 'CAST(1 AS INT)', 'toInternal'),
    ('GLUENT_TO_INTERNAL_NUMBER', 'STRING', 'BIGINT', 'CAST(1 AS BIGINT)', 'toInternal'),
    ('GLUENT_TO_INTERNAL_DATE', 'STRING', 'TIMESTAMP', 'CAST(0 AS TIMESTAMP)', 'toInternalDate'),
    ('GLUENT_TO_INTERNAL_DATE', 'STRING', 'STRING', '\'0100-01-01 00:00:00.000000000\'', 'toInternalDate'),
    ...
    ('GLUENT_TRANSFORM', 'STRING', 'STRING,STRING,STRING', "'UPPER', 'abc', '[^a]'", 'gluentTransform'),
    ('GLUENT_TRANSCODE', 'STRING', 'STRING, STRING, STRING', '\'foo\',\'UTF8\',\'UTF16\'', ('gluentTranscode', 'gluentTranscodePrepare', 'gluentTranscodeClose')),
    ('GLUENT_BUCKET', 'SMALLINT', 'BIGINT,SMALLINT', '1,16', 'gluentBucket'),
    ('GLUENT_BUCKET', 'SMALLINT', 'DECIMAL(38,0),SMALLINT', 'CAST(1234 AS DECIMAL(38,0)),16', 'gluentBucket')
]

IMPALA_FPQ_UDF_SPECS = [
    ('GLUENT_FPQ_NULL', 'STRING', 'SMALLINT', 'CAST(1 AS SMALLINT)', 'gluentFpqNull'),
    ('GLUENT_FPQ_ROW', 'STRING', 'BOOLEAN, STRING', 'TRUE,%(fn_db)sGLUENT_TO_FPQ(\'rowrow\')', 'gluentFpqRow'),
    ('GLUENT_FPQ_ROW', 'STRING', 'BOOLEAN, STRING, STRING ...', 'TRUE,%(fn_db)sGLUENT_TO_FPQ(\'foo\'),%(fn_db)sGLUENT_TO_FPQ(\'bar\'),%(fn_db)sGLUENT_TO_FPQ(\'baz\')', 'gluentFpqRow')
    ...
    ('GLUENT_TO_FPQ', 'STRING', 'SMALLINT', 'CAST(1 AS SMALLINT)', 'gluentToFpqNumber'),
    ('GLUENT_TO_FPQ', 'STRING', 'TINYINT', 'CAST(1 AS TINYINT)', 'gluentToFpqNumber'),
    ('GLUENT_TO_FPQ', 'STRING', 'TIMESTAMP', 'CAST(0 AS TIMESTAMP)', 'gluentToFpq'),
    ('GLUENT_TO_FPQ_TIMESTAMP', 'STRING', 'TIMESTAMP', 'CAST(0 AS TIMESTAMP)', 'gluentToFpqTimestamp'),
    ('GLUENT_TO_FPQ_TIMESTAMP', 'STRING', 'TIMESTAMP, SMALLINT', 'CAST(0 AS TIMESTAMP), 5180', 'gluentToFpqTimestamp'),
    ...
    ('GLUENT_TO_FPQ_INTERVAL', 'STRING', 'STRING,BOOLEAN', '\'44517008-8\',TRUE', 'gluentToFpqInterval')
]
...
class BackendImpalaApi(BackendHadoopApi):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False, do_not_connect=False):
        """ CONSTRUCTOR
        """
        super(BackendImpalaApi, self).__init__(connection_options, backend_type, messages, dry_run=dry_run,
    ...
    def _create_impala_udf(self, function_name, function_spec, function_symbol, function_return, udf_db=None, sync=None):
        """ Impala UDF.
        ...
        if isinstance(function_symbol, tuple):
            symbol_clause = 'SYMBOL=\'%s\' PREPARE_FN=\'%s\' CLOSE_FN=\'%s\'' % function_symbol
        else:
        ...
        target_uri = hdfs_client.gen_uri(self._connection_options.offload_fs_scheme,
                                         ...
                                         self._connection_options.offload_fs_prefix) if self._connection_options.offload_fs_scheme in OFFLOAD_NON_HDFS_FS_SCHEMES else self._connection_options.hdfs_home
        target_file = os.path.join(target_uri, IMPALA_UDF_LIB)
        function_body = "LOCATION \'%s\' %s" % (target_file, symbol_clause)

        return self.create_udf(udf_db, function_name, function_return, None, function_body,
                               sync=sync, log_level=log_level, spec_as_string=function_spec)

    def _execute_ddl_or_dml(self, sql, sync=None, query_options=None, log_level=VERBOSE, profile=None, no_log_items=None):
    ...
    def _fetch_show_functions(self, db_name, udf_name_filter=None):
        assert db_name
        if udf_name_filter:
            sql = 'SHOW FUNCTIONS IN %s LIKE \'%s\'' % (self.enclose_identifier(db_name), udf_name_filter.lower())
        else:
...
"""
    ...
    def _get_table_stats(self, hive_stats, as_dict=False, part_stats=False):
        ...
        """
        if part_stats:
            tab_stats, col_stats, part_stats = hive_stats.get_table_stats(self._backend_type, as_dict=as_dict,
                                                                          messages=None, partstats=part_stats)
        else:
            part_stats = None
            tab_stats, col_stats = hive_stats.get_table_stats(self._backend_type, as_dict=as_dict, messages=None,
    ...
    def _insert_literal_values_format_sql(self, db_name, table_name, column_names, literal_csv_list, split_by_cr=True):
        return self._insert_literals_using_insert_values_sql_text(db_name, table_name, column_names, literal_csv_list,
    ...
    def _udf_installation_sql(self, udf_db=None):
        ...
        assert [tupl[0] for tupl in gluent_impala_functions_all
                if len(tupl) != 5] == [], 'Unexpected item count in IMPALA_UDF_SPECS'
        ...
        if (udf_db and self.database_exists(udf_db)) or not udf_db:
            ...
            if self.refresh_functions_supported():
                self.execute_ddl('REFRESH FUNCTIONS %s' % self.enclose_identifier(udf_db if udf_db else 'default'),
                                 sync=None, log_level=log_level)
            rows = self.execute_query_fetch_all('SHOW FUNCTIONS%s' % db_clause, log_level=log_level)
            ...
            for fnc in gluent_impala_functions_all:
                if '%s(%s)' % (fnc[0].upper(), fnc[2].upper().replace(' ','')) in installed_udfs:
            ...
            for i, (f_name, _, f_type, _, _) in enumerate(gluent_impala_functions_drop):
                ...
                cmds.extend(self._drop_udf(f_name, udf_db=udf_db, function_spec=f_type, sync=sync, if_exists=False))

        for i, (f_name, f_ret, f_type, _, f_symbol) in enumerate(IMPALA_UDF_SPECS):
            sync = get_sync(i, len(IMPALA_UDF_SPECS))
            cmds.extend(self._create_impala_udf(f_name, f_type, f_symbol, f_ret, udf_db=udf_db, sync=sync))
    ...
    def _udf_test_sql(self, udf_db=None):
        ...
        for f_name, f_arg in test_args:
            sql = 'SELECT %s%s(%s)' \
                  % (db_clause, self.enclose_identifier(f_name), f_arg % {'fn_db': db_clause})
            sqls.append(sql)
        return sqls
    ...
    def alter_sort_columns(self, db_name, table_name, sort_column_names, sync=None):
        ...
        sql = 'ALTER TABLE %s.%s SORT BY (%s)' % (self.enclose_identifier(db_name),
                                                  self.enclose_identifier(table_name), sort_csv)
    ...
    def compute_stats(self, db_name, table_name, incremental=None, for_columns=False, partition_tuples=None, sync=None):
        ...
        if not self.table_stats_compute_supported():
            return
        partition_clause = ' PARTITION ({})'.format(self._format_partition_clause_for_sql(db_name, table_name, partition_tuples)) if partition_tuples else ''
        incremental_clause = ' INCREMENTAL' if incremental or partition_tuples else ''
        sql = 'COMPUTE%s STATS %s.%s%s' % (incremental_clause, self.enclose_identifier(db_name), self.enclose_identifier(table_name), partition_clause)
    ...
    def create_table(self, db_name, table_name, column_list, partition_column_names, storage_format=None, location=None,
        ...
        if partition_column_names:
            ...
            for part_col in partition_column_names:
                ...
                part_col_pairs.append('%s %s' % (self.enclose_identifier(part_col.lower()), real_col.format_data_type()))
...
%(col_projection)s
    ...
    def create_udf(self, db_name, udf_name, return_data_type, parameter_tuples, udf_body, or_replace=False,
        ...
        sql = 'CREATE FUNCTION %s%s(%s) RETURNS %s %s'\
              % (db_clause, udf_name, udf_param_clause, return_data_type, udf_body)
    ...
    def from_canonical_column(self, column, decimal_padding_digits=0):
        ...
        if column.data_type == GLUENT_TYPE_BINARY:
            ...
            return super(BackendImpalaApi, self).from_canonical_column(column, decimal_padding_digits=decimal_padding_digits)
...
FROM   %(from_db_table)s%(where)s""" % {'db_table': self.enclose_object_reference(db_name, table_name),
    ...
    def get_table_partitions(self, db_name, table_name):
        ...
        # {'Format', '#Rows', 'Size'}
        table_partitions = {k: self._table_partition_info(partition_id=k, num_rows=v['#Rows'],
    ...
    def is_valid_partitioning_data_type(self, data_type):
        if not data_type:
            return False
        valid_types = [HADOOP_TYPE_BIGINT, HADOOP_TYPE_CHAR, HADOOP_TYPE_DECIMAL, HADOOP_TYPE_DOUBLE, HADOOP_TYPE_FLOAT,
                       HADOOP_TYPE_INT, HADOOP_TYPE_REAL, HADOOP_TYPE_STRING, HADOOP_TYPE_SMALLINT,
                       HADOOP_TYPE_TIMESTAMP, HADOOP_TYPE_TINYINT, HADOOP_TYPE_VARCHAR]
    ...
    def list_udfs(self, db_name, udf_name_filter=None, case_sensitive=True):
        ...
        return [convert_show_output_to_returnable_list(_)
    ...
    def populate_sequence_table(self, db_name, table_name, starting_seq, target_seq, split_by_cr=False):
        ...
        insert_data = 'INSERT INTO %s%s VALUES ' % (db_clause, self.enclose_identifier(table_name))
    ...
    def refresh_table_files(self, db_name, table_name, sync=None):
        ...
        """
        assert db_name and table_name
        sql = 'REFRESH %s.%s' % (self.enclose_identifier(db_name), self.enclose_identifier(table_name))
        return self.execute_ddl(sql, sync=sync)
    ...
    def sequence_table_max(self, db_name, table_name):
        ...
        assert table_name
        db_clause = (self.enclose_identifier(db_name) + '.') if db_name else ''
        sql = 'SELECT MAX(n) FROM %s%s' % (db_clause, self.enclose_identifier(table_name))
        try:
            row = self.execute_query_fetch_one(sql, log_level=VVERBOSE)
            if row:
                return row[0]
    ...
    def supported_backend_data_types(self):
        data_types = [HADOOP_TYPE_BOOLEAN, HADOOP_TYPE_BIGINT, HADOOP_TYPE_CHAR, HADOOP_TYPE_DECIMAL,
                      HADOOP_TYPE_DOUBLE, HADOOP_TYPE_FLOAT, HADOOP_TYPE_INT, HADOOP_TYPE_REAL, HADOOP_TYPE_STRING,
                      HADOOP_TYPE_SMALLINT, HADOOP_TYPE_TIMESTAMP, HADOOP_TYPE_TINYINT, HADOOP_TYPE_VARCHAR]
        if self.canonical_date_supported():
    ...
    def udf_installation_os(self, user_udf_version):
        ...
        cmds.extend(self._udf_installation_copy_library_to_hdfs(udf_lib_source, udf_lib_destination))
