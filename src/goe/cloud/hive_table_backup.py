...
from goe.util.hs2_connection import hs2_connection, hs2_section_options, hs2_db_type
from goe.util.parallel_exec import ParallelExecutor

from goe.cloud.hdfs_store import HdfsStore, hdfs_strip_host
from goe.cloud.xfer_xfer import XferXfer
from goe.cloud.offload_logic import XferSetup
...
# EXCEPTIONS
###############################################################################
class HiveTableBackupException(Exception):
    pass
...
class HiveCfgConnection(object):
    ...
    def _connect(self):
        ...
        self.hive = HiveConnection.fromconnection(
            hs2_connection(self.options), db_type=self.db_type, messages=self._messages
        )
...
class HiveTableBackup(OffloadMessagesMixin, object):
    ...
    def __init__(
        ...
        assert (
            ...
            and db_name
            and table_name
            and (not cfg or isinstance(cfg, GOERemoteConfig))
        )
        ...
        self.step = partial(
            step,
            execute=get_option(options, "execute"),
        ...
        logger.debug(
            "HiveTableBackup() object successfully initialized for table: %s %s -> %s"
    ...
    def _create_table(self, hdfs_cli, hive_conn, db_table, table_ddl):
        def step_fn():
            table_location = "%s/%s/%s" % (
                self._cfg.get(hdfs_cli.section, "hdfs_url").strip("/"),
    ...
    def _transform_table_ddl(self, section, db_table, table_ddl, transform_options):
        ...
        def get_base(section):
            ...
            if not base:
                raise HiveTableBackupException(
                    "'hive_base' is not set for table: %s, section: %s. Please, define!"
                    % (db_table, section)
                )
            return base
        ...
        def adjust_dependent_transformations(section, transform):
            if transform:
                ...
                if "schema" in transform:
                    transform["location_db_name"] = self._cfg.db_url(
                        section, transform["schema"]
                    ).rstrip("/")
            else:
                transform = {}
        ...
        def step_fn():
            ...
            logger.debug(
                "Transforming DDL for table: %s with options: %s"
                % (db_table, transform_ddl)
            )
            return (
                table_ddl
                if not transform_ddl
                else DDLTransform().transform(table_ddl, transform_ddl)
            )
            ...
            if "partitions" in fl:
                ...
                if partition_key not in partitions_to_add:
                    partitions_to_add[partition_key] = {
                        ...
                        "spec": fl["partitions"],
                        "location": "%s%s"
                        % (end_by(root, "/"), os.path.dirname(fl["key_name"])),
                    }
    ...
    def _add_single_partition(self, hive_table, db_table, partition_spec):
        ...
        if hive_table.partition_exists(spec):
            self.log_vverbose(
                "Partition: %s already exists for table: %s" % (spec, db_table)
            )
        else:
    ...
    def _delete_files(self, driver, db_table, file_list, fast_path):
        ...
        def step_fn():
            if fast_path:
                self.warn(
                    "\nSkipping delete-on-target for table: %s due to 'fast_path' mode"
                    % db_table
                )
            elif file_list:
        ...
        return self.step(
            "Deleting 'deletable' files on destination: %s" % driver.section, step_fn
        )
    ...
    def _remove_empty_partitions(self, hive_conn, hive_table, db_table, file_list):
        ...
        return self.step(
            "Dropping non-conforming partitions for table: %s" % db_table, step_fn
        )
    ...
    def _drop_single_partition(self, hive_table, db_table, partition_spec):
        ...
        if hive_table.partition_exists(partition_spec["spec"]):
            ...
            self.log_vverbose(
                "Partition: %s does not exists for table: %s"
    ...
    def _move_files(self, file_list, diff, parallel, overwrite):
        ...
        def step_fn():
            ...
            xfer.listcopy(
                ...
                dst_remap=lambda x: x.replace(
                    diff.source_db_table_url, diff.target_db_table_url
                ),
                parallel=parallel,
            ...
            if (xfer.successful + xfer.noop) != xfer.submitted:
                raise HiveTableBackupException(
                    "Unsuccessful data file copy. Submitted: %d. Failed: %d"
        ...
        return self.step(
            "Copying files: %s -> %s using %d parallel slaves"
            % (diff.source, diff.target, parallel),
            step_fn,
        )
    ...
    def _refresh_table(self, hive_conn, hive_table, db_table, invalidate):
        ...
        return self.step(
            "%s table: %s" % ("Invalidating" if invalidate else "Refreshing", db_table),
            step_fn,
        )
    ...
    def _backup(
        ...
        """
        to_move_files = (
            diff.source_all if (overwrite or fast_path) else diff.offloadable_source
        )

        # Copy files SRC -> DST
        files_moved = self._move_files(
            file_list=to_move_files, diff=diff, parallel=parallel, overwrite=overwrite
        )

        # Remove files on SRC that are not on DST
        if delete_on_target:
            self._delete_files(
                diff.target_drv, db_table, diff.deletable_keys, fast_path
            )

        # Save DDL as meta file
        table_ddl = self._transform_table_ddl(
            diff.source,
            self._db_table,
            self.hive_table(hive_backup_conn(), self._db_table).table_ddl(),
            transform_ddl,
        )
    ...
    def _restore(
        ...
        to_move_files = (
            diff.source_all
            if (overwrite or drop_table_if_exists or fast_path)
            else diff.offloadable_source
        )
        ...
        if delete_on_target:
            if fast_path:
                ...
                self._remove_empty_partitions(
                    hive_restore_conn, hive_table, db_table, diff.deletable_keys
                )

        # Important - future commands might fail if not refreshed
        # Invalidate only if there were partitions 'lost'
        self._refresh_table(
            hive_restore_conn, hive_table, db_table, invalidate=delete_on_target
        )
    ...
    def _extract_counts(self, sources):
        ...
        def step_fn():
            task_list = [
                [select_count, obj.section, obj, table] for obj, table in sources
            ]
    ...
    def _validate_sources(self, src_section, dst_section, command, remote_db_table):
        src_type, dst_type = self._cfg.section_type(
            src_section
        ), self._cfg.section_type(dst_section)

        if SECTION_TYPE_S3 == src_type and command in (CMD_CLONE):
            raise HiveTableBackupException(
                "Cannot %s from S3 source: '%s'" % (command, src_section)
            )
        if SECTION_TYPE_S3 == dst_type and command in (CMD_RESTORE, CMD_CLONE):
            raise HiveTableBackupException(
                "Cannot %s to S3 destination: '%s'" % (command, dst_section)
            )
        if src_section == dst_section and (self._db_table == remote_db_table):
            raise HiveTableBackupException(
                "Source and destination refer to the same section: '%s'" % src_section
            )
    ...
    def _validate_diff(self, diff, override_empty, fast_path):
        if not self.option("execute"):
            ...
            if override_empty:
                self.warn(
                    "Overriding table: %s from EMPTY source: %s"
                    % (self._db_table, diff.source)
                )
            else:
                raise HiveTableBackupException(
                    "Source: %s is EMPTY for table: %s" % (diff.source, self._db_table)
                )
    ...
    def hive_table(self, conn, db_table):
        ...
        if self.table_exists(conn, db_table):
            ...
            logger.debug(
                "Unable to create HiveTable() object as table: %s does not exist"
                % db_table
            )
    ...
    def backup(
        ...
        if not self.table_exists(hive_conn(), self._db_table):
            raise HiveTableBackupException(
                "Table to be backed up: %s does NOT exist" % self._db_table
            )
        ...
        logger.debug(
            "Parallel: %d. Overwite: %s. Delete on target: %s Override empty: %s"
    ...
    def restore(
        ...
        if self.table_exists(hive_conn(), remote_db_table):
            self.warn(
                "Table to be restored: %s already exists on destination: %s"
                % (remote_db_table, self._local)
            )
        ...
        self.log_normal("Hive connection: %s" % hive_conn)
        logger.debug(
            "Parallel: %d. Overwrite: %s, Delete_on_target: %s, Drop_table_if_exists: %s, Skip_statistics: %s"
            % (
                parallel,
                overwrite,
                delete_on_target,
        ...
        logger.debug(
            "Rescan_partitions: %s, validate_counts: %s, transform_ddl: %s, override_empty: %s"
        ...
        restored = self._restore(
            ...
            remote_db_name,
            remote_db_table,
            self._transform_table_ddl(
                diff.target,
                self._db_table,
                self._extract_ddl_from_meta(diff.source_drv, self._db_table),
                transform_ddl,
            ),
            overwrite,
            delete_on_target,
        ...
        if validate_counts and self.option("execute"):
            self.log_normal(
                "Records in table: %s: %d"
                % (
                    remote_db_table,
                    self._extract_counts([(hive_conn, remote_db_table)])[
                        hive_conn.section
                    ],
                )
            )
    ...
    def clone(
        ...
        if not self.table_exists(hive_src(), self._db_table):
            raise HiveTableBackupException(
                "Table to be cloned: %s does NOT exist" % self._db_table
            )
        hive_dst = HiveCfgConnection(self._cfg, self._remote, messages=self._messages)
        if self.table_exists(hive_dst(), remote_db_table):
            self.warn(
                "Table to be cloned: %s already exists on destination: %s"
                % (remote_db_table, self._remote)
            )
        ...
        if hive_src.id == hive_dst.id and self._db_table == remote_db_table:
            raise HiveTableBackupException(
                "Source and Destination refer to the same object: %s on the same Hiveserver2: %s"
                % (self._db_table, hive_src.id)
            )
        if hive_src.db_type != hive_dst.db_type:
            self.warn(
                "SOURCE %s db type: %s is different from DESTINATION %s db type: %s"
        ...
        cloned = self._restore(
            ...
            self._transform_table_ddl(
                diff.target,
                self._db_table,
                self.hive_table(hive_src(), self._db_table).table_ddl(),
                transform_ddl,
            ),
            overwrite,
            delete_on_target,
        ...
        if validate_counts and self.option("execute"):
            results = self._extract_counts(
                [(hive_src, self._db_table), (hive_dst, remote_db_table)]
            )
            self._validate_counts(
                results, self._local, self._db_table, self._remote, remote_db_table
            )
    ...
    def evaluate(self, source, target, remote_db_table, fast_path=False):
        ...
        """
        diff = XferSetup(
            source, target, self._db_table, remote_db_table, self._cfg, lite=fast_path
        )
        ...
        def step_fn():
            ...
            if not fast_path:
                self.log_normal(
                    "\nDetected: %d files to copy: %s -> %s"
                    % (len(diff.offloadable), source, target)
                )
                self.log_normal(
                    "Detected: %d files to remove on: %s"
                    % (len(diff.deletable), target)
                )
                self.log_normal(
                    "Detected: %d files that are the same in %s, %s"
                    % (diff.the_same, source, target)
                )
        ...
        return (
            self.step(
                "Evaluating sources%s" % (" with FastPath" if fast_path else ""),
                step_fn,
                execute=True,
            )
            or diff
...
if __name__ == "__main__":
    ...
    def usage(prog_name):
        print(
            "usage: %s <backup|restore|clone> db_table src dst "
    ...
    def main():
        ...
        bkp = HiveTableBackup(
            db_name, table_name, source, target, messages=messages, options=options
        )
