...
from gluentlib.util.orchestration_lock import OrchestrationLockTimeout,\
    orchestration_lock_for_table, orchestration_lock_from_hybrid_metadata
...
class TestOrchestrationLock(TestCase):
    ...
    def test_orchestration_lock_from_hybrid_metadata(self):
        ...
        metadata = OrchestrationMetadata.from_attributes(messages=messages,
                                                         offloaded_owner=LOCK_OWNER,
                                                         offloaded_table=LOCK_TABLE)
        # Ensure works as context manager
    ...
    def test_orchestration_blocking_lock(self):
        ...
        self.assertEqual(exceptions_caught_in_threads['waiter']['exception']['type'], OrchestrationLockTimeout)
        t1.join()
