...
ORACLE_TIMESTAMP_LOCAL_TZ_RE = re.compile(r'^TIMESTAMP\(([0-9])\) WITH LOCAL TIME ZONE$', re.I)
ORACLE_TIMESTAMP_LOCAL_TZ_SUB_PATTERN = 'TIMESTAMP(%s) WITH LOCAL TIME ZONE'
ORACLE_INTERVAL_DS_RE = re.compile(r'^INTERVAL DAY(\([0-9]\))? TO SECOND(\([0-9]\))?$', re.I)
ORACLE_INTERVAL_DS_SUB_PATTERN = 'INTERVAL DAY(%s) TO SECOND(%s)'
ORACLE_INTERVAL_YM_RE = re.compile(r'^INTERVAL YEAR(\([0-9]\))? TO MONTH(\([0-9]\))?$', re.I)
...
class OracleColumn(ColumnMetadataInterface):
    ...
    def __init__(self, name, data_type, data_length=None, data_precision=None, data_scale=None, nullable=None,
                 data_default=None, hidden=None, char_semantics=None, char_length=None, safe_mapping=True,
                 partition_info=None):
        super(OracleColumn, self).__init__(name, data_type, data_length=data_length, data_precision=data_precision,
        ...
        if self.char_semantics is None and self.data_type in [ORACLE_TYPE_NCHAR, ORACLE_TYPE_NCLOB,
    ...
    def from_oracle(attribute_list):
        ...
        """
        name, _, nullable, data_type, data_precision, data_scale, data_length, data_default, hidden, char_used, char_length = attribute_list
        if data_type == ORACLE_TYPE_FLOAT and data_precision:
            # Precision for FLOAT does not match with our use of precision for NUMBER
            data_precision = None
        if char_used:
            if data_type in [ORACLE_TYPE_NCHAR, ORACLE_TYPE_NCLOB, ORACLE_TYPE_NVARCHAR2]:
            ...
            char_semantics = None
        col = OracleColumn(name, data_type, data_length=data_length, data_precision=data_precision,
                           data_scale=data_scale, nullable=nullable, data_default=data_default,
                           hidden=hidden, char_semantics=char_semantics, char_length=char_length)
        return col
    ...
    def format_data_type(self):
        ...
        if self.data_type in (ORACLE_TYPE_NUMBER, ORACLE_TYPE_FLOAT):
            ...
            if self.char_length:
                return '%s(%s %s)' % (self.data_type, self.char_length, char_semantic(self.char_semantics))
            else:
                # Oracle -> Oracle will always have char_length so this must be Backend -> Oracle
                return '%s(%s %s)' % (self.data_type, self.data_length, char_semantic(CANONICAL_CHAR_SEMANTICS_BYTE))
    ...
    def is_binary(self):
        return bool(self.data_type in [ORACLE_TYPE_BLOB, ORACLE_TYPE_RAW, ORACLE_TYPE_LONG, ORACLE_TYPE_LONG_RAW])
    ...
    def is_number_based(self):
        ...
        """
        return bool(self.data_type in [ORACLE_TYPE_NUMBER, ORACLE_TYPE_FLOAT,
                                       ORACLE_TYPE_BINARY_DOUBLE, ORACLE_TYPE_BINARY_FLOAT])

    def is_date_based(self):
        """ Is the column date in class
        """
        return bool(self.data_type in [ORACLE_TYPE_DATE, ORACLE_TYPE_TIMESTAMP,
                                       ORACLE_TYPE_TIMESTAMP_TZ, ORACLE_TYPE_TIMESTAMP_LOCAL_TZ])
    ...
    def is_string_based(self):
        ...
        """
        return bool(self.data_type in [ORACLE_TYPE_VARCHAR, ORACLE_TYPE_VARCHAR2, ORACLE_TYPE_NVARCHAR2,
