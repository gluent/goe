...
# EXCEPTIONS
###############################################################################
class Hs2ConnectionException(Exception): pass
...
def hs2_connect_using_options(opts, ldap=False, plain=False, kerberos=False, unsecured=False):
    ...
    """
    assert opts
    assert len([_ for _ in [ldap, plain, kerberos, unsecured] if _]) == 1, \
...
def hs2_connection(orchestration_config, host_override=None, port_override=None):
    ...
    optvars = vars(orchestration_config)  # Converting to dict as it's easier to deal with
    ...
    if hs2_connect_using_options(optvars, ldap=True):
        ldap_user, ldap_password = optvars.get('ldap_user'), optvars.get('ldap_password')
        ...
        return hs2.connect(host=host, port=port, use_ssl=optvars.get('use_ssl'), ca_cert=optvars.get('ca_cert'),
                           ...
                           use_http_transport=optvars.get('hiveserver2_http_transport'), http_path=optvars.get('hiveserver2_http_path'))

    elif hs2_connect_using_options(optvars, plain=True):
        hive_user = os.environ.get('HIVE_SERVER_USER')
        logger.debug("Connecting via PLAIN authentication with username %s to HiveServer2" % hive_user)
        return hs2.connect(host=host, port=port, use_ssl=optvars.get('use_ssl'), ca_cert=optvars.get('ca_cert'),
                           auth_mechanism='PLAIN', user=hive_user, timeout=timeout,
                           use_http_transport=optvars.get('hiveserver2_http_transport'), http_path=optvars.get('hiveserver2_http_path'))

    elif hs2_connect_using_options(optvars, kerberos=True):
        kerberos_service = optvars.get('kerberos_service')
        logger.debug("Connecting via Kerberos service: %s to HiveServer2" % kerberos_service)
        return hs2.connect(host=host, port=port, use_ssl=optvars.get('use_ssl'), ca_cert=optvars.get('ca_cert'),
                           auth_mechanism='GSSAPI', kerberos_service_name=kerberos_service, timeout=timeout,
                           use_http_transport=optvars.get('hiveserver2_http_transport'), http_path=optvars.get('hiveserver2_http_path'))

    else:
        logger.debug("Establishing unauthenticated connection to HiveServer2")
        return hs2.connect(host=host, port=port, use_ssl=optvars.get('use_ssl'), ca_cert=optvars.get('ca_cert'),
                           timeout=timeout, auth_mechanism=optvars.get('hiveserver2_auth_mechanism'),
                           use_http_transport=optvars.get('hiveserver2_http_transport'), http_path=optvars.get('hiveserver2_http_path'))
...
def hs2_env_options(override=None):
    ...
    options.target = 'hive' if os.environ.get('QUERY_ENGINE').upper() == 'HIVE' else 'impala'
    default_port = '10000' if 'HIVE' == os.environ.get('QUERY_ENGINE', 'IMPALA').upper() else '21050'
    ...
    options.use_ssl = True if 'true' == os.environ.get('SSL_ACTIVE', 'false').lower() else False
    ...
    if options.kerberos_service:
        ...
        if 'hiveserver2_auth_mechanism' not in vars(options):
            options.hiveserver2_auth_mechanism = os.environ.get('HIVE_SERVER_AUTH_MECHANISM', 'NOSASL')
    ...
    if options.hadoop_host and ',' in options.hadoop_host:
        ...
        logger.debug("Randomly selecting host: %s out of: %s" % (host, options.hadoop_host))
...
def hs2_db_type(options=None):
    ...
    """
    def from_port(port): return DEFAULT_PORTS.get(port, None)

    db_type = (options and vars(options).get('target', None)) or \
        os.environ.get('QUERY_ENGINE', None) or \
        from_port(os.environ.get('HIVE_SERVER_PORT', None))
...
if __name__ == "__main__":
    ...
    from gluentlib.util.misc_functions import set_gluentlib_logging, options_list_to_namespace
