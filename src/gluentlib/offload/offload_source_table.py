...
from gluentlib.offload.column_metadata import ColumnMetadataInterface, ColumnPartitionInfo, \
...
from gluentlib.util.misc_functions import get_integral_part_magnitude
...
COLUMN_INTEGRAL_MAGNITUDE_MESSAGE_TEXT = Template('Data in column $column has integral magnitude beyond backend capability')
...
HYBRID_BASE_OBJECTS = 'BASE'
HYBRID_AAPD_OBJECTS = 'AAPD'
HYBRID_INCREMENTAL_UPDATE_OBJECTS = 'INCREMENTAL_UPDATE'
HYBRID_JOIN_OBJECTS = 'JOIN'
HYBRID_DEPENDENT_OBJECTS = 'DEPENDENT_VIEW'
HYBRID_ALL_OBJECTS = [HYBRID_BASE_OBJECTS, HYBRID_AAPD_OBJECTS, HYBRID_INCREMENTAL_UPDATE_OBJECTS,
                      HYBRID_JOIN_OBJECTS, HYBRID_DEPENDENT_OBJECTS]
HYBRID_DEPENDENT_HYBRID_VIEWS = ['HYBRID_VIEW', 'JOIN_VIEW', 'AAPD_VIEW']
(HYBRID_OBJECTS_OFFLOADED_OWNER,
 HYBRID_OBJECTS_OFFLOADED_TABLE,
 HYBRID_OBJECTS_HYBRID_ROOT_OWNER,
 HYBRID_OBJECTS_HYBRID_ROOT_NAME,
 HYBRID_OBJECTS_HYBRID_ROOT_TYPE,
...
class RdbmsPartition():
    ...
    def __str__(self):
        return "RdbmsPartition(partition_name=%s, partition_count=%s, partition_position=%s, subpartition_count=%s, subpartition_name=%s, subpartition_names=%s, subpartition_position=%s, high_values_csv=%s, high_values_python=%s, partition_size=%s, num_rows=%s, high_values_individual=%s)" \
            % (self.partition_name,
    ...
    @staticmethod
    def by_name(partition_name=None, partition_count=None, partition_position=None, subpartition_count=None,
                subpartition_name=None, subpartition_names=None, subpartition_position=None, high_values_csv=None,
                high_values_python=None, partition_size=None, num_rows=None, high_values_individual=None):
...
class OffloadSourceTableInterface(metaclass=ABCMeta):
    ...
    def __init__(self, schema_name, table_name, connection_options, messages,
        ...
        self._db_api = frontend_api_factory(connection_options.db_type, connection_options, messages,
                                            existing_connection=conn, dry_run=dry_run,
    ...
    def get_distinct_column(self, column, partition_name=None):
        ...
        return self._db_api.get_distinct_column_values(self.owner, self.table_name, column_name,
    ...
    def get_suitable_sample_size(self, bytes_override=None) -> int:
        ...
        self._messages.log('Sampling plans to scan %sG out of %sG' % (plan_to_scan_gb, gb), detail=VVERBOSE)
        ...
        if self._is_compression_enabled():
            ...
            self._messages.log('Applying factor %s to sample percent due to table compression'
                               % self._sample_data_types_compression_factor(), detail=VVERBOSE)
    ...
    def sample_rdbms_data_types(self, columns_to_sample, data_sample_pct, data_sample_parallelism,
        ...
        def check_backend_max_decimal_scale(col_name, scale, allow_decimal_scale_rounding):
            if not allow_decimal_scale_rounding and (scale or 0) > backend_max_decimal_scale:
                # This data won't fit and we need to reject the offload
                self._messages.warning('%s: %s > %s' % (COLUMN_SAMPLE_SCALE_MESSAGE_TEXT.substitute(column=col_name),
                                                        scale, backend_max_decimal_scale), ansi_code='red')
                return False
            return True

        def format_sampling_query(columns_to_sample, data_sample_pct, data_sample_parallelism,
                                  columns_affected_by_sampling, columns_in_results):
            ...
            for i, col in enumerate(columns_to_sample):
                ...
                if col.is_date_based():
                    ...
                    if min_value:
                        self._messages.log('%s min_value (%s): %s' % (col.name, type(min_value), min_value),
                                           detail=VVERBOSE)
                        if min_value < backend_min_possible_date:
                            ...
                            self._messages.notice('Detected incompatible value "%s" for %s after %s, offloading via %s'
                                                  % (min_value, col.name, DATETIME_STATS_SAMPLING_OPT_ACTION_TEXT,
                    ...
                    if col.data_precision is not None and (col.data_precision - max(col.data_scale, 0)) > backend_max_decimal_integral_magnitude:
                        ...
                        if min_value or max_value:
                            self._messages.log('%s min_value/max_value: %s (%s)/%s (%s)'
                                               % (col.name, min_value, type(min_value), max_value, type(max_value)),
                                               detail=VVERBOSE)
                            ...
                            if not check_backend_max_decimal_integral_magnitude(col.name, num_value=max_value):
                                abort = True
                                continue
                    proj.append("""MAX(CASE WHEN "%(col_alias)s" = 0 THEN LENGTH("%(col)s") ELSE "%(col_alias)s"-1 END)""" % subs)
                    proj.append("""MAX(CASE WHEN "%(col_alias)s" = 0 THEN 0 ELSE LENGTH("%(col)s")-"%(col_alias)s" END)""" % subs)
        ...
        self._messages.log('Minimum valid date for backend: %s' % str(backend_min_possible_date), detail=VVERBOSE)

        columns_affected_by_sampling = []
        columns_in_results = []
        sample_sql = format_sampling_query(
            columns_to_sample, local_data_sample_pct, local_data_sample_parallelism,
            columns_affected_by_sampling, columns_in_results,
        )
        ...
        if not sample_output_row or sample_output_row[0] == 0:
            self._messages.warning('Unable to sample column data (row count %s), using default data types'
                                   % (sample_output_row[0] if sample_output_row else None), ansi_code='red')
            return
        ...
        for col in columns_in_results:
            if col.is_date_based():
                ...
                if min_value and datetime64(min_value) < backend_min_possible_date:
                    columns_affected_by_sampling.append(self._sample_data_types_date_as_string_column(col.name))
                    self._messages.notice('Detected incompatible value "%s" for %s after sampling column data, offloading via %s'
                ...
                if not check_backend_max_decimal_scale(col.name, scale, allow_decimal_scale_rounding):
                    ...
                    continue
                if suspect:
                    self._messages.warning('Unable to sample column data for %s due to suspect data'
                                           % col.name, ansi_code='red')
                    continue
                precision = None if int_part is None and scale is None else (int_part + scale)
                self._messages.log('Detected precision=%s, scale=%s for %s after sampling column data'
                                   % (precision, scale, col.name), VERBOSE)
    ...
    def check_data_types_supported(self, backend_max_datetime_scale, backend_nan_supported,
        ...
        if self.max_datetime_scale() > 6 and not self.check_nanosecond_offload_allowed(
                backend_max_datetime_scale, allow_nanosecond_timestamp_columns=allow_nanosecond_timestamp_columns
        ):
            return False

        if self.nan_capable_data_types() and not self.check_nan_offload_allowed(backend_nan_supported,
    ...
    def to_canonical_column_with_overrides(self, rdbms_column, canonical_overrides=None, auto_detect_num=True,
        ...
        """
        assert rdbms_column
        assert isinstance(rdbms_column, ColumnMetadataInterface), '%s is not an instance of ColumnMetadataInterface'\
                                                                  % type(rdbms_column)
        ...
        if match_table_column(rdbms_column.name, canonical_overrides or []):
            ...
            if not self.valid_canonical_override(rdbms_column, new_col):
                raise OffloadSourceTableException('%s %s: %s -> %s'
                                                  % (INVALID_DATA_TYPE_CONVERSION_EXCEPTION_TEXT, rdbms_column.name,
                                                     rdbms_column.data_type, new_col.data_type))
