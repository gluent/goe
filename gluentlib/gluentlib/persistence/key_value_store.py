...
def build_kv_store(kv_store_flavor,
    ...
    assert kv_store_flavor in SUPPORTED_KV_STORE_FLAVORS, \
    ...
    if kv_store_flavor == HDFS_FILEPATH_FLAVOR:
        ...
        assert location_for_dataset, '{} kv store requires location_for_dataset'.format(kv_store_flavor)
        assert hdfs_client_factory, '{} kv store requires hdfs_client_factory'.format(kv_store_flavor)
        return HdfsFilepathKeyValueStore(location_for_dataset, hdfs_client_factory, dry_run=dry_run)
    elif kv_store_flavor == GLUENT_REPO_FLAVOR:
        assert db and table_name
        assert orchestration_config, '{} kv store requires orchestration_config'.format(kv_store_flavor)
        ...
        return GluentRepoKeyValueStore(db, table_name, orchestration_config, messages, dry_run=dry_run,
                                       repo_client=repo_client, execution_id=execution_id)
