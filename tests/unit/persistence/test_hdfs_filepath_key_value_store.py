...
class TestHdfsFilepathKeyValueStore(unittest.TestCase):
    def _check_expected_values_for_kv_store(self, kv_store, expected_values_dict):
        self.assertEqual(
            expected_values_dict, kv_store.get_all(list(expected_values_dict.keys()))
        )
    ...
    def _do_test_multi_put(self, client_factory=None):
        ...
        self.assertIsNone(foo_bar_kv_store.get("baz"))  # 'baz' is initially undefined
        foo_bar_kv_store.put_all(
            {"baz": "bam", "foo": "foo/bar foo-NEW", "bar": "foo/bar bar-NEW"}
        )
        foo_bar_expected = {
            "baz": "bam",
            "foo": "foo/bar foo-NEW",
        ...
        self.assertIsNone(foo_baz_kv_store.get("baz"))  # 'baz' is initially undefined
        foo_baz_kv_store.put_all(
            {"baz": "bam", "foo": "foo/baz foo-NEW", "bar": "foo/baz bar-NEW"}
        )
        foo_baz_expected = {
            "baz": "bam",
            "foo": "foo/baz foo-NEW",
...
class MockHdfsClient(object):
    def __init__(self, paths_to_listings):
        self._paths_to_listings = {
            os.path.normpath(path): sorted(map(os.path.normpath, listings))
            for path, listings in paths_to_listings.items()
        }
...
def get_mock_hdfs_client(path_to_listing_dict_list):
    ...
    for path, listing_dict_list in path_to_listing_dict_list.items():
        listings = [
            HdfsFilepathKeyValueStoreEntry(**_).encode(path) for _ in listing_dict_list
        ]
