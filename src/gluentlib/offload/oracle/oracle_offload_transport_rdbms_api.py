...
from gluentlib.offload.frontend_api import FRONTEND_TRACE_ID
from gluentlib.offload.offload_constants import OFFLOAD_TRANSPORT_VALIDATION_POLLER_DISABLED
...
from gluentlib.offload.offload_transport_functions import get_rdbms_connection_for_oracle, split_ranges_for_id_range
from gluentlib.offload.offload_transport_rdbms_api import (
...
from gluentlib.offload.oracle.oracle_offload_source_table import oracle_version_supports_exadata
from gluentlib.util.misc_functions import id_generator
...
class OffloadTransportOracleApi(OffloadTransportRdbmsApiInterface):
    ...
    def __init__(self, rdbms_owner, rdbms_table_name, offload_options, messages,
        ...
        self._transport_row_source_query_split_methods = [TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_PARTITION,
                                                          TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_SUBPARTITION,
                                                          TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_EXTENT,
                                                          TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_MOD,
                                                          TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_ID_RANGE]
    ...
    def _get_adm_connection(self):
        if not self._rdbms_adm_conn:
            self._rdbms_adm_conn = get_rdbms_connection_for_oracle(self._offload_options.ora_adm_user,
    ...
    def _get_app_connection(self):
        if not self._rdbms_app_conn:
            self._rdbms_app_conn = get_rdbms_connection_for_oracle(self._offload_options.rdbms_app_user,
    ...
    def _get_fixed_sqoop_parameters(self, max_ts_scale):
        ...
        return {'TRACEFILE_IDENTIFIER': "'%s'" % FRONTEND_TRACE_ID,
                'TIME_ZONE': "'UTC'",
    ...
    def _oracle_alter_session_statements(self, session_parameter_dict, for_plsql=False, include_semi_colons=False,
    ...
    def _get_id_range(self, rdbms_col_name, partition_chunk=None):
        """ Function to get the MIN and MAX values for an id column, used to create
            ...
            The query is structured specifically to take advantage of Oracle's MIN/MAX optimization, e.g.:
                ...
                |   5 |      INDEX FULL SCAN (MIN/MAX)| GL_RANGE_IOT_MONTH_PK |
                |   6 |   VIEW                        |                       |
                |   7 |    SORT AGGREGATE             |                       |
                |   8 |     PARTITION RANGE SINGLE    |                       |
                |   9 |      INDEX FULL SCAN (MIN/MAX)| GL_RANGE_IOT_MONTH_PK |
                ---------------------------------------------------------------
        """
        min_max_row = None
...
, (
    ...
    FROM   "%(owner)s"."%(table)s"%(partition_clause)s
        ...
        cx = get_rdbms_connection_for_oracle(self._offload_options.rdbms_app_user,
                                             self._offload_options.rdbms_app_pass,
                                             self._offload_transport_dsn,
                                             self._offload_options.use_oracle_wallet)
        try:
            ...
            if min_max_row:
                self.log('MIN/MAX: %s (%s)/%s (%s)' % (min_max_row[0], type(min_max_row[0]),
                                                       min_max_row[1], type(min_max_row[1])), detail=VVERBOSE)
        finally:
            try:
                ora_cursor.close()
            except:
    ...
    def get_rdbms_query_cast(self, column_expression, rdbms_column, staging_column, max_ts_scale,
        ...
        if rdbms_column.data_type == ORACLE_TYPE_TIMESTAMP_LOCAL_TZ:
            cast_expression = "CONCAT(CAST(%s AS VARCHAR2(64)),' UTC')" % column_expression
            ...
            cast_expression = "TO_CHAR(%s,'YYYY-MM-DD HH24:MI:SS.FF%s TZH:TZM')" % (column_expression, ff_scale)
            ...
            cast_expression = "TO_CHAR(%s,'YYYY-MM-DD HH24:MI:SS.FF%s')" % (column_expression, ff_scale)
        elif (staging_column.is_string_based() and rdbms_column.data_type in (ORACLE_TYPE_NUMBER, ORACLE_TYPE_FLOAT)
              and convert_expressions_on_rdbms_side):
            cast_expression = "TO_CHAR(%s,'TM')" % column_expression
        elif rdbms_column.data_type in (ORACLE_TYPE_INTERVAL_DS, ORACLE_TYPE_INTERVAL_YM) and for_spark:
            cast_expression = "TO_CHAR(%s)" % column_expression
        elif (rdbms_column.data_type == ORACLE_TYPE_NUMBER and rdbms_column.data_precision and rdbms_column.data_scale
              and rdbms_column.data_scale > rdbms_column.data_precision and for_spark):
            ...
            cast_expression = "CASE WHEN %s IN ('NaN','Infinity','-Infinity') THEN NULL ELSE %s END" % (column_expression, column_expression)
    ...
    def get_offload_transport_sql_stats_function(self, rdbms_module, rdbms_action, conn_action=None):
        ...
        def stats_function():
            return cursor.execute(LOG_SQL_STATS_QUERY_TEXT, {'module': rdbms_module, 'action': rdbms_action}).fetchall()
    ...
    def get_rdbms_session_setup_hint(self, custom_session_parameters, max_ts_scale):
        ...
        def to_opt_param(kv_dict):
            if not kv_dict:
                return ''
            return ' '.join('OPT_PARAM({}, {})'.format(enquote(k), enquote(v))
    ...
    def get_rdbms_scn(self):
        ...
        try:
            ...
            try:
                ...
                self.log('Failed to close RDBMS session after error: %s' % str(exc), detail=VVERBOSE)
            raise
    ...
    def get_transport_split_type(self, partition_chunk, rdbms_table, parallelism, rdbms_partition_type,
        ...
        if partition_count > 0 and partition_count >= parallelism and partition_count <= MAX_UNION_ALL_SPLITS:
            ...
            self.log('Splitting subpartitioned table by subpartition as there are fewer partitions than requested parallelism: %s < %s'
                     % (str(partition_count), str(parallelism)), detail=VVERBOSE)
            partition_by = TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_SUBPARTITION
        elif is_iot:
            pk_col_types = [_.data_type for _ in rdbms_columns if _.name.upper() == rdbms_pk_cols[0].upper()]
            # partition_count > 1 below because we treat a single partition IOT offload like a non-partitioned one.
            if partition_count > 1 and partition_count <= MAX_UNION_ALL_SPLITS and rdbms_partition_type in ('RANGE',
                                                                                                            'LIST',
                                                                                                            'HASH'):
                ...
                if partition_count == 1:
                    self.log('Splitting IOT into id ranges for single partition', detail=VVERBOSE)
                else:
                    self.log('Splitting non-partitioned IOT into id ranges', detail=VVERBOSE)
    ...
    def get_transport_row_source_query(self, partition_by_prm, rdbms_table, consistent_read, parallelism,
        ...
        if consistent_read:
            ...
            self.log('Using RDBMS SCN %s for consistent data retrieval' % str(transport_scn), detail=VVERBOSE)
        ...
        if partition_by == TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_ID_RANGE:
            ...
            if not rdbms_pk_cols:
                raise OffloadTransportRdbmsApiException('Table %s cannot be split by range without primary key'
                                                        % owner_table)
            col_min, col_max = self._get_id_range(rdbms_pk_cols[0], partition_chunk=partition_chunk)
            if col_min is None or col_max is None:
                self.log('Switching from range to mod data split due to blank values', detail=VVERBOSE)
                partition_by = TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_MOD

        union_all = self._row_source_query_union_all_clause(pad)

        if partition_by in (TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_PARTITION,
                            TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_SUBPARTITION):
            ...
            if incremental_update_extract_predicate:
                union_branch_template += ' WHERE (%s)' % incremental_update_extract_predicate
            row_source = union_all.join([union_branch_template
                                         % {'batch': (i % parallelism),
                                            ...
                                            'part_name': p} for i, p in enumerate(partition_names)])
        elif partition_by == TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_EXTENT:
            partition_param, subpartition_param = 'NULL', 'NULL'
            if partition_chunk and partition_chunk.count() > 0:
                partition_param = 'offload_vc2_ntt(%s)' % ','.join(["'%s'" % _
                                                                    for _ in partition_chunk.partition_names()])
                if offload_by_subpartition:
                    subpartition_param = partition_param
                    partition_param = 'NULL'
            union_branch_template = 'SELECT /*+ NO_INDEX(g) USE_NL(g) LEADING(r) */ g.*%(incremental_update_meta_columns)s, %(batch)s AS %(batch_col)s ' +\
            ...
            if predicate_offload_clause:
                ...
                union_branch_template += ' AND (%s)' % incremental_update_extract_predicate
            row_source = union_all.join([union_branch_template
                                         % {'batch': i,
                                            'batch_col': TRANSPORT_ROW_SOURCE_QUERY_SPLIT_COLUMN,
                                            'incremental_update_meta_columns': incremental_update_meta_columns,
                                            ...
                                            'subpartition_param': subpartition_param} for i in range(parallelism)])
        elif partition_by == TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_MOD:
            if not mod_column:
                raise OffloadTransportRdbmsApiException('Table %s cannot be split by mod without suitable column'
                                                        % owner_table)
            batch_expr = 'MOD(ORA_HASH(%(batch_source_col)s), %(degree)s)'\
                         % {'batch_source_col': mod_column, 'degree': parallelism}
            ...
            if predicate_offload_clause:
                ...
                extra_where_clause = ' WHERE (%s)' % incremental_update_extract_predicate
            row_source = union_branch_template % {'batch': batch_expr,
                                                  'batch_col': TRANSPORT_ROW_SOURCE_QUERY_SPLIT_COLUMN,
                                                  'incremental_update_meta_columns': incremental_update_meta_columns,
                                                  'owner_table': owner_table,
            ...
            row_source = union_all.join([union_branch_template
                                         % {'batch': (i % parallelism),
                                            ...
                                            'low_val': lowhigh[0],
                                            'high_val': lowhigh[1]} for i, (lowhigh) in enumerate(id_ranges)])
        return row_source
    ...
    def log_sql_stats(self, module, action, payload,
        ...
        def log_sql_info(sql_info):
            if sql_info:
                ...
                self.log(row_format.format('SQL Id', 'Child#', 'Executions', 'Rows', 'CPU(s)', 'IO Wait(s)'),
                         detail=VVERBOSE)
                for sql_id, child_number, rows_processed, cpu_seconds, io_wait_seconds, executions in sql_info:
                    self.log(row_format.format(sql_id, child_number, executions, rows_processed, cpu_seconds,
                                               io_wait_seconds), detail=VVERBOSE)
                self.log(row_format.format('Total', '', tot_executions, tot_rows_processed, tot_cpu_seconds,
                                           tot_io_wait_seconds), detail=VVERBOSE)
        ...
        try:
            if payload:
                ...
                for sql_id_child_num, row_data in groupby(sorted(row_snapshots), lambda x: '%s:%s' % (x[0], x[1])):
                    ...
                    sql_info.append((sql_id, child_number, max([_[2] for _ in rows]), max([_[3] for _ in rows]),
                                     max([_[4] for _ in rows]),
                                     max([_[5] for _ in rows])))
            else:
                q_params = {'module': module,
                            'action': action}
                self.log('Searching for transport SQL matching: %s' % str(q_params), detail=VVERBOSE)
...
ORDER BY seconds DESC"""
        q_params = {'rdbms_app_user': str(self._offload_options.rdbms_app_user.upper()),
    ...
    def query_import_extraction(self, staging_columns, source_query, source_binds,
        ...
        cx = get_rdbms_connection_for_oracle(
            self._offload_options.rdbms_app_user, self._offload_options.rdbms_app_pass,
            self._offload_transport_dsn, self._offload_options.use_oracle_wallet
        )
        ora_cursor = cx.cursor()
        ...
        self.log('Importing load data with arraysize=%s, compression=%s'
                 % (ora_cursor.arraysize, compress), detail=VERBOSE)
    ...
    def sqoop_rdbms_specific_jvm_overrides(self, rdbms_session_setup_commands) -> list:
        oraoop_d_options = ['-Doracle.sessionTimeZone=UTC',
                            '-Doraoop.timestamp.string=true',
                            '-Doraoop.jdbc.url.verbatim=true']
        if rdbms_session_setup_commands:
            oraoop_d_options += ["-Doraoop.oracle.session.initialization.statements=%s"
    ...
    def sqoop_rdbms_specific_jvm_table_options(self, partition_chunk, partition_type=None,
        ...
        if is_iot and partition_type == OFFLOAD_PARTITION_TYPE_RANGE:
            ...
            oraoop_options += ['-Doraoop.import.consistent.read={true_or_false}'.format(true_or_false='true' if consistent_read else 'false')]

        if partition_chunk and partition_chunk.count() > 0:
            oraoop_options += ['-Doraoop.import.partitions='
    ...
    def sqoop_rdbms_specific_table_options(self, rdbms_owner, table_name) -> list:
        return ['--table', self._ssh_cli_safe_value((rdbms_owner + '.' + table_name).upper())]
