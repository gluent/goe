...
class MSSQLFrontendTestingApi(FrontendTestingApiInterface):
    ...
    def _create_new_testing_client(self, existing_connection,
        ...
        return MSSQLFrontendTestingApi(self._frontend_type, self._connection_options, self._messages,
    ...
    def _gl_chars_column_definitions(self, ascii_only=False, all_chars_notnull=False,
        ...
        raise NotImplementedError('MSSQL _gl_chars_column_definitions() not yet implemented')

    def _gl_type_mapping_column_definitions(self, max_backend_precision, max_backend_scale,
                                            max_decimal_integral_magnitude, filter_column=None):
        raise NotImplementedError('MSSQL _gl_type_mapping_column_definitions() not yet implemented')

    def _gl_types_column_definitions(self, all_chars_ascii7=False, all_chars_notnull=False,
                                     backend_supports_canonical_float=True, include_interval_columns=True) -> list:
        raise NotImplementedError('MSSQL _gl_types_column_definitions() not yet implemented')
    ...
    def _gl_wide_column_definitions(self, ascii_only=False, all_chars_notnull=False,
        ...
        raise NotImplementedError('MSSQL _gl_wide_column_definitions() not yet implemented')
    ...
    def gl_type_mapping_generated_table_col_specs(self, max_backend_precision, max_backend_scale,
                                                  ...
                                                  ascii_only=False):
        raise NotImplementedError('MSSQL gl_type_mapping_generated_table_col_specs() not yet implemented')
    ...
    def host_compare_sql_projection(self, column_list: list) -> str:
        ...
        for column in column_list:
            if column.data_type == MSSQL_TYPE_DATETIMEOFFSET:
                projection.append("REPLACE(CONVERT({},{},127),'Z',' ')".format(MSSQL_TYPE_VARCHAR,
                    self._db_api.enclose_identifier(column.name)))
            elif column.is_date_based():
                # CONVERT() style 21:
                #   yyyy-mm-dd hh:mi:ss.mmm (24h)
                projection.append("CONVERT({},{},21)".format(MSSQL_TYPE_VARCHAR,
                    self._db_api.enclose_identifier(column.name)))
                ...
                projection.append("CONVERT({},{},3)".format(MSSQL_TYPE_VARCHAR,
                    self._db_api.enclose_identifier(column.name)))
            else:
    ...
    def sales_based_fact_truncate_partition_ddl(self, schema: str, table_name: str,
                                                ...
                                                dropping_oldest: Optional[bool]=None) -> list:
    ...
    def sales_based_fact_late_arriving_data_sql(self, schema: str, table_name: str, time_id_literal: str):
