...
class KeyValueDatasetSnapshot(object):
    ...
    def get(self, key_string):
        return (
            self._kv_updates[key_string]
            if key_string in self._kv_updates
            else self._kv_dict_original[key_string]
        )
    ...
    def get_optional(self, key_string, default_value):
        return (
            self._kv_updates[key_string]
            if key_string in self._kv_updates
            else self._kv_dict_original.get(key_string, default_value)
        )
...
class KeyValueDatasetSpecification(object):
    def __init__(
            ...
            def do_deserialize(value_string):
                try:
                    ...
                    raise SerializationError(
                        'error deserializing value, "{}", for field, {}: {}'.format(
        ...
        def get_serializer_func(fieldname, serializer):
            def do_serialize(value):
                try:
                    ...
                    raise SerializationError(
                        "error serializing value, {}, for field, {}: {}".format(
                            value, fieldname, e
                        )
                    )
        ...
        for fieldname in self.required_fields.union(self.optional_fields):
            ...
            self._field_deserializers[fieldname] = get_deserializer_func(
        ...
        for fieldname in self.required_fields.union(self.optional_fields):
            ...
            self._field_serializers[fieldname] = get_serializer_func(
    ...
    def check_for_missing_fields(self, kv_dict, error_msg_prefix):
        ...
        assert not missing, "{} -- missing keys: ({})".format(
            error_msg_prefix, ", ".join(missing)
        )

    def check_for_unrecognized_fields(self, kv_dict, error_msg_prefix):
        unrecognized = set(kv_dict.keys()) - self.all_fields
        assert not unrecognized, "{} -- unrecognized keys: ({})".format(
            error_msg_prefix, ", ".join(unrecognized)
        )
...
class KeyValueDatasetPersistence(object):
    ...
    def load_dataset_snapshot(self):
        kv_string_dict = self.key_value_store.get_all(
            self.key_value_dataset_spec.all_fields
        )
        kv_dict = {
            k: self.key_value_dataset_spec.deserialize(k, v)
            for k, v in kv_string_dict.items()
            if v is not None
        }
        self.key_value_dataset_spec.check_for_missing_fields(
            kv_dict, "error loading dataset"
        )
    ...
    def persist_updates(self, kv_dataset_snapshot):
        ...
        self.key_value_dataset_spec.check_for_unrecognized_fields(
            updates_dict, "error persisting updates"
        )
        kv_serialized = {
            k: self.key_value_dataset_spec.serialize(k, v)
    ...
    def persist_full_dataset_snapshot(self, kv_dataset_snapshot):
        ...
        self.key_value_dataset_spec.check_for_missing_fields(
            full_dataset_dict, "error persisting full dataset"
        )
        self.key_value_dataset_spec.check_for_unrecognized_fields(
            full_dataset_dict, "error persisting full dataset"
        )
        kv_serialized = {
            k: self.key_value_dataset_spec.serialize(k, v)
            for k, v in full_dataset_dict.items()
        }
        self.key_value_store.put_all(kv_serialized)
