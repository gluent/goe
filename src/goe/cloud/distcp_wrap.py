...
# EXCEPTIONS
###############################################################################
class DistcpWrapException(Exception): pass
...
class DistcpWrap(object):
    ...
    def _construct_distcp_command(self, source, destination, update, delete):
        ...
        no_of_mappers = self._cfg.getint(DEFAULT_CFG_SECTION, 'distcp_mappers') if self._cfg else None
        ...
        if isinstance(source, list):
            ...
            cmd = "hadoop distcp -i %s %s %s -f file://%s %s" % \
                (mappers, update_semantics, delete_marker, source_file, destination)
        else:
            cmd = "hadoop distcp -i %s %s %s %s %s" % \
                (mappers, update_semantics, delete_marker, source, destination)
    ...
    def prepare(self, source, destination, user=None, update=True, delete=False):
        ...
        logger.debug("PREPARING DISTCP: Source: %s, Destination: %s, User: %s" % \
            ('<files>' if isinstance(source, list) else source, \
             destination, user if user else "current user"))
