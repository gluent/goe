...
from gluentlib.offload import offload_constants, predicate_offload
from gluentlib.offload.column_metadata import valid_column_list
from gluentlib.offload.offload_functions import get_dsl_threshold_clauses, datetime_literal_to_python
...
from gluentlib.offload.oracle.oracle_column import ORACLE_TYPE_TIMESTAMP, ORACLE_TYPE_TIMESTAMP_TZ
from gluentlib.persistence.orchestration_metadata import INCREMENTAL_PREDICATE_TYPE_PREDICATE,\
    INCREMENTAL_PREDICATE_TYPE_RANGE, INCREMENTAL_PREDICATE_TYPE_RANGE_AND_PREDICATE, INCREMENTAL_PREDICATE_TYPE_LIST,\
...
OFFLOAD_PARTITION_APPEND_CAPABLE_TYPES = [OFFLOAD_PARTITION_TYPE_RANGE, OFFLOAD_PARTITION_TYPE_LIST]

OFFLOAD_SOURCE_CLIENT_OFFLOAD = 'offload'
OFFLOAD_OP_TERMS = {
    OFFLOAD_SOURCE_CLIENT_OFFLOAD: {'name': 'offload', 'now': 'offloading', 'past': 'offloaded'},
}
...
INVALID_HV_FOR_LIST_AS_RANGE_EXCEPTION_TEXT = 'Partitions have key values which are incompatible with LIST_AS_RANGE offloading'
...
PREDICATE_TYPE_NO_MODIFY_HV_EXCEPTION_TEXT = 'Option --no-modify-hybrid-view cannot be used with INCREMENTAL_PREDICATE_TYPE'
PREDICATE_TYPE_NO_MODIFY_RESET_EXCEPTION_TEXT = 'Options --no-modify-hybrid-view and --reset-hybrid-view are not compatible'
...
RANGE_AND_PREDICATE_WITHOUT_PART_KEY_EXCEPTION_TEXT = 'Offload predicate must contain a predicate for all existing partition key columns'
...
def offload_source_data_factory(offload_source_table, offload_target_table, offload_operation,
                                hybrid_metadata, messages, source_client_type, rdbms_partition_columns_override=None,
    ...
    if offload_operation.offload_predicate \
    or offload_operation.ipa_predicate_type == INCREMENTAL_PREDICATE_TYPE_PREDICATE:
        ...
        return OffloadSourceDataPredicate(offload_source_table, offload_target_table, offload_operation,
                                          hybrid_metadata, messages,
        ...
        return OffloadSourceDataFull(offload_source_table, offload_target_table, offload_operation,
                                     hybrid_metadata, messages, source_client_type=source_client_type)
    elif offload_source_table.partition_type not in OFFLOAD_PARTITION_APPEND_CAPABLE_TYPES:
        messages.log('Offload source data type: OffloadSourceDataFullPartitioned', detail=VVERBOSE)
        return OffloadSourceDataFullPartitioned(offload_source_table, offload_target_table, offload_operation,
                                                hybrid_metadata, messages,
                                                source_client_type=source_client_type,
                                                rdbms_partition_columns_override=rdbms_partition_columns_override,
                                                col_offload_source_table_override=col_offload_source_table_override)
    elif offload_source_table.partition_type in (OFFLOAD_PARTITION_TYPE_RANGE, OFFLOAD_PARTITION_TYPE_LIST):
        bad_data_types = offload_source_table.unsupported_partition_data_types()
        if bad_data_types:
            messages.log('Table %s.%s is %s partitioned by %s (not %s), incremental %s is not supported'
                         % (offload_source_table.owner, offload_source_table.table_name,
                            offload_source_table.partition_type, ', '.join(bad_data_types),
                            ', '.join(offload_source_table.supported_partition_data_types()), terms['name']),
                         detail=VERBOSE)
            messages.log('Offload source data type: OffloadSourceDataFullPartitioned', detail=VVERBOSE)
            return OffloadSourceDataFullPartitioned(offload_source_table, offload_target_table, offload_operation,
                                                    hybrid_metadata, messages,
        ...
        if offload_source_table.partition_type == OFFLOAD_PARTITION_TYPE_RANGE:
            ...
            return OffloadSourceDataIpaRange(offload_source_table, offload_target_table, offload_operation,
                                             hybrid_metadata, messages, source_client_type=source_client_type,
                                             rdbms_partition_columns_override=rdbms_partition_columns_override,
                                             col_offload_source_table_override=col_offload_source_table_override)
        elif offload_source_table.partition_type == OFFLOAD_PARTITION_TYPE_LIST \
         and offload_operation.ipa_predicate_type in (INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE,
                                                      INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE_AND_PREDICATE):
            messages.log('Offload source data type: OffloadSourceDataIpaListAsRange', detail=VVERBOSE)
            return OffloadSourceDataIpaListAsRange(offload_source_table, offload_target_table, offload_operation,
                                                   hybrid_metadata, messages, source_client_type=source_client_type,
                                                   rdbms_partition_columns_override=rdbms_partition_columns_override,
                                                   col_offload_source_table_override=col_offload_source_table_override)
        elif offload_source_table.partition_type == OFFLOAD_PARTITION_TYPE_LIST:
            messages.log('Offload source data type: OffloadSourceDataIpaList', detail=VVERBOSE)
            return OffloadSourceDataIpaList(offload_source_table, offload_target_table, offload_operation,
                                            hybrid_metadata, messages, source_client_type=source_client_type,
                                            rdbms_partition_columns_override=rdbms_partition_columns_override,
                                            col_offload_source_table_override=col_offload_source_table_override)
        else:
            raise NotImplementedError('Offload source data method not implemented for partition type: %s'
...
def get_offload_type_for_config(owner, table_name, user_requested_offload_type, incr_append_capable,
                                ida_options_specified, hybrid_metadata, messages, with_messages=True):
    ...
    if user_requested_offload_type:
        ...
        if offload_type == OFFLOAD_TYPE_INCREMENTAL and not incr_append_capable:
            if with_messages:
                messages.warning('Offload type %s incompatible with table %s.%s, continuing with type "FULL"' \
                                 % (offload_type, owner, table_name), ansi_type='red')
            offload_type = OFFLOAD_TYPE_FULL
        ...
        if hybrid_metadata.incremental_high_value or hybrid_metadata.incremental_predicate_value:
            if with_messages:
                if hybrid_metadata.incremental_high_value:
                    messages.log('Including HWM in hybrid view due to existing INCREMENTAL_HIGH_VALUE metadata',
                                 detail=VVERBOSE)
                else:
                    messages.log('Including HWM in hybrid view due to existing INCREMENTAL_PREDICATE_VALUE metadata',
                                 detail=VVERBOSE)
    ...
    if not pred_for_90_10_in_hybrid_view:
        if offload_type == OFFLOAD_TYPE_INCREMENTAL:
            ...
            if with_messages:
                messages.log('Including HWM in hybrid view due to command options', detail=VVERBOSE)
            pred_for_90_10_in_hybrid_view = True
...
def is_default_partition(partition):
    ...
    return True if [upper_fn(_) for _ in partition.partition_values_python] == [offload_constants.PART_OUT_OF_LIST] else False
...
class OffloadSourcePartition(object):
    ...
    """
    def __init__(self, partition_name, partition_literal, partition_values_python, partition_values_individual,
                 size_in_bytes, row_count, common_partition_literal, subpartitions):
    ...
    def __str__(self):
        return 'OffloadSourcePartition(partition_name=%s, partition_literal=%r)' % (self.partition_name,
...
class OffloadSourcePartitions(object):
    ...
    def __str__(self):
        if self._partitions:
            return 'OffloadSourcePartitions(partitions=%s, partition range=%s-%s)' % (len(self._partitions), self._partitions[0].partition_name, self._partitions[-1].partition_name)
        else:
    ...
    def from_source_table(offload_source_table, partition_append_capable):
        ...
        if rdbms_partitions:
            for ora_partition in rdbms_partitions:
                partitions.append(OffloadSourcePartition(name_fn(ora_partition),
    ...
    def report_partitions(self, offload_by_subpartition, messages, include_stats=True):
        ...
        if include_stats:
            messages.log('%s %s (%s - %s), total size %s, total rows (from stats) %s'
                         % (len(self._partitions), part_description, self._partitions[-1].partition_name,
                            self._partitions[0].partition_name, size, rows))
        else:
    ...
    def get_partition(self, partition_name=None, partition_values_python=None):
        ...
        if not filtered_partitions:
            ...
            raise OffloadSourceDataException('Unexpected partition count for partition name %s: %s' % (partition_name, len(filtered_partitions)))
        else:
    ...
    def get_prior_partition(self, partition=None, partition_name=None, partition_values_python=None):
        ...
        start_partition = partition or self.get_partition(partition_name=partition_name, partition_values_python=partition_values_python)
        if not start_partition:
            if partition_name:
                raise OffloadSourceDataException('%s: %s' % (NO_MATCHING_PARTITION_EXCEPTION_TEXT, partition_name))
            else:
                raise OffloadSourceDataException('No partition found matching high value: %s' % str(partition_values_python))
    ...
    def has_maxvalue_partition(self):
        ...
        if (not self._partitions) or (self._partitions[0].partition_literal is None):
            return None
        elif offload_constants.PART_OUT_OF_RANGE in self._partitions[0].partition_values_individual:
...
class OffloadSourceDataInterface(metaclass=ABCMeta):
    ...
    def __init__(self, offload_source_table, offload_target_table, offload_operation, hybrid_metadata,
                 messages, source_client_type, rdbms_partition_columns_override=None,
                 col_offload_source_table_override=None):
        ...
        if col_offload_source_table_override:
            assert col_offload_source_table_override and isinstance(col_offload_source_table_override,
        ...
        if col_offload_source_table_override:
            self.log('Overriding source RDBMS columns with: %s.%s.columns'
                     % (col_offload_source_table_override.owner, col_offload_source_table_override.table_name),
                     detail=VVERBOSE)
        ...
        if rdbms_partition_columns_override:
            self.log('Overriding source data partition columns with: %s' % str(rdbms_partition_columns_override),
                     detail=VVERBOSE)
    ...
    def _get_backend_synth_part_expr(self):
        ...
        if not self._target_table_exists():
            ...
            self._backend_synth_part_expr = self._offload_target_table.gen_synthetic_partition_col_expressions(as_python_fns=True)
    ...
    def _set_offload_type(self):
        self._offload_type, self._pred_for_90_10_in_hybrid_view = \
            get_offload_type_for_config(self._part_offload_source_table.owner,
    ...
    def _populate_partitions_to_offload(self):
        ...
        if not self._partitions_to_offload:
            partition_delta = [_ for _ in self._source_partitions.get_partitions()
    ...
    def _make_rpa_hwm_gte_clause(self):
        ...
        hv_tuple = self._part_offload_source_table.decode_partition_high_values(prior_rdbms_hv) if prior_rdbms_hv else None
        ...
        if hv_tuple:
            ...
            backend_literal = self._offload_target_table.to_backend_literal(converted_hv, data_type=matching_backend_part_col.data_type)
    ...
    def _combine_part_cols_with_values(self, row_values):
        ...
        if not row_values:
            return None
        new_row = [(self._datetime_literal_to_python(row_val) if part_col.is_date_based() else row_val)
    ...
    def _select_target_table_max(self):
        ...
        ts_columns = [_.name for _ in self._partition_columns if _.data_type in (ORACLE_TYPE_TIMESTAMP,
        ...
        try:
            max_row = self._offload_target_table.get_max_column_values(column_names,
                                                                       columns_to_cast_to_string=ts_columns,
                                                                       optimistic_prune_clause=prune_clause)
        finally:
            t2 = datetime.now().replace(microsecond=0)
            self.log('Elapsed time to retrieve maximum backend partition key data: %s' % (t2 - t1), detail=VVERBOSE)
    ...
    def _report_source_partitions(self):
        ...
        self._source_partitions.report_partitions(self._part_offload_source_table.offload_by_subpartition,
                                                  self._messages)
    ...
    def _report_offloaded_partitions(self):
        ...
        self._offloaded_partitions.report_partitions(self._part_offload_source_table.offload_by_subpartition,
                                                     self._messages)
    ...
    def _report_partitions_to_offload(self):
        ...
        self._partitions_to_offload.report_partitions(self._part_offload_source_table.offload_by_subpartition,
                                                      self._messages)
    ...
    def populate_source_partitions(self, sort_by_hv=False):
        ...
        if not self._source_partitions:
            source_partitions = OffloadSourcePartitions.from_source_table(self._part_offload_source_table, \
                partition_append_capable=self._partition_append_capable)
            if sort_by_hv:
                self.log('Sorting %s partitions by partition value' % source_partitions.count(), detail=VVERBOSE)
    ...
    def get_partitions_to_offload_chunks(self):
        ...
        while remaining:
            ...
            chunk.reverse()
            yield (OffloadSourcePartitions(chunk), OffloadSourcePartitions(remaining[:]))
...
class OffloadSourceDataPredicate(OffloadSourceDataInterface):
    ...
    def __init__(self, offload_source_table, offload_target_table, offload_operation, hybrid_metadata,
        ...
        super(OffloadSourceDataPredicate, self).__init__(
            offload_source_table, offload_target_table, offload_operation, hybrid_metadata, messages,
            source_client_type=source_client_type, rdbms_partition_columns_override=rdbms_partition_columns_override,
            col_offload_source_table_override=col_offload_source_table_override,
        )

        self._incremental_hvs = []

        self._messages.log('Predicates (%s) supplied, %s selected rows from %s.%s'
                           % (offload_operation.offload_predicate.dsl, self._terms['now'], offload_source_table.owner,
                              offload_source_table.table_name), detail=VERBOSE)
    ...
    def _no_offload_type_full(self, predicate_type):
        if self._user_requested_offload_type == OFFLOAD_TYPE_FULL or self._offload_type == OFFLOAD_TYPE_FULL:
            raise OffloadSourceDataException('%s: %s' % (PREDICATE_TYPE_OFFLOAD_TYPE_FULL_EXCEPTION_TEXT,
                                                         predicate_type))

    def _offload_predicate_case_1(self):
        case_1 = bool(not self._pre_offload_hybrid_metadata
                      or self.get_pre_offload_predicate_type() == INCREMENTAL_PREDICATE_TYPE_PREDICATE)
        if case_1:
            self.log('Pre-offload predicate type is case 1: %s'
                     % self.get_pre_offload_predicate_type(), detail=VVERBOSE)
        return case_1
    ...
    def _offload_predicate_case_2_type(self):
        ...
        if not self._pre_offload_hybrid_metadata:
            ...
            return None
        elif (pre_offload_offload_type == OFFLOAD_TYPE_INCREMENTAL
        ...
        elif (pre_offload_offload_type == OFFLOAD_TYPE_FULL
    ...
    def _offload_predicate_case_2(self):
        ...
        if case_2:
            pre_offload_offload_type = self._get_pre_offload_offload_type()
            if (self._user_requested_offload_type and pre_offload_offload_type
            ...
            self.log('Pre-offload/user requested predicate type is case 2 (%s late arriving data): %s/%s'
                     % (lapbo_type, self.get_pre_offload_predicate_type(),
    ...
    def _offload_predicate_case_3(self):
        case_3 = bool(
            (self.get_pre_offload_predicate_type() in (INCREMENTAL_PREDICATE_TYPE_RANGE,
                                                       INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE)
             and self._user_requested_predicate_type in (INCREMENTAL_PREDICATE_TYPE_RANGE_AND_PREDICATE,
                                                         INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE_AND_PREDICATE))
            or self.get_pre_offload_predicate_type() in (INCREMENTAL_PREDICATE_TYPE_RANGE_AND_PREDICATE,
                                                         INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE_AND_PREDICATE))
        if case_3:
            self.log('Pre-offload/user requested predicate type is case 3 (intra day): %s/%s'
                     % (self.get_pre_offload_predicate_type(), self._user_requested_predicate_type),
                     detail=VVERBOSE)
        return case_3
    ...
    def _offload_predicate_contains_hwm_cols(self):
        ...
        hv_columns, hv_values, hv_sql_exprs = decode_metadata_incremental_high_values_from_metadata(
    ...
    def offload_data_detection(self):
        ...
        if self._offload_predicate_case_1():
            if not self._offload_predicate_modify_hybrid_view:
                raise OffloadSourceDataException('%s: %s' % (PREDICATE_TYPE_NO_MODIFY_HV_EXCEPTION_TEXT,
                                                             INCREMENTAL_PREDICATE_TYPE_PREDICATE))
            if self._user_requested_predicate_type not in (None, INCREMENTAL_PREDICATE_TYPE_PREDICATE):
                raise OffloadSourceDataException('%s: %s' % (PREDICATE_TYPE_INCOMPATIBLE_EXCEPTION_TEXT,
                                                             self._user_requested_predicate_type))
            ...
            if self._offload_predicate_modify_hybrid_view:
                ...
                self.log('Setting --no-modify-hybrid-view for late arriving predicate offload', detail=VVERBOSE)

            self._post_offload_predicates = self.get_pre_offload_predicates()
            self._partition_append_predicate_type = pre_offload_predicate_type

            hv_columns, _, self._incremental_hvs = decode_metadata_incremental_high_values_from_metadata(
                self._pre_offload_hybrid_metadata, self._col_offload_source_table
            )

            if self._offload_predicate_case_2_type() in (LAPBO_TYPE_90_10, LAPBO_TYPE_100_10):
                self.populate_source_partitions()
                target_table_max = self._select_target_table_max()
                max_lt_dsl, _ = get_dsl_threshold_clauses(hv_columns, pre_offload_predicate_type, target_table_max, equality_with_gt=False)
            ...
            _, _, self._incremental_hvs = decode_metadata_incremental_high_values_from_metadata(
                self._pre_offload_hybrid_metadata, self._col_offload_source_table
            )
            ...
            self.log('pre_offload_predicate_type: %s' % pre_offload_predicate_type, detail=VVERBOSE)
            self.log('offload_predicate_modify_hybrid_view: %s' % self._offload_predicate_modify_hybrid_view, detail=VVERBOSE)
            if pre_offload_predicate_type in (INCREMENTAL_PREDICATE_TYPE_RANGE,
                                              INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE):
                raise OffloadSourceDataException(f'{PREDICATE_TYPE_REQUIRED_EXCEPTION_TEXT}: {pre_offload_predicate_type}')
            else:
...
class OffloadSourceDataFull(OffloadSourceDataInterface):
    ...
    def __init__(self, offload_source_table, offload_target_table, offload_operation,
                 hybrid_metadata, messages, source_client_type):
        """ CONSTRUCTOR
        """
        self._partition_append_capable = False
        ...
        super(OffloadSourceDataFull, self).__init__(offload_source_table, offload_target_table, offload_operation,
                                                    hybrid_metadata, messages,
                                                    source_client_type=source_client_type)

        self._messages.notice('Table %s.%s is not partitioned, %s entire table'
                              % (offload_source_table.owner, offload_source_table.table_name, self._terms['now']))
...
class OffloadSourceDataFullPartitioned(OffloadSourceDataInterface):
    ...
    def __init__(self, offload_source_table, offload_target_table, offload_operation, hybrid_metadata,
        ...
        messages.notice('Table %s.%s is %s partitioned, %s entire table'
                        % (offload_source_table.owner, offload_source_table.table_name,
                           offload_source_table.partition_type, self._terms['now']))
    ...
    def offloading_open_ended_partition(self):
        if not self.partitions_to_offload:
            return False
        return bool(self.partitions_to_offload.has_maxvalue_partition()
...
class OffloadSourceDataIpaRange(OffloadSourceDataInterface):
    ...
    def __init__(self, offload_source_table, offload_target_table, offload_operation, hybrid_metadata,
        ...
        self.log('Table %s.%s is %s partitioned by %s (%s)'
                 % (offload_source_table.owner, offload_source_table.table_name, offload_source_table.partition_type,
                    ', '.join([_.data_type for _ in self._partition_columns]),
                    ', '.join([_.name for _ in self._partition_columns])), detail=VERBOSE)
    ...
    def _check_number_of_partition_append_filters(self, check_tokens):
        ...
        if len(check_tokens) != len(self._partition_columns):
            raise OffloadSourceDataException('Partition filter %s has incorrect value. %d partition values required' % \
    ...
    def _normalise_rpa_partition_filter(self, user_requested_less_than_value):
        ...
        for part_col, col_val in zip(self._partition_columns, rpa_filter_tuple):
            if part_col.is_date_based() != isinstance(col_val, (datetime64, date)):
                raise OffloadSourceDataException('Filter %s is incompatible with partition key %s of type %s'
    ...
    def _normalise_rpa_partition_name(self):
        ...
        if len(self._user_requested_partition_names) != 1:
            raise OffloadSourceDataException('%s for %s offloaded table' \
                % (TOO_MANY_PARTITION_NAMES_EXCEPTION_TEXT, self._partition_append_predicate_type))
    ...
    def _rpa_hv_not_offloaded(self, partition):
        ...
        self.debug('Partition %s not offloaded check: %s: %s > %s'
                   % (partition.partition_name, match, tuple(partition.partition_values_python),
    ...
    def _rpa_partition_already_offloaded(self, partition, partition_already_marked_offloaded):
        ...
        def rpa_hv_already_offloaded(partition):
            match = bool((self._rdbms_min is not None and self._rdbms_min <= self._target_table_max)
                         or tuple(partition.partition_values_python) <= self._target_table_max)
            self.debug('Partition %s already offloaded check: %s: %s <= %s'
                       % (partition.partition_name,
                          bool(tuple(partition.partition_values_python) <= self._target_table_max),
                          tuple(partition.partition_values_python), self._target_table_max))
            self.log('Partition %s already offloaded check: %s' % (partition.partition_name, match), detail=VVERBOSE)
            return match
        if partition_already_marked_offloaded:
            self.debug('Partition %s already marked as offloaded: %s'
            ...
            for i, p in enumerate(reversed_offload_partitions):  # traverse old to new
                ...
                if self._rpa_hv_not_offloaded(p):
                    ...
                    if self._part_offload_source_table.offload_by_subpartition and i < len(reversed_offload_partitions):
                        ...
                        with_matching_hwm = self._partitions_of_matching_hwm(p.partition_values_python,
                        ...
                        if with_matching_hwm:
                            ...
                            self.log('Skipping %s partitions with matching HWM' % len(with_matching_hwm),
                                     detail=VVERBOSE)
    ...
    def _set_incremental_high_values(self):
        ...
        if self._ida_options_have_been_specified():
            ...
            if filtered_partitions:
                self.log('HWM partition %s derived from HWM filter' % filtered_partitions[0].partition_name,
                         detail=VVERBOSE)
    ...
    def _check_hwm_is_a_valid_boundary(self, hwm_partition, retained_partitions, offload_type):
        ...
        def is_valid_common_boundary(check_partition, retained_partitions_ascending):
            ...
            """
            return bool(check_partition.common_partition_literal and \
                        retained_partitions_ascending and \
                        check_partition.partition_values_python != retained_partitions_ascending[-1].partition_values_python)
        ...
        if is_valid_common_boundary(hwm_partition, hwm_check_retained):
            ...
            self.log('Usable common boundary found: %s' % more_human_readable_python_hwm(hwm_partition.partition_values_python), detail=VVERBOSE)
        else:
            ...
            if next_common_boundary and is_valid_common_boundary(next_common_boundary, hwm_check_retained):
                self._messages.warning('Minimum recommended common boundary is: --less-than-values=%s' % more_human_readable_python_hwm(next_common_boundary.partition_values_python))
            elif offload_type == OFFLOAD_TYPE_INCREMENTAL:
                # It's the last HWM in the table...
                raise OffloadSourceDataException('Table is not valid for range subpartition offloading due to incompatible high value boundaries. Offload with --offload-type=FULL')
            if offload_type == OFFLOAD_TYPE_INCREMENTAL:
                # regardless of the messages and tests above, we don't have a valid HWM so need to stop here
                if self._get_rpa_filter():
                    raise OffloadSourceDataException('No common boundary at HWM filter: %s' % more_human_readable_python_hwm(self._get_rpa_filter()))
                else:
                    raise OffloadSourceDataException('No common boundary at HWM: %s' % more_human_readable_python_hwm(hwm_partition.partition_values_python))
    ...
    def _get_less_than_rpa_filter_fn(self):
        ...
        def filter_fn(p):
            try:
                ...
                self.log('{} <= {} throws: {}'.format(str(p.partition_values_python), str(less_than_list), str(exc)),
                         detail=VERBOSE)
                raise
            self.debug('Matching partition %s (%s <= %s): %s'
                       % (p.partition_name, str(p.partition_values_python), str(less_than_list), match))
            return match
    ...
    def _set_rdbms_min(self):
        ...
        try:
            ...
            t2 = datetime.now().replace(microsecond=0)
            self.log('Elapsed time to retrieve minimum RDBMS partition key data: %s' % (t2 - t1), detail=VVERBOSE)
    ...
    def _discard_partitions_using_filters(self):
        ...
        if self._ida_options_have_been_specified():
            filtered_partitions, discarded_partitions = self._split_partitions_to_offload_by_rpa_filter()
            ...
            if self._offload_type == OFFLOAD_TYPE_INCREMENTAL:
                ...
                if filtered_partition_list:
                    self.log('Offloading all partitions when OFFLOAD_TYPE is %s' % self._offload_type, detail=VERBOSE)

        if removed_open_partition:
            # Add the MAXVALUE/DEFAULT partition to the retained list
            # TODO need to review this for Teradata when there could be 2 open (MAXVALUE) partitions
            self._messages.notice('Not %s partition %s %s'
                                  % (self._terms['now'], removed_open_partition.partition_name,
    ...
    def offload_data_detection(self):
        ...
        if self.partitions_to_offload.count() == 0:
            ...
            keep_partitions, discard_partitions = self.split_partitions_to_offload_by_no_segment()
            ...
            if discard_partitions.count() > 0:
                ...
                discard_partitions.report_partitions(self._part_offload_source_table.offload_by_subpartition, self._messages)

            if self.partitions_to_offload.has_maxvalue_partition():
                self._messages.notice('%s partition %s with high_value of %s'
            ...
            if self._target_table_max and (not self._rdbms_min or self._target_table_max < self._rdbms_min):
                ...
                self._messages.warning('Offloaded data is less than RDBMS data, data verification may fail due to purged historical RDBMS data')
...
class OffloadSourceDataIpaList(OffloadSourceDataInterface):
    ...
    def _default_partition_has_matching_keys(self, list_of_keys):
        ...
        self.log(f'LPA table has {offload_constants.PART_OUT_OF_LIST} partition, fetching partition keys from {offload_constants.PART_OUT_OF_LIST} partition: {default_partition}',
                 detail=VVERBOSE)
        ...
        if matching_keys:
            self.log('LPA keys indicating open partition has been offloaded: %s' % str(matching_keys), detail=VVERBOSE)
            return True
        return False
        ...
        if self._lpa_partition_filters is None:
            ...
            if self._user_requested_lpa_filters:
                ...
                if type(self._user_requested_lpa_filters) not in (list, tuple):
                    raise OffloadSourceDataException('Invalid type for partition append filter: %s'
                ...
                for partition_name in self._user_requested_partition_names:
                    ...
                    if matched_partition:
                        ...
                        raise OffloadSourceDataException('%s: %s'
    ...
    def _get_lpa_keys_from_target(self):
        ...
        column_names = [_.name for _ in self._partition_columns]
        ts_columns = [_.name for _ in self._partition_columns if _.data_type in (ORACLE_TYPE_TIMESTAMP, ORACLE_TYPE_TIMESTAMP_TZ)]
    ...
    def _get_lpa_keys_for_default_partition(self, default_partition_name=None):
        ...
        try:
            ...
            t2 = datetime.now().replace(microsecond=0)
            self.log('Elapsed time to retrieve distinct RDBMS partition key data: %s' % (t2 - t1), detail=VVERBOSE)
    ...
    def _set_incremental_high_values(self):
        ...
        def get_metadata_list():
            if self._pre_offload_hybrid_metadata:
                _, _, hv_individual_list = decode_metadata_incremental_high_values_from_metadata(
                    self._pre_offload_hybrid_metadata, self._col_offload_source_table
                )
        ...
        if self._pred_for_90_10_in_hybrid_view:
            ...
            if self._get_lpa_partition_filters():
                ...
                matching_list_hvs = [_.partition_values_individual for _ in self._get_source_partitions_matching_lpa_filters()]
            elif self._partitions_to_offload.count() > 0:
                self.debug('new HVs from partitions to offload')
                matching_list_hvs = [_.partition_values_individual for _ in self._partitions_to_offload.get_partitions()]
            else:
                matching_list_hvs = []
    ...
    def _populate_offloaded_partitions(self):
        ...
        if not self._offloaded_partitions:
            ...
            if self._offload_type == OFFLOAD_TYPE_INCREMENTAL and self._default_partition_has_been_offloaded():
                raise OffloadSourceDataException('%s: %s' % (INCREMENTAL_OFFLOAD_DEFAULT_PARTITION_EXCEPTION_TEXT,
                                                             self._offload_type))
    ...
    def _lpa_partition_filter_fn(self, lpa_filters):
        ...
        def filter_fn(p):
            ...
            if is_default_partition(p):
                upper_fn = lambda s: s.upper() if isinstance(s, str) else s
                if any(upper_fn(list_item) == offload_constants.PART_OUT_OF_LIST
                       for lpa_filter in lpa_filters for list_item in lpa_filter):
    ...
    def _discard_partitions_using_filters(self):
        ...
        if self._offload_type == OFFLOAD_TYPE_INCREMENTAL and self.partitions_to_offload.has_default_partition():
            raise OffloadSourceDataException('%s %s for offload type %s'
...
class OffloadSourceDataIpaListAsRange(OffloadSourceDataIpaRange):
    ...
    def _check_for_unsupported_partitions(self):
        ...
        if matching_partitions:
            raise OffloadSourceDataException('%s: %s' \
                % (INVALID_HV_FOR_LIST_AS_RANGE_EXCEPTION_TEXT, [_.partition_name for _ in matching_partitions]))
    ...
    def _get_less_than_rpa_filter_fn(self):
        ...
        def filter_fn(p):
            if is_default_partition(p):
                self.log(f'Rejecting {offload_constants.PART_OUT_OF_LIST} partition {p.partition_name}',
                         detail=VVERBOSE)
                return False
            if less_than_list:
                match = bool(tuple(p.partition_values_python) < tuple(less_than_list))
                ...
                self.debug('Matching partition %s (%s <= %s): %s' \
                    % (p.partition_name, str(p.partition_values_python), str(lte_list), match))
            return match
    ...
    def _ida_options_have_been_specified(self):
        if self._user_requested_rpa_filter or self._user_requested_partition_names or self._user_requested_list_rpa_lte_filter:
            return True
        return False
...
class OffloadSourceDataIUPartitioned(OffloadSourceDataInterface):
    ...
    def __init__(self, offload_source_table, offload_target_table, offload_operation, hybrid_metadata,
        ...
        messages.notice('Table %s.%s is %s partitioned, %s across entire table'
                        % (offload_source_table.owner, offload_source_table.table_name,
                           offload_source_table.partition_type, self._terms['now']))
