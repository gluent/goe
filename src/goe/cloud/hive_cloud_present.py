...
from goe.cloud.hive_table_backup import HiveCfgConnection
...
# EXCEPTIONS
###############################################################################
class HiveCloudPresentException(Exception):
    pass
...
class HiveCloudPresent(OffloadMessagesMixin, object):
    ...
    def __init__(
        self, db_name, table_name, hive_section, cfg, messages=None, options=None
    ):
        """CONSTRUCTOR"""
    ...
    def _change_partition_location(self, partition_spec):
        ...
        self.log_vverbose(
            "Changing location for partition: %s to: %s"
            % (partition_spec["spec"], partition_spec["location"])
        )
        self.hive_table.change_partition_location(
            partition_spec["spec"], partition_spec["location"], self.option("sync_ddl")
        )

    def _create_partition(self, partition_spec):
        """Create partition according to partition_spec"""
        self.log_vverbose(
            "Adding partition: %s with location: %s"
            % (partition_spec["spec"], partition_spec["location"])
        )
        self.hive_table.add_partition(
            partition_spec["spec"], partition_spec["location"], self.option("sync_ddl")
        )
    ...
    def _task_change_partition_location(self, partition_spec, job_id):
        ...
        try:
            if partition_spec["create"]:
                logger.info(
                    "[Job: %d] Starting partition creation for: %s"
                    % (job_id, partition_spec)
                )
                self._create_partition(partition_spec)
            else:
                logger.info(
                    "[Job: %d] Starting partition location change for: %s"
                    % (job_id, partition_spec)
                )
                self._change_partition_location(partition_spec)

            logger.debug(
                "[Job: %d] Change for: %s - SUCCESS" % (job_id, partition_spec)
            )
    ...
    def _analyze_partitions(self, storage_section, file_root, file_data):
        ...
        def step_fn():
            logger.info(
                "Analyzing partition info for: %d files @ %s"
                % (len(file_data), file_root)
            )
            task_list = []
            ...
            for file_item in file_data:
                if "partitions" in file_item:
                    ...
                    p_location = "%s%s%s" % (
                        end_by(file_root, "/"),
                        end_by(
                            self._cfg.table_url(storage_section, self._db_table), "/"
                        ),
                    ...
                    if not self.hive_table.partition_exists(p_spec):
                        logger.debug(
                            "Partition: %s does NOT exist. Scheduling creation" % p_spec
                        )
                        p_create = True
                    ...
                    task_list.append(
                        [self._task_change_partition_location, partition_spec, job_id]
                    )
                    job_id += 1
                else:
                    logger.warn(
                        "Cannot find 'partitions' marker in file item: %s" % file_time
                    )
    ...
    def _execute_partition_tasks(self, parallel, task_list):
        ...
        def step_fn():
            logger.info(
                "Executing: %d partition tasks with: %d parallel slaves"
                % (len(task_list), parallel)
            )
    ...
    def _job_change_partition_locations(
        self, storage_section, file_root, file_data, parallel
    ):
        ...
        """
        logger.info(
            "Changing partition location for: %d files: -> %s using %d parallel slaves"
    ...
    def _allocate_storage(self, storage_section):
        ...
        if self._storage and storage_section == self._storage.section:
            logger.debug(
                "Reusing previous storage driver for section: %s" % storage_section
            )
            return self._storage

        storage_type = self._cfg.section_type(storage_section)
        logger.info(
            "Identified type: %s for section: %s" % (storage_type, storage_section)
        )
        ...
        if DST_S3 == storage_type:
            ...
            self._storage = HdfsStore(
                self._cfg,
                storage_section,
                self._cfg.table_url(storage_section, self._db_table),
            )
        else:
            raise HiveCloudPresentException(
                "Storage type: %s for section: %s is NOT supported"
    ...
    def _query_storage(self, storage_section, filters):
        ...
        def step_fn():
            logger.info(
                "Querying storage section: %s with filters: %s"
        ...
        return (
            self.step(
                "Querying storage: %s with filters: %s" % (storage_section, filters),
                step_fn,
            )
            or []
        )
    ...
    def _refresh_metadata(self):
        ...
        return (
            self.step("Invalidating metadata for table: %s" % self._db_table, step_fn)
            or True
        )
    ...
    def _summarize_partition_stats(self, partition_specifications=None):
        ...
        def step_fn():
            ...
            logger.info(
                "Summarizing partition stats for: %d partitions" % len(partition_specs)
            )
            ...
            for p in partition_specs:
                ...
                report[p_type]["partitions"].append(
                    self.hive_table._make_partition_str(p)
                )

                report[p_type]["files"] += p_details["#Files"]
                report[p_type]["rows"] += p_details["#Rows"]
                report[p_type]["size"] += from_impala_size(p_details["Size"])
                report[p_type]["min_ts"] = (
                    min(p_goets, report[p_type]["min_ts"])
                    if report[p_type]["min_ts"] is not None
                    else p_goets
                )
                report[p_type]["max_ts"] = (
                    max(p_goets, report[p_type]["max_ts"])
                    if report[p_type]["max_ts"] is not None
                    else p_goets
                )
    ...
    def _calculate_projected_stats(self, task_list):
        ...
        if projected_partitions:
            projected_state["create"] = sum(
                1 for _ in projected_partitions if _["create"]
            )
            projected_state["relocate"] = sum(
                1 for _ in projected_partitions if not _["create"]
            )

            current_state = self._summarize_partition_stats(
                [_["spec"] for _ in projected_partitions]
            )
            for p_type in current_state:
                ...
                if "max_ts" in projected_state:
                    projected_state["max_ts"] = max(
                        current_state[p_type]["max_ts"], projected_state["max_ts"]
                    )
                else:
                    projected_state["max_ts"] = current_state[p_type]["max_ts"]
                if "min_ts" in projected_state:
                    projected_state["min_ts"] = min(
                        current_state[p_type]["min_ts"], projected_state["min_ts"]
                    )
                else:
    ...
    def hive_conn(self):
        ...
        if current_thread not in self._hive_conn:
            self._hive_conn[current_thread] = HiveCfgConnection(
                self._cfg, self._hive_section, messages=self._messages
            )()
    ...
    def hive_table(self):
        ...
        if current_thread not in self._hive_table:
            self._hive_table[current_thread] = HiveTable(
                self._db_name, self._table_name, self.hive_conn
            )
    ...
    def present(self, storage_section, filters=None, parallel=1):
        ...
        """
        logger.info(
            "Starting partition present for table: %s using storage section: %s"
        ...
        if filtered_data:
            success, work_done = self._job_change_partition_locations(
                storage_section,
                self._make_base_url(storage_section),
                filtered_data,
                parallel,
            )
            if work_done:
                ...
                logger.info(
                    "No work was needed for storage section: %s, filters: %s"
                    % (storage_section, filters)
                )
        else:
            logger.info(
                "No relevant data to process for storage section: %s, filters: %s"
                % (storage_section, filters)
            )
    ...
    def project(self, storage_section, filters=None):
        ...
        """
        logger.info(
            "Projecting partition present for table: %s using storage section: %s"
            % (self._db_table, storage_section)
        )
        success = True
        ...
        task_list = self._analyze_partitions(
            storage_section, self._make_base_url(storage_section), filtered_data
        )
        projected_state = self._calculate_projected_stats(task_list)

        logger.debug(
            "Projected changes after applying: %s filters for table: %s is: %s"
    ...
    def report(self):
        ...
        logger.debug(
            "Current state for table: %s is: %s" % (self._db_table, current_state)
        )
...
if __name__ == "__main__":
    ...
    def usage(prog_name):
        print(
            "usage: %s db_table section <comparison: <, >, <=, etc> yyyy-mm-dd [options] [log level]"
            % prog_name
        )
        sys.exit(1)
    ...
    def main():
        ...
        pre = HiveCloudPresent(
            db_name, table_name, section, messages=messages, options=options
        )
