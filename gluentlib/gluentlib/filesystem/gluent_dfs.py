...
VALID_OFFLOAD_FS_SCHEMES = [OFFLOAD_FS_SCHEME_INHERIT, OFFLOAD_FS_SCHEME_GS, OFFLOAD_FS_SCHEME_HDFS,
                            OFFLOAD_FS_SCHEME_S3A, OFFLOAD_FS_SCHEME_MAPRFS, OFFLOAD_FS_SCHEME_S3,
                            OFFLOAD_FS_SCHEME_WASB, OFFLOAD_FS_SCHEME_WASBS, OFFLOAD_FS_SCHEME_ADL,
                            OFFLOAD_FS_SCHEME_ABFS, OFFLOAD_FS_SCHEME_ABFSS]
OFFLOAD_FS_SCHEMES_REQUIRING_ACCOUNT = [OFFLOAD_FS_SCHEME_AZURE]
OFFLOAD_FS_SCHEMES_REQUIRING_CONTAINER = [OFFLOAD_FS_SCHEME_GS, OFFLOAD_FS_SCHEME_S3A, OFFLOAD_FS_SCHEME_S3,
                                          OFFLOAD_FS_SCHEME_WASB, OFFLOAD_FS_SCHEME_WASBS, OFFLOAD_FS_SCHEME_ADL,
                                          OFFLOAD_FS_SCHEME_ABFS, OFFLOAD_FS_SCHEME_ABFSS]
OFFLOAD_NON_HDFS_FS_SCHEMES = [_ for _ in VALID_OFFLOAD_FS_SCHEMES
                               if _ not in (OFFLOAD_FS_SCHEME_INHERIT, OFFLOAD_FS_SCHEME_HDFS)]
OFFLOAD_WEBHDFS_COMPATIBLE_FS_SCHEMES = [_ for _ in VALID_OFFLOAD_FS_SCHEMES
                                         if _ in (OFFLOAD_FS_SCHEME_INHERIT, OFFLOAD_FS_SCHEME_HDFS)]
AZURE_OFFLOAD_FS_SCHEMES = [OFFLOAD_FS_SCHEME_WASB, OFFLOAD_FS_SCHEME_WASBS, OFFLOAD_FS_SCHEME_ADL,
                            OFFLOAD_FS_SCHEME_ABFS, OFFLOAD_FS_SCHEME_ABFSS]
...
def uri_component_split(uri):
    ...
    if scheme and scheme not in VALID_OFFLOAD_FS_SCHEMES:
        raise GluentDfsException('%s: %s' % (UNSUPPORTED_URI_SCHEME_EXCEPTION_TEXT, scheme))
    else:
...
def gen_fs_uri(path_prefix, db_path_suffix=None, scheme=None, container=None, backend_db=None, table_name=None):
...
def gen_load_uri_from_options(offload_options, hadoop_db=None, table_name=None, scheme_override=None):
    """ Returns a Hadoop file URI based on options config.
    """
    uri = gen_fs_uri(offload_options.hdfs_load,
...
class GluentDfs(object, metaclass=ABCMeta):
    ...
    def get_perms(self, dfs_path):
        """ Return permissions of a file\path in numeric form, e.g. "755"
    ...
    @retry.Retry(predicate=retry.if_exception_type(GluentDfsFilesNotVisible, google_exceptions.ServiceUnavailable),
