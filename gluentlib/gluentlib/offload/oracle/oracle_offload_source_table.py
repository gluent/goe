...
from gluentlib.offload.offload_source_table import OffloadSourceTableInterface, OffloadSourceTableException,\
    RdbmsPartition, convert_high_values_to_python, \
    OFFLOAD_PARTITION_TYPE_HASH, OFFLOAD_PARTITION_TYPE_LIST, OFFLOAD_PARTITION_TYPE_RANGE
from gluentlib.offload.oracle.oracle_column import OracleColumn, \
    ORACLE_TYPE_CHAR, ORACLE_TYPE_NCHAR, ORACLE_TYPE_CLOB, ORACLE_TYPE_NCLOB, ORACLE_TYPE_LONG, ORACLE_TYPE_VARCHAR, \
    ORACLE_TYPE_VARCHAR2, ORACLE_TYPE_NVARCHAR2, \
    ORACLE_TYPE_RAW, ORACLE_TYPE_BLOB, ORACLE_TYPE_LONG_RAW, ORACLE_TYPE_NUMBER, ORACLE_TYPE_FLOAT, \
...
ORACLE_SUPPORTED_RANGE_DATA_TYPES = [ORACLE_TYPE_DATE, ORACLE_TYPE_NUMBER, ORACLE_TYPE_TIMESTAMP,
                                     ORACLE_TYPE_VARCHAR2, ORACLE_TYPE_NVARCHAR2]
ORACLE_SUPPORTED_LIST_DATA_TYPES = [ORACLE_TYPE_DATE, ORACLE_TYPE_NUMBER, ORACLE_TYPE_TIMESTAMP, ORACLE_TYPE_CHAR,
                                    ORACLE_TYPE_VARCHAR2, ORACLE_TYPE_NCHAR, ORACLE_TYPE_NVARCHAR2]
...
class OracleSourceTable(OffloadSourceTableInterface):
    ...
    def __init__(self, schema_name, table_name, connection_options, messages,
                 dry_run=False, conn=None, do_not_connect=False):
        assert schema_name and table_name and connection_options
        assert hasattr(connection_options, 'ora_adm_user') and hasattr(connection_options, 'ora_adm_pass') and hasattr(connection_options, 'rdbms_dsn')

        super(OracleSourceTable, self).__init__(schema_name, table_name, connection_options, messages, dry_run=dry_run,
                                                conn=conn, do_not_connect=do_not_connect)

        logger.info('OracleSourceTable setup: (%s, %s, %s)' % (schema_name, table_name, connection_options.ora_adm_user))
        if dry_run:
            logger.info('* Dry run *')
    ...
    def _get_table_details(self):
        ...
        if row:
            ...
            self._iot_type, self._stats_num_rows, self._partitioned, self._partition_type, self._subpartition_type, self._dependencies, self._parallelism = row
        else:
    ...
    def _get_table_stats(self):
        ...
        if not self._table_stats:
            ...
            if self.is_partitioned():
                ...
                for row in self._db_api.execute_query_fetch_all(q, query_params={'owner': self.owner,
            ...
            logger.debug('stats: num_rows=%s, num_bytes=%s, avg_row_len=%s, #cols=%s, #parts=%s'
                         % (table_stats['num_rows'], table_stats['num_bytes'], table_stats['avg_row_len'],
                            len(table_stats['column_stats']), len(table_stats['partition_stats'])))
    ...
    def _get_hash_bucket_candidate(self):
        ...
        q = """SELECT column_name
            ...
            WHERE ROWNUM = 1""" % ','.join(sorted("'%s'" % dtype for dtype in self.supported_data_types().difference(self.hash_bucket_unsuitable_data_types())))
        rows = self._db_api.execute_query_fetch_one(q, query_params={'owner': self.owner, 'table_name': self.table_name})
    ...
    def _get_partitions(self, strict=True, populate_hvs=True) -> list:
        ...
        rows = self._db_api.execute_query_fetch_all(q, query_params={'owner': self.owner,
                                                                     'table_name': self.table_name})
        partitions = []
        for row in rows:
            partition_rownum, partition_name, partition_position, subpartition_count, subpartition_name, \
                subpartition_position, partition_high_value, partition_bytes, num_rows = row
            if partition_rownum == 1:
                # Initialise new partition on first occurrence of this partition...
                partition = RdbmsPartition.by_name(partition_count=1,
    ...
    def _get_subpartitions(self, strict=True, populate_hvs=True):
        ...
        partitions = []
        for row in rows:
            partition_count, partition_name, partition_position, subpartition_count, subpartition_name, \
                subpartition_position, subpartition_high_value, subpartition_size, num_rows = row
            partitions.append(RdbmsPartition.by_name(partition_count=partition_count,
                                                     partition_name=partition_name,
                                                     partition_position=partition_position,
                                                     subpartition_count=subpartition_count,
    ...
    def _is_compression_enabled(self) -> bool:
        ...
        if self._compression_enabled is None:
            ...
            rows = self._db_api.execute_query_fetch_one(sql, query_params={'owner': self.owner,
                                                                           'table_name': self.table_name})
    ...
    def _decode_partition_high_values(self, hv_csv, strict=True):
        ...
        hv_list = convert_high_values_to_python(self.partition_columns, hv_literals,
                                                self.partition_type, self, strict=strict)
    ...
    def _get_column_low_high_values(self, column_name, from_stats=True, sample_perc=1):
        ...
        """
        logger.debug('get_column_low_high_values: %s, %s, %s, %s'
        ...
        if from_stats:
            ...
            params = {'owner': self.owner, 'table_name': self.table_name, 'column_name': str(column_name.upper())}
            if col.data_type in [ORACLE_TYPE_NUMBER, ORACLE_TYPE_FLOAT, ORACLE_TYPE_VARCHAR2, ORACLE_TYPE_NVARCHAR2,
                                 ORACLE_TYPE_BINARY_DOUBLE, ORACLE_TYPE_BINARY_FLOAT]:
                raw_fn = 'UTL_RAW.CAST_TO_%s' % (ORACLE_TYPE_NUMBER if col.data_type == ORACLE_TYPE_FLOAT else col.data_type)
                ...
                raise OffloadSourceTableException('Unable to get values for column "%s" of type "%s" from stats'
            ...
            q = """SELECT MIN(%(col)s)
                ...
                SAMPLE (%(perc)s)""" % {'owner': self.owner.upper(), 'table': self.table_name.upper(),
                                        'col': column_name, 'perc': sample_perc or 1}
    ...
    def _partition_has_rows(self, partition_name):
        ...
        sql = 'SELECT 1 FROM "%s"."%s" PARTITION ("%s") WHERE ROWNUM = 1' % (self.owner.upper(),
    ...
    def _sample_data_types_execute_query(self, sample_sql):
        ...
        return self._db_api.execute_query_fetch_one(sample_sql, query_options=query_options,
                                                    log_level=VERBOSE, profile=True)
    ...
    def _sample_data_types_decimal_column(self, column, data_precision, data_scale):
        return OracleColumn(column.name, ORACLE_TYPE_NUMBER, data_precision=data_precision,
    ...
    def check_nanosecond_offload_allowed(self, backend_max_datetime_scale, allow_nanosecond_timestamp_columns=None):
        nano_cols = [_ for _ in self.columns
    ...
    def get_current_scn(self, return_none_on_failure=False):
        ...
        if (self._size_in_bytes or 0) < MAX_SCN_SCAN_TABLE_SIZE:
            try:
                q = 'SELECT MAX(ora_rowscn) FROM "%s"."%s"' % (self.owner.upper(), self.table_name.upper())
                rows = self._db_api.execute_query_fetch_one(q)
                return rows[0] if rows else rows
            except DatabaseError as e:
                if return_none_on_failure:
                    self._messages.warning('Unable to retrieve current SCN: %s' % str(e))
                    logger.warn(str(e))
                else:
                    raise
        else:
    ...
    def gen_column(self, name, data_type, data_length=None, data_precision=None, data_scale=None, nullable=None,
        ...
        return OracleColumn(name, data_type, data_length=data_length, data_precision=data_precision,
        ...
        if len(self.partition_columns) == 1:
            ...
            if self._offload_by_subpartition and LooseVersion(self._db_version) < LooseVersion(ORACLE_VERSION_WITH_SUBPART_MIN_MAX_OPTIMIZATION):
                ...
                if len(top_level_partitions) <= MAX_UNIONS_IN_MIN_PARTITION_CHECK:
                    min_sql_unions = '\nUNION ALL\n'.join('SELECT %s %s FROM %s PARTITION (%s)'
                    ...
                    self._messages.log(
                        ...
                        % MAX_UNIONS_IN_MIN_PARTITION_CHECK, detail=VVERBOSE
                    )
    ...
    def predicate_has_rows(self, predicate):
        ...
        sql = 'SELECT 1 FROM "%s"."%s" WHERE (%s) AND ROWNUM = 1' % (self.owner.upper(), self.table_name.upper(),
                                                                     where_clause)
    ...
    def rdbms_literal_to_python(self, rdbms_column, rdbms_literal, partition_type, strict=True):
        ...
        if rdbms_literal.upper() in ('DEFAULT', offload_constants.PART_OUT_OF_LIST):
            ...
            if strict:
                raise OffloadSourceTableException('Unsupported partition key type for %s: %s'
    ...
    def to_rdbms_literal_with_sql_conv_fn(self, py_val, rdbms_data_type):
        ...
        if rdbms_data_type == ORACLE_TYPE_DATE:
        ...
        elif rdbms_data_type in (ORACLE_TYPE_TIMESTAMP_TZ, ORACLE_TYPE_TIMESTAMP_LOCAL_TZ):
            ...
            raise NotImplementedError('Oracle to_rdbms_literal_with_sql_conv_fn() data type \"%s\" not implemented.'
                                      % rdbms_data_type)
    ...
    def offload_by_subpartition_capable(self, valid_for_auto_enable=False):
        ...
        if not partition_key_data_types_are_valid(get_partition_columns(self._columns_with_subpartition_info)):
            return False
        if (self.partition_type, self.subpartition_type) in [(OFFLOAD_PARTITION_TYPE_LIST, OFFLOAD_PARTITION_TYPE_RANGE),
                                                             (OFFLOAD_PARTITION_TYPE_HASH, OFFLOAD_PARTITION_TYPE_RANGE)]:
            return True
        if (self.partition_type, self.subpartition_type) == (OFFLOAD_PARTITION_TYPE_RANGE, OFFLOAD_PARTITION_TYPE_RANGE):
    ...
    def supported_data_types() -> set:
        return {ORACLE_TYPE_VARCHAR, ORACLE_TYPE_VARCHAR2, ORACLE_TYPE_NVARCHAR2, ORACLE_TYPE_CHAR, ORACLE_TYPE_NCHAR,
                ORACLE_TYPE_CLOB, ORACLE_TYPE_NCLOB, ORACLE_TYPE_BLOB, ORACLE_TYPE_RAW, ORACLE_TYPE_NUMBER,
                ORACLE_TYPE_BINARY_DOUBLE, ORACLE_TYPE_BINARY_FLOAT, ORACLE_TYPE_FLOAT, ORACLE_TYPE_DATE,
                ORACLE_TYPE_TIMESTAMP, ORACLE_TYPE_TIMESTAMP_TZ, ORACLE_TYPE_INTERVAL_DS, ORACLE_TYPE_INTERVAL_YM,
                ORACLE_TYPE_XMLTYPE}
    ...
    def get_subpartition_boundary_info(self):
        ...
        for p in reversed(self.get_subpartitions()):
            ...
            if p.high_values_python in subs_by_hwm:
                ...
                subs_by_hwm[p.high_values_python] = {'sub_count': 1, 'expected_subs': expected_subs, 'common': False}
            ...
            subs_by_hwm[p.high_values_python]['common'] = \
                bool(subs_by_hwm[p.high_values_python]['sub_count'] == subs_by_hwm[p.high_values_python]['expected_subs'])
    ...
    def to_canonical_column(self, column):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, safe_mapping=None):
            ...
            char_semantics = CANONICAL_CHAR_SEMANTICS_UNICODE if (
                column.data_type == ORACLE_TYPE_NCHAR or
                column.data_type == ORACLE_TYPE_NVARCHAR2 or
                column.data_type == ORACLE_TYPE_NCLOB) else col.char_semantics
            return CanonicalColumn(col.name, data_type, data_length=data_length, data_precision=data_precision,
                                   ...
                                   char_length=col.char_length, char_semantics=char_semantics)
    ...
    def from_canonical_column(self, column):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, char_length=None,
            ...
            """
            safe_mapping = is_safe_mapping(col.safe_mapping, safe_mapping)
            return OracleColumn(col.name.upper(), data_type, data_length=data_length, data_precision=data_precision,
                                data_scale=data_scale, nullable=col.nullable, data_default=col.data_default,
                                safe_mapping=safe_mapping, char_length=char_length, char_semantics=col.char_semantics)
        ...
        def nchar_or_char(data_type, char_semantics):
            ...
            elif data_type == ORACLE_TYPE_CLOB and char_semantics == CANONICAL_CHAR_SEMANTICS_UNICODE:
                return ORACLE_TYPE_NCLOB
            elif data_type == ORACLE_TYPE_VARCHAR2 and char_semantics == CANONICAL_CHAR_SEMANTICS_UNICODE:
        ...
        if column.data_type == GLUENT_TYPE_FIXED_STRING:
            ...
            if (char_length if column.char_semantics in [CANONICAL_CHAR_SEMANTICS_CHAR,
                                                         CANONICAL_CHAR_SEMANTICS_UNICODE] else data_length) > max_length:
    ...
    def transform_regexp_replace_expression(self, rdbms_column, regexp_replace_pattern, regexp_replace_string):
        return 'REGEXP_REPLACE(%s, %s, %s)' % (self.enclose_identifier(rdbms_column.name),
                                               regexp_replace_pattern, regexp_replace_string)

    def transform_translate_expression(self, rdbms_column, from_string, to_string):
        return 'TRANSLATE(%s, %s, %s)' % (self.enclose_identifier(rdbms_column.name), from_string, to_string)
