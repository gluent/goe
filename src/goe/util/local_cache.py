...
# EXCEPTIONS
###############################################################################
class LocalCacheException(Exception): pass
...
class LocalCache(object, metaclass=ABCMeta):
    ...
    def __init__(self, cache_id, cache_dir, expiration=DEFAULT_EXPIRATION):
        ...
        logger.debug("LocalCache() object: %s successfully initialized with file: %s" % \
    ...
    def _expired(self):
        ...
        cache_expiration_str = datetime.datetime.fromtimestamp(cache_expiration + self._expiration).strftime("%Y-%m-%d %H:%M:%S")
        ...
        if time.time() - cache_expiration >= self._expiration:
            ...
            logger.debug("Cache file: %s is not yet expired. Current expiration: %s" % \
