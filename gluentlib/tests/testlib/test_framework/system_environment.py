...
# EXCEPTIONS
###############################################################################
class SystemEnvironmentException(Exception): pass
...
class SystemEnvironment(object):
    ...
    def __init__(self, root_dir=None):
        ...
        logger.debug("SystemEnvironment() object successfully initialized for: %s" % self._root_dir)
    ...
    def _oracle_connect(self):
        ...
        if not db_user or not db_passwd or not db_dsn:
            logger.warn("Unable to instantiate ORACLE query executor. Please, enable ORA_ADM_ environment variables")
    ...
    def _get_oracle_version(self):
        ...
        result = self.oracle.execute(sql, cursor_fn=lambda c: c.fetchall(), as_dict=True)
    ...
    def report(self, environment_id=None):
        ...
        report_body += "\tHost:\n\t\t%s\n" % self.host
        report_body += "\tOS:\n\t\tRelease: %s\n\t\tIssue: %s\n" % (self.os_release, self.os_issue)
        report_body += "\tORACLE:\n\t\tVersion: %s\n" % self.oracle_version
        distro_override = " (environment override as: %s)" % self.goe_hadoop_distro \
            if self.goe_hadoop_distro != self.hadoop_distro else ""
        report_body += "\tHadoop:\n\t\tDistro: %s%s\n\t\tVersion: %s\n" % \
            (self.hadoop_distro, distro_override, self.hadoop_version)
        report_body += "\tHadoop Db:\n\t\tType: %s\n\t\tVersion: %s\n" % (self.hadoop_db_type, self.hadoop_db_version)
...
class DevSystemEnvironment(SystemEnvironment):
    ...
    def __init__(self, offload_dev_root=None):
        ...
        logger.debug("DevSystemEnvironment() object successfully initialized with dev root: %s" % \
    ...
    def _parse_hg_tip(self):
        ...
        if not os.path.isdir(self._offload_dev_root):
            ...
            return ITEM_UNDISCOVERED, ITEM_UNDISCOVERED, ITEM_UNDISCOVERED, ITEM_UNDISCOVERED
    ...
    def hg_changeset(self):
        if not self._hg_changeset:
            self._hg_changeset, self._hg_user, self._hg_date, self._hg_summary = self._parse_hg_tip()
    ...
    def report(self):
        ...
        report_body += "\tGOE:\n\t\tMajor Version: %s\n\t\tChangeset: %s\n\t\tBy: %s\n\t\tAt: %s\n\t\tSummary: %s\n" % \
            (self.goe_major_version, self.hg_changeset, self.hg_user, self.hg_date, self.hg_summary)
...
class ProdSystemEnvironment(SystemEnvironment):
    ...
    def __init__(self, offload_root=None):
        ...
        logger.debug("ProdSystemEnvironment() object successfully initialized with root: %s" % self._offload_root)
    ...
    def _get_hg_changeset(self):
        ...
        if not os.path.isfile(self._changeset_file):
            logger.warn("Unable to extract 'changeset' as 'changeset file': %s does not exist" % \
    ...
    def _get_hg_date(self):
        ...
        if not os.path.isfile(self._changeset_file):
            logger.warn("Unable to extract 'changeset date' as 'changeset file': %s does not exist" % \
                self._changeset_file)
        else:
            logger.debug("Extracing changeset date from file: %s" % self._changeset_file)
            hg_date_unix = os.path.getmtime(self._changeset_file)
            # Format to mimic what hg command returns
            hg_date = datetime.datetime.fromtimestamp(hg_date_unix).strftime("%a %b %d %H:%M:%s %Y +0000")
...
def system_environment(env_type, offload_root=None):
    ...
    if env_type not in ('dev', 'prod'):
        raise SystemEnvironmentException("Environment type must be one of: ['dev', 'prod']")
