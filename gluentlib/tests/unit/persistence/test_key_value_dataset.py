...
from gluentlib.persistence.key_value_dataset import KeyValueDatasetSnapshot, KeyValueDatasetSpecification, KeyValueDatasetPersistence, SerializationError
...
class TestKeyValueDataset(unittest.TestCase):
  ...
  def test_get_full_dataset_dict(self):
    ...
    self.assertEqual(kv_snapshot.full_dataset_dict, { 'k1': 'k1-original', 'k2': 'k2-value' })
    kv_snapshot.update('k1', 'k1-new')
    self.assertEqual(kv_snapshot.full_dataset_dict, { 'k1': 'k1-new', 'k2': 'k2-value' })
    kv_snapshot.update('k3', 'k3-value')
    self.assertEqual(kv_snapshot.full_dataset_dict, { 'k1': 'k1-new', 'k2': 'k2-value', 'k3': 'k3-value' })
    kv_snapshot.update('k1', 'k1-newer')
    self.assertEqual(kv_snapshot.full_dataset_dict, { 'k1': 'k1-newer', 'k2': 'k2-value', 'k3': 'k3-value' })
  ...
  def test_deep_copies(self):
    ...
    self.assertEqual(kv_snapshot.full_dataset_dict, { 'k1': ['k1-original'], 'k2': ['k2-original'] })
    ...
    self.assertEqual(kv_snapshot.updates_dict, { 'k1': ['k1-new'] })
    self.assertEqual(kv_snapshot.full_dataset_dict, { 'k1': ['k1-new'], 'k2': ['k2-original'] })
  ...
  def test_kv_datset_spec_constructor_fields(self):
    ...
    kvd_spec = KeyValueDatasetSpecification(required_fields=required_fields, optional_fields=optional_fields)
    ...
    self.assertEqual(kvd_spec.all_fields, set(required_fields).union(optional_fields))

    kvd_spec_no_required = KeyValueDatasetSpecification(optional_fields=optional_fields)
    ...
    kvd_spec_no_optional = KeyValueDatasetSpecification(required_fields=required_fields)
  ...
  def test_kv_dataset_check_for_missing_fields(self):
    ...
    kvd_spec = KeyValueDatasetSpecification(required_fields=required_fields, optional_fields=optional_fields)
    kvd_spec.check_for_missing_fields({ 'req1': 'present', 'req2': 'present', 'opt1': 'present', 'opt2': 'present' }, '')
    ...
    self.assertTrue(str(cm.exception).startswith('arbitrary test prefix'), msg="unexpected exception message: '{}'".format(str(cm.exception)))

    with self.assertRaises(AssertionError) as cm:
      kvd_spec.check_for_missing_fields({ 'req1': 'present', 'opt1': 'present', 'opt2': 'present' }, 'arbitrary test prefix')
  ...
  def test_kv_dataset_check_for_unrecognized_fields(self):
    ...
    kvd_spec.check_for_unrecognized_fields({ 'req1': 'present', 'req2': 'present' }, '')
    kvd_spec.check_for_unrecognized_fields({ 'opt1': 'present', 'opt2': 'present' }, '')
    kvd_spec.check_for_unrecognized_fields({ 'req1': 'present', 'req2': 'present', 'opt1': 'present', 'opt2': 'present' }, '')
    with self.assertRaises(AssertionError) as cm:
      kvd_spec.check_for_unrecognized_fields({ 'invalid-field': 'present' }, 'arbitrary test prefix')

    self.assertTrue(str(cm.exception).startswith('arbitrary test prefix'), msg="unexpected exception message: '{}'".format(str(cm.exception)))

    with self.assertRaises(AssertionError) as cm:
      kvd_spec.check_for_unrecognized_fields({ 'req1': 'present', 'req2': 'present', 'opt1': 'present', 'opt2': 'present', 'invalid-field': 'present' }, 'arbitrary test prefix')
  ...
  def test_kv_dataset_spec_serialization(self):
    ...
    kvd_spec = KeyValueDatasetSpecification(required_fields=required_fields, optional_fields=optional_fields,
        field_names_to_deserializers={ 'req_int_field': int, 'req_float_field': float, 'opt_list_field': json.loads, 'opt_dict_field': json.loads },
        field_names_to_serializers={ 'opt_list_field': json.dumps, 'opt_dict_field': json.dumps })
    ...
    self.assertEqual(kvd_spec.serialize('req_float_field', 666.789987), '666.789987')

    list_val = ['foo','bar',1,2]
    list_val_json_serialized = json.dumps(list_val)
    self.assertEqual(kvd_spec.serialize('opt_list_field', list_val), list_val_json_serialized)

    dict_val = {'s': 'string', 'l': [1,2], 'd': {'k': 'v'}}
    dict_val_json_serialized = json.dumps(dict_val)
    self.assertEqual(kvd_spec.serialize('opt_dict_field', dict_val), dict_val_json_serialized)

    # next, test DESERIALIZATION
    self.assertEqual(kvd_spec.deserialize('req_string_field', 'a_string'), 'a_string')
    self.assertEqual(kvd_spec.deserialize('req_int_field', '666'), 666)
    self.assertEqual(kvd_spec.deserialize('req_float_field', '666.789987'), 666.789987)

    self.assertEqual(kvd_spec.deserialize('opt_list_field', list_val_json_serialized), list_val)
    self.assertEqual(kvd_spec.deserialize('opt_dict_field', dict_val_json_serialized), dict_val)

  def test_kv_dataset_spec_serialization_errors(self):
    required_fields = [ 'dict_field', 'int_field' ]

    kvd_spec = KeyValueDatasetSpecification(required_fields=required_fields,
    ...
    with self.assertRaises(SerializationError):
      kvd_spec.deserialize('dict_field', 'this statement is neither true nor valid json')
  ...
  def test_initialize_and_run_gc(self):
    ...
    kvdp = KeyValueDatasetPersistence(mock_kv_store, YET_ANOTHER_DATASET_SPEC, YetAnotherSnapshotClass)

    self.assertTrue(mock_kv_store.initialized, msg='KeyValueStore for KeyValueDatasetPersistence was not initialized')

    self.assertEqual(mock_kv_store.run_gc_count, 0, msg='MockKeyValueStore should start out with a run_gc_count of 0...')
    kvdp.run_gc()
    self.assertEqual(mock_kv_store.run_gc_count, 1, msg='KeyValueStore#run_gc was NOT called when KeyValueDatasetPersistence#run_gc was called...')

  def test_load_dataset_snapshot(self):
    required_values_dict = { INT_FIELD: 123, FLOAT_FIELD: -123.456, STRING_FIELD: 'foo' }
    required_values_serialized_dict = { INT_FIELD: '123', FLOAT_FIELD: '-123.456', STRING_FIELD: 'foo' }
    ...
    optional_values_serialized_dict = { LIST_FIELD: json.dumps([1,'two',[3]]), DICT_FIELD: json.dumps({ 'bar': 'baz' }) }
  ...
  def test_persist_updates(self):
    ...
    self.assertFalse(updated_snapshot is snapshot) # paranoia -- check against trivial equality
  ...
  def test_persist_full_dataset_snapshot(self):
    values_dict = { INT_FIELD: 123, FLOAT_FIELD: -123.456, STRING_FIELD: 'foo',
                    LIST_FIELD: [1,'two',[3]], DICT_FIELD: { 'bar': 'baz' } }

    snapshot = YetAnotherSnapshotClass(values_dict)

    mock_kv_store = MockKeyValueStore() # in this case, the mock KV store starts out empty
    kvdp = KeyValueDatasetPersistence(mock_kv_store, YET_ANOTHER_DATASET_SPEC, YetAnotherSnapshotClass)

    kvdp.persist_full_dataset_snapshot(snapshot)

    loaded_snapshot = kvdp.load_dataset_snapshot()
    self.assertFalse(loaded_snapshot is snapshot) # paranoia -- check against trivial equality
...
YET_ANOTHER_DATASET_SPEC = KeyValueDatasetSpecification(
  ...
  field_names_to_deserializers={ INT_FIELD: int, FLOAT_FIELD: float,
      LIST_FIELD: lambda _: list(json.loads(_)), DICT_FIELD: lambda _: dict(json.loads(_)) },
...
class MockKeyValueStore(object):
  def __init__(self, initial_fieldname_to_serialized_values_dict={}):
    self._fieldnames_to_serialized_values_dict = initial_fieldname_to_serialized_values_dict
