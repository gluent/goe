...
from gluentlib.offload.offload_transport_functions import split_lists_for_id_list, split_ranges_for_id_range
from gluentlib.offload.offload_transport_rdbms_api import OffloadTransportRdbmsApiInterface,\
    OffloadTransportRdbmsApiException,\
    TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_PARTITION, TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_ID_RANGE, \
    TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_TERADATA_AMP, TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_TERADATA_HASHAMP, \
    TRANSPORT_ROW_SOURCE_QUERY_SPLIT_COLUMN
from gluentlib.offload.teradata.teradata_column import TERADATA_TYPE_BIGINT, TERADATA_TYPE_BYTEINT,\
    TERADATA_TYPE_DECIMAL, TERADATA_TYPE_INTEGER, TERADATA_TYPE_NUMBER, TERADATA_TYPE_SMALLINT, \
    TERADATA_TYPE_DATE, TERADATA_TYPE_TIME, TERADATA_TYPE_TIMESTAMP, TERADATA_TYPE_TIMESTAMP_TZ, \
    TERADATA_TYPE_INTERVAL_DS, TERADATA_TYPE_INTERVAL_YM
...
class OffloadTransportTeradataApi(OffloadTransportRdbmsApiInterface):
    ...
    def __init__(self, rdbms_owner, rdbms_table_name, offload_options, messages,
        ...
        self._transport_row_source_query_split_methods = [TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_ID_RANGE,
    ...
    def _get_id_range(self, rdbms_col_name, partition_chunk=None):
        ...
        if partition_chunk and partition_chunk.count() == 1:
            partition_clause = ' WHERE PARTITION = {}'.format(partition_chunk.partition_names().pop())
        min_max_qry = 'SELECT MIN(%(col)s), MAX(%(col)s) FROM "%(owner)s"."%(table)s"%(partition_clause)s'\
    ...
    def _get_transport_frontend_api(self):
        ...
        if self._transport_frontend_api is None:
            self._transport_frontend_api = frontend_api_factory(
                self._offload_options.db_type, self._offload_options, self._messages,
    ...
    def _table_amps(self, rdbms_table) -> list:
        if self._teradata_amps is None:
            ...
            rows = frontend_api.execute_query_fetch_all(sql, query_params=[rdbms_table.owner, rdbms_table.table_name],
                                                        log_level=VVERBOSE)
    ...
    def get_rdbms_query_cast(self, column_expression, rdbms_column, staging_column, max_ts_scale,
        ...
        if rdbms_column.data_type == TERADATA_TYPE_TIMESTAMP_TZ:
            ...
            cast_expression = "TO_CHAR(%s AT TIME ZONE '00:00','YYYY-MM-DD HH24:MI:SS.FF%s TZH:TZM')" % (column_expression, ff_scale)
            ...
            cast_expression = "TO_CHAR(%s AT TIME ZONE '00:00','YYYY-MM-DD HH24:MI:SS.FF%s')" % (column_expression, ff_scale)
        elif rdbms_column.data_type == TERADATA_TYPE_TIME:
            cast_expression = "TO_CHAR(%s AT TIME ZONE '00:00','HH24:MI:SS.FF%s')" % (column_expression, ff_scale)
        elif (staging_column.is_string_based()
              and rdbms_column.data_type in (TERADATA_TYPE_NUMBER, TERADATA_TYPE_BIGINT, TERADATA_TYPE_BYTEINT,
                                             TERADATA_TYPE_INTEGER, TERADATA_TYPE_SMALLINT, TERADATA_TYPE_DECIMAL)
              and convert_expressions_on_rdbms_side):
            cast_expression = "TO_CHAR(%s,'TM')" % column_expression
        elif (rdbms_column.data_type in (TERADATA_TYPE_INTERVAL_DS, TERADATA_TYPE_INTERVAL_YM)
    ...
    def get_transport_split_type(self, partition_chunk, rdbms_table, parallelism, rdbms_partition_type,
        ...
        """
        def get_first_pk_col(pk_cols):
            if pk_cols:
                return match_table_column(pk_cols[0], rdbms_columns)
            else:
                return None
        ...
        first_pk_col = get_first_pk_col(rdbms_pk_cols)
        ...
        if partition_count > 0 and partition_count >= parallelism:
        ...
        elif first_pk_col and first_pk_col.is_number_based():
            ...
            self.log('Splitting table by HASHAMP and reducing parallelism to AMP count', detail=VVERBOSE)
            partition_by = TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_TERADATA_HASHAMP
            tuned_parallelism = len(self._table_amps(rdbms_table))
        elif partition_count == 0 and not predicate_offload_clause:
            self.log('Splitting table by AMP and reducing parallelism to AMP count', detail=VVERBOSE)
    ...
    def get_transport_row_source_query(self, partition_by_prm, rdbms_table, consistent_read, parallelism,
        ...
        assert not self._incremental_update_extractor,\
        ...
        if partition_by_prm == TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_ID_RANGE:
            ...
            if col_min is None or col_max is None:
                # TODO In Oracle we switch to MOD splitter here - we may choose to do the same if we implement it.
                raise OffloadTransportRdbmsApiException(
                    f'Table {owner_table} cannot be split by range with blank primary key values')
        ...
        if partition_by_prm == TRANSPORT_ROW_SOURCE_QUERY_SPLIT_BY_PARTITION:
            ...
            row_source = union_all.join([union_branch_template
                                         % {'batch': (i % parallelism),
                                            ...
                                            'part_csv': csv} for i, csv in enumerate(part_csvs) if csv])
            ...
            union_branch_template = 'SELECT g.*, %(batch)s AS %(batch_col)s FROM %(owner_table)s g WHERE %(batch_source_col)s >= %(low_val)s AND %(batch_source_col)s < %(high_val)s%(partition_filter)s'
            ...
            row_source = union_all.join([union_branch_template
                                         % {'batch': (i % parallelism),
                                            ...
                                            'owner_table': owner_table,
                                            'amp_csv': csv}
                                         for i, csv in enumerate(amp_csvs) if csv])
            ...
            table_amps = self._table_amps(rdbms_table)
            amp_csvs = split_lists_for_id_list(table_amps, parallelism, as_csvs=True)
            pi_col_csv = ','.join(rdbms_table.enclose_identifier(_.name)
    ...
    def query_import_extraction(self, staging_columns, source_query, source_binds,
        ...
        self.log('Importing load data with arraysize=%s, compression=%s' % (fetch_size, compress), detail=VERBOSE)

        try:
            db_cursor = transport_frontend_api.execute_query_get_cursor(source_query, query_params=source_binds,
                                                                        log_level=VERBOSE)
            yield db_cursor
        finally:
            db_cursor.close()
