...
from gluentlib.data_governance.hadoop_data_governance import data_governance_register_new_db_step, \
    data_governance_register_new_table_step, \
    data_governance_register_new_view_step, get_data_governance_register
from gluentlib.data_governance.hadoop_data_governance_constants import DATA_GOVERNANCE_GLUENT_OBJECT_TYPE_JOIN_VIEW, \
...
from gluentlib.offload.column_metadata import ColumnMetadataInterface, ColumnBucketInfo, ColumnPartitionInfo, \
    get_column_names, get_partition_columns, invalid_column_list_message, \
    match_table_column, regex_real_column_from_part_column, str_list_of_columns, valid_column_list,\
...
from gluentlib.offload.offload_functions import get_hybrid_threshold_clauses, hvs_to_backend_sql_literals, \
    load_db_name
...
from gluentlib.util.exception_trigger import trigger_exception
...
TYPICAL_DATE_GRANULARITY_TERMS = {PART_COL_GRANULARITY_YEAR: 'YEAR',
...
class BackendTableInterface(metaclass=ABCMeta):
    ...
    def __init__(self, db_name, table_name, backend_type, orchestration_options, messages, orchestration_operation=None,
        ...
        if len(self.db_name) > self.max_db_name_length():
            raise BackendTableException('Database name %s is too long for %s: %s > %s'
                                        % (self.db_name, self._db_api.backend_db_name(),
                                           len(self.db_name), self.max_db_name_length()))
        ...
        if self._db_api.load_db_transport_supported() and len(self._load_db_name) > self.max_db_name_length():
            raise BackendTableException('Load database name %s is too long for %s: %s > %s'
                                        % (self._load_db_name, self._db_api.backend_db_name(),
                                           len(self._load_db_name), self.max_db_name_length()))
        ...
        if orchestration_operation:
            ...
            self._user_requested_allow_decimal_scale_rounding = getattr(orchestration_operation,
                                                                        'allow_decimal_scale_rounding', False)
            self._user_requested_compute_load_table_stats = getattr(orchestration_operation,
                                                                    'compute_load_table_stats', False)
    ...
    def _cast_validation_columns(self, staging_columns: list):
        ...
        for col in self.get_non_synthetic_columns():
            if self._final_table_casts[col.name.upper()]['cast_type']:
                ...
                cast_validation_cols.append((staging_col.staging_file_column_name, self.get_verification_cast(col)))
        ...
        for part_col in self.get_synthetic_columns():
            if part_col.partition_info and not part_col.partition_info.function:
                cast_validation_cols.append((part_col.partition_info.source_column_name,
    ...
    def _check_partition_info_function(self, source_backend_column, partition_info):
        ...
        if udf.parameters[0].data_type != source_backend_column.data_type:
            raise BackendTableException(
                '%s for column %s: %s != %s' % (PARTITION_FUNCTION_ARG_TYPE_EXCEPTION_TEXT,
                                                source_backend_column.name, udf.parameters[0].data_type,
    ...
    def _check_partition_info_range_start_end(self, canonical_column, backend_column):
        ...
        if canonical_column.partition_info.range_start is None:
            raise BackendTableException('--partition-lower-value is required for partition column/type: %s/%s'
                                        % (backend_column.name, backend_column.data_type))
        elif range_min and int(canonical_column.partition_info.range_start) < range_min:
            raise BackendTableException('--partition-lower-value is below minimum supported value: %s < %s'
        ...
        if canonical_column.partition_info.range_end is None:
            raise BackendTableException('--partition-upper-value is required for partition column/type: %s/%s'
                                        % (backend_column.name, backend_column.data_type))
        elif range_max and int(canonical_column.partition_info.range_end) > range_max:
            raise BackendTableException('--partition-upper-value is above maximum supported value: %s > %s'
    ...
    def _create_db(self, db_name, comment=None, properties=None):
        ...
        if self._db_api.database_exists(db_name):
            self._log('%s already exists: %s' % (self.db_name_label(initcap=True), db_name), detail=VERBOSE)
            return []
        elif not self.create_database_supported():
            return []
        else:
            return self._db_api.create_database(db_name, comment=comment, properties=properties)

    def _create_final_db(self, location=None):
        comment = BACKEND_DB_COMMENT_TEMPLATE.format(db_name_type='Offload',
                                                     db_name_label=self.db_name_label())
        return self._create_db(self.db_name, comment=comment, properties={'location': location})

    def _create_load_db(self, location=None):
        comment = BACKEND_DB_COMMENT_TEMPLATE.format(db_name_type='Offload load',
                                                     db_name_label=self.db_name_label())
        return self._create_db(self._load_db_name, comment=comment, properties={'location': location})

    def _create_result_cache_db(self, location=None):
        comment = BACKEND_DB_COMMENT_TEMPLATE.format(db_name_type='Result Cache',
                                                     db_name_label=self.db_name_label())
        return self._create_db(self._result_cache_db_name, comment=comment,
    ...
    def _derive_partition_info(self, column, partition_columns):
        ...
        self._debug('Deriving partition info for %s.%s.%s' % (self.db_name, self._base_table_name, column_name))
        partition_info = None
        if not partition_columns:
            ...
            partition_columns = self._db_api.get_partition_columns(self.db_name, self._base_table_name)
        part_col_names = [_ for _ in get_column_names(partition_columns, conv_fn=str.upper)
                          if not self._is_synthetic_bucket_column(_)]

        if column_name.upper() in (part_col_names or []) and not self._is_synthetic_bucket_column(column_name):
            ...
            if self.is_synthetic_partition_column(column_name):
                ...
                if self._partition_functions:
                    try:
                        ...
                        self._log('IndexError reading partition_functions[position]: {}[{}]'.format(
                            self._partition_functions, position),
                            detail=VERBOSE
                        )
                        raise
                # First see if the backend has its own metadata we can interrogate
                partition_info = self._db_api.derive_native_partition_info(self.db_name, self._base_table_name,
                ...
                if partition_info:
                    ...
                    if isinstance(column, ColumnMetadataInterface):
                        ...
                        backend_column = self._db_api.get_column(self.db_name, self._base_table_name, column_name)
                    ...
                    if digits and not backend_column.is_string_based():
                        ...
                        digits = None
                    partition_info = ColumnPartitionInfo(position=position, granularity=granularity, digits=digits,
                                                         source_column_name=source_column_name)

                partition_info.function = part_fn_name
                self._log('Derived synthetic partition info for %s: %s' % (column_name, partition_info),
                          detail=VVERBOSE)
            else:
                # Backends have their own metadata therefore call overloaded derive_native_partition_info()
                partition_info = self._db_api.derive_native_partition_info(self.db_name, self._base_table_name,
                                                                           column_name, position)
                self._log('Derived native partition info for %s: %s' % (column_name, partition_info), detail=VVERBOSE)
    ...
    def _execute_query_fetch_all(self, sql, query_options=None, log_level=VVERBOSE, not_when_dry_running=False,
                                 query_params=None, profile=None):
        assert sql
        return self._db_api.execute_query_fetch_all(sql, query_options=query_options, log_level=log_level,
                                                    time_sql=True, not_when_dry_running=not_when_dry_running,
                                                    query_params=query_params, profile=profile)

    def _execute_query_fetch_one(self, sql, query_options=None, log_level=VVERBOSE, not_when_dry_running=False,
                                 query_params=None, profile=None):
        assert sql
    ...
    def _execute_dml(self, sql, query_options=None, sync=None, log_level=VERBOSE, profile=None):
        assert sql
        return self._db_api.execute_dml(sql, query_options=query_options, sync=sync,
    ...
    def _gen_final_insert_sqls(self, select_expr_tuples, threshold_clauses=None, sort_expr_list=None,
        ...
        if chunk_filter_clauses:
            ...
            for chunk_clause in chunk_filter_clauses:
                sqls.append(self._final_insert_format_sql(select_expr_tuples, threshold_clauses + [chunk_clause],
                                                          sort_expr_list, for_materialized_join=for_materialized_join))
        else:
            sqls.append(self._final_insert_format_sql(select_expr_tuples, threshold_clauses, sort_expr_list,
    ...
    def _gen_final_insert_subchunk_expressions(self):
        ...
        subchunk_sql = 'SELECT %s, COUNT(*)\nFROM %s.%s\nGROUP BY %s%s' % (
            subchunk_expr, self.enclose_identifier(self._load_db_name),
            self.enclose_identifier(self._load_table_name), subchunk_expr, order_by
        )
    ...
    def _gen_final_table_casts(self, rdbms_columns, staging_columns) -> dict:
        ...
        for backend_col in backend_columns:
            ...
            if backend_col.partition_info and self.is_synthetic_partition_column(backend_col):
                self._log('Picking up source column for partition column %s: %s'
                          % (backend_col.name, backend_col.partition_info.source_column_name), detail=VVERBOSE)
                source_backend_col = self.get_column(backend_col.partition_info.source_column_name)
            elif backend_col.bucket_info:
                self._log('Picking up source column for bucket column %s: %s'
                          % (backend_col.bucket_info.source_column_name, backend_col.name), detail=VVERBOSE)
                source_backend_col = self.get_column(backend_col.bucket_info.source_column_name)
            ...
            if backend_col.partition_info and self.is_synthetic_partition_column(backend_col):
                ...
                final_table_casts[backend_col.name.upper()] = {'cast': partition_expr,
                                                               'cast_type': backend_col.data_type,
                                                               'verify_cast': partition_vexpr}
            elif backend_col.bucket_info:
                _, bucket_expr = self.synthetic_bucket_filter_capable_column(source_backend_col)
                udf_db_prefix = (self._udf_db + '.') if self._udf_db else ''
                bucket_expr = bucket_expr % {'dividend': cast_expr, 'divisor': backend_col.bucket_info.num_buckets,
                                             'udf_db': udf_db_prefix}
                bucket_vexpr = bucket_expr % {'dividend': vcast_expr, 'divisor': backend_col.bucket_info.num_buckets,
                                              'udf_db': udf_db_prefix}
                final_table_casts[backend_col.name.upper()] = {'cast': bucket_expr,
                                                               'cast_type': backend_col.data_type,
                                                               'verify_cast': bucket_vexpr}
            else:
                final_table_casts[backend_col.name.upper()] = {'cast': cast_expr,
                                                               'cast_type': cast_type,
                                                               'verify_cast': vcast_expr}
        return final_table_casts

    def _gen_mat_join_insert_sqls(self, select_expr_tuples, threshold_cols=None, gte_threshold_vals=None,
                                  lt_threshold_vals=None, insert_predicates=None, sort_expr_list=None):
        ...
        def get_threshold_where_clauses(threshold_cols, lt_threshold_vals, gte_threshold_vals):
            ...
            if threshold_cols and lt_threshold_vals:
                lt_threshold_vals = hvs_to_backend_sql_literals(threshold_cols, lt_threshold_vals,
                                                                self._ipa_predicate_type, self.to_backend_literal)
                lt_threshold_clause, _ = get_hybrid_threshold_clauses(threshold_cols, threshold_cols,
                                                                      self._ipa_predicate_type, lt_threshold_vals,
                                                                      col_enclosure_fn=self.enclose_identifier)
                threshold_clauses.append(lt_threshold_clause)
                if gte_threshold_vals:
                    gte_threshold_vals = hvs_to_backend_sql_literals(threshold_cols, gte_threshold_vals,
                                                                     self._ipa_predicate_type, self.to_backend_literal)
                    _, gte_threshold_clause = get_hybrid_threshold_clauses(threshold_cols, threshold_cols,
                                                                           self._ipa_predicate_type, gte_threshold_vals,
                                                                           col_enclosure_fn=self.enclose_identifier)
        ...
        return self._gen_final_insert_sqls(select_expr_tuples, threshold_clauses=threshold_clauses,
                                           sort_expr_list=sort_expr_list, for_materialized_join=True)
    ...
    def _gen_synthetic_part_date_truncated_sql_expr(self, date_value, granularity, synthetic_name, source_column_cast):
        ...
        return self._gen_synthetic_partition_date_truncated_sql_expr(synthetic_name, date_value, granularity,
                                                                     source_column_cast)
    ...
    def _gen_synthetic_part_date_as_string_sql_expr(self, date_value, granularity, source_column_cast):
        ...
        for component_fn, size_or_literal in component_fn_list:
            assert source_column_cast
            partition_expr.append(self._gen_synthetic_partition_date_as_string_sql_expr(component_fn, size_or_literal,
                                                                                        source_column_cast))
    ...
    def _gen_synthetic_part_number_sql_expr(self, number_value, backend_col, granularity, synthetic_partition_digits):
        ...
        return self._gen_synthetic_part_number_granularity_sql_expr(number_value, backend_col, granularity,
                                                                    synthetic_partition_digits)

    def _gen_synthetic_part_string_sql_expr(self, string_value, granularity):
        assert granularity
        assert isinstance(granularity, int), 'Granularity must be integral not: %s' % type(granularity)
        return self._gen_synthetic_part_string_granularity_sql_expr(self._format_staging_column_name(string_value),
                                                                    granularity)
    ...
    def _gen_synthetic_literal_function(self, partition_column):
        ...
        if partition_column.partition_info.function:
            ...
            def partition_fn(source_value):
                ...
                sql = 'SELECT {}'.format(self._partition_function_sql_expression(partition_column.partition_info,
                                                                                 sql_literal))
                row = self._db_api.execute_query_fetch_one(sql)
                return row[0] if row else None
        else:
            partition_fn = SyntheticPartitionLiteral.gen_synthetic_literal_function(partition_column, self.get_columns())
        return partition_fn
    ...
    def _gen_synthetic_sql_cast(self, partition_column, source_column_cast):
        ...
        partition_expr = None
        if partition_column.partition_info.function:
            partition_expr = self._partition_function_sql_expression(partition_column.partition_info,
                                                                     source_column_cast)
        elif source_column.is_date_based() and partition_column.is_date_based():
            partition_expr = self._gen_synthetic_part_date_truncated_sql_expr(source_column_name, granularity,
                                                                              partition_column.name, source_column_cast)
        elif source_column.is_date_based() and partition_column.is_string_based():
            partition_expr = self._gen_synthetic_part_date_as_string_sql_expr(source_column_name, granularity,
                                                                              source_column_cast)
            ...
            partition_expr = self._gen_synthetic_part_number_sql_expr(source_column_cast, source_column,
                                                                      int(granularity), digits)
        else:
    ...
    def _get_dfs_client(self):
        if self._dfs_client is None:
            self._dfs_client = get_dfs_from_options(self._orchestration_config, messages=self._messages)
    ...
    def _offload_step(self, step_constant: str, step_fn: Callable, command_type: Optional[str]=None,
                      optional=False, mandatory_step=False):
        return self._messages.offload_step(step_constant, step_fn, command_type=command_type,
                                           execute=bool(not self._dry_run),
                                           optional=optional, mandatory_step=mandatory_step)
    ...
    def _partition_function_sql_expression(self, partition_info, sql_input_expression):
        ...
        return '{}({})'.format(self._db_api.enclose_object_reference(udf_db, udf_name),
    ...
    def _validate_final_table_casts(self, staging_columns: list, log_profile=None):
        ...
        cast_check_preds = ['(%s AND %s IS NULL)' % (self._staging_column_is_not_null_sql_expr(cname), ccast)
        ...
        verify_cast_set = self._execute_query_fetch_one(verify_cast_sql,
                                                        query_options=self._cast_verification_query_options(),
                                                        log_level=VERBOSE, not_when_dry_running=True,
                                                        profile=log_profile)
        ...
        if verify_cast_set and verify_cast_set[0] > 0:
            ...
            identify_projection = ["MAX(CASE WHEN %s THEN %s END)" % (pred, i)
            ...
            identify_cast_set = self._execute_query_fetch_one(identify_cast_sql,
                                                              query_options=self._cast_verification_query_options())
            ...
            self._log('CAST() of load data will cause data loss due to lack of precision in target data type in %s rows'
                      % verify_cast_set[0], ansi_code='red')
            self._log('Failing casts are:\n%s' % '\n'.join(failing_casts), ansi_code='red')
            if failing_casts:
                limit_sample_row_count = 50
                suggest_sql = self._validate_final_table_casts_verification_sql(failing_cols, failing_casts,
                                                                                min(verify_cast_set[0],
                                                                                    limit_sample_row_count))
    ...
    def _validate_staged_data_rules(self, rdbms_part_cols, rdbms_columns, staging_columns):
        ...
        def validation_rules_for_decimal_scale(pred_list):
            for scale_check_col in [_ for _ in self.get_non_synthetic_columns() if _.is_number_based()]:
                ...
                if rdbms_col.data_scale and rdbms_col.data_scale > 0 and rdbms_col.data_scale < backend_scale:
                    self._debug('RDBMS column scale is safely below the backend so no need to check it: %s < %s'
                ...
                pred_list.append(
                    Validation(column_name=scale_check_col.name.upper(),
                               expression=is_invalid_scale_expr,
                               fatal=True,
                               message='Scale for column %s exceeds column maximum: %s\n%s'
                                       % (scale_check_col.name.upper(), backend_scale,
        ...
        def validation_rules_for_not_null_cols(pred_list, meta_op_filter):
            ...
            if self.not_null_column_supported():
                ...
                for col_name in nn_columns:
                    ...
                    pred_list.append(
                        Validation(column_name=col_name,
                                   expression='%s IS NULL%s'
                                              % (self._format_staging_column_name(staging_col), meta_op_filter),
                                   fatal=True,
            ...
            if self.get_partition_columns():
                source_column_names = [_.partition_info.source_column_name.upper()
                                       for _ in self.get_partition_columns() if _.partition_info]
                extra_cols = set(source_column_names).difference(cols_already_checked)
                for col_name in extra_cols:
                    staging_col = match_table_column(col_name, staging_columns)
                    pred_list.append(
                        Validation(column_name=col_name.upper(),
                                   expression='%s IS NULL%s'
                                              % (self._format_staging_column_name(staging_col), meta_op_filter),
                                   fatal=False,
        ...
        def validation_rules_for_synthetic_part_cols(pred_list):
            for partition_col in self.get_partition_columns():
                if (self._is_synthetic_partition_column(partition_col.name)
                    ...
                    if source_backend_col.is_number_based():
                        granularity_expr = self._gen_synthetic_part_number_granularity_sql_expr(
                            ...
                            source_backend_col, partition_col.partition_info.granularity,
                            partition_col.partition_info.digits, with_padding=False)
                        length_expr = self._db_api.length_sql_expression(granularity_expr)
                        pred_list.append(
                            Validation(column_name=partition_col.partition_info.source_column_name.upper(),
                                       expression='%s > %s' % (length_expr,
                ...
                elif not self._is_synthetic_bucket_column(partition_col.name) \
                        and not self._is_synthetic_partition_column(partition_col.name):
                    # Native partition column, check range start/end
                    if partition_col.partition_info.range_start is not None \
                            and partition_col.partition_info.range_end is not None:
                        pred_list.append(
                            Validation(column_name=partition_col.name.upper(),
    ...
    def _validate_staged_data(self, rdbms_part_cols, rdbms_columns, expected_rows, staging_columns, log_profile=None):
        ...
        projection = ["MAX(CASE WHEN %s THEN %s END)" % (_.expression, i)
                      for i, _ in enumerate(pred_list)] + ["COUNT(*)"]
        pred_messages = '\n,      '.join(projection)
        sql = """SELECT %s\nFROM   %s""" % (pred_messages, self._format_staging_object_name())
        validation_set = self._execute_query_fetch_one(sql, query_options=self._validate_staged_data_query_options(),
                                                       log_level=VERBOSE, not_when_dry_running=True,
                                                       profile=log_profile)
        if validation_set:
            ...
            if expected_rows is None:
                ...
                if expected_rows == count_star:
                    self._log('Offload row count matches load table row count (%s == %s)'
                              % (expected_rows, count_star), detail=VERBOSE)
                else:
                    self._log('Offload row count != load table row count (%s != %s)'
                              % (expected_rows, count_star), detail=VVERBOSE)
                    raise DataValidationException('Offload row count does not match load table row count (%s != %s)'
            ...
            errors = [pred_list[msg_code].message
    ...
    @abstractmethod
    def _gen_synthetic_partition_date_truncated_sql_expr(self, synthetic_name, column_name,
                                                         granularity, source_column_cast):
        pass
    ...
    def check_partition_function(self, db_name, udf_name):
        ...
        if not udfs:
            raise BackendTableException('{} in {}: {}.{}'.format(
                PARTITION_FUNCTION_DOES_NOT_EXIST_EXCEPTION_TEXT, self.backend_db_name(), db_name, udf_name
            ))
        self._log('Partition function details: {}'.format(str(udfs)), detail=VVERBOSE)
        if len(udfs) != 1:
            # We do not support UDF overloading
            raise BackendTableException('Multiple partition functions are matched by {}.{}'.format(db_name, udf_name))

        udf = udfs[0]
        if not self._db_api.is_supported_partition_function_return_data_type(udf.return_type):
            raise BackendTableException('Partition function {}.{} has invalid return type: {}'.format(
                db_name, udf_name, udf.return_type
            ))
        if len(udf.parameters or []) != 1:
            raise BackendTableException('Partition function {}.{} {}: {}'.format(
                db_name, udf_name, PARTITION_FUNCTION_ARG_COUNT_EXCEPTION_TEXT, udf.parameter_spec_string()
            ))
        if udf.parameters[0].data_type not in self._db_api.supported_partition_function_parameter_data_types():
            raise BackendTableException('Partition function {}.{} has invalid parameter type: {} not in {}'.format(
                db_name, udf_name, udf.parameters[0].data_type,
        ...
        for canonical_column in canonical_columns:
            ...
            if canonical_column.partition_info:
                if canonical_column.partition_info.function:
                    partition_data_type = self._check_partition_info_function(backend_column,
                                                                              canonical_column.partition_info)
                else:
                    partition_data_type = backend_column.data_type

                if not self.is_valid_partitioning_data_type(partition_data_type):
                    raise BackendTableException('Partition column data type is not supported in this version: %s'
                                                % partition_data_type)
                if self._partition_column_requires_synthetic_column(backend_column, canonical_column.partition_info):
                    ...
                    if canonical_column.partition_info.function:
                        new_synthetic_columns.append(
                            self._db_api.gen_column_object(synthetic_name, data_type=partition_data_type,
                                                           partition_info=canonical_column.partition_info))
                    else:
                        new_synthetic_columns.append(self._gen_synthetic_partition_column_object(synthetic_name,
                                                                                                 canonical_column))
            ...
            if canonical_column.bucket_info:
                new_synthetic_columns.append(
                    self._gen_synthetic_bucket_column_object(bucket_info=canonical_column.bucket_info))
    ...
    def create_backend_view(self, column_tuples, new_column_list, ansi_joined_tables,
                            filter_clauses=None, db_name_override=None):
        ...
        self._db_api.create_view(view_db_name, self.table_name, column_tuples, ansi_joined_tables,
                                 filter_clauses=filter_clauses, sync=True)
    ...
    def create_conversion_view(self, column_tuples):
        ...
        self._db_api.create_view(self._conv_view_db, self._conv_view_name, column_tuples, ansi_joined_tables, sync=True)
    ...
    def derive_unicode_string_columns(self, as_csv=False):
        ...
        """
        unicode_columns = [_ for _ in self.get_canonical_columns()
    ...
    def drop_table(self, purge=False):
        ...
        if self.is_view():
            ...
            if self.table_name != self._base_table_name:
                cmds_executed.extend(self._db_api.drop_table(self.db_name, self._base_table_name, purge=purge))
        else:
            cmds_executed.extend(self._db_api.drop_table(self.db_name, self._base_table_name, purge=purge))
    ...
    def gen_synthetic_partition_col_expressions(self, expr_from_columns=None, as_python_fns=False):
        ...
        for partition_col in self.get_partition_columns():
            ...
            if partition_col.bucket_info and self.synthetic_bucketing_supported():
                if expr_from_columns and match_table_column(partition_col.name, expr_from_columns):
                    bucket_expr = [partition_col.name, partition_col.format_data_type(),
                                   self.enclose_identifier(partition_col.name), partition_col.name]
                    self._log('Bucket expression from supporting columns: %s' % bucket_expr, detail=VVERBOSE)
                else:
                    bucket_expr = [partition_col.name, partition_col.format_data_type(),
            ...
            if partition_col.partition_info and self.synthetic_partitioning_supported():
                partition_expr = None
                if expr_from_columns and not as_python_fns and match_table_column(partition_col.name,
                                                                                  expr_from_columns):
                ...
                if partition_expr:
                    pcs.append([partition_col.name, partition_col.format_data_type(),
                                partition_expr, partition_col.partition_info.source_column_name])
    ...
    def get_columns(self):
        ...
        if self._columns is None:
            ...
            if self.is_view(object_name_override=self._base_table_name):
                ...
                self._log('Faking partition columns for view: %s.%s' % (self.db_name, self._base_table_name),
                          detail=VVERBOSE)
                ...
                part_cols = self._db_api.get_partition_columns(self.db_name, self._base_table_name)
    ...
    def get_distinct_column_values(self, column_name_list, columns_to_cast_to_string=None, order_results=False):
        ...
        """
        return self._db_api.get_distinct_column_values(self.db_name, self.table_name, column_name_list,
    ...
    def get_max_column_values(self, column_name_list, columns_to_cast_to_string=None, optimistic_prune_clause=None):
        ...
        """
        return self._db_api.get_max_column_values(self.db_name, self.table_name, column_name_list,
                                                  columns_to_cast_to_string=columns_to_cast_to_string,
    ...
    def get_non_synthetic_columns(self):
        ...
        if not self._columns and self.exists():
            self.get_columns()
        return [_ for _ in self._columns
                if not self.is_synthetic_partition_column(_) and not self._is_synthetic_bucket_column(_)]
    ...
    def get_synthetic_columns(self):
        ...
        """
        return [_ for _ in self.get_partition_columns()
                if self.is_synthetic_partition_column(_) or self._is_synthetic_bucket_column(_)]
    ...
    def get_table_partition_count(self):
        return self._db_api.get_table_partition_count(self.db_name, self._base_table_name)

    def get_table_row_count_from_metadata(self):
        return self._db_api.get_table_row_count_from_metadata(self.db_name, self._base_table_name)

    def get_table_size(self, no_cache=False):
        return self._db_api.get_table_size(self.db_name, self._base_table_name, no_cache=no_cache)

    def get_table_size_and_row_count(self):
        return self._db_api.get_table_size_and_row_count(self.db_name, self._base_table_name)

    def get_table_stats(self, as_dict=False):
        return self._db_api.get_table_stats(self.db_name, self._base_table_name, as_dict=as_dict)

    def get_table_stats_partitions(self):
        return self._db_api.get_table_stats_partitions(self.db_name, self._base_table_name)
    ...
    def refresh_operational_settings(self, offload_operation=None, rdbms_columns=None, data_gov_client=None,
    ...
    def set_partition_stats(self, new_partition_stats, additive_stats):
        return self._db_api.set_partition_stats(self.db_name, self._base_table_name,
                                                new_partition_stats, additive_stats)

    def set_table_stats(self, new_table_stats, additive_stats):
        return self._db_api.set_table_stats(self.db_name, self._base_table_name, new_table_stats, additive_stats)
    ...
    def to_canonical_column_with_overrides(self, backend_column, canonical_overrides,
        ...
        if match_table_column(backend_column.name, canonical_overrides):
            ...
            if not self._db_api.valid_canonical_override(backend_column, new_col):
                raise BackendTableException('%s %s: %s -> %s'
                                            % (INVALID_DATA_TYPE_CONVERSION_EXCEPTION_TEXT, backend_column.name.upper(),
                                               backend_column.data_type.upper(), new_col.data_type.upper()))
        else:
            if detect_sizes and backend_column.is_unbound_string():
                ...
                self._log('Padded length %s to %s' % (str(longest_string), str(safe_length)), detail=VVERBOSE)
                ...
                if self.detect_column_has_fractional_seconds(backend_column):
                    ...
                    self._log('Detected fractional seconds, using data_scale=%s' % str(detect_col.data_scale),
                              detail=VVERBOSE)
                else:
                    self._log('No fractional seconds detected, using data_scale=0', detail=VVERBOSE)
    ...
    def validate_staged_data(self, rdbms_part_cols, rdbms_columns, expected_rows, staging_columns,
        ...
        """ Validate the staged data before we insert it into the final backend table.
            ...
            across all backends.
        """
        self._validate_staged_data(rdbms_part_cols, rdbms_columns, expected_rows, staging_columns,
    ...
    def compute_final_table_stats_step(self, incremental_stats, materialized_join=False):
        if self.table_stats_compute_supported():
            self._offload_step(command_steps.STEP_COMPUTE_STATS,
                               lambda: self.compute_final_table_stats(incremental_stats,
                                                                      materialized_join=materialized_join),
                               optional=True)
    ...
    def validate_staged_data_step(self, rdbms_part_cols, rdbms_columns, expected_rows, staging_columns,
                                  incremental_update_extraction=False):
        self._offload_step(
            command_steps.STEP_VALIDATE_DATA,
            lambda: self.validate_staged_data(rdbms_part_cols, rdbms_columns, expected_rows, staging_columns,
