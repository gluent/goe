...
class HdfsFilepathKeyValueStore(KeyValueStore):
    ...
    def get_all(self, key_string_list):
        ...
        return {
            k: entry.value_string if entry else None
            for k, entry in keys_and_entries.items()
        }

    def put_all(self, key_string_to_value_string_map):
        keys_and_entries = self._get_max_seqno_entries(
            list(key_string_to_value_string_map.keys())
        )
        keys_and_seqnos = {
            k: (1 + entry.seqno) if entry else 0
            for k, entry in keys_and_entries.items()
        }
    ...
    def _list_entries(self, entry_filter=lambda _: True):
        ...
        return [
            _
            for _ in (HdfsFilepathKeyValueStoreEntry.decode(_) for _ in dir_listing)
            if entry_filter(_)
        ]

    def _extract_max_seqno_entries_from_entries_list(
        self, entries_list, keys_to_include=[]
    ):
    ...
    def _get_max_seqno_entries(self, key_string_list=[]):
        ...
        entry_filter = (
            lambda entry: entry.key_string in key_string_set
            if key_string_set
            else lambda _: True
        )
        entries = self._list_entries(entry_filter=entry_filter)
        return self._extract_max_seqno_entries_from_entries_list(
            entries, key_string_list
        )
...
class HdfsFilepathKeyValueStoreEntry(object):
    ...
    def encode(self, path_prefix):
        encoded_entry = urllib.parse.urlencode(
            {"s": self.seqno, "k": self.key_string, "v": self.value_string}
        )
    ...
    def decode(full_path):
        ...
        assert not missing, "error decoding kv string -- missing keys: {}".format(
            ", ".join(missing)
        )
