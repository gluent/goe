...
class AddOracleColumn(SchemaSyncStep):
    ...
    def __init__(self, options, orchestration_options, messages, execution_id, repo_client):
        ...
        super().__init__(schema_sync_constants.ADD_ORACLE_COLUMN, options,
    ...
    def _do_run(self, run_params):
        ...
        log('Adding columns to %s.%s table' % (double_quote_sandwich(rdbms_owner),
                                               double_quote_sandwich(rdbms_table_name)))

        new_rdbms_cols = ['%s %s' % (double_quote_sandwich(_.name), _.format_data_type()) for _ in run_params['columns']]
        sql = 'ALTER TABLE %s.%s ADD (%s)' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), ', '.join(new_rdbms_cols))
        log('Oracle sql: ' + sql, verbose)

        try:
            if self._options.execute:
                conn = get_oracle_connection(self._orchestration_options.ora_adm_user,
            ...
            source_name = '%s.%s' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name))
            log('%s\n%s' % (schema_sync_constants.EXCEPTION_ADD_ORACLE_COLUMN, traceback.format_exc()), verbose)
            self._messages.warning('%s for table %s' % (schema_sync_constants.EXCEPTION_ADD_ORACLE_COLUMN, source_name), ansi_code='red')
