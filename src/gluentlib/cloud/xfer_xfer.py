...
from gluentlib.util.parallel_exec import ParallelExecutor, STATUS_NOOP
...
from gluentlib.cloud.xfer_file import XferFile
from gluentlib.cloud.hdfs_xfer_file import HdfsXferFile
from gluentlib.cloud.s3_xfer_file import S3XferFile


###############################################################################
# EXCEPTIONS
###############################################################################
class XferXferException(Exception): pass
...
class XferXfer(OffloadMessagesMixin, object):
    ...
    def copy(self, src_section, src_key, dst_section, dst_key, job_id=1, force=False):
        ...
        logger.debug("[Job: %d] Starting single file copy from: %s to: %s" % (job_id, src, dst))
        self.log_vverbose("Copying: %s -> %s" % (src, dst))

        # Run xfer
        if not src.exists:
            raise XferXferException("[FILE COPY job: %d] FAILED: File: %s does NOT exist" % (job_id, src))
        ...
        src.copy(dst)
        logger.debug("[Job: %d] Finish single file copy from: %s to: %s. Success" % (job_id, src, dst))
    ...
    def multicopy(self, copy_pairs, parallel=1, kill_on_first_error=False, force=False):
        ...
        def emit_job(job_id, file_item):
            ...
            return [self.copy, src_section, src_key, dst_section, dst_key, job_id, force]
    ...
    def listcopy(self, src_section, dst_section, src_files, dst_remap=lambda x: x, parallel=1, \
        kill_on_first_error=False, force=False):
        ...
        logger.debug("Starting list copy from: %s to: %s with: %s parallel slaves" % \
            (src_section, dst_section, parallel))

        copy_pairs = ((src_section, src_f, dst_section, dst_remap(src_f)) for src_f in src_files)
