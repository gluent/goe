...
# EXCEPTIONS
###############################################################################
class HdfsStoreException(Exception):
    pass
...
class HdfsStore(GOEHdfsLayout, JMESPathSearch, object):
    ...
    def __init__(self, cfg, section, root, dry_run=False):
        ...
        if not self._cfg_section:
            raise HdfsStoreException(
                "Configuration 'section' was not supplied to HdfsStore() object"
            )
        ...
        if not isinstance(self._hdfs, GOEWebHdfsClient):
            raise HdfsStoreException(
                "Only webhdfs client is supported in HdfsStore so far"
            )

        options = self._cfg.extract_options(
            self._cfg_section, ("webhdfs_url", "hdfs_root")
        )
        ...
        logger.debug(
            "HdfsStore() object successfully initialized for root: '%s'" % self._root
        )
    ...
    def hdfscli_goe_client(cfg, section, dry_run=False):
        ...
        def web_client(webhdfs_url):
            ...
            args = {
                ...
                "use_kerberos": parse_python_from_string(
                    cfg.get(section, "hdfs_use_kerberos", "False")
                ),
                "verify_ssl_cert": parse_python_from_string(
                    cfg.get(section, "hdfs_verify_ssl_cert", "None")
                ),
                "mutual_auth": cfg.get(section, "hdfs_mutual_auth", "OPTIONAL"),
                "max_concurrency": cfg.getint(
                    section, "hdfs_request_max_concurrency", 1
                ),
                "dry_run": dry_run,
            }
        ...
        def ssh_client(user):
            ...
            hdfs_host = cfg.get(section, "hdfs_cmd_host") or cfg.get(
                section, "hadoop_host"
            )
            if not user or not hdfs_host:
                raise HdfsStoreException(
                    "Invalid setup for HDFS ssh section. Define: 'hadoop_host', 'ssh_user'"
                )
        ...
        if url:
            ...
            raise HdfsStoreException(
                "Unable to create HDFS client from section: %s. Supply either: 'webhdfs_url' or 'ssh_user'"
                % section
            )
    ...
    def _create_root(self):
        ...
        if not self._cfg_section:
            raise HdfsUserException(
                "Configuration 'section' needs to be supplied to 'create root': %s"
                % self._root
            )

        permissions = self._cfg.getint(
            self._cfg_section, "hdfs_newdir_permissions", DEFAULT_ROOT_PERMISSIONS
        )
        if not permissions:
            raise HdfsUserException(
                "('hdfs_newdir_permissions') "
        ...
        try:
            logger.debug(
                "Creating HDFS root: %s with permissions: %s"
                % (self._root, permissions)
            )
            self._hdfs.makedirs(self._root, permissions)
        except hdfs.util.HdfsError as e:
            raise HdfsStoreException(
                "HDFS exception: %s while creating root: %s" % (e, self._root)
            )
    ...
    def _extract_files(self):
        ...
        for root, dirs, files in self._hdfs.walk(
            hdfs_path=self._root, depth=0, status=True
        ):
            if skip_meta == root[0]:
                continue
            for f in files:
                ...
                logger.debug(
                    "Emitting file: %s with status: %s" % (abs_file_name, file_status)
                )
    ...
    def _get_file_properties(self, file_name, file_status):
        ...
        last_modified = datetime.datetime.fromtimestamp(
            int(file_status["modificationTime"] / 1000), pytz.utc
        )
    ...
    def create(self):
        ...
        if self._root_exists():
            logger.warning(
                "Ignoring attempt to create already existing root: %s" % self._root
            )
        else:
            self._create_root()
    ...
    def save_meta_note(self, file_name, data, overwrite=True, stats=None):
        ...
        logger.debug(
            "Saving HDFS note: %s. Overwrite: %s. With stats: %s"
            % (key_name, overwrite, stats)
        )
        args = {"hdfs_path": key_name, "data": data, "overwrite": overwrite}
