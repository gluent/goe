...
from testlib.test_framework.test_constants import PARTITION_FUNCTION_TEST_FROM_DEC1,\
...
from testlib.test_framework.backend_testing_api import BackendTestingApiInterface, \
    STORY_TEST_BACKEND_DOUBLE_COL, STORY_TEST_BACKEND_INT_8_COL, STORY_TEST_BACKEND_DECIMAL_DEF_COL,\
    STORY_TEST_BACKEND_VAR_STR_COL, STORY_TEST_BACKEND_DATE_COL, STORY_TEST_BACKEND_DATETIME_COL,\
    STORY_TEST_BACKEND_TIMESTAMP_COL, STORY_TEST_BACKEND_BLOB_COL, STORY_TEST_BACKEND_NULL_STR_COL,\
    ...
    STORY_TEST_OFFLOAD_NUMS_DEC_10_0, STORY_TEST_OFFLOAD_NUMS_DEC_13_9, STORY_TEST_OFFLOAD_NUMS_DEC_38_3
...
class BackendBigQueryTestingApi(BackendTestingApiInterface):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False,
        ...
        """ CONSTRUCTOR
        """
        super(BackendBigQueryTestingApi, self).__init__(connection_options, backend_type, messages, dry_run=dry_run,
    ...
    def _define_test_partition_function(self, udf_name):
        arg_types = {PARTITION_FUNCTION_TEST_FROM_INT8: BIGQUERY_TYPE_INT64,
        ...
        udf_expr = {PARTITION_FUNCTION_TEST_FROM_INT8: 'CAST(TRUNC(p_arg,-1) AS INT64)',
    ...
    def _gl_type_mapping_column_definitions(self, filter_column=None):
        ...
        all_columns = {
            name(BIGQUERY_TYPE_BIGNUMERIC):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_BIGNUMERIC), BIGQUERY_TYPE_BIGNUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_BIGNUMERIC), GLUENT_TYPE_DECIMAL)},
            name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_INTEGER_8):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_INTEGER_8), BIGQUERY_TYPE_BIGNUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_INTEGER_8), GLUENT_TYPE_INTEGER_8),
                 'present_options': {'integer_8_columns_csv': name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_INTEGER_8)}},
            name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_DECIMAL, '38', '18'):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_DECIMAL, '38', '18'),
                                    BIGQUERY_TYPE_BIGNUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_DECIMAL, '38', '18'),
                                     GLUENT_TYPE_DECIMAL),
                 'present_options': {'decimal_columns_csv_list': [name(BIGQUERY_TYPE_BIGNUMERIC, GLUENT_TYPE_DECIMAL,
                                                                       '38', '18')],
                                     'decimal_columns_type_list': ['38,18']}},
            name(BIGQUERY_TYPE_BYTES):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_BYTES), BIGQUERY_TYPE_BYTES),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_BYTES), GLUENT_TYPE_BINARY)},
            name(BIGQUERY_TYPE_BYTES, GLUENT_TYPE_LARGE_BINARY):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_BYTES, GLUENT_TYPE_LARGE_BINARY), BIGQUERY_TYPE_BYTES),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_BYTES, GLUENT_TYPE_LARGE_BINARY), GLUENT_TYPE_LARGE_BINARY),
                 'present_options': {'large_binary_columns_csv': name(BIGQUERY_TYPE_BYTES, GLUENT_TYPE_LARGE_BINARY)}},
            name(BIGQUERY_TYPE_DATE):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_DATE), BIGQUERY_TYPE_DATE),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_DATE), GLUENT_TYPE_DATE)},
            name(BIGQUERY_TYPE_DATE, GLUENT_TYPE_TIMESTAMP):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_DATE, GLUENT_TYPE_TIMESTAMP), BIGQUERY_TYPE_DATE),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_DATE, GLUENT_TYPE_TIMESTAMP), GLUENT_TYPE_TIMESTAMP),
                 'present_options': {'timestamp_columns_csv': name(BIGQUERY_TYPE_DATE, GLUENT_TYPE_TIMESTAMP)}},
            name(BIGQUERY_TYPE_DATETIME):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_DATETIME), BIGQUERY_TYPE_DATETIME),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_DATETIME), GLUENT_TYPE_TIMESTAMP)},
            name(BIGQUERY_TYPE_DATETIME, GLUENT_TYPE_DATE):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_DATETIME, GLUENT_TYPE_DATE), BIGQUERY_TYPE_DATETIME),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_DATETIME, GLUENT_TYPE_DATE), GLUENT_TYPE_DATE),
                 'present_options': {'date_columns_csv': name(BIGQUERY_TYPE_DATETIME, GLUENT_TYPE_DATE)}},
            name(BIGQUERY_TYPE_FLOAT64):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_FLOAT64), BIGQUERY_TYPE_FLOAT64),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_FLOAT64), GLUENT_TYPE_DOUBLE)},
            name(BIGQUERY_TYPE_FLOAT64, GLUENT_TYPE_DECIMAL, '38', '9'):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_FLOAT64, GLUENT_TYPE_DECIMAL, '38', '9'), BIGQUERY_TYPE_FLOAT64),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_FLOAT64, GLUENT_TYPE_DECIMAL, '38', '9'), GLUENT_TYPE_DECIMAL),
                 'present_options': {'decimal_columns_csv_list':
                                         [name(BIGQUERY_TYPE_FLOAT64, GLUENT_TYPE_DECIMAL, '38', '9')],
                                     'decimal_columns_type_list': ['38,9']}},
            name(BIGQUERY_TYPE_INT64):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_INT64), BIGQUERY_TYPE_INT64),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_INT64), GLUENT_TYPE_INTEGER_8)},
            name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_1):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_1), BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_1), GLUENT_TYPE_INTEGER_1),
                 'present_options': {'integer_1_columns_csv': name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_1)}},
            name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_2):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_2), BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_2), GLUENT_TYPE_INTEGER_2),
                 'present_options': {'integer_2_columns_csv': name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_2)}},
            name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_4):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_4), BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_4), GLUENT_TYPE_INTEGER_4),
                 'present_options': {'integer_4_columns_csv': name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_4)}},
            name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_8):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_8), BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_8), GLUENT_TYPE_INTEGER_8),
                 'present_options': {'integer_8_columns_csv': name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_8)}},
            name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_38):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_38), BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_38), GLUENT_TYPE_INTEGER_38),
                 'present_options': {'integer_38_columns_csv': name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_INTEGER_38)}},
            name(BIGQUERY_TYPE_NUMERIC):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC), BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC), GLUENT_TYPE_DECIMAL)},
            name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_DECIMAL, '38', '18'):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_DECIMAL, '38', '18'),
                                    BIGQUERY_TYPE_NUMERIC),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_DECIMAL, '38', '18'),
                                     GLUENT_TYPE_DECIMAL),
                 'present_options': {'decimal_columns_csv_list': [name(BIGQUERY_TYPE_NUMERIC, GLUENT_TYPE_DECIMAL,
                                                                       '38', '18')],
                                     'decimal_columns_type_list': ['38,18']}},
            name(BIGQUERY_TYPE_STRING):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING), GLUENT_TYPE_VARIABLE_STRING,
                                     char_semantics=CANONICAL_CHAR_SEMANTICS_CHAR)},
            name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING), GLUENT_TYPE_LARGE_STRING),
                 'present_options': {'large_string_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING)}},
            name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING, UNICODE_NAME_TOKEN):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING, UNICODE_NAME_TOKEN),
                                    BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING, UNICODE_NAME_TOKEN),
                                     GLUENT_TYPE_LARGE_STRING),
                 'present_options':
                     {'large_string_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING,
                                                       UNICODE_NAME_TOKEN),
                      'unicode_string_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_STRING,
                                                         UNICODE_NAME_TOKEN)}},
            name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_BINARY):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_BINARY), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_BINARY), GLUENT_TYPE_BINARY),
                 'present_options': {'binary_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_BINARY)}},
            name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_BINARY):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_BINARY), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_BINARY), GLUENT_TYPE_LARGE_BINARY),
                 'present_options': {'large_binary_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_LARGE_BINARY)}},
            name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_DS):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_DS), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_DS), GLUENT_TYPE_INTERVAL_DS),
                 'present_options': {'interval_ds_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_DS)}},
            name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_YM):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_YM), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_YM), GLUENT_TYPE_INTERVAL_YM),
                 'present_options': {'interval_ym_columns_csv': name(BIGQUERY_TYPE_STRING, GLUENT_TYPE_INTERVAL_YM)}},
            name(BIGQUERY_TYPE_STRING, UNICODE_NAME_TOKEN):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_STRING, UNICODE_NAME_TOKEN), BIGQUERY_TYPE_STRING),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_STRING, UNICODE_NAME_TOKEN), GLUENT_TYPE_VARIABLE_STRING,
            ...
            name(BIGQUERY_TYPE_TIME):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_TIME), BIGQUERY_TYPE_TIME),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_TIME), GLUENT_TYPE_TIME)},
            name(BIGQUERY_TYPE_TIMESTAMP):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_TIMESTAMP), BIGQUERY_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_TIMESTAMP), GLUENT_TYPE_TIMESTAMP_TZ)},
            name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_DATE):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_DATE), BIGQUERY_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_DATE), GLUENT_TYPE_DATE),
                 'present_options': {'date_columns_csv': name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_DATE)}},
            name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP):
                {'column':
                     BigQueryColumn(name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP), BIGQUERY_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP), GLUENT_TYPE_TIMESTAMP),
                 'present_options': {'timestamp_columns_csv': name(BIGQUERY_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP)}},
        }
        if filter_column:
    ...
    def create_partitioned_test_table(self, db_name, table_name, source_table_name,
        ...
        create_cols.append(self.gen_column_object('YEARMON', data_type=BIGQUERY_TYPE_INT64,
        ...
        insert_col_tuples.append(("CAST(%(fn)s('%%E4Y%%m', %(source_name)s) AS %(cast_type)s)"
                                  % {'fn': extract_fn, 'source_name': partition_source_column.name,
                                     'cast_type': BIGQUERY_TYPE_INT64}, 'YEARMON'))
...
FROM %(db_table)s""" % {'db_table': self._db_api.enclose_object_reference(db_name, table_name),
    ...
    def drop_database(self, db_name, cascade=False):
        ...
        if not self._db_api.database_exists(db_name):
            self._log('Dataset does not exist, not attempting to drop: %s' % db_name, detail=VVERBOSE)
            return []
        log_cmd = 'drop_dataset(%s)' % self._db_api.bq_dataset_id(db_name)
        self._log('BigQuery call: %s' % log_cmd, detail=VERBOSE)
        if not self._dry_run:
            self._db_api.bq_client().delete_dataset(self._db_api.bq_dataset_id(db_name), delete_contents=cascade)
        return [log_cmd]
    ...
    def expected_backend_column(self, canonical_column, override_used=None, decimal_padding_digits=None):
        def is_bigquery_numeric(column):
            return bool(
                (column.data_type == GLUENT_TYPE_DECIMAL and
                 column.data_precision and column.data_precision <= 38 and
                 column.data_scale and column.data_scale <= 9)
                or
                (column.data_type == GLUENT_TYPE_INTEGER_38 and
                 column.data_precision and column.data_precision <= 29)
            )
    ...
    def expected_canonical_to_backend_type_map(self, override_used=None):
        ...
        return {GLUENT_TYPE_FIXED_STRING: BIGQUERY_TYPE_STRING,
                GLUENT_TYPE_LARGE_STRING: BIGQUERY_TYPE_STRING,
                GLUENT_TYPE_VARIABLE_STRING: BIGQUERY_TYPE_STRING,
                GLUENT_TYPE_BINARY: BIGQUERY_TYPE_BYTES,
                GLUENT_TYPE_LARGE_BINARY: BIGQUERY_TYPE_BYTES,
                GLUENT_TYPE_INTEGER_1: BIGQUERY_TYPE_INT64,
                GLUENT_TYPE_INTEGER_2: BIGQUERY_TYPE_INT64,
                GLUENT_TYPE_INTEGER_4: BIGQUERY_TYPE_INT64,
                GLUENT_TYPE_INTEGER_8: BIGQUERY_TYPE_INT64,
                GLUENT_TYPE_INTEGER_38: BIGQUERY_TYPE_BIGNUMERIC,
                GLUENT_TYPE_DECIMAL: numeric_override or BIGQUERY_TYPE_BIGNUMERIC,
                GLUENT_TYPE_DOUBLE: BIGQUERY_TYPE_FLOAT64,
                GLUENT_TYPE_DATE: BIGQUERY_TYPE_DATE,
                GLUENT_TYPE_TIME: BIGQUERY_TYPE_TIME,
                GLUENT_TYPE_TIMESTAMP: BIGQUERY_TYPE_DATETIME,
                GLUENT_TYPE_TIMESTAMP_TZ: BIGQUERY_TYPE_TIMESTAMP,
                GLUENT_TYPE_INTERVAL_DS: BIGQUERY_TYPE_STRING,
                GLUENT_TYPE_INTERVAL_YM: BIGQUERY_TYPE_STRING,
                GLUENT_TYPE_BOOLEAN: BIGQUERY_TYPE_BOOLEAN}
    ...
    def expected_customers_offload_predicates(self):
        return [
            ...
            ('column(CUST_EFF_FROM) = datetime(1970-01-01)', '`CUST_EFF_FROM` = DATETIME \'1970-01-01\''),
            ('column(CUST_EFF_FROM) = datetime(1970-01-01 12:13:14)',
             '`CUST_EFF_FROM` = DATETIME \'1970-01-01 12:13:14\''),
        ]
    ...
    def expected_customers_synthetic_offload_predicates(self):
        ...
        return [
            ((date_column, 'D', None), [
                ('column(CUST_EFF_FROM) = datetime(2010-01-01)', '`CUST_EFF_FROM` = DATETIME \'2010-01-01\''),
                ('datetime(2010-01-01) = column(CUST_EFF_FROM)', 'DATETIME \'2010-01-01\' = `CUST_EFF_FROM`'),
                ('column(CUST_EFF_FROM) != datetime(2010-01-01)', '`CUST_EFF_FROM` != DATETIME \'2010-01-01\''),
                ('column(CUST_EFF_FROM) < datetime(2010-01-01)', '`CUST_EFF_FROM` < DATETIME \'2010-01-01\''),
                ('column(CUST_EFF_FROM) <= datetime(2010-01-01)', '`CUST_EFF_FROM` <= DATETIME \'2010-01-01\''),
                ('column(CUST_EFF_FROM) > datetime(2010-01-01)', '`CUST_EFF_FROM` > DATETIME \'2010-01-01\''),
    ...
    def gl_type_mapping_generated_table_col_specs(self):
        ...
        for col_dict in [definitions[col_name] for col_name in sorted(definitions.keys())]:
            ...
            if backend_column.data_type in [BIGQUERY_TYPE_BIGNUMERIC,
                                            BIGQUERY_TYPE_NUMERIC] and col_dict.get('present_options'):
                ...
                if present_options.get('decimal_columns_type_list'):
                    precision, scale = present_options['decimal_columns_type_list'][0].split(',')
                ...
                elif col_dict['expected_canonical_column'].data_type in [GLUENT_TYPE_INTEGER_1, GLUENT_TYPE_INTEGER_2,
            ...
            elif col_dict['expected_canonical_column'].data_type == GLUENT_TYPE_BINARY:
                gl_type_mapping_cols.append({'column': backend_column,
    ...
    def rename_column(self, db_name, table_name, column_name, new_name, sync=None):
        ...
        def rename_ddl_fn(ddl_line, column_name, new_name):
            if ddl_line and self.enclose_identifier(column_name).lower() in ddl_line.lower():
                return ddl_line.lower().replace(self.enclose_identifier(column_name).lower(),
        ...
        self._log('Recreating %s with column %s renamed to %s'
                  % (self._db_api.enclose_object_reference(db_name, table_name), column_name, new_name),
                  detail=VVERBOSE)
        orig_ddl = self._db_api.get_table_ddl(db_name, table_name, as_list=True, for_replace=True)
        ...
        rename_sel_fn = lambda x: '{} AS {}'.format(x, new_name) if x.lower() == column_name.lower() else x
...
FROM %(db_table)s""" % {'db_table': self._db_api.enclose_object_reference(db_name, table_name),
    ...
    def story_test_table_extra_col_info(self):
        ...
        """
        extra_cols = {STORY_TEST_BACKEND_DOUBLE_COL: {'sql_expression': 'CAST(123.123 AS %s)'
                                                                        % BIGQUERY_TYPE_FLOAT64},
                      STORY_TEST_BACKEND_INT_8_COL: {'sql_expression': 'CAST(1234567890123 AS %s)'
                                                                       % BIGQUERY_TYPE_INT64},
                      STORY_TEST_BACKEND_DECIMAL_DEF_COL: {'sql_expression': 'CAST(123.123 AS %s)'
                                                                             % BIGQUERY_TYPE_NUMERIC},
                      STORY_TEST_BACKEND_VAR_STR_COL: {'sql_expression': "CAST('this is string' AS %s)"
                                                                         % BIGQUERY_TYPE_STRING},
                      ...
                      STORY_TEST_BACKEND_BLOB_COL: {'sql_expression': "CAST('this is string' AS %s)"
                                                                      % BIGQUERY_TYPE_BYTES},
                      STORY_TEST_BACKEND_NULL_STR_COL: {'sql_expression': 'CAST(NULL AS %s)'
                                                                          % BIGQUERY_TYPE_STRING}}
        return extra_cols
    ...
    def transient_query_error_identification_strings(self) -> list:
        ...
        return self._transient_query_error_identification_global_strings() + bigquery_error_strings
