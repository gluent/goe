...
from gluentlib.data_governance.hadoop_data_governance_interface import HadoopDataGovernanceInterface
from gluentlib.data_governance.hadoop_data_governance_constants import *

###############################################################################
# EXCEPTIONS
###############################################################################

class ClouderaNavigatorException(Exception): pass
class ClouderaNavigatorNoMetadataException(Exception): pass
...
CUSTOM_METADATA_TEMPLATE = {METADATA_DESCRIPTION: None,
                            METADATA_TAGS: [],
                            METADATA_PROPERTIES: {}}
...
class ClouderaNavigator(HadoopDataGovernanceInterface):
    def __init__(self, url, user, password, messages, dry_run=False):
        ...
        self._messages.debug('Initialize ClouderaNavigator(%s, %s)' % (url, user), detail=SUPPRESS_STDOUT)
    ...
    def _get_api_header(self):
        ...
        if http_code != requests.codes.ok:
            raise ClouderaNavigatorException('Navigator API URL returned status %s: %s' % (http_code, self._navigator_api_url))
        return http_code
    ...
    def _get_full_api_url(self):
        ...
        if not self._full_api_url:
            self._full_api_url = URL_SEP.join([self._navigator_api_url, self._get_api_version()])
    ...
    def _resp_as_json(self, resp, return_key=None):
        ...
        if return_key and js and type(js) is not dict:
            raise NotImplementedError('Return key from JSON with type %s is not currently supported' % type(js))
    ...
    def _nav_put(self, sub_url, json_payload, return_key=None):
        ...
        resp = requests.put(url, auth=(self._navigator_user, self._navigator_password), headers=headers, data=json_payload)
        if resp.status_code == requests.codes.ok:
            return self._resp_as_json(resp, return_key=return_key)
        self._messages.log('Response code: %s' % resp.status_code, VERBOSE)
        self._messages.log('Response text: %s' % resp.text, VERBOSE)
    ...
    def _nav_put_entity(self, entity_id, json_payload, return_key=None):
        ...
        assert type(json_payload) is str
        if self._dry_run:
            self._messages.debug('Skipping entity PUT in dry run mode', detail=SUPPRESS_STDOUT)
            return None
        return self._nav_put('entities/%s' % entity_id, json_payload, return_key=return_key)
    ...
    def _nav_post(self, sub_url, json_payload, return_key=None):
        ...
        url = URL_SEP.join([self._get_full_api_url(), sub_url])
        headers = {'Content-Type': 'application/json'}
        resp = requests.post(url, auth=(self._navigator_user, self._navigator_password), headers=headers, data=json_payload)
        if resp.status_code == requests.codes.ok:
            return self._resp_as_json(resp, return_key=return_key)
        self._messages.log('Response code: %s' % resp.status_code, VERBOSE)
        self._messages.log('Response text: %s' % resp.text, VERBOSE)
    ...
    def _search_metadata(self, search_terms):
        ...
        url_search_string = 'AND'.join(search_token(k, v) for k, v in search_terms.items())
        self._messages.debug('Search query syntax: %s' % url_search_string, detail=SUPPRESS_STDOUT)
        metadata_list = self._nav_get('entities?query=%s' % url_search_string)
        self._messages.debug('Found %s matching metadata entries' % (metadata_list if metadata_list is None else len(metadata_list)), detail=SUPPRESS_STDOUT)
    ...
    def healthcheck_api(self):
        ...
        self._messages.log('Healthchecking Navigator API (%s@%s)' % (self._navigator_user, self._get_full_api_url()), VVERBOSE)
    ...
    def validate_metadata(self, custom_metadata):
        ...
        unexpected_keys = [_ for _ in custom_metadata if _ not in CUSTOM_METADATA_TEMPLATE]
        assert not unexpected_keys, 'Unexpected keys: %s' % unexpected_keys
        # no unexpected value types in custom_metadata
        assert not custom_metadata.get(METADATA_DESCRIPTION) or isinstance(custom_metadata[METADATA_DESCRIPTION], str)
        assert not custom_metadata.get(METADATA_TAGS) or type(custom_metadata[METADATA_TAGS]) in (list, tuple)
        assert not custom_metadata.get(METADATA_PROPERTIES) or type(custom_metadata[METADATA_PROPERTIES]) is dict
        return True
    ...
    def get_full_metadata(self, entity_id, ignore_missing_metadata=False):
        ...
        if not metadata:
            if ignore_missing_metadata:
                return {}
            raise ClouderaNavigatorNoMetadataException('No metadata retrieved for entity id: %s' % entity_id)
        return metadata

    def get_hive_object_metadata(self, db, object_name, ignore_missing_metadata=False, object_type=None, null_source_service_id_workaround=False):
        ...
        self._messages.debug('get_hive_object_metadata(%s,%s)' % (db, object_name), detail=SUPPRESS_STDOUT)
        ...
        search_terms = {'parentPath': [db, '\\/' + db], 'originalName': object_name, 'sourceId': self._hive_source_entity, 'deleted': ['false', None]}
        if null_source_service_id_workaround:
            ...
            search_terms = {'parentPath': [db, '\\/' + db], 'originalName': object_name, 'sourceId': [self._hive_source_entity, None], 'extractorRunId': '%s*' % self._hive_source_entity, 'sourceType': 'HIVE', 'deleted': ['false', None]}
        if object_type:
            search_terms['type'] = object_type.upper()

        self._messages.log('Searching Navigator metadata with: %s' % search_terms, detail=VVERBOSE)
        metadata_list = self._search_metadata(search_terms)
        if not metadata_list and not null_source_service_id_workaround:
            return self.get_hive_object_metadata(db, object_name, ignore_missing_metadata=ignore_missing_metadata, object_type=object_type, null_source_service_id_workaround=True)
        if not metadata_list:
            if ignore_missing_metadata:
                self._messages.log('No existing metadata found', detail=VVERBOSE)
                return {}
            raise ClouderaNavigatorNoMetadataException('No metadata retrieved for Hive object: %s.%s' % (db, object_name))
        elif type(metadata_list) is list:
            if null_source_service_id_workaround:
                trustworthy_entries = [_ for _ in metadata_list if _.get('extractorRunId', '')[:len(self._hive_source_entity)+1] == '%s#' % self._hive_source_entity]
                if trustworthy_entries:
                    self._messages.log('Verified trustworthy metadata x%s' % len(trustworthy_entries), detail=VVERBOSE)
                    metadata_list = trustworthy_entries
            if len(metadata_list) == 1:
                return metadata_list[0]
            else:
                raise ClouderaNavigatorException('%s metadata matches for Hive object: %s.%s' % (len(metadata_list), db, object_name))
        self._messages.log('Malformed metadata response: %s' % metadata_list, detail=VVERBOSE)
        raise ClouderaNavigatorException('Malformed metadata response for Hive object: %s.%s' % (db, object_name))

    def get_hive_db_metadata(self, db, ignore_missing_metadata=False, null_source_service_id_workaround=False):
        """ Get the metadata dict for a Hive object (table or view) identified by:
              - sourceType is Hive
        ...
        search_terms = {'originalName': db, 'sourceId': self._hive_source_entity, 'deleted': ['false', None]}
        self._messages.log('Searching Navigator metadata with: %s' % search_terms, detail=VVERBOSE)
        metadata_list = self._search_metadata(search_terms)
        if not metadata_list:
            if ignore_missing_metadata:
                self._messages.log('No existing metadata found', detail=VVERBOSE)
                return {}
            raise ClouderaNavigatorNoMetadataException('No metadata retrieved for Hive db: %s' % db)
        elif type(metadata_list) is list:
            if len(metadata_list) == 1:
                return metadata_list[0]
            else:
                raise ClouderaNavigatorException('%s metadata matches for Hive db: %s' % (len(metadata_list), db))
        self._messages.log('Malformed metadata response: %s' % metadata_list, detail=VVERBOSE)
        raise ClouderaNavigatorException('Malformed metadata response for Hive db: %s' % db)

    def get_hive_table_metadata(self, db, table_name, ignore_missing_metadata=False):
        return self.get_hive_object_metadata(db, table_name, ignore_missing_metadata=ignore_missing_metadata, object_type='TABLE')

    def get_hive_view_metadata(self, db, view_name, ignore_missing_metadata=False):
        return self.get_hive_object_metadata(db, view_name, ignore_missing_metadata=ignore_missing_metadata, object_type='VIEW')

    def get_custom_metadata_from_full_metadata(self, full_metadata):
        return {k: full_metadata.get(k) for k in CUSTOM_METADATA_TEMPLATE} if full_metadata else full_metadata
    ...
    def get_custom_metadata(self, entity_id, ignore_missing_metadata=False):
        ...
        """
        metadata = self.get_full_metadata(entity_id, ignore_missing_metadata=ignore_missing_metadata)
        ...
        for metadata_key, new_value in new_metadata.items():
            if new_value:
                if not merged_metadata[metadata_key]:
                    self._messages.log('No existing metadata[%s], populating with %s' % (metadata_key, new_value), detail=VVERBOSE)
                    merged_metadata[metadata_key] = new_value
                elif metadata_key == METADATA_TAGS:
                    new_tags = [_ for _ in new_value if _ not in merged_metadata[metadata_key]]
                    self._messages.log('Appending new tags to existing metadata[%s]: %s' % (metadata_key, new_tags), detail=VVERBOSE)
                    ...
                    for new_prop_key, new_prop_val in new_value.items():
                        ...
                        if not reset and new_prop_key in (DATA_GOVERNANCE_DYNAMIC_PROPERTY_INITIAL_GLUENT_VERSION, DATA_GOVERNANCE_DYNAMIC_PROPERTY_INITIAL_OPERATION_DATETIME) and merged_metadata[metadata_key].get(new_prop_key):
                            ...
                            if merged_metadata[metadata_key].get(new_prop_key):
                                ...
                                old_val_list = merged_metadata[metadata_key][new_prop_key].split(',')
                                new_val_list = new_prop_val.split(',')
                                merged_metadata[metadata_key][new_prop_key] = ','.join(sorted(set(old_val_list + new_val_list)))
                            else:
                                merged_metadata[metadata_key][new_prop_key] = new_prop_val
                        else:
                            # anything else can be overwritten
                            if new_prop_key in merged_metadata[metadata_key] and new_prop_val and merged_metadata[metadata_key][new_prop_key] != new_prop_val:
                                self._messages.log('Overwriting property %s: %s => %s' % (new_prop_key, merged_metadata[metadata_key][new_prop_key], new_prop_val), detail=VVERBOSE)
                            merged_metadata[metadata_key][new_prop_key] = new_prop_val
                    self._messages.debug('Post merge property metadata: %s' % merged_metadata[metadata_key], detail=SUPPRESS_STDOUT)
                elif not reset and metadata_key == METADATA_DESCRIPTION and merged_metadata.get(METADATA_DESCRIPTION) and new_value:
                    self._messages.log('Leaving existing description in place: %s' % merged_metadata.get(METADATA_DESCRIPTION), detail=VVERBOSE)
                else:
                    self._messages.log('Updating existing metadata[%s]: %s' % (metadata_key, new_value), detail=VVERBOSE)
                    merged_metadata[metadata_key] = new_value
        return merged_metadata

    def merge_with_pre_rename_custom_metadata(self, new_metadata, renaming_from_db, renaming_from_object_name):
        ...
        """
        pre_rename_metadata = self.get_hive_object_custom_metadata(renaming_from_db, renaming_from_object_name, ignore_missing_metadata=True)
        self._messages.debug('pre_rename_metadata: %s' % pre_rename_metadata, detail=SUPPRESS_STDOUT)
        if pre_rename_metadata:
            new_metadata = self.merge_new_custom_metadata_over_old(new_metadata, pre_rename_metadata)
        self._messages.log('Merged new metadata with pre-rename metadata: %s' % new_metadata, detail=VVERBOSE)
        return new_metadata

    def update_custom_metadata(self, entity_id, new_metadata, reset=False, renaming_from_db=None, renaming_from_object_name=None):
        ...
        """
        self._messages.debug('update_custom_metadata(%s)' % entity_id, detail=SUPPRESS_STDOUT)
        ...
        self._messages.debug('existing_metadata: %s' % existing_metadata, detail=SUPPRESS_STDOUT)
        if renaming_from_db and renaming_from_object_name:
            new_metadata = self.merge_with_pre_rename_custom_metadata(new_metadata, renaming_from_db, renaming_from_object_name)

        merged_metadata = self.merge_new_custom_metadata_over_old(new_metadata, existing_metadata, reset=reset)
        self._messages.log('Merged new metadata with existing metadata: %s' % merged_metadata, detail=VVERBOSE)
    ...
    def check_hive_source_entity(self):
        ...
        try:
            ...
            if e.response.status_code == requests.codes.not_found:
                ...
                raise ClouderaNavigatorException('Hive service entity %s does not exist' % self._hive_source_entity)
            raise

        if hive_source_metadata and hive_source_metadata.get('type') == 'SOURCE' and hive_source_metadata.get('sourceType') == 'HIVE':
            if 'name' in hive_source_metadata:
                self._messages.log('Data governance Hive service: %s' % hive_source_metadata['name'], VVERBOSE)
            return True

        self._messages.log('Metadata not for Hive service: {type: %s, sourceType: %s}' % (hive_source_metadata.get('type'), hive_source_metadata.get('sourceType')), VVERBOSE)
        raise ClouderaNavigatorException('Entity %s is not for a Hive service' % self._hive_source_entity)

    def set_hive_source_entity(self, entity_id):
        self._hive_source_entity = entity_id
        self.check_hive_source_entity()

    def preregister_object_entity(self, db, object_name, custom_metadata, renaming_from_db=None, renaming_from_object_name=None):
        """ Pre-register metadata for a Hive table we are about to create
            ...
            we may choose to ignore these parameters
        """
        self._messages.debug('preregister_object_entity(%s, %s)' % (db, object_name), detail=SUPPRESS_STDOUT)
        assert self._hive_source_entity
        payload = {'sourceId': self._hive_source_entity,
                   'parentPath': db,
                   'originalName': object_name}
        if renaming_from_db and renaming_from_object_name:
            custom_metadata = self.merge_with_pre_rename_custom_metadata(custom_metadata, renaming_from_db, renaming_from_object_name)
        payload.update(custom_metadata)
        self._messages.debug('Pre-registration payload: %s' % payload, detail=SUPPRESS_STDOUT)
        if self._dry_run:
            self._messages.debug('Skipping entity POST in dry run mode', detail=SUPPRESS_STDOUT)
            return {}
        new_entity_id = self._nav_post('entities', json.dumps(payload), return_key=METADATA_IDENTITY)
        self._messages.log('Registered Navigator entity: %s' % new_entity_id, detail=VVERBOSE)
    ...
    def register_custom_object_metadata(self, db, table_name, custom_metadata, renaming_from_db=None, renaming_from_object_name=None):
        return self.preregister_object_entity(db, table_name, custom_metadata, renaming_from_db=renaming_from_db, renaming_from_object_name=renaming_from_object_name)
