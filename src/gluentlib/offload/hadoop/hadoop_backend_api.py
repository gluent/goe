...
from gluentlib.offload.backend_api import BackendApiInterface, BackendApiException, BackendApiConnectionException,\
...
from gluentlib.offload.offload_constants import DBTYPE_IMPALA, DBTYPE_HIVE, FILE_STORAGE_COMPRESSION_CODEC_GZIP,\
...
from gluentlib.util.better_impyla import HiveConnection, HiveTable, HiveServer2Error, BetterImpylaException, \
...
from gluentlib.util.hive_table_stats import HiveTableStats
from gluentlib.util.hs2_connection import hs2_connection, HS2_OPTIONS
from gluentlib.util.misc_functions import backtick_sandwich
...
HADOOP_DATA_TYPE_DECODE_RE = re.compile(r'^([a-z0-9_]+)[\(]?([0-9]*)?[\,]?([0-9]*)?[\)]?$', re.I)
...
def hive_enable_dynamic_partitions_for_insert_sqls(max_dynamic_partitions=None, max_dynamic_partitions_pernode=None,
                                                   as_dict=False):
    """ Hive session options for use when inserting into a partitioned table.
    """
    hive_settings = {'hive.exec.dynamic.partition': 'true',
...
class BackendHadoopApi(BackendApiInterface):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False,
        ...
        """ CONSTRUCTOR
        """
        super(BackendHadoopApi, self).__init__(connection_options, backend_type, messages, dry_run=dry_run,
    ...
    def _align_decimal_scale_to_udfs(self, column_name, data_precision, data_scale):
        ...
        private_precision = data_precision if data_precision else self.max_decimal_precision()
    ...
    def _align_decimal_precision_to_udfs(self, column_name, data_precision, data_scale):
        ...
        new_precision = data_precision if data_precision else self.max_decimal_precision()
        # Precision should be 18 or self.max_decimal_precision()
        new_precision = max(new_precision, 18) if new_precision < 19 else max(new_precision,
    ...
    def _alter_table_external(self, db_name, table_name, external=True):
        ...
        alter_sql = "ALTER TABLE {}.{} SET TBLPROPERTIES ('EXTERNAL'='{}')".format(self.enclose_identifier(db_name),
    ...
    def _connect_to_backend(self, host_override=None, port_override=None):
        ...
        try:
            return HiveConnection.fromconnection(hs2_connection(self._connection_options, host_override=host,
                                                                port_override=port),
    ...
    def _drop_udf(self, function_name, udf_db=None, function_spec=None, sync=None, if_exists=True):
        ...
        drop_sql = 'DROP FUNCTION %s%s%s%s' \
            % (exists_clause, db_clause, self.enclose_identifier(function_name), spec_clause)
    ...
    def _format_partition_clause(self, partition_tuples, sep_char='/'):
        ...
        if not partition_tuples:
            return None
        return sep_char.join('{part_col}={part_value}'.format(part_col=_[0], part_value=str_value_fn(_[1])) for _ in partition_tuples)
    ...
    def _get_hadoop_connection_exception_message_template(self):
        raise NotImplementedError('_get_hadoop_connection_exception_message_template() is not implemented for common Hadoop class')
    ...
    def _insert_literal_values_format_sql(self, db_name, table_name, column_names, literal_csv_list, split_by_cr=True):
        raise NotImplementedError('_insert_literal_values_format_sql() is not implemented for common Hadoop class')
    ...
    def _legacy_column_list_to_type(self, legacy_columns):
        ...
        for col_name, data_type_spec, _ in legacy_columns:
            data_type, data_length, precision, scale = self._decode_hadoop_data_type_spec(data_type_spec)
            backend_columns.append(HadoopColumn(col_name, data_type=data_type, data_length=data_length,
                                                data_precision=precision, data_scale=scale,
                                                nullable=None, data_default=None))
        return backend_columns
    ...
    def _udf_installation_sql(self, udf_db=None):
        raise NotImplementedError('_udf_installation_sql() is not implemented for common Hadoop class')

    def _udf_installation_copy_library_to_hdfs(self, udf_lib_source, udf_lib_destination):
        ...
        cmds = []
        hdfs_client = get_dfs_from_options(self._connection_options, messages=self._messages, dry_run=self._dry_run)
        if self._connection_options.offload_fs_scheme in OFFLOAD_NON_HDFS_FS_SCHEMES:
            target_uri = hdfs_client.gen_uri(self._connection_options.offload_fs_scheme,
    ...
    def _udf_test_sql(self, udf_db=None):
        raise NotImplementedError('_udf_test_sql() is not implemented for common Hadoop class')
    ...
    def add_columns(self, db_name, table_name, column_tuples, sync=None):
        ...
        column_clause = ', '.join('{} {}'.format(self.enclose_identifier(col_name.lower()),
                                                 col_type.lower()) for col_name, col_type in column_tuples)
        sql = 'ALTER TABLE %s.%s ADD COLUMNS (%s)' % (self.enclose_identifier(db_name),
                                                      self.enclose_identifier(table_name),
                                                      column_clause)
    ...
    def backend_report_info(self):
        ...
        """
        return {REPORT_ATTR_BACKEND_CLASS: 'Hadoop',
                ...
                REPORT_ATTR_BACKEND_HOST_INFO: '%s:%s' % (self._connection_options.hadoop_host,
    ...
    def create_sequence_table(self, db_name, table_name):
        ...
        sequence_table_columns = [HadoopColumn(SEQUENCE_COLUMN_NAME, data_type=HADOOP_TYPE_INT)]
        return self.create_table(db_name, table_name, sequence_table_columns, None,
                                 FILE_STORAGE_FORMAT_PARQUET, without_db_name=without_db_name, sync=True)
...
LIMIT 1""" % {'db_table': self.enclose_object_reference(db_name, table_name),
    ...
    def gen_default_numeric_column(self, column_name, data_scale=18):
        ...
        """
        return HadoopColumn(column_name, data_type=HADOOP_TYPE_DECIMAL, data_precision=self.max_decimal_precision(),
    ...
    def gen_ctas_sql_text(self, db_name, table_name, storage_format, column_tuples, from_db_name=None,
        ...
        projection = self._format_select_projection(column_tuples)
        from_clause = '\nFROM   {}'.format(self.enclose_object_reference(from_db_name, from_table_name)) if from_db_name and from_table_name else ''
        limit_clause = '\nLIMIT  {}'.format(row_limit) if row_limit is not None else ''
        if table_properties:
            table_prop_clause = '\nTBLPROPERTIES (%s)' \
                % ', '.join('%s=%s' % (self.to_backend_literal(k), self.to_backend_literal(v)) for k, v in table_properties.items())
        else:
            table_prop_clause = ''
...
SELECT %(projection)s%(from_clause)s%(limit_clause)s""" \
    ...
    def get_table_size_and_row_count(self, db_name, table_name):
        return (self.get_table_size(db_name, table_name),
    ...
    def get_view_ddl(self, db_name, view_name, as_list=False, terminate_sql=False, for_replace=False):
        ...
        return self.get_table_ddl(db_name, view_name, as_list=as_list, terminate_sql=terminate_sql,
    ...
    def insert_literal_values(self, db_name, table_name, literal_list, column_list=None, max_rows_per_insert=250,
        ...
        def add_cast(literal, formatted_data_type):
            if formatted_data_type.upper() in [HADOOP_TYPE_STRING, HADOOP_TYPE_TIMESTAMP]:
                return literal
            elif self._backend_type == DBTYPE_HIVE and HADOOP_TYPE_VARCHAR in formatted_data_type.upper():
        ...
        while remaining_rows:
            ...
            for row in this_chunk:
                formatted_rows.append(','.join(add_cast(gen_literal(py_val), data_type)
                                               for py_val, data_type in zip(row, data_type_strs)))
            sql = self._insert_literal_values_format_sql(db_name, table_name, column_names,
    ...
    def is_valid_partitioning_data_type(self, data_type):
        ...
        return bool(data_type.upper() in [HADOOP_TYPE_BIGINT, HADOOP_TYPE_CHAR, HADOOP_TYPE_DECIMAL,
                                          HADOOP_TYPE_DOUBLE, HADOOP_TYPE_FLOAT, HADOOP_TYPE_INT, HADOOP_TYPE_REAL,
                                          HADOOP_TYPE_STRING, HADOOP_TYPE_SMALLINT, HADOOP_TYPE_TIMESTAMP,
    ...
    def is_valid_storage_compression(self, user_requested_codec, user_requested_format):
        if user_requested_codec == 'NONE':
            return True
        elif user_requested_format == FILE_STORAGE_FORMAT_ORC\
         and user_requested_codec in [FILE_STORAGE_COMPRESSION_CODEC_SNAPPY, FILE_STORAGE_COMPRESSION_CODEC_ZLIB]:
            return True
        elif user_requested_format == FILE_STORAGE_FORMAT_PARQUET\
         and user_requested_codec in [FILE_STORAGE_COMPRESSION_CODEC_SNAPPY, FILE_STORAGE_COMPRESSION_CODEC_GZIP]:
            return True
        else:
            return False
    ...
    def native_integer_types(self):
        return [HADOOP_TYPE_BIGINT, HADOOP_TYPE_INT, HADOOP_TYPE_SMALLINT, HADOOP_TYPE_TINYINT]
    ...
    def rename_table(self, from_db_name, from_table_name, to_db_name, to_table_name, sync=None):
        ...
        if not self._dry_run and self.exists(to_db_name, to_table_name):
            raise BackendApiException('Target table already exists, cannot rename table to: %s.%s' % (to_db_name, to_table_name))
        ...
        if fs_scheme == OFFLOAD_FS_SCHEME_S3A and not self.transactional_tables_default():
            self._log('Renaming S3 based table using external table management: %s.%s' % (from_db_name, from_table_name), detail=VERBOSE)
    ...
    def set_column_stats(self, db_name, table_name, new_column_stats, ndv_cap, num_null_factor):
        if not self.table_stats_get_supported():
            return
        hive_stats = self._get_hive_stats_table(db_name, table_name)
        hive_stats.set_column_stats(new_column_stats, self._backend_type, ndv_cap=ndv_cap,
                                    num_null_factor=num_null_factor, dry_run=self._dry_run, messages=self._messages)

    def set_partition_stats(self, db_name, table_name, new_partition_stats, additive_stats):
        if not self.table_stats_get_supported():
            return
        hive_stats = self._get_hive_stats_table(db_name, table_name)
        hive_stats.set_partition_stats(new_partition_stats, self._backend_type, additive=additive_stats,
                                       dry_run=self._dry_run, messages=self._messages)
    ...
    def set_table_stats(self, db_name, table_name, new_table_stats, additive_stats):
        if not self.table_stats_get_supported():
            return
        hive_stats = self._get_hive_stats_table(db_name, table_name)
        hive_stats.set_table_stats(new_table_stats, self._backend_type, additive=additive_stats,
                                   dry_run=self._dry_run, messages=self._messages)

    def supported_date_based_partition_granularities(self):
        return [PART_COL_GRANULARITY_YEAR, PART_COL_GRANULARITY_MONTH, PART_COL_GRANULARITY_DAY]
    ...
    def udf_installation_sql(self, create_udf_db, udf_db=None):
        ...
        if udf_db and not self.database_exists(udf_db):
            if create_udf_db:
                ...
                raise BackendApiException('Database: %s does not exist. Specify --create-backend-db flag to create'
                                          % udf_db)
    ...
    def udf_installation_test(self, udf_db=None):
        ...
        try:
            for sql in self._udf_test_sql(udf_db=udf_db):
                cmds.append(sql)
                row = self.execute_query_fetch_one(sql, log_level=VVERBOSE, not_when_dry_running=True)
                if row:
    ...
    def from_canonical_column(self, column, decimal_padding_digits=0):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, safe_mapping=None):
            ...
            """
            safe_mapping = is_safe_mapping(col.safe_mapping, safe_mapping)
            return HadoopColumn(col.name, data_type=data_type, data_length=data_length, data_precision=data_precision,
        ...
        if column.data_type == GLUENT_TYPE_FIXED_STRING:
            ...
            if column.data_precision is None and column.data_scale is None:
                ...
                new_precision, new_scale = self._align_decimal_scale_to_udfs(column.name, column.data_precision,
                                                                             column.data_scale)
                new_precision, new_scale = self._apply_decimal_padding_digits(column.name, new_precision, new_scale,
                ...
                if self._unsupported_decimal_precision_scale(new_precision, new_scale):
                    raise NotImplementedError('Unsupported precision/scale for %s column %s: %s/%s' \
                ...
                if new_precision != column.data_precision or new_scale != column.data_scale:
                    self._log('UDF aligned/padded precision/scale for %s: %s,%s -> %s,%s'
                              % (column.name, column.data_precision, column.data_scale, new_precision, new_scale),
                              detail=VERBOSE)
