...
class AddBackendColumn(SchemaSyncStep):
    ...
    def __init__(self, options, orchestration_options, messages, execution_id, repo_client):
        ...
        super().__init__(schema_sync_constants.ADD_BACKEND_COLUMN, options,
                         orchestration_options, messages, execution_id, repo_client)
    ...
    def _do_run(self, run_params):
        ...
        log('Adding columns to %s.%s table' % (self._backend_api.enclose_identifier(backend_owner),
        ...
        for rdbms_column in run_params['columns']:
            ...
            log('Mapped column %s to canonical column: %s' \
                % (rdbms_column.name.upper(), str(canonical_column)), detail=VVERBOSE)
            backend_column = self._backend_api.from_canonical_column(canonical_column)
            data_type = backend_column.format_data_type()
            log('Canonical to backend mapping: %s->%s' % (canonical_column.data_type, data_type), detail=VVERBOSE)
        ...
        try:
            ...
            if self._orchestration_options.backend_distribution == BACKEND_DISTRO_GCP:
                ...
                sqls.insert(-1, 'Missing columns should be added via the API or the BigQuery console. The API call is shown below:\n')
            return None, sqls

        except Exception as exc:
            source_name = '%s.%s' % (double_quote_sandwich(rdbms_owner.upper()),
                                     double_quote_sandwich(rdbms_table_name.upper()))
            log('%s\n%s' % (schema_sync_constants.EXCEPTION_ADD_BACKEND_COLUMN, traceback.format_exc()), verbose)
            self._messages.warning('%s for table %s'
                                   % (schema_sync_constants.EXCEPTION_ADD_BACKEND_COLUMN, source_name), ansi_code='red')
