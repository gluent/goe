...
from gluentlib.offload.bigquery.bigquery_column import BIGQUERY_TYPE_DATETIME,\
    BIGQUERY_TYPE_INT64, BIGQUERY_TYPE_TIMESTAMP
from gluentlib.offload.column_metadata import ColumnMetadataInterface, ColumnPartitionInfo, get_partition_columns
...
class TestBackendApi(TestCase):
    ...
    def setUp(self):
        ...
        self.api = backend_api_factory(self.target, self.config, messages, dry_run=True,
    ...
    def _test_add_columns(self):
        if self.connect_to_backend:
            column_list = [self.api.gen_default_numeric_column('col1'),
    ...
    def _test_create_database(self):
        if self.connect_to_backend:
            try:
                ...
                self.assertIsInstance(self.api.create_database(self.db, properties={'location': '/some/place',
                                                                                    'transient': True}), list)
            except NotImplementedError:
                pass

    def _test_create_table(self):
        column_list = [self.api.gen_column_object('col1', data_type=self.test_api.backend_test_type_canonical_int_8()),
                       self.api.gen_default_numeric_column('col2'),
                       self._gen_string_column('col3')]
        self.assertIsInstance(self.api.create_table(self.db, self.table, column_list, None,
                                                    FILE_STORAGE_FORMAT_PARQUET, location='/some/location',
                                                    table_properties={'some_property': 'some_value'}), list)
        try:
            self.assertIsInstance(self.api.create_table(self.db, self.table, column_list, None,
                                                        FILE_STORAGE_FORMAT_AVRO, location='/some/location',
                                                        external=True,
                                                        table_properties={'some_property': 'some_value'}), list)
        except NotImplementedError:
            pass
        ...
        if self.api.partition_by_column_supported():
            if self.target == DBTYPE_BIGQUERY:
                ...
                partition_column = self.api.gen_column_object('part_col',
                                                              data_type=self.test_api.backend_test_type_canonical_int_8(),
                                                              partition_info=partition_info)
            else:
                partition_info = ColumnPartitionInfo(position=0, granularity=10, range_start=0,
                                                     range_end=1000,digits=6)
                partition_column = self.api.gen_column_object('gl_part_000010_col1',
                                                              data_type=self.test_api.backend_test_type_canonical_string(),
                                                              partition_info=partition_info)
            part_col_list = [partition_column.name]
            column_list.append(partition_column)
            self.assertIsInstance(self.api.create_table(self.db, self.table, column_list, part_col_list,
                                                        FILE_STORAGE_FORMAT_PARQUET, sort_column_names=['col2']), list)

    def _test_create_view(self):
        column_list = [('col1', 'col21_alias'),
                       ('col2', 'col2_alias')]
        filter_clause = 'other_col < %s' % self.api.to_backend_literal(456)
        cmds = self.api.create_view(self.db, 'some_view', column_list, self.db + '.' + self.table,
    ...
    def _test_derive_native_partition_info(self):
        if self.connect_to_backend:
            part_cols = get_partition_columns(self.api.get_partition_columns(self.db, self.part_table),
    ...
    def _test_detect_column_has_fractional_seconds(self):
        if self.connect_to_backend:
            if self.target == DBTYPE_BIGQUERY:
                ts_column = [_ for _ in self.api.get_columns(self.db, self.table)
    ...
    def _test_execute_ddl(self):
        ...
        self.assertIsInstance(self.api.execute_ddl('DDL ON A TABLE', sync=True, profile=True,
        ...
        if self.connect_to_backend:
            ...
            if self.api.parameterized_queries_supported() and num_columns:
                if self.api.backend_type() == DBTYPE_BIGQUERY:
                    ...
                    where_clause = 'WHERE %s < %s' % (column_name, self.api.format_query_parameter('num_value'))
                elif self.api.backend_type() == DBTYPE_SNOWFLAKE:
                    params = [42]
                    where_clause = 'WHERE %s < :1' % column_name
                elif self.api.backend_type() == DBTYPE_SYNAPSE:
                    params = [42]
                    where_clause = 'WHERE %s < %s' % (column_name, self.api.format_query_parameter('not-applicable'))
                else:
                ...
                sql = self.test_api.unit_test_single_row_sql_text(
                    self.db, self.table, column_name, row_limit=10,
    ...
    def _test_execute_query_fetch_one(self):
        if self.connect_to_backend:
            num_columns = [_ for _ in self.api.get_columns(self.db, self.table) if _.is_number_based()]
            column_name = num_columns[0].name if num_columns else '*'
            row = self.api.execute_query_fetch_one(self._test_execute_query_text(column_name, 1),
                                                   time_sql=True)
            if row:
                self.assertIsInstance(row, (list, tuple))
            row = self.api.execute_query_fetch_one(self._test_execute_query_text(column_name, 1),
                                                   time_sql=True, as_dict=True)
            if row:
            ...
            if self.api.parameterized_queries_supported() and num_columns:
                if self.api.backend_type() == DBTYPE_BIGQUERY:
                    ...
                    raise NotImplementedError('Missing a test for implementation: %s' % self.api.backend_type())
                sql = self.test_api.unit_test_single_row_sql_text(
                    self.db, self.table, column_name, row_limit=1,
                    where_clause=where_clause
                )
    ...
    def _test_extract_date_part_sql_expression(self):
        self.assertRaises(AssertionError, lambda: self.api.extract_date_part_sql_expression('x', 'col_name'))
        self.assertRaises(AssertionError, lambda: self.api.extract_date_part_sql_expression('YEAR', 123))
        for date_part in ['YEAR', 'MONTH', 'YEAR']:
            self.assertIsInstance(self.api.extract_date_part_sql_expression(date_part, 'col_name'), str)
            self.assertIsInstance(self.api.extract_date_part_sql_expression(date_part.lower(), 'col_name'), str)
    ...
    def _test_format_column_comparison(self):
        ...
        self.assertIsInstance(self.api.format_column_comparison(col1, '=', col1, left_alias='a', right_alias='b'), str)
        col2 = self._gen_string_column('str_col')
        self.assertIsInstance(self.api.format_column_comparison(col2, '=', col2), str)
        self.assertIsInstance(self.api.format_column_comparison(col2, '=', col2, left_alias='a', right_alias='b'), str)

    def _test_gen_insert_select_sql_text(self):
        column_tuples = [('123', 'a_number'),
                         ("'blah'", 'a_string')]
        self.assertIsNotNone(self.api.gen_insert_select_sql_text(self.db, self.table, self.db, 'some_other_table',
                                                                 column_tuples, filter_clauses=['col_1 < 12345']))

    def _test_gen_ctas_sql_text(self):
        column_tuples = [(123, 'a_number'),
                         ("'blah'", 'a_string')]
        self.assertIsNotNone(self.api.gen_ctas_sql_text(self.db, self.table, FILE_STORAGE_FORMAT_PARQUET, column_tuples,
                                                        from_db_name=self.db, from_table_name='other_table', row_limit=2))
        if self.api.backend_type() == DBTYPE_IMPALA:
            self.assertIsNotNone(
                self.api.gen_ctas_sql_text(self.db, self.table, FILE_STORAGE_FORMAT_PARQUET, column_tuples,
                                           from_db_name=self.db, from_table_name='other_table', row_limit=2,
                                           external=True, table_properties={'Gluent': 'rocks'}))
    ...
    def _test_gen_native_range_partition_key_cast(self):
        def test_granularity_query(source_col, partition_info):
            ...
            row = self.api.execute_query_fetch_one('SELECT %s FROM %s.%s LIMIT 1'
                                                   % (cast_expr, self.db, self.table))
            ...
            if self.connect_to_backend:
                ...
                if self.api.backend_type() == DBTYPE_BIGQUERY:
                    ...
                    if int_column:
                        ...
                        test_granularity_query(int_column, ColumnPartitionInfo(position=0, granularity=10,
    ...
    def _test_gen_sql_text(self):
        ...
        self.assertIn(self.table.lower(), self.api.gen_sql_text(self.db, self.table, column_names=column_names,
                                                                filter_clauses=filter_clauses).lower())
        self.assertIn('col2', self.api.gen_sql_text(self.db, self.table, column_names=column_names,
                                                    measures=['col2', 'col3'], agg_fns=['min', 'max', 'sum']))
        self.assertIn('col2', self.api.gen_sql_text(self.db, self.table,
                                                    measures=['col2', 'col3'], agg_fns=['min', 'max', 'sum']))
    ...
    def _test_get_distinct_column_values(self):
        if self.connect_to_backend:
            ...
            self.assertIsInstance(self.api.get_distinct_column_values(self.db, self.table, two_col_names,
    ...
    def _test_get_max_column_values(self):
        if self.connect_to_backend:
            two_col_names = self.api.get_column_names(self.db, self.table)[:2]
            one_col_name = two_col_names[0]
            self.assertIsInstance(self.api.get_max_column_values(self.db, self.table, [one_col_name]), tuple)
            self.assertIsInstance(self.api.get_max_column_values(self.db, self.table, [one_col_name],
                                                                 optimistic_prune_clause='1 = 2'), tuple)
            self.assertIsInstance(self.api.get_max_column_values(self.db, self.table, two_col_names,
                                                                 columns_to_cast_to_string=[one_col_name],
                                                                 optimistic_prune_clause='1 = 2'), tuple)
    ...
    def _test_insert_literal_values(self):
        ...
        column_list = [
            ...
            self.api.gen_column_object('col2', data_type=self.test_api.backend_test_type_canonical_timestamp()),
            self.api.gen_column_object('col3', data_type=self.test_api.backend_test_type_canonical_string())
        ]
        literal_list = [[1, '2019-12-01', 'row 1'],
                        [2, '2019-12-02', 'row 2'],
                        [3, '2019-12-03', 'row 3'],
                        [4, None, 'row 4'],
                        [5, '2019-12-05', None]]
        if self.connect_to_backend:
            self.assertIsInstance(self.api.insert_literal_values(self.db, self.table, literal_list,
                                                                 column_list=column_list, max_rows_per_insert=3), list)
            self.assertIsInstance(self.api.insert_literal_values(self.db, self.table, literal_list,
                                                                 column_list=column_list, split_by_cr=False), list)
    ...
    def _test_length_sql_expression(self):
        for literal, expected_length in [('ABCDEF', 6), ('', 0)]:
            ...
            if self.connect_to_backend:
                self.assertEqual(self.api.execute_query_fetch_one('SELECT %s' % length_expr)[0], expected_length)
    ...
    def _test_list_databases(self):
        if self.connect_to_backend:
            ...
            self.assertIsInstance(self.api.list_databases(db_name_filter=self.db, case_sensitive=False), list)
    ...
    def _test_list_tables(self):
        if self.connect_to_backend:
            ...
            self.assertIsInstance(self.api.list_tables(self.db, table_name_filter=self.table, case_sensitive=False),
                                  list)
    ...
    def _test_list_udfs(self):
        if self.connect_to_backend:
            ...
            if all_udfs_in_db:
                ...
                self.assertIsInstance(self.api.list_udfs(self._get_udf_db(), udf_name_filter=row[0],
    ...
    def _test_regexp_extract_decimal_scale_pattern(self):
        ...
        for test_str, expected_scale_length in [
            ...
            self.assertEqual(len(re.match(pattern, test_str).group(1)), expected_scale_length,
                             f'Source string {test_str}')

    def _test_regexp_extract_sql_expression(self):
        regexp_expr = self.api.regexp_extract_sql_expression(self.api.to_backend_literal('123.456'),
                                                             self.api.regexp_extract_decimal_scale_pattern())
        self.assertIsNotNone(regexp_expr)
        if self.connect_to_backend:
            # Test numbers along with the expected scale
            test_pairs = [('123', None), ('-123', None), ('', None), ('0', None),
                          ('123.123E5', None), ('-123.123E5', None),
                          ('123.001', '001'), ('-123.001', '001'), ('0.1', '1'), ('-0.1', '1')]
            for test_str, expected_scale in test_pairs:
                regexp_expr = self.api.regexp_extract_sql_expression(self.api.to_backend_literal(test_str),
                                                                     self.api.regexp_extract_decimal_scale_pattern())
    ...
    def _test_rename_table(self):
        if self.connect_to_backend:
            self.assertIsInstance(self.api.rename_table(self.db, self.table, 'other_db', 'other_table'), list)
    ...
    def _test_snowflake_file_format_exists(self):
        if self.connect_to_backend:
            self.assertFalse(self.api.snowflake_file_format_exists(self.db, 'no-ffs-were-given'))
            file_format = add_suffix_in_same_case(self.config.snowflake_file_format_prefix,
    ...
    def _test_snowflake_stage_exists(self):
        if self.connect_to_backend:
            ...
            self.assertIsInstance(self.api.snowflake_stage_exists(self.db, self.config.snowflake_stage), bool)
    ...
    def _test_to_backend_literal(self):
        ...
        literal = self.api.to_backend_literal(int(123456), self.test_api.backend_test_type_canonical_int_8())
        self.assertIn('123456', str(literal))
        test_by_select(literal)
        literal = self.api.to_backend_literal(float(1.23), self.test_api.backend_test_type_canonical_decimal())
        self.assertIn('1.23', str(literal))
        test_by_select(literal)
        literal = self.api.to_backend_literal(int(12345678901234567), self.test_api.backend_test_type_canonical_decimal())
        ...
        test_by_select(literal)
        literal = self.api.to_backend_literal(datetime64(datetime.now()),
