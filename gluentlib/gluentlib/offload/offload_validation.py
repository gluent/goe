...
from gluentlib.util.misc_functions import is_number, isclose, str_floatlike
...
from gluentlib.offload.factory.frontend_api_factory import frontend_api_factory, frontend_api_factory_ctx
...
from gluentlib.offload.offload_functions import expand_columns_csv, hybrid_view_combine_hv_and_pred_clauses,\
    get_hybrid_predicate_clauses, get_hybrid_threshold_clauses, hvs_to_backend_sql_literals
from gluentlib.offload.offload_metadata_functions import decode_metadata_incremental_high_values_from_metadata
from gluentlib.offload.offload_messages import OffloadMessagesMixin, VERBOSE, VVERBOSE
from gluentlib.persistence.orchestration_metadata import OrchestrationMetadata, \
    INCREMENTAL_PREDICATE_TYPE_LIST, INCREMENTAL_PREDICATE_TYPE_RANGE, INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE
...
class CrossDbValidatorException(Exception): pass
...
def build_verification_clauses(offload_source_table, ipa_predicate_type, verification_hvs, prior_hvs,
                               offload_predicate=None, with_binds=True, backend_table=None):
    ...
    def get_bind_and_literal(col_hv, bind_counter, bind_pattern, data_type):
        ...
        if backend_table:
            ...
            where_clause_expr = str(where_clause_expr) if isinstance(where_clause_expr,
                                                                     (int, float)) else where_clause_expr
        else:
    ...
    if offload_predicate:
        if backend_table:
            ...
            if with_binds:
                where_clause, binds = offload_source_table.predicate_to_where_clause_with_binds(offload_predicate)
    ...
    if rdbms_part_cols and verification_hvs:
        ...
        if prior_literals:
            _, prior_gte_clause = get_hybrid_threshold_clauses(rdbms_part_cols, rdbms_part_cols,
                                                               ipa_predicate_type, prior_literals,
                                                               col_enclosure_fn=enclosure_fn)
            threshold_clauses.append(prior_gte_clause)
        if new_literals:
            new_lt_clause, _ = get_hybrid_threshold_clauses(rdbms_part_cols, rdbms_part_cols,
                                                            ipa_predicate_type, new_literals,
                                                            col_enclosure_fn=enclosure_fn)
...
class CrossDbValidator(OffloadMessagesMixin, object):
    ...
    def __init__(self, db_name, table_name, connection_options, backend_obj=None, messages=None,
                 backend_db=None, backend_table=None):
        """ CONSTRUCTOR
        ...
        self._frontend = frontend_api_factory(connection_options.db_type, connection_options, messages,
                                              conn_user_override=connection_options.rdbms_app_user,
                                              trace_action=self.__class__.__name__)
        if backend_obj:
            self._backend = backend_obj
        else:
            self._backend = backend_api_factory(connection_options.target, connection_options,
                                                messages, dry_run=bool(not connection_options.execute))
        self._connection_options = connection_options
        self._messages = messages

        if not backend_db and not backend_table:
            self._offload_metadata = OrchestrationMetadata.from_name("%s_H" % self._db_name.upper(),
    ...
    def _get_frontend_table(self):
        if not self._frontend_table:
            ...
            if self._offload_metadata:
                self._frontend_table = OffloadSourceTable.create(self._offload_metadata.offloaded_owner,
                                                                 self._offload_metadata.offloaded_table,
                                                                 self._connection_options, self._messages)
            else:
                self._frontend_table = OffloadSourceTable.create(self._db_name, self._table_name,
                                                                 self._connection_options, self._messages)
    ...
    def _get_sample_cols(self, required_no):
        ...
        """
        return self._frontend.agg_validate_sample_column_names(self._db_name.upper(), self._table_name.upper(),
                                                               num_required=required_no)
    ...
    def _get_group_bys(self, group_bys):
        ...
        if not group_bys:
            ...
            group_bys = expand_columns_csv(','.join(group_bys), frontend_table.columns,
    ...
    def _get_select_cols(self, select_cols, aggs):
        ...
        if is_number(select_cols):
            ...
            frontend_table = self._get_frontend_table()
            select_cols = expand_columns_csv(','.join(select_cols), frontend_table.columns,
                                             retain_non_matching_names=True)
        else:
    ...
    def _construct_simple_agg_sql(self, selects, filters, group_bys, aggs, as_of_scn=None,
                                  db_name=None, table_name=None, frontend_hint_block=None):
    ...
    def _construct_simple_front_agg_sql(self, selects, filters, group_bys, aggs, as_of_scn, frontend_hint_block):
        ...
        return self._construct_simple_agg_sql(selects, self._adjust_filters_front(filters), group_bys, aggs,
                                              as_of_scn=as_of_scn, frontend_hint_block=frontend_hint_block)
    ...
    def _construct_simple_back_agg_sql(self, selects, filters, group_bys, aggs):
        ...
        return self._backend.gen_sql_text(self._backend_db, self._backend_table, column_names=group_bys,
                                          filter_clauses=backend_filters, measures=selects, agg_fns=aggs)
    ...
    def _run_sqls(self, front_sql, back_sql, front_binds=None):
        ...
        self._pexec.execute_in_threads(2,
    ...
    def _compare_results(self, results, front_selects, group_bys):
        ...
        def merge_front_end_results(front_results, back_results):
            ...
            for row_no, _ in enumerate(front_results):
                ...
                for col_no in range(no_columns):
                    row_results.append((
                        row_pk(front_results[row_no]), col_name(col_no),
                        front_results[row_no][col_no], back_results[row_no][col_no]
                    ))
        ...
        def process_columns(column):
            ...
            if not matched:
                self.warn("Results for [%s]: %s do NOT match - FRONT: %s vs BACK: %s" % \
                    (row_pk, col_name, str_floatlike(front), str_floatlike(back)))
                return False
            else:
                logger.debug("Results for [%s]: %s match - FRONT: %s vs BACK: %s" % \
        ...
        def cols(results):
            return 0 if not results or not isinstance(results, list) or len(results) == 0 \
                or not isinstance(results[0], (list, tuple)) else len(results[0])
        ...
        if [[], []] == results:
            ...
            logger.warning("Result (row) dimensions do not match: FRONT: %d BACK: %d" % (rows(front_results), rows(back_results)))
            return False
        elif cols(front_results) != cols(back_results):
            logger.warning("Result (column) dimensions do not match: FRONT: %d BACK: %d" % (cols(front_results), cols(back_results)))
            return False
        else:
    ...
    def _extract_offload_boundary(self, frontend=False):
        ...
        if frontend:
            ...
            backend_table = backend_table_factory(self._offload_metadata.backend_owner,
                                                  self._offload_metadata.backend_table,
                                                  self._backend.backend_type(), self._connection_options,
                                                  self._messages, hybrid_metadata=self._offload_metadata,
                                                  existing_backend_api=self._backend)
        inc_keys, hv_real_vals, hv_indiv_vals = decode_metadata_incremental_high_values_from_metadata(
        ...
        if frontend:
            ...
            lt_threshold_vals = hvs_to_backend_sql_literals(backend_threshold_cols, hv_real_vals, inc_predicate_type,
                                                            self._backend.to_backend_literal)
            enclosure_fn = self._backend.enclose_identifier
        lt_clause, _ = get_hybrid_threshold_clauses(inc_keys, inc_keys, inc_predicate_type, lt_threshold_vals,
    ...
    def validate(self, selects=DEFAULT_SELECT_COLS, filters=None, group_bys=None, aggs=DEFAULT_AGGS,
                 safe=True, as_of_scn=None, execute=True, frontend_filters=None, frontend_query_params=None,
        ...
        logger.debug("Parameters: SELECT: %s, WHERE: %s, GROUP BY: %s AS OF: %s"
        ...
        self._frontend_sql, front_selects = self._construct_simple_front_agg_sql(selects, frontend_filters, group_bys,
                                                                                 aggs, as_of_scn,
                                                                                 frontend_hint_block)
...
class HybridCountValidator(object):
    ...
    def __init__(self, table_owner, table_name, hybrid_schema, hybrid_view,
                 connection_options, messages=None, dry_run=False):
        """ CONSTRUCTOR
    ...
    def validate(self, frontend_filters, frontend_query_params, frontend_hint_block=None):
        def crlfping(filter_str):
            """ crlfping - for those who've seen a typewriter """
            return '\n                  %s' % filter_str

        if frontend_filters:
            assert isinstance(frontend_filters, list), '{} is not list'.format(type(frontend_query_params))
        if frontend_query_params:
            assert isinstance(frontend_query_params, list), '{} is not list'.format(type(frontend_query_params))

        threshold_clause = ''
        if frontend_filters:
            threshold_clause = crlfping('WHERE %s') % crlfping('AND   ').join(frontend_filters)

        hint_clause = f' {frontend_hint_block}' if frontend_hint_block else ''
        params = {'owner': self._table_owner,
        ...
        FROM hybrid_rows, origin_rows""") % params

        with frontend_api_factory_ctx(self._connection_options.db_type, self._connection_options, self._messages,
                                      conn_user_override=self._connection_options.rdbms_app_user,
                                      trace_action=self.__class__.__name__) as frontend_api:
            results = frontend_api.execute_query_fetch_one(
                q, query_params=frontend_query_params, log_level=VERBOSE, not_when_dry_running=True
            )
...
class BackendCountValidator(object):
    ...
    def validate(self, frontend_filters, frontend_query_params, backend_filters, frontend_hint_block=None):
        ...
        frontend_count = frontend_api.get_table_row_count(self._frontend_table.owner, self._frontend_table.table_name,
                                                          filter_clause=frontend_filter,
                                                          filter_clause_params=frontend_query_params,
                                                          hint_block=frontend_hint_block,
                                                          not_when_dry_running=True, log_level=VERBOSE)
        backend_count = backend_api.get_table_row_count(self._backend_table.db_name, self._backend_table.table_name,
                                                        filter_clause=backend_filter, not_when_dry_running=True,
                                                        log_level=VERBOSE)
        if self._dry_run:
            return 0, 0, 0
        else:
