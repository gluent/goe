...
def connector_sql_engine_supports_nanoseconds():
    ...
    return bool(
        ...
        in (DBTYPE_IMPALA, DBTYPE_HIVE, DBTYPE_SNOWFLAKE)
    )


def get_offload_test_fn(
    base_schema,
    table_name,
    frontend_api,
    backend_api,
    ...
    test_messages,
    run_offload=True,
    offload_modifiers=None,
    config_modifiers=None,
    create_backend_db=True,
    verification_mode=False,
    with_assertions=False,
):
    desc_prefix = "Table %soffload %s" % (
    ...
    def test_fn():
        ...
        if run_offload:
            ...
            if (
                orchestration_config.target == DBTYPE_IMPALA
                and table_name.lower()
                in ("gl_range_large_number_pos", "gl_range_large_number_neg")
                and not verification_mode
            ):
                if StrictVersion(backend_api.target_version()) >= StrictVersion(
                    "3.0.0"
                ):
                    test_messages.log(
                        "Skipping offload test of %s on Impala version %s (GOE-1542)"
            ...
            (
                offload_options["offload_partition_columns"],
                offload_options["offload_partition_granularity"],
                offload_options["offload_partition_lower_value"],
                offload_options["offload_partition_upper_value"],
            ) = get_suitable_offload_granularity_options(
                base_schema, table_name, orchestration_config, test_messages
            )
            offload_options[
                "partition_names_csv"
            ...
            ] = setup_constants.OFFLOAD_CUSTOM_DIGITS.get(table_name, "15")
            ...
            if table_name in ["gl_clobs", "gl_blobs"]:
                ...
                "gl_exotic_chars",
                "gl_exotic_chars_hash",
                "gl_range_exotic_chars",
            ]:
            ...
            offload_messages = OffloadMessages.from_options_dict(
                offload_options,
                log_fh=get_log_fh(),
            ...
            try:
                OrchestrationRunner(
                    config_overrides=config_modifiers, dry_run=verification_mode
                ).offload(
                    offload_options, reuse_log=True, messages_override=offload_messages
                )
                if table_name in expected_failed_offloads:
                    # We expect this test to fail, therefore we should never get to here
                    raise TestOffloadException(
                        f"offload({table_name}) should have thrown an exception and didn't"
                    )
            except Exception as exc:
                if table_name in expected_failed_offloads and expected_failed_offloads[
                    table_name
                ] in str(exc):
        ...
        if run_assertions:
            ...
            assert not text_in_warnings(test_messages, MISSING_ROWS_IMPORTED_WARNING), (
                desc_prefix + " (MISSING_ROWS_IMPORTED_WARNING)"
            )
            assert not text_in_warnings(test_messages, MISSING_ROWS_SPARK_WARNING), (
...
def get_suitable_offload_granularity_options(owner, table_name, config, messages):
    offload_source_table = OffloadSourceTable.create(
        owner, table_name, config, messages
    )
    if not offload_source_table.columns:
        raise TestOffloadException(
            f"Offload source table does not exist: {owner}.{table_name}"
        )
    ...
    if (
        len(offload_source_table.partition_columns or []) == 1
        and offload_source_table.partition_columns[0].is_date_based()
    ):
    ...
    for i, part_col in enumerate(offload_source_table.partition_columns or []):
        if part_col.is_date_based():
            gran_list.append(
                ...
                else PART_COL_GRANULARITY_MONTH
            )
            ...
            if part_col.name.upper() in ["YEAR", "MONTH"]:
                gran_list.append("1")
                if (
                    offload_partition_upper_value is None
                    or offload_partition_upper_value < 10000
                ):
                    offload_partition_upper_value = 10000
            elif offload_source_table.partition_type == "LIST":
                gran_list.append("1000")
                partitions_hvs = flatten_lpa_individual_high_values(
                    [
                ...
                if (
                    offload_partition_lower_value is None
                    or offload_partition_lower_value > min_partitions_hvs - 1000
                ):
                ...
                granularity_magnitude = math.floor(
                    math.log10(max(math.floor(max_partitions_hvs / 1000), 100))
                )
                gran_list.append(str(10**granularity_magnitude))
            else:
                gran_list.append("1000")
                if offload_partition_upper_value:
                    offload_partition_upper_value = max(
...
def get_suitable_offload_partition_name(owner, table_name, config, messages):
    ...
    if not offload_source_table.columns:
        raise TestOffloadException(
            "Offload source table does not exist: %s.%s" % (owner, table_name)
        )
    if offload_source_table.offload_by_subpartition_capable(valid_for_auto_enable=True):
        offload_source_table.enable_offload_by_subpartition()
    ltv_operation = OffloadOperation.from_dict(
        {"owner_table": "%s.%s" % (owner, table_name), "reset_backend_table": True},
        config,
        messages,
    )
    ...
    if (
        ...
        raise NotImplementedError(
            "get_suitable_partition_name does not support predicate type: %s"
            % source_data_client.get_partition_append_predicate_type()
        )
    messages.log(
        "get_suitable_partition_name: --partition_names %s" % partition_csv,
        detail=VERBOSE,
    )
