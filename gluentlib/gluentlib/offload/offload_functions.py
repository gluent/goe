...
from gluentlib.offload.predicate_offload import GenericPredicate, create_or_relation_predicate
from gluentlib.persistence.orchestration_metadata import INCREMENTAL_PREDICATE_TYPE_LIST,\
    INCREMENTAL_PREDICATE_TYPE_RANGE, INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE, \
    INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE_AND_PREDICATE, \
    INCREMENTAL_PREDICATE_TYPE_RANGE_AND_PREDICATE
from gluentlib.util.misc_functions import backtick_sandwich, case_insensitive_in, csv_split, substitute_in_same_case,\
...
def hvs_to_backend_sql_literals(threshold_cols, threshold_values, ipa_predicate_type, to_literal_fn):
    ...
    assert inspect.isfunction(to_literal_fn) or inspect.ismethod(to_literal_fn), 'Invalid function type %s'\
                                                                                 % type(to_literal_fn)
    ...
    if ipa_predicate_type == INCREMENTAL_PREDICATE_TYPE_LIST:
        ...
        new_threshold_values = tuple([to_literal_fn(val, col.data_type)
                                      for col, val in zip(threshold_cols, threshold_values)])
    return new_threshold_values


def hybrid_threshold_clauses(lower_threshold_cols, upper_threshold_cols, offload_high_values, equality_with_gt=True,
                             col_quoting_char='"', as_predicate_dsl=False, col_enclosure_fn=None):
    ...
    def format_predicate(col, op, hv):
        if as_predicate_dsl:
            return '(column(%s) %s %s(%s))' % (col.name, op, get_dsl_literal_name(col), format_dsl_hv(col, hv))
        else:
    ...
    def dsl_and_join(exp, join_term='AND'):
        ...
        if len(exp) > 1:
            return '({} {} {})'.format(dsl_and_join(exp[:-1], join_term=join_term), join_term, exp[-1])
        else:
            return exp[-1]
...
def hybrid_view_list_clauses(lower_threshold_cols, upper_threshold_cols, incremental_high_values,
                             col_quoting_char='"', in_list_chunk_size=1000, col_enclosure_fn=None):
    ...
    upper_threshold_clause = ' AND '.join('%s NOT IN (%s)' % (quote_fn(upper_threshold_cols[0].name.upper()), _)
    ...
    lower_threshold_clause = ' OR '.join('%s IN (%s)' % (quote_fn(lower_threshold_cols[0].name.upper()), _)
                                         for _ in sub_in_lists)
...
def get_hybrid_threshold_clauses(lower_cols, upper_cols, ipa_predicate_type,
                                 incremental_high_values, col_quoting_char='"', col_enclosure_fn=None):
    if ipa_predicate_type in (INCREMENTAL_PREDICATE_TYPE_RANGE, INCREMENTAL_PREDICATE_TYPE_RANGE_AND_PREDICATE):
        return hybrid_threshold_clauses(lower_cols, upper_cols, incremental_high_values,
                                        col_quoting_char=col_quoting_char, col_enclosure_fn=col_enclosure_fn)
    elif ipa_predicate_type == INCREMENTAL_PREDICATE_TYPE_LIST:
        return hybrid_view_list_clauses(lower_cols, upper_cols, incremental_high_values,
                                        col_quoting_char=col_quoting_char, col_enclosure_fn=col_enclosure_fn)
    elif ipa_predicate_type in (INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE,
                                INCREMENTAL_PREDICATE_TYPE_LIST_AS_RANGE_AND_PREDICATE):
        return hybrid_threshold_clauses(lower_cols, upper_cols, incremental_high_values, equality_with_gt=False,
                                        col_quoting_char=col_quoting_char, col_enclosure_fn=col_enclosure_fn)
    else:
        return '', ''
...
def get_dsl_threshold_clauses(cols, ipa_predicate_type, incremental_high_values, equality_with_gt=None):
    if ipa_predicate_type in (INCREMENTAL_PREDICATE_TYPE_RANGE,
        ...
        return hybrid_threshold_clauses(cols, cols, incremental_high_values, as_predicate_dsl=True,
                                        equality_with_gt=equality_with_gt)
    else:
        raise NotImplementedError('get_dsl_threshold_clauses() is not implemented for INCREMENTAL_PREDICATE_TYPE: %s'
                                  % ipa_predicate_type)
...
def get_hybrid_predicate_clauses(offload_predicates, table_object, columns_override=None):
    ...
    if offload_predicates:
        hybrid_predicate = table_object.predicate_to_where_clause(create_or_relation_predicate(offload_predicates),
...
def datetime_literal_to_python(dt_literal):
    ...
    if isinstance(dt_literal, datetime64):
        ...
        try:
            ...
            raise OffloadFunctionException('Failed to parse datetime value: %s (%s)'
