...
from gluentlib.offload.snowflake.snowflake_column import SnowflakeColumn,\
    SNOWFLAKE_TYPE_BINARY, SNOWFLAKE_TYPE_TIMESTAMP_NTZ, SNOWFLAKE_TYPE_TIME
...
from gluentlib.offload.backend_table import BackendTableInterface, BackendTableException
from gluentlib.offload.offload_constants import FILE_STORAGE_FORMAT_AVRO
from gluentlib.offload.offload_messages import VERBOSE, VVERBOSE
from gluentlib.filesystem.gluent_dfs import AZURE_OFFLOAD_FS_SCHEMES, OFFLOAD_FS_SCHEME_S3A, OFFLOAD_FS_SCHEME_GS,\
...
from gluentlib.filesystem.gluent_azure import azure_fq_container_name
...
class BackendSnowflakeTable(BackendTableInterface):
    ...
    def __init__(self, db_name, table_name, backend_type, orchestration_options, messages, orchestration_operation=None,
        ...
        """ CONSTRUCTOR
        """
        super(BackendSnowflakeTable, self).__init__(db_name, table_name, backend_type, orchestration_options, messages,
        ...
        self._snowflake_file_format = add_suffix_in_same_case(orchestration_options.snowflake_file_format_prefix,
    ...
    def _final_insert_format_sql(self, select_expr_tuples, filter_clauses, sort_expr_list, for_materialized_join=False):
        ...
        if for_materialized_join:
            ...
            return self._db_api.gen_insert_select_sql_text(self.db_name, self._base_table_name,
                                                           self._load_view_db_name, self._load_table_name,
            ...
            return self._db_api.gen_copy_into_sql_text(self.db_name, self._base_table_name,
    ...
    def _format_staging_object_name(self):
        return "'@%(stage)s/%(table)s/' (FILE_FORMAT=>%(file_format)s, PATTERN=>'.*\.%(file_ext)s')" % {
            'stage': self._db_api.enclose_object_reference(self._load_db_name, self._snowflake_stage),
            'table': self._load_table_name,
            'file_format': self._db_api.enclose_object_reference(self._load_db_name, self._snowflake_file_format),
            'file_ext': self._offload_staging_format.lower()
        }

    def _gen_create_file_format_sql_text(self, file_format_name, staging_format):
        null_if = " NULL_IF = 'null'" if staging_format == FILE_STORAGE_FORMAT_AVRO else ''
        return "CREATE FILE FORMAT %s TYPE = %s%s" % (self._db_api.enclose_object_reference(self.db_name,
                                                                                            file_format_name),
                                                      staging_format, null_if)

    def _gen_create_stage_sql_text(self):
        stage_url = self._get_dfs_client().gen_uri(self._offload_fs_scheme_override(),
                                                   self._orchestration_config.offload_fs_container,
                                                   self._orchestration_config.offload_fs_prefix,
        ...
        return dedent("""\
            ...
            % (self._db_api.enclose_object_reference(self.db_name, self._snowflake_stage),
               stage_url,
    ...
    def _gen_synthetic_part_number_granularity_sql_expr(self, column_name, backend_col, granularity,
    ...
    def _offload_fs_container_override(self):
        ...
        if self._orchestration_config.offload_fs_scheme in AZURE_OFFLOAD_FS_SCHEMES:
            ...
            container_override = azure_fq_container_name(self._orchestration_config.offload_fs_azure_account_name,
    ...
    def _offload_fs_scheme_override(self):
        ...
        mappings = {OFFLOAD_FS_SCHEME_GS: 'gcs',
    ...
    def _staging_column_is_not_null_sql_expr(self, column_name):
        ...
        if self._offload_staging_format == FILE_STORAGE_FORMAT_AVRO:
            ...
            return super(BackendSnowflakeTable, self)._staging_column_is_not_null_sql_expr(column_name)
    ...
    def _staging_to_backend_cast(self, rdbms_column, backend_column, staging_column) -> tuple:
        ...
        def cast_date(expr, data_type, safe_cast=False):
            if safe_cast:
                ...
                return "TRY_TO_{}(CAST({} AS TEXT),'YYYY-MM-DD')".format(data_type, expr)
            else:
        ...
        def cast_datetime(expr, data_type, safe_cast=False):
            if safe_cast:
                # TRY_TO_... only works with TEXT but all staging columns are VARIANT. Std CAST is fine.
                return "TRY_TO_{}(CAST({} AS TEXT),'YYYY-MM-DD HH24:MI:SS.FF')".format(data_type, expr)
            else:
                return 'TO_{}({})'.format(data_type, expr)
        ...
        def cast_time(expr, data_type, safe_cast=False):
            if safe_cast:
                # TRY_TO_... only works with TEXT but all staging columns are VARIANT. Std CAST is fine.
                return "TRY_TO_{}(CAST({} AS TEXT),'HH24:MI:SS.FF')".format(data_type, expr)
            else:
                return 'TO_{}({})'.format(data_type, expr)
        ...
        def cast_tz(expr, data_type, safe_cast=False):
            if safe_cast:
                # TRY_TO_... only works with TEXT but all staging columns are VARIANT. Std CAST is fine.
                return "TRY_TO_{}(CAST({} AS TEXT),'YYYY-MM-DD HH24:MI:SS.FF TZH:TZM')".format(data_type, expr)
            else:
                return 'TO_{}({})'.format(data_type, expr)
        ...
        if isinstance(staging_column, SnowflakeColumn) and staging_column.data_type == backend_column.data_type:
            ...
            return_cast = cast(self._format_staging_column_name(staging_column),
                               backend_column.format_data_type().upper())
            return_vcast = cast(self._format_staging_column_name(staging_column),
                                backend_column.format_data_type().upper(), safe_cast=True)
        elif rdbms_column.is_time_zone_based():
            if not backend_column.is_string_based():
                # Time zoned so we do not normalise to UTC.
                return_cast = cast_tz(self._format_staging_column_name(staging_column), backend_column.data_type)
                return_vcast = cast_tz(self._format_staging_column_name(staging_column), backend_column.data_type,
                                       safe_cast=True)
            ...
            if backend_column.is_string_based():
                if rdbms_column.has_time_element():
                    ...
                    return_cast = cast_datetime(self._format_staging_column_name(staging_column),
                                                SNOWFLAKE_TYPE_TIMESTAMP_NTZ)
                    return_vcast = cast_datetime(self._format_staging_column_name(staging_column),
                                                 SNOWFLAKE_TYPE_TIMESTAMP_NTZ, safe_cast=True)
                    cast_staged_date_to_string_template = """CASE
                    WHEN {col} IS NOT NULL THEN
                        TO_CHAR({cast},'YYYY-MM-DD HH24:MI:SS.FF9')
                    END"""
                else:
                    return_cast = cast_date(self._format_staging_column_name(staging_column),
                                                SNOWFLAKE_TYPE_TIMESTAMP_NTZ)
                    return_vcast = cast_date(self._format_staging_column_name(staging_column),
                                                 SNOWFLAKE_TYPE_TIMESTAMP_NTZ, safe_cast=True)
                    cast_staged_date_to_string_template = """CASE
                    WHEN {col} IS NOT NULL THEN
                ...
                if rdbms_column.has_time_element():
                    ...
                    return_vcast = cast_datetime(self._format_staging_column_name(staging_column),
                                                 backend_column.data_type, safe_cast=True)
                else:
                    return_cast = cast_date(self._format_staging_column_name(staging_column),
                                                backend_column.data_type)
                    return_vcast = cast_date(self._format_staging_column_name(staging_column),
            ...
            return_cast = cast_time(self._format_staging_column_name(staging_column), backend_column.data_type)
            return_vcast = cast_time(self._format_staging_column_name(staging_column), backend_column.data_type,
                                     safe_cast=True)
    ...
    def create_backend_table(self):
        ...
        cmds = self._db_api.create_table(self.db_name, self.table_name, self.get_columns(),
                                         no_partition_columns, sort_column_names=self._sort_columns)
    ...
    def create_db(self):
        ...
        if self._db_api.snowflake_stage_exists(self.db_name, self._snowflake_stage):
            self._log('Snowflake stage already exists: %s.%s' % (self.db_name, self._snowflake_stage), detail=VERBOSE)
        else:
            sqls.append(self._gen_create_stage_sql_text())

        for staging_format in self._db_api.valid_staging_formats():
            file_format = add_suffix_in_same_case(self._orchestration_config.snowflake_file_format_prefix,
                                                  '_' + staging_format)
            if self._db_api.snowflake_file_format_exists(self.db_name, file_format):
                self._log('Snowflake file format already exists: %s.%s' % (self.db_name, file_format), detail=VERBOSE)
            else:
                sqls.append(self._gen_create_file_format_sql_text(file_format, staging_format))
    ...
    def load_materialized_join(self, threshold_cols=None, gte_threshold_vals=None, lt_threshold_vals=None,
        ...
        self._debug('Loading %s.%s from %s.%s' % (self.db_name, self._base_table_name,
                                                  self._load_view_db_name, self._load_table_name))
        select_expression_tuples = [(self.enclose_identifier(col.name), self.enclose_identifier(col.name))
    ...
    def predicate_has_rows(self, predicate):
        ...
        sql = "SELECT 1 FROM %s WHERE (%s) LIMIT 1" % (
            ...
            snowflake_predicate.predicate_to_where_clause(self.get_columns(), predicate)
        )
    ...
    def predicate_to_where_clause(self, predicate, columns_override=None):
        return snowflake_predicate.predicate_to_where_clause(columns_override or self.get_columns(), predicate)
    ...
    def staging_area_exists(self):
        ...
        if not self._db_api.snowflake_integration_exists(self._snowflake_integration):
            raise BackendTableException('Snowflake integration does not exist: %s' % self._snowflake_integration)
        status = True
        expected_stage_prefix = self._get_dfs_client().gen_uri(self._offload_fs_scheme_override(),
                                                               self._orchestration_config.offload_fs_container,
                                                               self._orchestration_config.offload_fs_prefix,
                                                               container_override=self._offload_fs_container_override())
        if not self._db_api.snowflake_stage_exists(self._load_db_name, self._snowflake_stage,
                                                   url_prefix=expected_stage_prefix):
            self._warning('Snowflake stage %s.%s does not exist' % (self._load_db_name, self._snowflake_stage))
            status = False
        if not self._db_api.snowflake_file_format_exists(self._load_db_name, self._snowflake_file_format,
                                                         format_type=self._offload_staging_format):
            self._warning('Snowflake file format %s.%s does not exist'
