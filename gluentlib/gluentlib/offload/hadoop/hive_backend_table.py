...
from gluentlib.offload.hadoop.hadoop_column import HADOOP_TYPE_BIGINT, HADOOP_TYPE_DECIMAL
from gluentlib.offload.hadoop.hadoop_backend_api import hive_enable_dynamic_partitions_for_insert_sqls
from gluentlib.offload.hadoop.hadoop_backend_table import BackendHadoopTable
from gluentlib.offload.offload_constants import FILE_STORAGE_FORMAT_ORC, FILE_STORAGE_COMPRESSION_CODEC_ZLIB, \
    OFFLOAD_STATS_METHOD_NATIVE, OFFLOAD_STATS_METHOD_NONE, OFFLOAD_STATS_METHOD_HISTORY
...
class BackendHiveTable(BackendHadoopTable):
    ...
    def _create_new_backend_table(self, sort_column_names=None):
        ...
        if self._orchestration_config.storage_format == FILE_STORAGE_FORMAT_ORC:
            table_properties = {'orc.compress': (self._user_requested_storage_compression
        ...
        return self._db_api.create_table(self.db_name, self.table_name, self.get_columns(), partition_column_names,
                                         ...
                                         location=location, table_properties=table_properties)
    ...
    def _final_insert_format_sql(self, select_expr_tuples, filter_clauses, sort_expr_list, for_materialized_join=False):
        ...
        return self._db_api.gen_insert_select_sql_text(self.db_name, self._base_table_name,
                                                       ...
                                                       filter_clauses=filter_clauses, sort_expr_list=sort_expr_list,
    ...
    def _final_insert_query_options(self):
        ...
        """
        query_options = hive_enable_dynamic_partitions_for_insert_sqls(max_dynamic_partitions=self._hive_max_dynamic_partitions,
        ...
        if self._offload_stats_method in [OFFLOAD_STATS_METHOD_NATIVE, OFFLOAD_STATS_METHOD_HISTORY] and not stats_autogather:
    ...
    def _gen_synthetic_part_number_granularity_sql_expr(self, column_expr, backend_col, granularity,
        ...
        to_synth_expr = 'CAST(FLOOR(CAST(%s AS %s) / %s) * %s AS STRING)' \
            % (column_expr, cast_type, granularity, granularity)
        if with_padding:
            return "LPAD(CAST(%s AS STRING), %s, '0')" % (to_synth_expr, synthetic_partition_digits)
        else:
