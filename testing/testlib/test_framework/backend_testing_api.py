...
from gluentlib.offload.column_metadata import CanonicalColumn, match_table_column,\
    GLUENT_TYPE_DATE, GLUENT_TYPE_INTEGER_1, GLUENT_TYPE_INTEGER_2, GLUENT_TYPE_INTEGER_4,\
    GLUENT_TYPE_INTEGER_8, GLUENT_TYPE_INTEGER_38, GLUENT_TYPE_VARIABLE_STRING
...
class BackendTestingApiInterface(metaclass=ABCMeta):
    ...
    def _gl_type_mapping_interval_ds_test_values(self):
        ...
        return ['+689991265 07:06:06.298066000', '-974757863 13:30:50.716052102', '+713917392 23:02:50']
    ...
    def _select_single_non_null_value_common(self, db_name, table_name, column_name, project_expression,
        ...
        if self._db_api.exists(db_name, table_name):
            ...
            if null_ne_empty_str and [_ for _ in columns
        ...
        sql = self._select_single_non_null_value_sql_template() \
            % (project_expression, self._db_api.enclose_object_reference(db_name, table_name),
               self.enclose_identifier(column_name), empty_string_clause)
    ...
    def compute_stats(self, db_name, table_name, incremental=None, for_columns=False, partition_tuples=None, sync=None):
        return self._db_api.compute_stats(db_name, table_name, incremental=incremental, for_columns=for_columns,
                                          partition_tuples=partition_tuples, sync=sync)
    ...
    def create_table(self, db_name, table_name, column_list, partition_column_names, storage_format=None, location=None,
        ...
        return self._db_api.create_table(db_name, table_name, column_list, partition_column_names,
                                         storage_format=storage_format, location=location, external=external,
                                         table_properties=table_properties, sort_column_names=sort_column_names,
                                         without_db_name=without_db_name, sync=sync)

    def create_test_partition_functions(self, db_name):
        ...
        """
        for udf_name in [test_constants.PARTITION_FUNCTION_TEST_FROM_INT8,
            ...
            if udf_body:
                self.create_udf(db_name, udf_name, return_type, [('p_arg', argument_type)],
                                udf_body, or_replace=True)

    def create_view(self, db_name, view_name, column_tuples, ansi_joined_tables, filter_clauses=None, sync=None):
        return self._db_api.create_view(db_name, view_name, column_tuples, ansi_joined_tables,
                                        filter_clauses=filter_clauses, sync=sync)
    ...
    def create_udf(self, db_name, udf_name, return_data_type, parameter_tuples, udf_body, or_replace=False,
        ...
        return self._db_api.create_udf(db_name, udf_name, return_data_type, parameter_tuples, udf_body,
                                       or_replace=or_replace, spec_as_string=spec_as_string,
                                       sync=sync, log_level=log_level)
    ...
    def get_distinct_column_values(self, db_name, table_name, column_name_list, order_results=False):
        return self._db_api.get_distinct_column_values(db_name, table_name, column_name_list,
    ...
    def get_max_column_values(self, db_name, table_name, column_name_list, columns_to_cast_to_string=None,
        ...
        return self._db_api.get_max_column_values(db_name, table_name, column_name_list,
    ...
    def gl_identifiers_generated_table_col_specs(self):
        ...
        canonical_columns = [
            ...
            CanonicalColumn('COL_STR_UPPER', GLUENT_TYPE_VARIABLE_STRING, data_length=10),
            CanonicalColumn('COL_DATE_UPPER', GLUENT_TYPE_DATE),
            CanonicalColumn('col_num_lower', GLUENT_TYPE_INTEGER_8),
            CanonicalColumn('col_str_lower', GLUENT_TYPE_VARIABLE_STRING, data_length=10),
            CanonicalColumn('col_date_lower', GLUENT_TYPE_DATE),
            CanonicalColumn('Col_Num_CamelCase', GLUENT_TYPE_INTEGER_8),
            CanonicalColumn('Col_Str_CamelCase', GLUENT_TYPE_VARIABLE_STRING, data_length=10),
        ...
        for column in canonical_columns:
            ...
            if column.is_number_based():
                ...
                literals = None
            gl_identifiers_cols.append({'column': backend_column,
                                        'literals': literals})
    ...
    def gl_type_mapping_present_options(self):
        ...
        for col_dict in definitions.values():
            if col_dict.get('present_options'):
                ...
                self._debug('Processing opts for %s: %s' % (col_dict['column'].name, str(opts)))
                for opt in opts:
                    if opt in present_options and isinstance(present_options[opt], list):
                        assert isinstance(opts[opt], list),\
                            'Expected type of option value is list, not: %s' % type(opts[opt])
                        present_options[opt] += opts[opt]
                    elif opt in present_options:
                        assert isinstance(opts[opt], str),\
                            'Expected type of option value is str, not: %s' % type(opts[opt])
                        present_options[opt] += ',' + opts[opt]
                    else:
                        present_options[opt] = opts[opt]
    ...
    def insert_literal_values(self, db_name, table_name, literal_list, column_list=None,
        ...
        return self._db_api.insert_literal_values(db_name, table_name, literal_list, column_list=column_list,
                                                  max_rows_per_insert=max_rows_per_insert, split_by_cr=split_by_cr)

    def insert_table_as_select(self, db_name, table_name, from_db_name, from_table_name, select_expr_tuples,
                               partition_expr_tuples, compute_stats=None, filter_clauses=None):
        ...
        """
        insert_sql = self._db_api.gen_insert_select_sql_text(db_name, table_name, from_db_name, from_table_name,
                                                             select_expr_tuples, partition_expr_tuples,
    ...
    def transient_error_rerunner(self, run_fn, max_retries=1):
        ...
        for i in range(max_retries + 1):
            try:
                ...
                transient_errors = [_ for _ in self.transient_query_error_identification_strings() if _ in str(exc)]
                if transient_errors and i < max_retries:
                    self._log('{}, exception: {}'.format(TRANSIENT_QUERY_RERUN_MARKER, str(exc)), detail=VERBOSE)
                    self._log('Matched transient error is: {}'.format(str(transient_errors)), detail=VERBOSE)
                    self._log('Sleeping for {} seconds before re-run'.format(str(TRANSIENT_QUERY_RERUN_PAUSE)),
                              detail=VERBOSE)
    ...
    @abstractmethod
    def create_partitioned_test_table(self, db_name, table_name, source_table_name, storage_format,
                                      compute_stats=False, filter_clauses=None):
