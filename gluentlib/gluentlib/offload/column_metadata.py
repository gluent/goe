...
def get_partition_columns(column_list, exclude_bucket_column=False):
    ...
    bucket_list = [] if exclude_bucket_column else [_ for _ in column_list if _.bucket_info]
    # Sort part_cols by partition_info.position
    return bucket_list + sorted(part_cols, key=lambda x: x.partition_info.position)


def get_partition_source_column_names(column_list, conv_fn=None, exclude_bucket_column=False):
    ...
    columns = get_partition_columns(column_list, exclude_bucket_column=exclude_bucket_column)
    if conv_fn:
        return [conv_fn(partition_or_bucket_info(_).source_column_name) for _ in columns or []]
    else:
...
def match_partition_column_by_source(source_column_name, column_list):
    ...
    match_cols = [_ for _ in part_cols_with_source
...
class ColumnPartitionInfo:
    ...
    """
    def __init__(self, position, source_column_name=None, granularity=None, digits=None,
                 range_start=None, range_end=None, function=None):
    ...
    def synthetic_name(self, full_column_list, source_column_name_override=None, position_override=None):
        ...
        return (SYNTHETIC_PARTITION_COLUMN_NAME_TEMPLATE
                % (synth_desc, source_column_name_override or self.source_column_name)).upper()
...
class ColumnMetadataInterface(metaclass=ABCMeta):
    ...
    def __init__(self, name, data_type, data_length=None, data_precision=None, data_scale=None, nullable=None,
        ...
        self.char_semantics = self._optional_upper_in(char_semantics, [CANONICAL_CHAR_SEMANTICS_BYTE,
    ...
    def _optional_integer(self, column_attribute):
        if column_attribute:
            assert isinstance(column_attribute, int), '{} is not int'.format(type(column_attribute))
        return column_attribute
    ...
    def clone(self, name=None, data_type=None, data_length=None, nullable=None,
              partition_info=None, bucket_info=None, char_length=None, char_semantics=None):
...
class CanonicalColumn(ColumnMetadataInterface):
    ...
    def __init__(self, name, data_type, data_length=None, data_precision=None, data_scale=None, nullable=None,
                 ...
                 from_override=False, char_length=None, char_semantics=None):
        """ CONSTRUCTOR
        """
        super(CanonicalColumn, self).__init__(name, data_type, data_length, data_precision, data_scale, nullable,
                                              data_default=data_default, safe_mapping=safe_mapping,
                                              partition_info=partition_info, bucket_info=bucket_info,
