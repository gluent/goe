...
# EXCEPTIONS
###############################################################################
class OracleDmlLoadException(Exception):
    pass
...
class RandomRowid(object):
    ...
    def __init__(
        ...
        table_owner,
        table_name,
        batch_size=DEFAULT_ROWID_BATCH_SIZE,
        sample_percent=DEFAULT_ROW_SAMPLE_PERCENT,
    ):
        ...
        logger.debug(
            "RandomRowid() object successfully initialized for table: %s"
            % self._db_table
        )
...
class OracleDmlLoad(object):
    ...
    def __init__(self, db_user, db_passwd, db_dsn, table_owner, table_name):
        ...
        self._rowid_factory = RandomRowid(
            self._ora, self._table_owner, self._table_name
        )
        ...
        logger.debug(
            "OracleDmlLoad() object successfully initialized for table: %s"
            % self._db_table
        )
    ...
    def _find_table_ranges(self):
        ...
        column_ranges = ", ".join(
            [
                "min(%s) as MIN_%s, max(%s) as MAX_%s" % (_, _, _, _)
                for _ in self._table_columns
            ]
        )
        ...
        query_results = self._ora.execute(
    ...
    def _generate_new_col_value(self, col_name):
        ...
        if col_name not in self._table_columns:
            raise OracleDmlLoadException(
                "Column: %s does not seem to exist in table: %s"
        ...
        if any([_ not in self._table_ranges for _ in (min_col, max_col)]):
            raise OracleDmlLoadException(
                "Column: %s does not seem to exist in 'ranges' for table: %s"
                % (col_name, self._db_table)
            )
    ...
    def _execute_update(self):
        ...
        """
        column_list = random.sample(
            self._table_columns, random.randint(1, len(self._table_columns))
        )
    ...
    def _get_commit_size(self, commit_size):
        ...
        """
        if commit_size is not None and (
            commit_size <= 0 or commit_size > MAX_COMMIT_SIZE
        ):
            raise OracleDmlLoadException(
                "Commit size must be in [1..%d] range" % MAX_COMMIT_SIZE
            )
    ...
    def load(self, no_records, ops=["insert", "update", "delete"], commit_size=None):
        ...
        """
        logger.debug(
            "Executing DML load: %d records, '%s' operations with %s commit size"
...
if __name__ == "__main__":
    ...
    def usage(prog_name):
        print(
            "%s: db_table load_records operations [commit_size] [debug level]"
            % prog_name
        )
