...
from gluentlib.offload.offload_constants import SPARK_BACKEND_CAPABILITIES
from gluentlib.offload.offload_messages import VERBOSE, VVERBOSE
from gluentlib.util.hs2_connection import HS2_OPTIONS
...
class BackendSparkThriftApi(BackendHiveApi):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False,
        ...
        if do_not_connect:
            ...
            self._hive_conn = self._connect_to_backend(host_override=self._spark_thrift_host,
    ...
    def get_columns(self, db_name, table_name):
        ...
        legacy_columns = self.execute_query_fetch_all('DESCRIBE %s'
    ...
    def get_table_ddl(self, db_name, table_name, as_list=False, terminate_sql=False, for_replace=False):
        ...
        ddl_row = self.execute_query_fetch_one('SHOW CREATE TABLE %s'
                                               % self.enclose_object_reference(db_name, table_name))
        if not ddl_row:
