...
from gluentlib.offload.oracle.oracle_column import ORACLE_TYPE_DATE, ORACLE_TYPE_TIMESTAMP, ORACLE_TIMESTAMP_RE,\
...
class OracleLiteral(FormatLiteralInterface):
    ...
    def _format_data_type(cls, str_val, data_type):
        if data_type == ORACLE_TYPE_DATE:
            ...
            return "TO_TIMESTAMP('%s','%s')" % (cls._strip_unused_time_scale(adjusted_str), fmt)
        else:
            return str_val
