...
from gluentlib.data_governance.hadoop_data_governance import data_governance_register_new_table_step,\
...
from gluentlib.filesystem.gluent_dfs import gen_load_uri_from_options
from gluentlib.offload.column_metadata import ColumnMetadataInterface, get_column_names
from gluentlib.offload.hadoop import hadoop_predicate
from gluentlib.offload.hadoop.hadoop_column import HADOOP_TYPE_BIGINT, HADOOP_TYPE_DATE, HADOOP_TYPE_DOUBLE,\
    HADOOP_TYPE_FLOAT, HADOOP_TYPE_INT, HADOOP_TYPE_SMALLINT, HADOOP_TYPE_STRING
from gluentlib.offload.offload_constants import FILE_STORAGE_FORMAT_AVRO, OFFLOAD_BUCKET_NAME, \
    OFFLOAD_STATS_METHOD_NATIVE, OFFLOAD_STATS_METHOD_HISTORY
from gluentlib.offload.offload_messages import VERBOSE, VVERBOSE
from gluentlib.offload.offload_transport_functions import load_db_hdfs_path, schema_paths
...
from gluentlib.offload.staging.avro.avro_staging_file import AVRO_TYPE_DOUBLE, AVRO_TYPE_FLOAT,\
    AVRO_TYPE_INT, AVRO_TYPE_LONG
from gluentlib.offload.staging.parquet.parquet_column import PARQUET_TYPE_DOUBLE, PARQUET_TYPE_FLOAT,\
...
class BackendHadoopTable(BackendTableInterface):
    ...
    def __init__(self, db_name, table_name, backend_type, orchestration_options, messages, orchestration_operation=None,
        ...
        if self._target_owner:
            _, self._avro_schema_hdfs_path = \
                schema_paths(self._target_owner, self.table_name, self._load_db_name, self._orchestration_config)
        else:
    ...
    def _compute_hive_table_statistics(self, incremental_stats, materialized_join=False):
        ...
        def progress_message(partitions_done, partitions_total):
            ...
            self._log('Partition progress %d%% (%d/%d)' % (int(perc), partitions_done, partitions_total), detail=VERBOSE)

        def gather_partition_stats(columns, partition):
            return self._db_api.compute_stats(self.db_name, self._base_table_name, for_columns=columns, partition_tuples=partition)
        ...
        def partitions_from_load_table():
            ...
            """
            part_exprs = '\n,    '.join('%s AS %s' % (self.get_final_table_cast(_), self.enclose_identifier(_))
                                        for _ in get_column_names(self.get_partition_columns()))

            sql = """SELECT DISTINCT
     %s
FROM %s.%s"""% (part_exprs, self.enclose_identifier(self._load_db_name), self.enclose_identifier(self._base_table_name))
        ...
        self._log('Stats automatically gathered by Hive for new partitions during insert', detail=VVERBOSE)
        ...
        if incremental_stats and not materialized_join:
            if self._offload_stats_method == OFFLOAD_STATS_METHOD_NATIVE and self._hive_column_stats_enabled:
                ...
                if not self._dry_run:
                    tab_stats, part_stats, col_stats = self._db_api.get_missing_hive_table_stats(
                        self.db_name, self._base_table_name, colstats=self._hive_column_stats_enabled, as_dict=True
                    )
                    self._log('Gathering stats on partitions that have none: %s' % str(list(part_stats.keys())), detail=VVERBOSE)
    ...
    def _create_load_table(self, staging_file):
        ...
        no_partition_cols = []
        location = gen_load_uri_from_options(self._orchestration_config, hadoop_db=self._load_db_name,
        ...
        self._db_api.create_table(
            ...
            no_partition_cols, storage_format=staging_file.file_format, location=location,
            external=True, table_properties=table_properties, sync=True
        )
    ...
    def _gen_synthetic_bucket_column_object(self, bucket_info=None):
        return self._db_api.gen_column_object(OFFLOAD_BUCKET_NAME, data_type=self.synthetic_bucket_data_type(),
    ...
    def _gen_synthetic_part_number_granularity_sql_expr(self, column_expr, backend_col, granularity,
        ...
        raise NotImplementedError('_gen_synthetic_part_number_granularity_sql_expr() is not implemented for common Hadoop class')
    ...
    def _gen_synthetic_partition_column_object(self, synthetic_name, canonical_column):
        ...
        """
        return self._db_api.gen_column_object(synthetic_name, data_type=HADOOP_TYPE_STRING,
    ...
    def _gen_synthetic_partition_date_as_string_sql_expr(self, extract_name, pad_size, source_column_cast):
        ...
        """
        return "LPAD(CAST(%s(%s) AS STRING), %s, '0')" % (extract_name, source_column_cast, pad_size)

    def _gen_synthetic_partition_date_truncated_sql_expr(self, synthetic_name, column_name, granularity, source_column_cast):
        ...
        return "TRUNC(%s, '%s')" % (source_column_cast, TYPICAL_DATE_GRANULARITY_TERMS[granularity])
    ...
    def _get_data_db_hdfs_dir(self):
        if self._data_db_hdfs_dir is None:
            self._data_db_hdfs_dir = self._get_dfs_client().gen_uri(self._orchestration_config.offload_fs_scheme,
    ...
    def _staging_to_backend_cast(self, rdbms_column, backend_column, staging_column) -> tuple:
        ...
        def staging_and_backend_types_match(staging_column, backend_column):
            if staging_column.format_data_type().upper() == backend_column.format_data_type().upper():
                self._log('No CAST() required for %s: %s->%s'
                          % (staging_column.name, staging_column.format_data_type(), backend_column.format_data_type()),
                          detail=VVERBOSE)
                return True
            else:
                return False
        ...
        def staging_file_int_match(backend_column, staging_column):
            return bool((backend_column.data_type == HADOOP_TYPE_BIGINT
                         and staging_column.data_type in [AVRO_TYPE_LONG, PARQUET_TYPE_INT64])
                        or (backend_column.data_type == HADOOP_TYPE_INT
        ...
        def staging_file_float_match(backend_column, staging_column):
            return bool((backend_column.data_type == HADOOP_TYPE_FLOAT
                         and staging_column.data_type in [AVRO_TYPE_FLOAT, PARQUET_TYPE_FLOAT])
                        or (backend_column.data_type == HADOOP_TYPE_DOUBLE
        ...
        if backend_column.is_number_based():
            if (staging_file_int_match(backend_column, staging_column)
                ...
                return_cast = 'CAST(%s AS %s)' % (self._format_staging_column_name(staging_column),
            ...
            if not backend_column.is_string_based():
                ...
                return_cast = """CASE WHEN %(tstz)s REGEXP '.*[+-][01]?[0-9]:[0-9][05]$' THEN
                    ...
                    END""" % {'tstz': self._format_staging_column_name(staging_column),
            ...
            if backend_column.is_string_based():
                ...
                if backend_column.data_type == HADOOP_TYPE_DATE:
                    ...
                    return_cast = 'CAST(%s AS %s)' % (self._format_staging_column_name(staging_column), return_type)
    ...
    def _synthetic_bucket_filter_non_udf_sql_expression(self):
        raise NotImplementedError('_synthetic_bucket_filter_non_udf_sql_expression() is not implemented for common Hadoop class')
    ...
    def load_final_table(self, sync=None):
        ...
        try:
            self._execute_dml(sqls, query_options=query_options, sync=sync,
        ...
        FROM (
            ...
            FROM %(db)s.%(table)s
        ) v""" % {'part_expr': partition_expressions, 'db': self._load_db_name, 'table': self._load_table_name}
                self._log(
    ...
    def get_default_location(self):
        if self._default_location is None:
            self._default_location = self._db_api.get_table_location(self.db_name, self._base_table_name)
