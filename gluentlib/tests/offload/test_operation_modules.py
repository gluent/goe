...
from gluentlib.offload.offload_constants import NOT_NULL_PROPAGATION_AUTO, NOT_NULL_PROPAGATION_NONE
from gluentlib.offload.offload_messages import OffloadMessages
from gluentlib.offload.operation.data_type_controls import OffloadDataTypeControlsException,\
    canonical_columns_from_columns_csv
from gluentlib.offload.operation.not_null_columns import OffloadNotNullControlsException, apply_not_null_columns_csv
from gluentlib.offload.column_metadata import CanonicalColumn, \
    GLUENT_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_2, GLUENT_TYPE_INTEGER_4
...
class TestOperationDataTypeControls(TestCase):
    ...
    def test_canonical_columns_from_columns_csv(self):
        reference_columns = [CanonicalColumn('COL1_ID', GLUENT_TYPE_INTEGER_2),
        ...
        col_list = canonical_columns_from_columns_csv(GLUENT_TYPE_INTEGER_4, 'COL1_ID,COL2_ID', [], reference_columns)
        self.assertEqual(len(col_list), 2)
        self.assertEqual(col_list[0].data_type, GLUENT_TYPE_INTEGER_4)
        col_list = canonical_columns_from_columns_csv(GLUENT_TYPE_INTEGER_4, '*_ID', [], reference_columns)
        self.assertEqual(len(col_list), 2)
        self.assertEqual(col_list[0].data_type, GLUENT_TYPE_INTEGER_4)
        col_list = canonical_columns_from_columns_csv(GLUENT_TYPE_INTEGER_4, '*_ID,*KEY', [], reference_columns)
        self.assertEqual(len(col_list), 4)
        col_list = canonical_columns_from_columns_csv(GLUENT_TYPE_DECIMAL, 'COL5_YEAR', [], reference_columns,
                                                      precision=4, scale=0)
        ...
        col_list = canonical_columns_from_columns_csv(GLUENT_TYPE_INTEGER_4, '*', [], reference_columns)
        self.assertEqual(len(col_list), len(reference_columns))
        # Ensure overlaps are caught
        self.assertRaises(OffloadDataTypeControlsException,
                          lambda: canonical_columns_from_columns_csv(GLUENT_TYPE_INTEGER_4, 'COL1_ID,COL2_ID',
...
class TestOperationNotNullColumns(TestCase):
    ...
    reference_columns = [CanonicalColumn('COL1_ID', GLUENT_TYPE_INTEGER_2, nullable=True),
    ...
    def test_not_null_columns_list(self):
        messages = OffloadMessages()
        names = [_.name for _ in self.reference_columns]
        new_cols = apply_not_null_columns_csv(self.reference_columns, '*_NN', NOT_NULL_PROPAGATION_AUTO, names, messages)
        for new_col, ref_col in zip(new_cols, self.reference_columns):
            self.assertEqual(new_col.name, ref_col.name)
        ...
        self.assertRaises(
            ...
            lambda: apply_not_null_columns_csv(self.reference_columns, '*XYZ', NOT_NULL_PROPAGATION_AUTO, names, messages)
        )
        self.assertRaises(
            OffloadNotNullControlsException,
            lambda: apply_not_null_columns_csv(self.reference_columns, 'COLX', NOT_NULL_PROPAGATION_AUTO, names, messages)
        )
