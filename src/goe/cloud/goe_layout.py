...
# EXCEPTIONS
###############################################################################
class GOELayoutException(Exception): pass
...
class GOEHdfsLayout(object):
    ...
    def extract_goets(self, file_key, missing_value=99999999):
        ...
        if match:
            ...
            logger.debug("Detected 'goe timestamp': %s.%s in file key: %s. Extracting timestamp: %d" % \
                (ts_resolution, ts, file_key, ts_number))
            return ts_number
        else:
            logger.debug("Did not detect 'goe' timestamp in file key: %s. Assigning default: %d" % \
