...
from gluentlib.offload.column_metadata import CanonicalColumn, ColumnBucketInfo, ColumnPartitionInfo, \
    get_column_names, get_partition_columns, is_synthetic_partition_column, \
    match_partition_column_by_source, match_table_column, match_table_column_position, \
    regex_real_column_from_part_column, valid_column_list, \
    GLUENT_TYPE_BINARY, GLUENT_TYPE_BOOLEAN, GLUENT_TYPE_DATE, GLUENT_TYPE_DECIMAL, \
    GLUENT_TYPE_FLOAT, GLUENT_TYPE_INTEGER_2, GLUENT_TYPE_TIME, GLUENT_TYPE_VARIABLE_STRING, \
...
class TestColumnMetadata(TestCase):
    ...
    def canonical_columns(self):
        return [
            ...
            CanonicalColumn('COL_COST', GLUENT_TYPE_DECIMAL, data_precision=10, data_scale=2),
            ...
            CanonicalColumn(SYNTHETIC_PARTITION_COLUMN_NAME_TEMPLATE % ('M', 'COL_DATE'),
                            GLUENT_TYPE_VARIABLE_STRING,
                            partition_info=ColumnPartitionInfo(0, source_column_name='COL_DATE', granularity='M')),
            CanonicalColumn(SYNTHETIC_PARTITION_COLUMN_NAME_TEMPLATE % ('1', 'COL_NAME'),
                            GLUENT_TYPE_VARIABLE_STRING,
                            partition_info=ColumnPartitionInfo(1, source_column_name='COL_NAME', granularity='1')),
            CanonicalColumn('NATIVE_PART_COL', GLUENT_TYPE_INTEGER_2, partition_info=ColumnPartitionInfo(2)),
            CanonicalColumn(OFFLOAD_BUCKET_NAME, GLUENT_TYPE_INTEGER_2,
                            bucket_info=ColumnBucketInfo(source_column_name='ID', num_buckets=16,
                                                         bucket_hash_method=None)),
        ]
    ...
    def test_get_column_names(self):
        ...
        self.assertEqual(get_column_names(self.canonical_columns(), conv_fn=str.upper)[0],
                         get_column_names(self.canonical_columns())[0].upper())
        self.assertEqual(get_column_names(self.canonical_columns(), conv_fn=str.lower)[0],
    ...
    def test_get_partition_columns(self):
        ...
        self.assertIsInstance(match_table_column('NATIVE_PART_COL', get_partition_columns(self.canonical_columns())),
                              CanonicalColumn)
        self.assertIsNone(match_table_column('COL_COST', get_partition_columns(self.canonical_columns())),
                          CanonicalColumn)
