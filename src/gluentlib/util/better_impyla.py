...
from gluentlib.offload.offload_messages import OffloadMessagesMixin, VERBOSE
from gluentlib.offload.offload_constants import DBTYPE_HIVE, DBTYPE_IMPALA, DBTYPE_SPARK

from gluentlib.util.hs2_connection import hs2_connection_from_env, hs2_cursor_user, hs2_db_type
from gluentlib.util.misc_functions import end_by


###############################################################################
# EXCEPTIONS
###############################################################################
class BetterImpylaException(Exception): pass
...
class HiveConnection(OffloadMessagesMixin, object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        logger.debug("BetterImpyla() object successfully initialized. Port: %s DbType: %s"
    ...
    def _use_existing_connection(self, *args, **kwargs):
        ...
        port = self._extract_port_from_args(*args, **kwargs)
        if port:
            logger.warn("Extracted port: %d from a list of parameters. May be misleading" % port)
        else:
            port = self._hack_into_impyla_for_port(conn)

        if not port:
            raise BetterImpylaException("Unable to extract port from a list of 'connect' parameters or existing connection")
    ...
    def _hack_into_impyla_for_port(self, connection):
        ...
        try:
            ...
            logger.warn("Attribute Error: %s while scanning impyla object for port" % e, exc_info=True)
    ...
    def execute(self, cmd, cursor_fn=None, suppress_exceptions=False, as_dict=False, detail=VERBOSE):
        ...
        if cursor_fn:
            ...
            if as_dict:
                logger.debug("AS_DICT transformation requested for: %s" % \
    ...
    def execute_ddl(self, cmd, cursor_fn=None, suppress_exceptions=False, as_dict=False, sync_ddl=True):
        """ Execute DDL command (has a special symantics in impala
        """
        ret = None

        if DBTYPE_IMPALA == self._db_type:
            ret = self.executemany(["SET SYNC_DDL=%s" % sync_ddl, cmd], cursor_fn, suppress_exceptions, as_dict)
        else:
            ret = self.execute(cmd, cursor_fn, suppress_exceptions, as_dict)
        ...
        if not self._sql_engine_version:
            ...
            try:
                ...
                if self._db_type == DBTYPE_HIVE:
                    ...
                    logger.warn("Unable to get version() on Hive (expected in Hive <2.1): %s" % str(e))
                else:
            ...
            if version_text:
                ...
                if self._db_type == DBTYPE_IMPALA:
                    m = re.search(r'^impalad version (\d+\.\d+\.\d+).*', version_text, re.M | re.I)
...
class HiveTable(object):
    ...
    def _parse_sections(self, describe_formatted):
        ...
        for col_name, data_type, comment in describe_formatted:
            ...
            if store_as_tuple:
                ...
                logger.debug("Identified table detail item %s=%s for section: %s" % \
    ...
    def _extract_sections(self, table_name, db_name):
        ...
        is_view = ('VIRTUAL_VIEW' == table_details['detailed table information']['table type:'])
    ...
    def _extract_original_text_for_hive_view(self, describe_formatted):
        ...
        """
        start = next((i for i in range(len(describe_formatted)) if (describe_formatted[i][0] or '').strip().lower() == 'view original text:'), -1)
        if start < 0:
    ...
    def _extract_table_details(self, table_name, db_name):
        ...
        if not is_view:
            ...
            if 'partition columns' not in table_details:
                ...
                if partition_cols:
                    table_details['partition columns'] = [_ for _ in table_details['table columns'] if _[0] in partition_cols]
            else:
    ...
    def _extract_table_partitions(self, table_name, db_name):
        ...
        def extract_partitions_impala(table_name, db_name):
            ...
            partitions_out, _ = self._show_partitions(table_name, db_name, cursor_fn=lambda c: c.fetchall(), as_dict=True)
            partition_data = {}
    ...
    def _get_partition_column_type(self, col_name):
        ...
        if not col_type:
            raise BetterImpylaException("Cannot find column: %s in partitioned columns for: %s.%s" % \
    ...
    def _parse_dependent_objects_from_view_definition(self, view_definition):
        ...
        if not ret:
            logger.warn("Unable to parse view definition: %s for 'dependent objects'" % view_definition)
        return ret
    ...
    def view_definition(self):
        ...
        if not self.is_view():
            logger.warn("Object: %s.%s is NOT a view" % (self._db_name, self._table_name))
            return None
    ...
    def table_ddl(self, terminate_sql=True):
        ...
        if not self._table_ddl:
            if self.is_view():
                self._table_ddl = "CREATE VIEW %s.%s AS %s" % (self._db_name, self._table_name, self.view_definition())
                if terminate_sql:
                    self._table_ddl += ";"
            else:
    ...
    def partition_details(self, partition_spec):
        ...
        if partition_str in table_partitions:
            ...
            logger.warn("Unable to find partition: %s when looking for details" % partition_spec)
    ...
    def partition_location(self, partition_spec):
        ...
        if partition_details:
            if 'Location' in partition_details:
                ...
                logger.warn("Unable to find 'location' in details for partition: %s" % partition_spec)
    ...
    def change_partition_location(self, partition_spec, new_location, sync_ddl=False):
        ...
        if not current_location:
            logger.warn("Unable to find current location for partition: %s. Does it exist ?" % partition_spec)
            return False
        elif current_location == new_location:
            logger.warn("Partition: %s is already located at: %s" % (partition_spec, new_location))
            return True

        sql = "ALTER TABLE %s.%s PARTITION %s SET LOCATION '%s'" % \
            (self._db_name, self._table_name, self._make_formal_partition_spec(partition_spec), new_location)
    ...
    def add_partition(self, partition_spec, location=False, sync_ddl=False):
        ...
        if self.partition_exists(partition_spec):
            logger.warn("Partition: %s already exists for table: %s.%s" % (partition_spec, self._db_name, self._table_name))
            return False
        ...
        sql = "ALTER TABLE %s.%s ADD PARTITION %s LOCATION '%s'" % \
            (self._db_name, self._table_name, self._make_formal_partition_spec(partition_spec), location)
    ...
    def drop_partition(self, partition_spec, sync_ddl=False):
        ...
        if not self.partition_exists(partition_spec):
            logger.warn("Partition: %s does NOT exists for table: %s.%s" % (partition_spec, self._db_name, self._table_name))
            return False

        sql = "ALTER TABLE %s.%s DROP PARTITION %s" % \
            (self._db_name, self._table_name, self._make_formal_partition_spec(partition_spec))
    ...
    def compute_stats(self):
        ...
        if DBTYPE_IMPALA == self._hive._db_type:
            ...
            logger.warn("Statistics collection is NOT yet supported for db type: %s" % self._hive.db_type)
