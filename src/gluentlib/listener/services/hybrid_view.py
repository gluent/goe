...
from gluentlib.offload.offload_messages import VVERBOSE, OffloadMessages
from gluentlib.offload.offload_validation import (
    CrossDbValidator,
...
class HybridViewService(object):
    ...
    def __init__(self, hybrid_schema, hybrid_view, dry_run=False, existing_log_fh=None):
        ...
        if (
            self._connection_options.log_level == LOG_LEVEL_DEBUG  # noqa: W291
            and not existing_log_fh  # noqa: W503
        ):
        ...
        # Initially store in state as None to allow
        # lazy collection via _get_backend_detail()
        self._backend_info = None
        # Initially store in state as None to allow
        # lazy collection via _get_frontend_detail()
    ...
    def backend_partition_by_column_supported(self):
        """
        Is Partitioning Supported

        Does the backend support partitioning,
        strictly speaking we don't need this info except for testing"""
    ...
    def backend_attributes(self, as_json=True):
        ...
        partition_key_values results in a backend query which can take some time
        so is excluded by default.
        """
    ...
    def validate_by_aggregation(self, lower_hv=None, upper_hv=None, as_json=True):
        """
        Return response from CrossDbValidator

        as JSON: {'success': True/False, 'message': "blah"}
        lower_hv, upper_hv: DBA_TAB_PARTITIONS.HIGH_VALUE based. Can have both empty,
        upper_hv only or both.
        """
        if lower_hv:
    ...
    def _get_backend_detail(self, attribute_name=None):
        ...
        if not self._backend_info:
            ...
            fetched_backend_partitions = backend_table.get_table_partitions()
            backend_partitions = []
            if isinstance(fetched_backend_partitions, list):
                backend_partitions = fetched_backend_partitions
            elif isinstance(fetched_backend_partitions, dict):
                for partition_key in fetched_backend_partitions:
                    backend_partitions.append(fetched_backend_partitions[partition_key])
            else:
                backend_partitions = list(fetched_backend_partitions)
