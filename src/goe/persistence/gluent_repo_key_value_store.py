...
class GluentRepoKeyValueStore(KeyValueStore):
    def __init__(
        ...
        if repo_client:
            ...
            self._repo_client = orchestration_repo_client_factory(
                orchestration_config, messages, dry_run=dry_run
            )
    ...
    def get_all(self, key_string_list=None):
        kv_dict = self._repo_client.get_incremental_update_extraction_metadata(
            self._hybrid_owner, self._hybrid_view
        )
    ...
    def put_all(self, key_string_to_value_string_map):
        if not self._dry_run:
            self._repo_client.save_incremental_update_extraction_metadata(
                self._hybrid_owner,
                self._hybrid_view,
                key_string_to_value_string_map,
                self._execution_id,
            )
