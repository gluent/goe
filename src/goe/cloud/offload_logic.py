...
# EXCEPTIONS
###############################################################################
class OffloadLogicException(Exception):
    pass
...
class XferSetupRaw(object):
    ...
    def _initialize(self, lite):
        ...
        self._lite = (
            lite  # True = "Lite" execution mode (aka: give me 'source' data afahp)
        )

        # Module outcome
        self._offloadable_difference = (
            {}
        )  # 'Offloadable' file difference between source and target
        self._offloadable_stats = {}  # 'Offloadable difference' 'statistics'

        self._deletable_difference = (
            {}
        )  # 'Deletable' file difference between source and target
    ...
    def _calculate_offloadable_difference(self, source_data, target_data):
        ...
        for s in source:
            if s not in target or source[s]["size"] != target[s]["size"]:
                ...
                result.append({"source": source[s], "target": target_s})
            else:
                self._the_same += 1
    ...
    def _calculate_deletable_stats(self, data):
        ...
        """
        target_last_modified = self._last_modified_from_data(data, sub_key="target")
        target_first_modified = self._last_modified_from_data(
            data, sub_key="target", min_last_modified=True
        )
        result = {
    ...
    def _evaluate(self, search_conditions, lite):
        ...
        self._source_data = (
            self._source.jmespath_search(where=search_conditions, data=src_data_access)
            if search_conditions
            else src_data_access
        )
        if self._lite:
            self._target_data = []
        else:
            self._target_data = (
                self._target.jmespath_search(
                    where=search_conditions, data=self._target.data
                )
                if search_conditions
                else self._target.data
            )

        if not self._lite:
            self._offloadable_difference = self._calculate_offloadable_difference(
                self._source_data, self._target_data
            )
            self._deletable_difference = self._calculate_deletable_difference(
                self._source_data, self._target_data
            )
            ...
            self._offloadable_stats = self._calculate_offloadable_stats(
                self._offloadable_difference
            )
            self._deletable_stats = self._calculate_deletable_stats(
                self._deletable_difference
            )
    ...
    def _last_modified_from_data(self, data, sub_key=None, min_last_modified=False):
        ...
        last_modified = (
            [
                to_str(get_last_modified(_))
                for _ in data
                if get_last_modified(_) is not None
            ]
            if data
            else None
        )
    ...
    def offloadable_source(self):
        ...
        if self._lite:
            logger.warn(
                "XferSetupRaw.offloadable_source() is undefined in 'lite' mode."
            )
            return []
        else:
            return sorted(
                [_["source"] for _ in self._offloadable_difference],
    ...
    def offloadable_keys(self):
        ...
        if self._lite:
            ...
            return sorted(
                [_["source"]["key_name"] for _ in self._offloadable_difference]
            )
    ...
    def offloadable_distcp(self):
        ...
        if self._lite:
            logger.warn(
                "XferSetupRaw.offloadable_distcp() is undefined in 'lite' mode."
            )
            return []
        else:
    ...
    def offloadable_extended(self):
        ...
        if self._lite:
            logger.warn(
                "XferSetupRaw.offloadable_extended() is undefined in 'lite' mode."
            )
            return []
        else:
            return sorted(
                self._offloadable_difference, key=lambda x: x["source"]["name"]
            )
    ...
    def deletable(self):
        ...
        if self._lite:
            ...
            return sorted(
                [
                    (_["source"]["name"], _["target"]["name"])
                    for _ in self._deletable_difference
                ],
                key=lambda x: x[0],
            )
    ...
    def deletable_extended(self):
        ...
        if self._lite:
            logger.warn(
                "XferSetupRaw.deletable_extended() is undefined in 'lite' mode."
            )
            return []
        else:
    ...
    def evaluate(self, search_conditions=None, lite=False):
        ...
        logger.debug(
            "Evaluating for offload. Search conditions: %s. Lite mode: %s"
            % (search_conditions, lite)
        )
...
class XferSetup(XferSetupRaw):
    ...
    def __init__(
        ...
        logger.debug(
            "XferSetup() object initialized for source: %s:%s, destination: %s:%s"
    ...
    def _parse_section(self, section, destination, db_table):
        ...
        if not self._cfg.has_section(section):
            raise OffloadLogicException(
                "Section: '%s' does NOT exist in configuration file" % section
            )
        if not self._cfg.has_option(section, "db_path_suffix"):
            raise OffloadLogicException(
                "'db_path_suffix' is missing in section: '%s'" % section
            )
        ...
        if DST_HDFS == result["type"]:
            ...
            raise OffloadLogicException(
                "Unsupported section type: %s for '%s'" % (result["type"], destination)
            )
    ...
    def _determine_section_type(self, section, destination):
        ...
        config_type = (
            self._cfg.get(section, "type")
            if self._cfg.has_option(section, "type")
            else None
        )
        ...
        if config_type and config_type in (DST_HDFS, DST_S3):
            ...
            raise OffloadLogicException(
                "Unable to determine section type for '%s'" % section
            )
    ...
    def _parse_section_hdfs(self, section):
        ...
        for component in ("hdfs_root", "hdfs_url", "webhdfs_url"):
            if not self._cfg.has_option(section, component):
                raise OffloadLogicException(
                    "Required components: '%s' is missing in HDFS section: %s"
                    % (component, section)
                )
    ...
    def _parse_section_s3(self, section):
        ...
        for component in ("s3_bucket", "s3_prefix"):
            if not self._cfg.has_option(section, component):
                raise OffloadLogicException(
                    "Required components: '%s' is missing in S3 section: %s"
                    % (component, section)
                )
...
if __name__ == "__main__":
    ...
    def usage(prog_name):
        print(
            "%s: property src src_db_table dst dst_db_table [lite=True|False] [debug level]"
            % prog_name
        )
        sys.exit(1)
