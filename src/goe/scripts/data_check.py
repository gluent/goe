...
PROG_BANNER = (
    "data_check: (randomnly) Test data consistency between oracle and hive/impala"
)
...
def parse_dsn(dsn):
    ...
    if not match_dsn:
        raise DataCheckException(
            "Invalid DSN (not printing actual dsn for security reasons)"
        )
...
def process_database(db, args):
    ...
    probability = args.sample_pct / 100.0
...
def parse_args():
    ...
    db_source.add_argument(
        "-d",
        "--dsn",
        help="ORACLE database connection. Default: ORA_ADM_ environment variables",
    )
    db_source.add_argument(
        "-m", "--manifest", help="Manifest file with ORACLE connection information"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        default=DEF_SAMPLE_PCT,
        help="Percent of data to sample. Represents probability (1-100) of each individual partition/small table to be selected for check. Default: %d"
        % DEF_SAMPLE_PCT,
    )
