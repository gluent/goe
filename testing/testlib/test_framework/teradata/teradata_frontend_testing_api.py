...
from testlib.setup import gen_test_data
from testlib.test_framework import test_constants
from testlib.test_framework.frontend_testing_api import (
    FrontendTestingApiException,
    FrontendTestingApiInterface,
)
from testlib.test_framework.test_functions import (
...
class TeradataFrontendTestingApi(FrontendTestingApiInterface):
    ...
    def _create_new_testing_client(self, existing_connection,
        ...
        return TeradataFrontendTestingApi(self._frontend_type, self._connection_options, self._messages,
    ...
    def _gen_column_data(self, column, row_index, from_list=None, ascii_only=False, all_chars_notnull=False,
        ...
        if column.data_type == TERADATA_TYPE_VARCHAR:
            ...
            return gen_test_data.gen_number(row_index, precision=column.data_precision, scale=column.data_scale,
                                            from_list=from_list, ordered=ordered)
    ...
    def _gl_type_mapping_column_definitions(self, max_backend_precision, max_backend_scale,
        ...
        all_columns = {
            name(TERADATA_TYPE_BIGINT):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_BIGINT), TERADATA_TYPE_BIGINT),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_BIGINT), GLUENT_TYPE_INTEGER_8)},
            name(TERADATA_TYPE_BIGINT, GLUENT_TYPE_INTEGER_4):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_BIGINT, GLUENT_TYPE_INTEGER_4), TERADATA_TYPE_BIGINT),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_BIGINT, GLUENT_TYPE_INTEGER_4), GLUENT_TYPE_INTEGER_4),
                 'literals': [TestDecimal.min(9), TestDecimal.rnd(9), TestDecimal.max(9)]},
            name(TERADATA_TYPE_BLOB):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_BLOB), TERADATA_TYPE_BLOB),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_BLOB), GLUENT_TYPE_LARGE_BINARY),
                 'literals': ['lob-a', 'lob-b', 'lob-c']},
            name(TERADATA_TYPE_BYTE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_BYTE), TERADATA_TYPE_BYTE, data_length=30),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_BYTE), GLUENT_TYPE_BINARY)},
            name(TERADATA_TYPE_BYTEINT):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_BYTEINT), TERADATA_TYPE_BYTEINT),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_BYTEINT), GLUENT_TYPE_INTEGER_1)},
            name(TERADATA_TYPE_BYTEINT, GLUENT_TYPE_INTEGER_2):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_BYTEINT, GLUENT_TYPE_INTEGER_2), TERADATA_TYPE_BYTEINT),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_BYTEINT, GLUENT_TYPE_INTEGER_2), GLUENT_TYPE_INTEGER_2)},
            name(TERADATA_TYPE_CHAR, '3'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_CHAR, '3'), TERADATA_TYPE_CHAR, data_length=3),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_CHAR, '3'), GLUENT_TYPE_FIXED_STRING, data_length=3),
                 'ascii_only': ascii_only,
                 'notnull': all_chars_notnull},
            name(TERADATA_TYPE_CHAR, '3', test_constants.UNICODE_NAME_TOKEN):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_CHAR, '3', test_constants.UNICODE_NAME_TOKEN),
                                  TERADATA_TYPE_CHAR, data_length=3),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_CHAR, '3', test_constants.UNICODE_NAME_TOKEN),
                                     GLUENT_TYPE_FIXED_STRING,
                                     char_semantics=CANONICAL_CHAR_SEMANTICS_UNICODE),
                 'offload_options': {'unicode_string_columns_csv': name(TERADATA_TYPE_CHAR, '3',
                                                                        test_constants.UNICODE_NAME_TOKEN)},
                 'ascii_only': ascii_only,
                 'notnull': all_chars_notnull},
            name(TERADATA_TYPE_CLOB):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_CLOB), TERADATA_TYPE_CLOB),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_CLOB), GLUENT_TYPE_LARGE_STRING),
                 'ascii_only': ascii_only,
                 'notnull': all_chars_notnull,
            ...
            name(TERADATA_TYPE_CLOB, test_constants.UNICODE_NAME_TOKEN):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_CLOB, test_constants.UNICODE_NAME_TOKEN), TERADATA_TYPE_CLOB),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_CLOB, test_constants.UNICODE_NAME_TOKEN),
                                     GLUENT_TYPE_LARGE_STRING,
                                     char_semantics=CANONICAL_CHAR_SEMANTICS_UNICODE),
                 'offload_options': {'unicode_string_columns_csv': name(TERADATA_TYPE_CLOB,
                                                                        test_constants.UNICODE_NAME_TOKEN)},
                 'ascii_only': ascii_only,
                 'notnull': all_chars_notnull,
                 # Keep row size small for mapping table
            ...
            name(TERADATA_TYPE_DATE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DATE), TERADATA_TYPE_DATE),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DATE), GLUENT_TYPE_DATE)},
            name(TERADATA_TYPE_DATE, GLUENT_TYPE_VARIABLE_STRING):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DATE, GLUENT_TYPE_VARIABLE_STRING), TERADATA_TYPE_DATE),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DATE, GLUENT_TYPE_VARIABLE_STRING),
                                     GLUENT_TYPE_VARIABLE_STRING),
                 'offload_options': {'variable_string_columns_csv': name(TERADATA_TYPE_DATE,
                                                                         GLUENT_TYPE_VARIABLE_STRING)}},
            name(TERADATA_TYPE_DECIMAL):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL), TERADATA_TYPE_DECIMAL),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL), GLUENT_TYPE_INTEGER_4),
                 # Naked DECIMAL defaults to DECIMAL(5,0) so we expect INT4 and insert suitable data below.
                 'literals': [TestDecimal.min(5, 0), TestDecimal.rnd(5, 0), TestDecimal.max(5, 0)]},
            name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_1):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_1), TERADATA_TYPE_DECIMAL),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_1), GLUENT_TYPE_INTEGER_1),
                 'offload_options': {'integer_1_columns_csv': name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_1)},
                 'literals': [TestDecimal.min(2), TestDecimal.rnd(2), TestDecimal.max(2)]},
            name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_2):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_2), TERADATA_TYPE_DECIMAL),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_2), GLUENT_TYPE_INTEGER_2),
                 'offload_options': {'integer_2_columns_csv': name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_2)},
                 'literals': [TestDecimal.min(4), TestDecimal.rnd(4), TestDecimal.max(4)]},
            name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_4):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_4), TERADATA_TYPE_DECIMAL),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_4), GLUENT_TYPE_INTEGER_4),
                 'offload_options': {'integer_4_columns_csv': name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_INTEGER_4)},
                 'literals': [TestDecimal.min(5), TestDecimal.rnd(5), TestDecimal.max(5)]},
            ...
            name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_DOUBLE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_DOUBLE), TERADATA_TYPE_DECIMAL),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_DOUBLE), GLUENT_TYPE_DOUBLE),
                 'offload_options': {'double_columns_csv': name(TERADATA_TYPE_DECIMAL, GLUENT_TYPE_DOUBLE)},
                 'literals': [1.5, 2.5, 3.5]},
            name(TERADATA_TYPE_DECIMAL, '2'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, '2'), TERADATA_TYPE_DECIMAL,
                                    data_precision=2, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, '2'), GLUENT_TYPE_INTEGER_1)},
            name(TERADATA_TYPE_DECIMAL, '4'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, '4'), TERADATA_TYPE_DECIMAL,
                                    data_precision=4, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, '4'), GLUENT_TYPE_INTEGER_2)},
            name(TERADATA_TYPE_DECIMAL, '9'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, '9'), TERADATA_TYPE_DECIMAL,
                                    data_precision=9, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, '9'), GLUENT_TYPE_INTEGER_4)},
            name(TERADATA_TYPE_DECIMAL, '18'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, '18'), TERADATA_TYPE_DECIMAL,
                                    data_precision=18, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, '18'), GLUENT_TYPE_INTEGER_8)},
            name(TERADATA_TYPE_DECIMAL, '19'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, '19'), TERADATA_TYPE_DECIMAL,
                                    data_precision=19, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, '19'), GLUENT_TYPE_INTEGER_38, data_precision=19)},
            name(TERADATA_TYPE_DECIMAL, str(max_decimal_integral_magnitude)):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, str(max_decimal_integral_magnitude)),
                                    TERADATA_TYPE_DECIMAL, data_precision=max_decimal_integral_magnitude, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, str(max_decimal_integral_magnitude)),
                                     GLUENT_TYPE_INTEGER_38)},
            name(TERADATA_TYPE_DECIMAL, str(max_precision), str(generic_scale)):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, str(max_precision), str(generic_scale)),
                                    TERADATA_TYPE_DECIMAL, data_precision=max_precision, data_scale=generic_scale),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, str(max_precision), str(generic_scale)),
            ...
            name(TERADATA_TYPE_DECIMAL, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DECIMAL, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3'),
                                    TERADATA_TYPE_DECIMAL, data_precision=9, data_scale=2),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DECIMAL, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3'),
                                     GLUENT_TYPE_DECIMAL, data_precision=10, data_scale=3),
                 'offload_options': {
                     'decimal_columns_csv_list': [name(TERADATA_TYPE_DECIMAL, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3')],
            ...
            name(TERADATA_TYPE_DOUBLE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_DOUBLE), TERADATA_TYPE_DOUBLE),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_DOUBLE), GLUENT_TYPE_DOUBLE)},
            name(TERADATA_TYPE_INTEGER):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_INTEGER), TERADATA_TYPE_INTEGER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_INTEGER), GLUENT_TYPE_INTEGER_4)},
            name(TERADATA_TYPE_INTEGER, GLUENT_TYPE_INTEGER_2):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_INTEGER, GLUENT_TYPE_INTEGER_2), TERADATA_TYPE_INTEGER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_INTEGER, GLUENT_TYPE_INTEGER_2), GLUENT_TYPE_INTEGER_2),
                 'offload_options': {'integer_2_columns_csv': name(TERADATA_TYPE_INTEGER, GLUENT_TYPE_INTEGER_2)},
                 'literals': [TestDecimal.min(4), TestDecimal.rnd(4), TestDecimal.max(4)]},
            name(TERADATA_TYPE_INTERVAL_DS):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_INTERVAL_DS), TERADATA_TYPE_INTERVAL_DS,
                                    data_precision=4, data_scale=6),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_INTERVAL_DS), GLUENT_TYPE_INTERVAL_DS)},
            name(TERADATA_TYPE_INTERVAL_YM):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_INTERVAL_YM), TERADATA_TYPE_INTERVAL_YM, data_precision=4),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_INTERVAL_YM), GLUENT_TYPE_INTERVAL_YM)},
            name(TERADATA_TYPE_NUMBER):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER), GLUENT_TYPE_DECIMAL)},
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_1):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_1), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_1), GLUENT_TYPE_INTEGER_1),
                 'offload_options': {'integer_1_columns_csv': name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_1)},
                 'literals': [TestDecimal.min(2), TestDecimal.rnd(2), TestDecimal.max(2)]},
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_2):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_2), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_2), GLUENT_TYPE_INTEGER_2),
                 'offload_options': {'integer_2_columns_csv': name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_2)},
                 'literals': [TestDecimal.min(4), TestDecimal.rnd(4), TestDecimal.max(4)]},
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_4):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_4), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_4), GLUENT_TYPE_INTEGER_4),
                 'offload_options': {'integer_4_columns_csv': name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_4)},
                 'literals': [TestDecimal.min(9), TestDecimal.rnd(9), TestDecimal.max(9)]},
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_8):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_8), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_8), GLUENT_TYPE_INTEGER_8),
                 'offload_options': {'integer_8_columns_csv': name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_8)},
                 'literals': [TestDecimal.min(18), TestDecimal.rnd(18), TestDecimal.max(18)]},
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_38):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_38), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_38), GLUENT_TYPE_INTEGER_38),
                 'offload_options': {'integer_38_columns_csv': name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_INTEGER_38)},
            ...
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DOUBLE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DOUBLE), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DOUBLE), GLUENT_TYPE_DOUBLE),
                 'offload_options': {'double_columns_csv': name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DOUBLE)},
                 'literals': [1.5, 2.5, 3.5]},
            name(TERADATA_TYPE_NUMBER, '2'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, '2'), TERADATA_TYPE_NUMBER,
                                    data_precision=2, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, '2'), GLUENT_TYPE_INTEGER_1)},
            name(TERADATA_TYPE_NUMBER, '4'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, '4'), TERADATA_TYPE_NUMBER,
                                    data_precision=4, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, '4'), GLUENT_TYPE_INTEGER_2)},
            name(TERADATA_TYPE_NUMBER, '9'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, '9'), TERADATA_TYPE_NUMBER,
                                    data_precision=9, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, '9'), GLUENT_TYPE_INTEGER_4)},
            name(TERADATA_TYPE_NUMBER, '18'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, '18'), TERADATA_TYPE_NUMBER,
                                    data_precision=18, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, '18'), GLUENT_TYPE_INTEGER_8)},
            name(TERADATA_TYPE_NUMBER, '19'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, '19'), TERADATA_TYPE_NUMBER,
                                    data_precision=19, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, '19'), GLUENT_TYPE_INTEGER_38, data_precision=19)},
            name(TERADATA_TYPE_NUMBER, str(max_decimal_integral_magnitude)):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, str(max_decimal_integral_magnitude)),
                                    TERADATA_TYPE_NUMBER, data_precision=max_decimal_integral_magnitude, data_scale=0),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, str(max_decimal_integral_magnitude)),
                                     GLUENT_TYPE_INTEGER_38)},
            name(TERADATA_TYPE_NUMBER, str(max_precision), str(generic_scale)):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, str(max_precision), str(generic_scale)),
                                    TERADATA_TYPE_NUMBER, data_precision=max_precision, data_scale=generic_scale),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, str(max_precision), str(generic_scale)),
                                     GLUENT_TYPE_DECIMAL, data_precision=max_precision, data_scale=generic_scale)},
            name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DECIMAL, '10', '3'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DECIMAL, '10', '3'), TERADATA_TYPE_NUMBER),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DECIMAL, '10', '3'),
                                     GLUENT_TYPE_DECIMAL, data_precision=10, data_scale=3),
                 'offload_options': {
                     'decimal_columns_csv_list': [name(TERADATA_TYPE_NUMBER, GLUENT_TYPE_DECIMAL, '10', '3')],
                     'decimal_columns_type_list': ['10,3']},
            ...
            name(TERADATA_TYPE_NUMBER, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3'):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_NUMBER, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3'),
                                    TERADATA_TYPE_NUMBER, data_precision=9, data_scale=2),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_NUMBER, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3'),
                                     GLUENT_TYPE_DECIMAL, data_precision=10, data_scale=3),
                 'offload_options': {
                     'decimal_columns_csv_list': [name(TERADATA_TYPE_NUMBER, '9', '2', GLUENT_TYPE_DECIMAL, '10', '3')],
                     'decimal_columns_type_list': ['10,3']},
                 'literals': [TestDecimal.min(9, 2), TestDecimal.rnd(9, 2), TestDecimal.max(9, 2)]
                 },
            name(TERADATA_TYPE_SMALLINT):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_SMALLINT), TERADATA_TYPE_SMALLINT),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_SMALLINT), GLUENT_TYPE_INTEGER_4)},
            name(TERADATA_TYPE_SMALLINT, GLUENT_TYPE_INTEGER_4):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_SMALLINT, GLUENT_TYPE_INTEGER_4), TERADATA_TYPE_SMALLINT),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_SMALLINT, GLUENT_TYPE_INTEGER_4), GLUENT_TYPE_INTEGER_4),
                 'offload_options': {'integer_4_columns_csv': name(TERADATA_TYPE_SMALLINT, GLUENT_TYPE_INTEGER_4)},
                 'literals': [TestDecimal.min(4), TestDecimal.rnd(4), TestDecimal.max(4)]},
            name(TERADATA_TYPE_TIME):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_TIME), TERADATA_TYPE_TIME),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_TIME), GLUENT_TYPE_TIME)},
            name(TERADATA_TYPE_TIMESTAMP):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_TIMESTAMP), TERADATA_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_TIMESTAMP), GLUENT_TYPE_TIMESTAMP)},
            name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_DATE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_DATE), TERADATA_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_DATE), GLUENT_TYPE_DATE),
                 'offload_options': {'date_columns_csv': name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_DATE)},
                 # Including 1970-01-01 because Python datetime64 understands this as False due to being Unix epoch.
                 'literals': [datetime.datetime(1970, 1, 1), datetime.datetime(1970, 1, 2),
                              datetime.datetime(1971, 2, 1), datetime.datetime(1971, 2, 2)]},
            name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP_TZ):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP_TZ), TERADATA_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP_TZ),
                                     GLUENT_TYPE_TIMESTAMP_TZ),
                 'offload_options': {'timestamp_tz_columns_csv':
                     name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_TIMESTAMP_TZ)}},
            name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_VARIABLE_STRING):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_VARIABLE_STRING), TERADATA_TYPE_TIMESTAMP),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_VARIABLE_STRING),
                                     GLUENT_TYPE_VARIABLE_STRING),
                 'offload_options': {'variable_string_columns_csv':
                     name(TERADATA_TYPE_TIMESTAMP, GLUENT_TYPE_VARIABLE_STRING)}},
            name(TERADATA_TYPE_TIMESTAMP_TZ):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_TIMESTAMP_TZ), TERADATA_TYPE_TIMESTAMP_TZ),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_TIMESTAMP_TZ), GLUENT_TYPE_TIMESTAMP_TZ)},
            name(TERADATA_TYPE_VARBYTE):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_VARBYTE), TERADATA_TYPE_VARBYTE, data_length=30),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_VARBYTE), GLUENT_TYPE_BINARY)},
            name(TERADATA_TYPE_VARCHAR):
                {'column': TeradataColumn(name(TERADATA_TYPE_VARCHAR), TERADATA_TYPE_VARCHAR, data_length=30),
                 'expected_canonical_column': CanonicalColumn(name(TERADATA_TYPE_VARCHAR), GLUENT_TYPE_VARIABLE_STRING),
                 'ascii_only': ascii_only,
                 'notnull': all_chars_notnull},
            name(TERADATA_TYPE_VARCHAR, test_constants.UNICODE_NAME_TOKEN):
                {'column':
                     TeradataColumn(name(TERADATA_TYPE_VARCHAR, test_constants.UNICODE_NAME_TOKEN),
                                    TERADATA_TYPE_VARCHAR, data_length=30),
                 'expected_canonical_column':
                     CanonicalColumn(name(TERADATA_TYPE_VARCHAR, test_constants.UNICODE_NAME_TOKEN),
                                     GLUENT_TYPE_VARIABLE_STRING),
                 'offload_options': {'unicode_string_columns_csv':
                     name(TERADATA_TYPE_VARCHAR, test_constants.UNICODE_NAME_TOKEN)},
                 'ascii_only': ascii_only,
                 'notnull': all_chars_notnull},
        }
    ...
    def _gl_types_column_definitions(self, ascii_only=False, all_chars_notnull=False,
        ...
        if include_interval_columns:
            ...
            column_list.extend([
                ...
                {'column': TeradataColumn(name(), TERADATA_TYPE_INTERVAL_DS, data_precision=4, data_scale=6)},
            ])
        column_list.extend([
    ...
    def _gl_wide_column_definitions(self, ascii_only=False, all_chars_notnull=False,
        ...
        column_list.extend(
            {'column': TeradataColumn(name(), TERADATA_TYPE_NUMBER, data_precision=9)} for _ in range(5)
        )
        column_list.extend(
        ...
        column_list.extend(
            {'column': TeradataColumn(name(), TERADATA_TYPE_TIMESTAMP, data_scale=0)} for _ in range(5)
        )

        extra_column_count = gl_wide_max_columns(self, backend_max_test_column_count) - 20
    ...
    def _populate_generated_test_table(self, schema, table_name, columns, rows, fastexecute=True):
        ...
        if fastexecute:
            ...
            inputsize_datatypes = [TERADATA_TYPE_INTERVAL_DS, TERADATA_TYPE_INTERVAL_YM, TERADATA_TYPE_BLOB,
                                   TERADATA_TYPE_CLOB, TERADATA_TYPE_VARBYTE, TERADATA_TYPE_BYTE]
            inputsizes = []
            if any(map(lambda x: x['column'].data_type in inputsize_datatypes, columns)):
                for i, col in enumerate(columns):
                    if col['column'].data_type in inputsize_datatypes:
                        ...
                        inputsize = (SQL_WVARCHAR, max(map(lambda x: len(x[i]) if x[i] else 0, data)), 0)
                    else:
                        inputsize = None
    ...
    def gl_type_mapping_generated_table_col_specs(self, max_backend_precision, max_backend_scale,
        ...
        raise NotImplementedError('Teradata gl_type_mapping_generated_table_col_specs() not yet implemented')
    ...
    def host_compare_sql_projection(self, column_list: list) -> str:
        ...
        for column in column_list:
            if column.data_type == TERADATA_TYPE_TIMESTAMP_TZ:
                ...
                projection.append("TO_CHAR({}, 'YYYY-MM-DD HH24:MI:SS.FF3')||' +00:00'".format(
                    self._db_api.enclose_identifier(column.name)))
                ...
                if column.is_nan_capable():
                    ...
                    projection.append("CASE WHEN CAST({} AS VARCHAR(1)) = '*' THEN 'NaN' ELSE TO_CHAR({}) END".format(
                        self._db_api.enclose_identifier(column.name), self._db_api.enclose_identifier(column.name)))
                else:
    ...
    def run_tpt_file(self, local_path, schema):
        self._log(f'Running {local_path}')
        tpt_job_variables = ["SourceSchema='%s'" % schema,
        ...
        cmd = ['tbuild', '-f', local_path, '-u', ','.join([j for j in tpt_job_variables])]
    ...
    def sales_based_fact_create_ddl(self, schema: str, table_name: str, maxval_partition: bool=False,
        ...
        assert part_key_type in [TERADATA_TYPE_NUMBER, TERADATA_TYPE_BIGINT, TERADATA_TYPE_VARCHAR,
                                 TERADATA_TYPE_DATE, TERADATA_TYPE_TIMESTAMP], \
        ...
        if part_key_type in (TERADATA_TYPE_NUMBER, TERADATA_TYPE_BIGINT):
            ...
            sql_params.update({'time_id_expr': "CAST(TO_CHAR(time_id,'YYYYMMDD') AS INTEGER)",
                               'step': '1'})
            ...
            sql_params.update({'time_id_expr': 'time_id',})
            ...
            part_literal_hv1 = SALES_BASED_FACT_PRE_LOWER if noseg_partition else SALES_BASED_FACT_PRE_HV
            part_literal_hv2 = SALES_BASED_FACT_HV_5_END if maxval_partition else SALES_BASED_FACT_HV_6_END
            sql_params.update({'time_id_expr': "CAST(time_id AS TIMESTAMP(0))",})
        else:
            # VC
            part_literal_template1 = part_literal_template2 = """'%s'"""
            part_literal_hv1 = SALES_BASED_FACT_PRE_LOWER_NUM if noseg_partition else SALES_BASED_FACT_PRE_HV_NUM
    ...
    def sales_based_fact_drop_partition_ddl(self, schema: str, table_name: str,
        ...
        WITH DELETE""".format(schema, table_name, range_start_hv, range_end_hv)
        return [sql]
    ...
    def sales_based_fact_truncate_partition_ddl(self, schema: str, table_name: str,
        ...
        partition_names = [_.partition_name
    ...
    def sales_based_fact_late_arriving_data_sql(self, schema: str, table_name: str, time_id_literal: str) -> list:
        ...
        FROM   %(schema)s.sales""" % {'schema': schema, 'table_name': table_name, 'time_id': time_id_literal}
        return [ins]
    ...
    def sales_based_list_fact_create_ddl(self, schema: str, table_name: str, default_partition: bool=False,
        ...
        assert part_key_type in [TERADATA_TYPE_NUMBER, TERADATA_TYPE_VARCHAR,
                                 TERADATA_TYPE_DATE, TERADATA_TYPE_TIMESTAMP], \
            'Unsupported part_key_type: %s' % part_key_type
        ...
        if part_key_type == TERADATA_TYPE_NUMBER:
            ...
            params.update({'yrmon_expr': "CAST(TRUNC(time_id,'MM') AS TIMESTAMP(0))",
    ...
    def sales_based_list_fact_late_arriving_data_sql(self, schema: str, table_name: str,
        ...
        FROM   %(schema)s.sales
        WHERE  time_id = DATE '%(yrmon)s'""" % {'schema': schema, 'table_name': table_name,
