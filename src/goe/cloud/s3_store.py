...
class S3Store(GOEHdfsLayout, JMESPathSearch, object):
    ...
    def __init__(self, cfg, section, prefix="/"):
        ...
        options = self._cfg.extract_options(
            self._cfg_section, ("s3_bucket", "s3_prefix")
        )
        ...
        logger.debug(
            "S3Store() object successfully initialized for bucket: '%s', prefix: '%s'"
    ...
    def __repr__(self):
        return "[section={section}]: s3://{bucket}/{prefix}".format(
            section=self._cfg_section, bucket=self._bucket, prefix=self._prefix
        )
    ...
    def _prefix_exists(self):
        ...
        items = [
            _ for _ in self._get_key_iterator(config={"MaxItems": 1, "PageSize": 1})
        ]
        exists = True if items else False

        logger.debug(
            "Prefix: s3://%s/%s exists: %s" % (self._bucket, self._prefix, exists)
        )
        return exists
    ...
    def _get_page_iterator(self, config=None):
        ...
        """
        logger.debug(
            "Getting keys for S3 bucket: %s starting from prefix: %s"
            % (self._bucket, self._prefix)
        )
        ...
        if self._bucket_exists():
            ...
            args = {"Bucket": self._bucket, "Prefix": self._prefix}
            if config:
    ...
    def _select_files_only(self, keys):
        ...
        reMeta = re.compile(
            "^%s%s/" % (end_by(self._prefix, "/"), METANOTE_FOLDER)
        )  # Meta folder (ignored)
    ...
    def create(self):
        """Create S3 prefix"""
        logger.debug(
            "S3 does not need to pre-create 'prefix', it will be done 'automatically'"
        )
    ...
    def delete(self, file_list):
        ...
        for record_range in chunk_list(file_list, MAX_S3_DELETE_CHUNK):
            delete_objects = {
                "Objects": [
                    {"Key": end_by(self._prefix, "/") + _} for _ in record_range
                ]
            }
