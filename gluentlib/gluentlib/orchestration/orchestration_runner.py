...
from gluent import OffloadOperation, \
    check_and_set_nls_lang, get_log_fh, get_log_fh_name, get_offload_target_table, \
    init, init_redis_execution_id, init_log, offload_table
...
class OrchestrationRunner:
    ...
    def __init__(self, config_overrides=None, dry_run=False, suppress_stdout=False):
        self._config = self._gen_config(config_overrides, dry_run, suppress_stdout=suppress_stdout)
    ...
    def _build_offload_source_table(self, operation):
        return OffloadSourceTable.create(operation.owner, operation.table_name, self._config, self._messages)

    def _build_repo_client(self, messages) -> "OrchestrationRepoClientInterface":
        return orchestration_repo_client_factory(self._config, messages, dry_run=bool(not self._config.execute))
    ...
    def _cleanup_objects(self, repo_client, frontend_table=None, backend_table=None):
        try:
            ...
            self._log_error('Exception cleaning up connections: {}'.format(str(exc)), detail=VVERBOSE)

    def _command_begin(self, command_type: str, params, repo_client: "OrchestrationRepoClientInterface",
                       operation=None) -> int:
        assert command_type
        assert params
        assert repo_client
        ...
        try:
            ...
            command_id = repo_client.start_command(self._execution_id, command_type, get_log_fh_name(),
                                                   command_input, operation_dict)
            self._log(f'Command id for execution {self._execution_id}: {command_id}', detail=VVERBOSE)
        except Exception as exc:
            self._log_error(f'Exception starting command {self._execution_id}: {str(exc)}', detail=VVERBOSE)
            self._log('Exception stack: {}'.format(traceback.format_exc()), detail=VVERBOSE)
            raise
        return command_id
    ...
    def _command_end(self, command_id: int, repo_client: "OrchestrationRepoClientInterface"):
        ...
        try:
            ...
            self._log_error(f'Exception closing command {command_id}: {str(exc)}', detail=VVERBOSE)
            self._log('Exception stack: {}'.format(traceback.format_exc()), detail=VVERBOSE)
            raise

    def _command_fail(self, command_id: int, external_exc: Exception, repo_client: "OrchestrationRepoClientInterface"):
        assert repo_client
        self._log_error(f'Failing command due to exception: {str(external_exc)}', detail=VVERBOSE)
        self._log('External exception stack: {}'.format(traceback.format_tb(external_exc.__traceback__)),
                  detail=VVERBOSE)
        try:
            repo_client.end_command(command_id, orchestration_constants.COMMAND_ERROR)
        except Exception as local_exc:
            self._log_error(f'Exception failing command {command_id}: {str(local_exc)}', detail=VVERBOSE)
            self._log('Exception stack: {}'.format(traceback.format_exc()), detail=VVERBOSE)
            raise

    def _gen_config(self, config_overrides, dry_run, suppress_stdout=False) -> OrchestrationConfig:
    ...
    def _gen_messages(self, execution_id, command_type):
        return OffloadMessages.from_options(self._config, log_fh=get_log_fh(), execution_id=execution_id,
                                            cache_enabled=orchestration_defaults.cache_enabled(),
                                            command_type=command_type)

    def _gen_offload_operation(self, params, repo_client: "OrchestrationRepoClientInterface"):
        ...
        try:
            if isinstance(params, dict):
                ...
                op = OffloadOperation.from_dict(params, self._config, self._messages,
                ...
                op = OffloadOperation.from_options(params, self._config, self._messages,
                                                   repo_client=repo_client, execution_id=self._execution_id,
                                                   max_hybrid_name_length=self._get_max_hybrid_identifier_length())
            return op
        except Exception as exc:
            self._log_error(f'Exception generating offload operation {self._execution_id}: {str(exc)}', detail=VVERBOSE)
            self._log('Exception stack: {}'.format(traceback.format_exc()), detail=VVERBOSE)
            raise

    def _get_execution_id(self, execution_id: Optional[ExecutionId]=None) -> ExecutionId:
    ...
    def _get_max_hybrid_identifier_length(self) -> int:
        ...
        if self._max_hybrid_name_length is None:
            frontend_api = frontend_api_factory(self._config.db_type, self._config, self._messages,
    ...
    def _init_command(self, command: str, params, execution_id: Optional[ExecutionId]=None,
                      reuse_log: bool=False, messages_override: Optional[OffloadMessages]=None
        ...
        try:
            ...
            self._log('{} ExecutionId: {}'.format('Using overridden' if execution_id else 'Generated', self._execution_id),
                      detail=VVERBOSE)
            init_redis_execution_id(self._execution_id)
            return self._build_repo_client(self._messages)
        except Exception as exc:
            self._log_error(f'Exception initializing command {command}: {str(exc)}', detail=VVERBOSE)
            self._log('Exception stack: {}'.format(traceback.format_exc()), detail=VVERBOSE)
            raise
    ...
    def _log_final_messages(self, command_type, repo_client=None):
        ...
        if self._messages.get_messages():
            self._messages.offload_step(
                command_steps.STEP_MESSAGES, self._messages.log_messages,
                command_type=command_type, execute=self._config.execute
            )

    def _offload(self, operation, offload_source_table, offload_target_table):
        with orchestration_lock_for_table(offload_source_table.owner, offload_source_table.table_name,
                                          dry_run=bool(not self._config.execute)):
            try:
                return offload_table(self._config, operation, offload_source_table,
                                     offload_target_table, self._messages)
            except Exception as exc:
                try:
                    self._log_error('Unhandled exception in offload_table(): {}'.format(str(exc)))
    ...
    def offload(self, params, execution_id: Optional[ExecutionId]=None,
                reuse_log: bool=False, messages_override: Optional[OffloadMessages]=None):
        """
        ...
        messages_override: Allows us to pass in an existing messages object so a parent can inspect the messages,
                           used for testing.
        """
        repo_client = self._init_command(orchestration_constants.COMMAND_OFFLOAD, params,
        ...
        try:
            ...
            self._log('Offloading to {}'.format(self._target_version_string(offload_target_table)))

            status = self._offload(operation, offload_source_table, offload_target_table)
            self._log_final_messages(orchestration_constants.COMMAND_OFFLOAD, repo_client=repo_client)

            self._command_end(command_id, repo_client)
            self._cleanup_objects(repo_client, frontend_table=offload_source_table, backend_table=offload_target_table)
        except Exception as exc:
    ...
    def offload_join(self, params, execution_id: Optional[ExecutionId]=None,
                     reuse_log: bool=False, messages_override: Optional[OffloadMessages]=None):
        """
        Run an offload join using incoming params.
        See offload() for parameter descriptions.
        """
        repo_client = self._init_command(orchestration_constants.COMMAND_OFFLOAD_JOIN, params,
                                         execution_id=execution_id,
                                         reuse_log=reuse_log,
                                         messages_override=messages_override)

        check_and_set_nls_lang(self._config, self._messages)
        command_id = self._command_begin(orchestration_constants.COMMAND_OFFLOAD_JOIN, params, repo_client)
        try:
    ...
    def present(self, params, execution_id: Optional[ExecutionId]=None,
                reuse_log: bool=False, messages_override: Optional[OffloadMessages]=None):
        """
        ...
        messages_override: Allows us to pass in an existing messages object so a parent can inspect the messages,
                           used for testing.
        """
        repo_client = self._init_command(orchestration_constants.COMMAND_PRESENT, params,
                                         execution_id=execution_id,
                                         reuse_log=reuse_log,
                                         messages_override=messages_override)

        command_id = self._command_begin(orchestration_constants.COMMAND_PRESENT, params, repo_client)
        try:
    ...
    def present_join(self, params, execution_id: Optional[ExecutionId]=None,
                     reuse_log: bool=False, messages_override: Optional[OffloadMessages]=None):
        """
        Run a present join using incoming params.
        """
        repo_client = self._init_command(orchestration_constants.COMMAND_PRESENT_JOIN, params,
                                         execution_id=execution_id,
                                         reuse_log=reuse_log,
                                         messages_override=messages_override)

        check_and_set_nls_lang(self._config, self._messages)
        command_id = self._command_begin(orchestration_constants.COMMAND_PRESENT_JOIN, params, repo_client)
        try:
            raise OrchestrationRunnerException('Offload Join no longer exists')
        except Exception as exc:
            self._command_fail(command_id, exc, repo_client)
    ...
    def schema_sync(self, params, execution_id: Optional[ExecutionId]=None,
                    reuse_log: bool=False, messages_override: Optional[OffloadMessages]=None):
        """
        Run Schema Sync using incoming params.
        """
        repo_client = self._init_command(orchestration_constants.COMMAND_SCHEMA_SYNC, params,
                                         execution_id=execution_id,
                                         reuse_log=reuse_log,
                                         messages_override=messages_override)
        ...
        try:
            status = schema_sync(params, self._messages, self._execution_id, repo_client)

            self._log_final_messages(orchestration_constants.COMMAND_SCHEMA_SYNC)
            self._command_end(command_id, repo_client)
