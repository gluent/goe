...
from gluentlib.util.misc_functions import timedelta_to_str, disable_terminal_colors, check_offload_env, \
...
from gluentlib.cloud.hive_cloud_present import HiveCloudPresent
...
def report_status(db_name, table_name, success, elapsed, args):
    ...
    if not success:
        log("CLOUD-PRESENT failed for table: %s.%s in: %s" % (db_name, table_name, elapsed_str), \
            ansi_code="red", options=args)
    elif STATUS_EVALUATION == success:
        log("CLOUD-PRESENT evaluation successful for table: %s.%s in: %s" % (db_name, table_name, elapsed_str), \
            ansi_code="green", options=args)
    elif STATUS_NOT_EXECUTED == success:
        log("CLOUD-PRESENT: no changes detected for table: %s.%s in: %s" % (db_name, table_name, elapsed_str), \
            ansi_code="yellow", options=args)
    else:
        log("CLOUD-PRESENT successful for table: %s.%s in: %s" % (db_name, table_name, elapsed_str), \
            ansi_code="green", options=args)
...
def report_present_summary(pre, db_name, table_name, args, changes_made):
    ...
    def print_section_subreport(report, section):
        log("\n%s part:\n\tPartitions: %d (%.2f%%)\n\tFiles: %d (%.2f%%)\n\tRows: %d (%.2f%%)\n\tTotal size: %s (%.2f%%)" % \
            (section.upper(), \
             report[section]['partitions'], (100. * report[section]['partitions'] / report['total']['partitions']), \
             report[section]['files'], (100. * report[section]['files'] / report['total']['files']), \
             report[section]['rows'], (100. * report[section]['rows'] / report['total']['rows']), \
             bytes_to_human_size(report[section]['size']), (100. * report[section]['size'] / report['total']['size']) \
            ), ansi_code='blue', options=args)
        log("\tRange: %s - %s" % (report[section]['min_ts'], report[section]['max_ts']), ansi_code='blue', options=args)
...
def report_projection(pre, section, section_type, filters, args):
    ...
    if projection and (projection['create'] > 0 or projection['relocate'] > 0):
        log("\nTo be moved to: %s\n\tPartitions: %d\n\tFiles: %d\n\tRows: %d\n\tSize: %s" % \
            (section_type.upper(), projection['partitions'], projection['files'], projection['rows'],
            bytes_to_human_size(projection['size'])), ansi_code='yellow', options=args)
        log("\tRange: %s - %s" % (projection['min_ts'], projection['max_ts']), ansi_code='yellow', options=args)
...
def do_cloud_present(db_name, table_name, cfg, args, messages):
    ...
    log('\nProcessing table: %s.%s' % (db_name, table_name), ansi_code='green', options=args)
    ...
    try:
        if pre.table_exists:
            ...
            targets = [
                (args.remote_config_section, cfg.section_type(args.remote_config_section), make_older_than_jmespath(args, '<')),
                (args.local_config_section, cfg.section_type(args.local_config_section), make_older_than_jmespath(args, '>='))
            ]
            ...
            for item in targets:
                ...
                if execution_needed and args.remap:
                    log('\nProcessing %s partitions for table: %s.%s' % (typ.upper(), db_name, table_name), \
                        ansi_code='blue', options=args)
            ...
            if executed:
                report_present_summary(pre, db_name, table_name, args, changes_made=True)
        else:
            raise CloudPresentException("Table: %s.%s does NOT exist" % (db_name, table_name))
    except Exception as e:
        msg = "Exception: '%s' detected for table: %s.%s" % \
...
def parse_args(cfg):
    ...
    parser.add_argument('-S', '--local-config-section', required=False, default=DEFAULT_SRC, \
        help="Config file section that represents HDFS location. Default: %s" % DEFAULT_SRC)
    parser.add_argument('-D', '--remote-config-section', required=False, default=DEFAULT_DST, \
        help="Config file section that represents CLOUD location. Default: %s" % DEFAULT_DST)
    parser.add_argument('-I', '--impala-config-section', required=False, default=None, \
    ...
    storage_divider.add_argument('--older-than-days', type=int, \
    ...
    storage_divider.add_argument('--older-than-date', \
...
def main():
    ...
    # Main loop
    try:
        process_all_databases(cfg, args, OffloadMessages.from_options_dict(vars(args), log_fh=get_default_log()))
