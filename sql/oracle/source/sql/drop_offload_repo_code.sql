...
prompt Dropping old repo code...

SET SERVEROUTPUT ON

DECLARE

    c_schema        CONSTANT VARCHAR2(128) := UPPER('&gluent_db_repo_user');

    v_packages      SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL('OFFLOAD_REPO');
    v_types         SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL('OFFLOAD_METADATA_OT', 'OFFLOAD_PARTITION_OT', 'OFFLOAD_PARTITION_NTT');
    v_functions     SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL();
    v_procedures    SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL();
    v_contexts      SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL();
    v_views         SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL('OFFLOAD_METADATA_V');
    v_tables        SYS.DBMS_DEBUG_VC2COLL := SYS.DBMS_DEBUG_VC2COLL();
