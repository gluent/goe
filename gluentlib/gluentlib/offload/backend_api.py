...
from gluentlib.filesystem.gluent_dfs import OFFLOAD_FS_SCHEME_S3A, OFFLOAD_FS_SCHEME_WASB, \
...
from gluentlib.offload.column_metadata import ColumnMetadataInterface,\
    get_column_names, is_synthetic_bucket_column, is_synthetic_partition_column, match_table_column
from gluentlib.offload.offload_messages import VERBOSE, VVERBOSE
from gluentlib.offload.offload_constants import CAPABILITY_BUCKET_HASH_COLUMN, CAPABILITY_CANONICAL_DATE,\
    ...
    CAPABILITY_SENTRY, CAPABILITY_SORTED_TABLE, \
    CAPABILITY_SORTED_TABLE_MODIFY, CAPABILITY_SQL_MICROSECOND_PREDICATE,\
    CAPABILITY_SYNTHETIC_BUCKETING, CAPABILITY_SYNTHETIC_PARTITIONING,\
    CAPABILITY_TABLE_STATS_COMPUTE, CAPABILITY_TABLE_STATS_GET, \
    CAPABILITY_TABLE_STATS_SET, DBTYPE_BIGQUERY, DBTYPE_IMPALA, \
    DBTYPE_HIVE, DBTYPE_SNOWFLAKE, DBTYPE_SPARK, DBTYPE_SYNAPSE
from gluentlib.offload.offload_transport_functions import run_os_cmd
from gluentlib.util.misc_functions import str_summary_of_self
from gluentlib.util.password_tools import PasswordTools


class BackendApiException(Exception): pass
class BackendStatsException(Exception): pass
class MissingSequenceTableException(Exception): pass
class BackendApiConnectionException(Exception): pass
...
VALID_REMOTE_DB_TYPES = [DBTYPE_BIGQUERY, DBTYPE_HIVE, DBTYPE_IMPALA, DBTYPE_SNOWFLAKE, DBTYPE_SPARK, DBTYPE_SYNAPSE]
...
class BackendApiInterface(metaclass=ABCMeta):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False,
        ...
        self._global_session_parameters = self._gen_global_session_parameters(
            connection_options.backend_session_parameters, log_clashes=bool(not do_not_connect)
        )
    ...
    def _apply_decimal_padding_digits(self, column_name, data_precision, data_scale, decimal_padding_digits):
        ...
        if data_precision is None or data_scale is None:
            self._log('No padding applied to %s for precision/scale: %s/%s'
                      % (column_name, data_precision, data_scale), detail=VVERBOSE)
            return data_precision, data_scale
        ...
        if new_precision != data_precision or new_scale != data_scale:
            self._log('Padded precision/scale for %s in line with DECIMAL_PADDING_DIGITS=%s: %s,%s -> %s,%s'
                      % (column_name, decimal_padding_digits, data_precision, data_scale, new_precision, new_scale),
                      detail=VVERBOSE)
    ...
    def _create_table_columns_clause_common(self, column_list, external=False):
        ...
        return '    ' + '\n,   '.join([col_template % (_[0], _[1], _[2]) for _ in sql_cols])
    ...
    def _format_select_projection(self, select_expr_tuples):
        ...
        return '\n,      '.join('{} AS {}'.format(e, self.enclose_identifier(n)) for e, n in select_expr_tuples)
    ...
    def _gen_max_column_values_sql(self, db_name, table_name, column_name_list, filter_clause=None,
        ...
        if len(column_name_list) == 1:
            ...
            sql = 'SELECT %s FROM %s%s GROUP BY %s ORDER BY %s LIMIT 1' \
                % (proj_cols, self.enclose_object_reference(db_name, table_name), where, cols, order_by)
        return sql
    ...
    def _gen_global_session_parameters(self, backend_session_parameters, log_clashes=True):
        ...
        if self._fixed_session_parameters():
            ...
            for key_to_delete in [_ for _ in list(global_session_parameters.keys()) if _.lower() in fixed_keys]:
                if log_clashes:
                    self._log('Removing OFFLOAD_BACKEND_SESSION_PARAMETERS[%s] because of fixed parameter clash'
                              % key_to_delete, detail=VVERBOSE)
    ...
    def _gen_insert_select_sql_assertions(self, db_name, table_name, from_db_name, from_table_name, select_expr_tuples,
    ...
    def _gen_sql_text_common(self, db_name, table_name, column_names=None, filter_clauses=None, measures=None, agg_fns=None):
        ...
        group_by_clause = ('\nGROUP BY ' + '\n,        '.join([self.enclose_identifier(_)
                                                               for _ in column_names])) if measures and column_names else ''
        order_by_clause = ('\nORDER BY ' + '\n,        '.join([self.enclose_identifier(_)
                                                               for _ in column_names])) if agg_fns and column_names else ''
...
FROM   %(db)s.%(table)s%(where_clause)s%(group_by)s%(order_by)s""" \
    ...
    def _gen_select_count_sql_text_common(self, db_name, table_name, filter_clause=None):
        ...
        return 'SELECT COUNT(*) FROM %s%s' % (self.enclose_object_reference(db_name, table_name), where_clause)

    def _get_max_column_values_common(self, db_name, table_name, column_name_list, columns_to_cast_to_string=None,
                                      optimistic_prune_clause=None, not_when_dry_running=False, query_options=None):
        ...
        sql = self._gen_max_column_values_sql(db_name, table_name, column_name_list,
                                              columns_to_cast_to_string=columns_to_cast_to_string)
        max_row = None
        if len(column_name_list) == 1:
            if optimistic_prune_clause:
                hwm_sql = self._gen_max_column_values_sql(db_name, table_name, column_name_list,
                                                          filter_clause=optimistic_prune_clause,
                                                          columns_to_cast_to_string=columns_to_cast_to_string)
                max_row = self.execute_query_fetch_one(hwm_sql, query_options=query_options,
                                                       not_when_dry_running=not_when_dry_running, log_level=VVERBOSE)

            if not max_row or max_row[0] is None:
                max_row = self.execute_query_fetch_one(sql, query_options=query_options,
                                                       not_when_dry_running=not_when_dry_running, log_level=VVERBOSE)
    ...
    def _insert_literals_using_insert_values_sql_text(self, db_name, table_name, column_names, literal_csv_list,
                                                      split_by_cr=True):
    ...
    def _run_os_cmd(self, cmd):
        ...
        """
        return run_os_cmd(cmd, self._messages, self._connection_options, dry_run=self._dry_run, silent=True)
    ...
    def _table_partition_info(self, partition_id, data_format=None, num_rows=None, size_in_bytes=None):
        ...
        """
        return {'data_format': data_format, 'num_rows': num_rows,
                'partition_id': partition_id, 'size_in_bytes': size_in_bytes}

    def _unsupported_decimal_precision_scale(self, data_precision, data_scale):
        return ((data_scale is not None and (data_scale < 0 or data_scale > self.max_decimal_scale()))
    ...
    def decimal_scale_validation_expression(self, column_name, column_scale):
        ...
        extract_scale_expr = self.regexp_extract_sql_expression(
            column_name,
            self.regexp_extract_decimal_scale_pattern()
        )
    ...
    def execute_ddl(self, sql, sync=None, query_options=None, log_level=VERBOSE, profile=None, no_log_items=None):
        ...
        """
        no_log_items = self._validate_no_log_items(no_log_items)
        return_sqls = self._execute_ddl_or_dml(sql, sync=sync, query_options=query_options, log_level=log_level,
                                               profile=profile, no_log_items=no_log_items)
        assert return_sqls is not None
        return return_sqls

    def execute_dml(self, sql, sync=None, query_options=None, log_level=VERBOSE, profile=None, no_log_items=None):
        no_log_items = self._validate_no_log_items(no_log_items)
        return_sqls = self._execute_ddl_or_dml(sql, sync=sync, query_options=query_options, log_level=log_level,
                                               profile=profile, no_log_items=no_log_items)
        assert return_sqls is not None
    ...
    def get_distinct_expressions(self, db_name, table_name, expression_list,
        ...
        projection = ','.join(['{} AS c{}'.format(_, i) for i, _ in enumerate(expression_list)])
        if order_results:
            aliases = ','.join(['c{}'.format(_) for _ in range(len(expression_list))])
            sql_template += ' ORDER BY {}'.format(aliases)
        sql = sql_template % {'projection': projection,
    ...
    def get_max_column_length(self, db_name, table_name, column_name):
        ...
        sql = 'SELECT MAX(%s) FROM %s' \
            % (self.length_sql_expression(self.enclose_identifier(column_name)), self.enclose_object_reference(db_name, table_name))
        row = self.execute_query_fetch_one(sql, log_level=VVERBOSE)
        return row[0] if row else None

    def get_non_synthetic_columns(self, db_name, table_name):
        return [_ for _ in self.get_columns(db_name, table_name)
                if not self.is_synthetic_partition_column(_) and not self.is_synthetic_bucket_column(_)]
    ...
    @abstractmethod
    def insert_literal_values(self, db_name, table_name, literal_list, column_list=None,
                              max_rows_per_insert=250, split_by_cr=True):
