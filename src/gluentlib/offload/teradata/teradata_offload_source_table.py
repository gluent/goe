...
from gluentlib.offload.teradata.teradata_column import TeradataColumn,\
    TERADATA_TYPE_BIGINT, TERADATA_TYPE_BLOB, TERADATA_TYPE_BYTE, TERADATA_TYPE_BYTEINT,\
    TERADATA_TYPE_CHAR, TERADATA_TYPE_CLOB, TERADATA_TYPE_DATE,\
    TERADATA_TYPE_DECIMAL, TERADATA_TYPE_DOUBLE, TERADATA_TYPE_INTEGER,\
    TERADATA_TYPE_INTERVAL_YM, TERADATA_TYPE_INTERVAL_DS,\
    TERADATA_TYPE_NUMBER, TERADATA_TYPE_SMALLINT,\
    TERADATA_TYPE_TIME, TERADATA_TYPE_TIMESTAMP, TERADATA_TYPE_TIME_TZ, TERADATA_TYPE_TIMESTAMP_TZ,\
    TERADATA_TYPE_VARBYTE, TERADATA_TYPE_VARCHAR, TERADATA_TIMESTAMP_RE
from gluentlib.offload.teradata.teradata_frontend_api import teradata_get_primary_partition_expression
from gluentlib.offload.teradata.teradata_partition_expression import TeradataPartitionExpression, \
...
from gluentlib.util.misc_functions import chunk_list
...
SUPPORTED_RANGE_DATA_TYPES = [TERADATA_TYPE_DATE, TERADATA_TYPE_TIMESTAMP,
                              TERADATA_TYPE_BIGINT, TERADATA_TYPE_INTEGER, TERADATA_TYPE_SMALLINT, TERADATA_TYPE_NUMBER]
...
class TeradataSourceTable(OffloadSourceTableInterface):
    ...
    def __init__(self, schema_name, table_name, connection_options, messages,
        ...
        super().__init__(schema_name, table_name, connection_options, messages, dry_run=dry_run,
                         conn=conn, do_not_connect=do_not_connect)

        logger.info('TeradataSourceTable setup: (%s, %s, %s)'
    ...
    def _get_column_low_high_values(self, column_name, from_stats=True, sample_perc=1):
        ...
        if from_stats:
            ...
            raise NotImplementedError(f'Low/high value from statistics is not supported')
        else:
            q = dedent("""\
                SELECT MIN(%(col)s)
                ,      MAX(%(col)s)
                ...
                SAMPLE %(perc)s""") % {
                    ...
                    'col': column_name, 'perc': self._sample_data_types_data_sample_pct(sample_perc or 1)}
    ...
    def _get_primary_partition_expression(self) -> Optional[TeradataPartitionExpression]:
        ...
        if self._primary_partition_expression is None:
            try:
                ...
                self._log('Cannot retrieve partition expression, treating table as not partitioned: {}'.format(str(exc)),
                          detail=VVERBOSE)
    ...
    def _get_partitions(self, strict=True, populate_hvs=True) -> list:
        ...
        if partition_column.data_type in (TERADATA_TYPE_DATE, TERADATA_TYPE_TIMESTAMP):
            ...
            cte_branches = [range_n_periods_cte_template.format(start=start, end=end, interval=interval,
                                                                end_margin=margin)
            ...
            for start, end, interval in pe.ranges:
                ...
                row = self._db_api.execute_query_fetch_one(f'SELECT ({end}-{start})/{interval} AS n', log_level=VVERBOSE)
        ...
        for row in rows:
            ...
            hv_list = convert_high_values_to_python(self.partition_columns, [partition_high_value],
    ...
    def _get_primary_index_columns(self) -> list:
        ...
        if rows:
            assert len(set([_[0] for _ in rows])) == 1, 'Multiple primary indexes unexpected: {}'.format(
    ...
    def _sample_data_types_decimal_column(self, column, data_precision, data_scale):
        return TeradataColumn(column.name, TERADATA_TYPE_NUMBER, data_precision=data_precision,
    ...
    def decode_partition_high_values(self, hv_csv, strict=True) -> tuple:
        ...
        hv_list = convert_high_values_to_python(self.partition_columns, [hv_literal],
                                                self.partition_type, self, strict=strict)
    ...
    def from_canonical_column(self, column):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, char_length=None,
            ...
            return TeradataColumn(col.name.upper(), data_type, data_length=data_length, data_precision=data_precision,
    ...
    def gen_column(self, name, data_type, data_length=None, data_precision=None, data_scale=None, nullable=None,
        ...
        return TeradataColumn(name, data_type, data_length=data_length, data_precision=data_precision,
    ...
    def get_minimum_partition_key_data(self):
        ...
        min_sql = 'SELECT MIN({}) FROM {}'.format(self.enclose_identifier(self.partition_columns[0].name),
    ...
    def supported_data_types() -> set:
        return {
            ...
            TERADATA_TYPE_INTERVAL_YM, TERADATA_TYPE_INTERVAL_DS,
            TERADATA_TYPE_NUMBER, TERADATA_TYPE_SMALLINT,
            TERADATA_TYPE_TIME, TERADATA_TYPE_TIMESTAMP, TERADATA_TYPE_TIME_TZ,
            TERADATA_TYPE_TIMESTAMP_TZ, TERADATA_TYPE_VARBYTE, TERADATA_TYPE_VARCHAR
        }
    ...
    def to_rdbms_literal_with_sql_conv_fn(self, py_val, rdbms_data_type):
        ...
        if rdbms_data_type == TERADATA_TYPE_DATE:
            ...
            raise NotImplementedError('Teradata to_rdbms_literal_with_sql_conv_fn() data type %s not implemented.'
                                      % rdbms_data_type)
