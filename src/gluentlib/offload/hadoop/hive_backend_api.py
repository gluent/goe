...
from gluentlib.offload.offload_constants import DBTYPE_HIVE, FILE_STORAGE_FORMAT_AVRO, FILE_STORAGE_FORMAT_ORC,\
    FILE_STORAGE_FORMAT_PARQUET, HIVE_BACKEND_CAPABILITIES
...
from gluentlib.util.better_impyla import HDFS_NULL_PART_KEY_CONSTANT,\
...
HIVE_UDF_SPECS = [
    ('GLUENT_TZOFFSET_TO_TIMESTAMP', 'com.gluent.udf.TzOffsetStrToTimestamp', "'2015-01-01 12:13:14.56789 +02:00'"),
    ...
    ('GLUENT_TO_INTERNAL_NUMBER', 'com.gluent.udf.ToOracleInternalNumberRaw', 'CAST(1 AS TINYINT)'),
    ('GLUENT_TO_INTERNAL_DATE', 'com.gluent.udf.ToOracleInternalDateRaw', 'CAST(0 AS TIMESTAMP)'),
    ('GLUENT_TO_INTERNAL_DOUBLE', 'com.gluent.udf.ToOracleInternalDouble', 'CAST(1.1 AS DOUBLE)'),
    ('GLUENT_TO_INTERNAL_FLOAT', 'com.gluent.udf.ToOracleInternalFloat', 'CAST(1.1 AS FLOAT)'),
    ('GLUENT_ROW_RUN_LENGTH', 'com.gluent.udf.OracleRowRunLengthRaw', 'CAST(\'rowrow\' AS BINARY), 1'),
    ('GLUENT_FIELD_RUN_LENGTH', 'com.gluent.udf.OracleFieldRunLengthRaw', 'CAST(\'foo\' AS BINARY), 1'),
    ('GLUENT_UTF8_RUN_LENGTH', 'com.gluent.udf.OracleUtf8RunLengthRaw', 'CAST(\'bar\' AS BINARY), 1'),
...
class BackendHiveApi(BackendHadoopApi):
    ...
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False, do_not_connect=False):
        """ CONSTRUCTOR
        """
        super(BackendHiveApi, self).__init__(connection_options, backend_type, messages, dry_run=dry_run,
    ...
    def _create_hive_udf(self, function_name, function_class, function_library, udf_db=None):
        """ Hive
        """
        ...
        sql = "CREATE FUNCTION %s%s AS '%s' USING JAR '%s'" \
            % (db_clause, function_name, function_class, function_library)
 ...
 4) Invalid hiveserver2 authentication method is used. In non-kerberized environments Hive
    ...
    def _get_table_stats(self, hive_stats, as_dict=False, part_stats=False):
        ...
        if metastore and 'AWSGLUE' in metastore.upper():
            ...
            col_stats = True
        tab_stats, part_stats, col_stats = hive_stats.get_table_stats(self._backend_type, as_dict=as_dict,
    ...
    def backend_version(self):
        ...
        try:
            ...
            self._log('Assuming VERSION()=None because of exception: %s' % str(exc), detail=VVERBOSE)
            row = None
    ...
    def compute_stats(self, db_name, table_name, incremental=None, for_columns=False, partition_tuples=None, sync=None):
        ...
        sql = 'ANALYZE TABLE %s.%s%s COMPUTE STATISTICS%s' % (self.enclose_identifier(db_name), \
...
%(col_projection)s
    ...
    def gen_insert_select_sql_text(self, db_name, table_name, from_db_name, from_table_name, select_expr_tuples,
        ...
        dist_by_clause = ('\nDISTRIBUTE BY %s' % ','.join(self.enclose_identifier(n)
                                                          for n in distribute_columns)) if distribute_columns else ''

        insert_sql = """INSERT INTO %(db_table)s%(part_clause)s
SELECT %(proj)s
FROM   %(from_db_table)s%(where)s%(dist_by)s%(sort_by)s""" % {'db_table': self.enclose_object_reference(db_name, table_name),
                                                              'part_clause': part_clause,
                                                              'proj': projection,
    ...
    def get_missing_hive_table_stats(self, db_name, table_name, colstats=True, as_dict=False):
        ...
        return hive_stats.get_table_stats(self._backend_type, as_dict=as_dict, messages=self._messages, missing=True, colstats=colstats)
    ...
    def get_table_row_count(self, db_name, table_name, filter_clause=None,
        ...
        row = self.execute_query_fetch_one(sql, query_options=query_options, log_level=log_level, time_sql=True,
                                           not_when_dry_running=not_when_dry_running)
        return row[0] if row else None
    ...
    def supported_backend_data_types(self):
        return [HADOOP_TYPE_BOOLEAN, HADOOP_TYPE_BIGINT, HADOOP_TYPE_BINARY, HADOOP_TYPE_CHAR, HADOOP_TYPE_DATE,
                HADOOP_TYPE_DECIMAL, HADOOP_TYPE_DOUBLE, HADOOP_TYPE_DOUBLE_PRECISION, HADOOP_TYPE_FLOAT,
                HADOOP_TYPE_INT, HADOOP_TYPE_INTERVAL_DS, HADOOP_TYPE_INTERVAL_YM, HADOOP_TYPE_REAL, HADOOP_TYPE_STRING,
