...
# EXCEPTIONS
###############################################################################
class CloudToolException(Exception):
    pass
...
IGNORED_DATABASES = [
    re.compile(_, re.I) for _ in ("^_impala_builtins$", "^default$", "^\w+_load$")
]
...
def get_databases_and_tables(cfg, options):
    ...
    def from_manifest(manifest_file):
        ...
        with open(manifest_file) as source:
            for line in source:
                ...
                logger.debug(
                    "[MANIFEST] Found %s.%s database to process" % (db_name, table_name)
                )
                yield db_name, table_name

    def from_db(cfg, options):
        db_pattern, table_pattern = [
            re.compile("^%s$" % _.replace("*", ".*"), re.I)
            for _ in options.table.split(".")
        ]
        logger.debug(
            "[DB] Scanning databases for: %s and tables for: %s"
            % (db_pattern.pattern, table_pattern.pattern)
        )

        hive = HiveConnection.fromconnection(
            hs2_connection_from_section(options.local_config_section, cfg=cfg)
        )

        for db_name in sorted(hive.show_databases()):
            # Skip non-matching databases
            if any(_.match(db_name) for _ in IGNORED_DATABASES) or not db_pattern.match(
                db_name
            ):
            ...
            for table_name in sorted(hive.show_tables(db_name)):
                if table_pattern.match(table_name):
                    logger.debug(
                        "[DB] Found table to process: %s.%s" % (db_name, table_name)
                    )
                    yield db_name, table_name
                else:
    ...
    def from_literal(db_table_pattern):
        ...
        logger.debug(
            "[LITERAL] Extracting db_table: %s.%s literally" % (db_name, table_name)
        )
        yield db_name, table_name
...
def make_older_than_jmespath(
    options, comparison="<", ts_column="timestamp", ts_format="%Y-%m-%d"
):
