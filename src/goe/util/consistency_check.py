...
# EXCEPTIONS
############################################################
class RandomDataCheckException(Exception):
    pass
...
class Verboser(object):
    ...
    def __init__(self, log_dir=None, stdout_level=0):
        ...
        logger.debug(
            "Verboser() object successfully initialized. Log: %s, level: %d"
    ...
    def begin(self, db_pattern, table_pattern, probability, max_mbytes):
        self._write(
            "Start data integrity check for: %s.%s with probability: %.2f, max small mbs: %d"
            % (db_pattern, table_pattern, probability, max_mbytes),
            level=1,
            color="blue",
        )
        if self._log_file:
    ...
    def end(self):
        error_rate = self._errors * 100.0 / (self._processed if self._processed else 1)
        self._write("\nEnd data integrity check", level=1, color="blue")
        self._write(
            "Detected error rate: %.2f%% [%d out of %d]"
            % (error_rate, self._errors, self._processed),
            level=1,
            end="\n\n",
            color="blue",
        )
...
class RandomDataCheck(object):
    ...
    def _resolve_oracle_date(self, oracle_date):
        ...
        query_result = self._ora.execute(
    ...
    def _get_goe_partition_hwms(self, table_owners, table_names):
        ...
        query_result = self._ora.execute(
            sql, binds, cursor_fn=lambda c: c.fetchall(), as_dict=True
        )

        # Transform result to {(owner, table_name): hwm} dictionary
        hwms = {}
        for rec in query_result:
            hwms[(rec["OWNER"], rec["TABLE_NAME"])] = self._resolve_oracle_date(
                json.loads(rec["HWM"])["INCREMENTAL_HIGH_VALUE"]
            )
    ...
    def _generate_validation_sql(self, partitions):
        ...
        def make_where(part):
            ...
            if min_value:
                ...
                raise RandomDataCheckException(
                    "This combination of min: %s, max: %s is NOT supported"
                    % (min_value, max_value)
                )
    ...
    def _select_by_probability(self, partitions, probability):
        ...
        for part in partitions:
            ...
            if spot < probability:
                logger.debug(
                    "Partition: %s randomly accepted as: %f < %f"
                    % (part, spot, probability)
                )
                yield part
            else:
                logger.debug(
                    "Partition: %s randomly declinde as: %f >= %f"
                    % (part, spot, probability)
                )
    ...
    def check(self, db_name, table_name, probability, max_mbytes):
        ...
        selected_partitions = self._select_by_probability(
            itertools.chain(minned_partitions, small_tables), probability
        )
...
if __name__ == "__main__":
    ...
    def main():
        ...
        db_table, sample_pct, max_mbytes = (
            sys.argv[1],
            int(sys.argv[2]),
            int(sys.argv[3]),
        )
