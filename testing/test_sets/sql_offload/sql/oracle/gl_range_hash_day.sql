...
create table gl_range_hash_day
( id   integer
, data varchar2(30)
, dt   date
, num  number )
...
insert into gl_range_hash_day
   ( id, data, dt, num )
with pdates as (
   select to_date(substr(partition_name, 2), 'yyyymmdd') as dt
   from   all_tab_partitions
   where  table_owner = sys_context('userenv','current_schema')
   and    table_name  = 'GL_RANGE_HASH_DAY'
   )
select rownum
,      dbms_random.string('u',30)
,      dt
       /* rounding because no need to test extreme fractions here */
,      round(dbms_random.value * 1e20,6)
from   pdates
