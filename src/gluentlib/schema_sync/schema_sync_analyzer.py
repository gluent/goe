...
from gluentlib.offload.column_metadata import get_column_names
...
from gluentlib.util.misc_functions import double_quote_sandwich, trunc_with_hash
...
def ddl_compare(cursor, source_object, target_object):
    ...
    sql = """
        ...
        ,       last_ddl_time
        FROM    dba_objects
        WHERE   (owner,object_name,object_type) IN ({source},{target})""".format(source=source_object,
...
class SchemaSyncAnalyzerException(Exception): pass
...
class SchemaSyncAnalyzer(object):
    ...
    def __init__(self, options, messages):
        ...
        cx_conn = get_oracle_connection(
            self._orchestration_options.ora_adm_user, self._orchestration_options.ora_adm_pass,
            self._orchestration_options.rdbms_dsn, self._orchestration_options.use_oracle_wallet
        )
        self._ora_adm_conn = OracleQuery.fromconnection(cx_conn)
        self._backend_api = backend_api_factory(self._orchestration_options.target, self._orchestration_options,
                                                self._messages, dry_run=(not self._options.execute))
    ...
    def expand_wildcard_scope(self, includes):
        ...
        log('Expanded --include: "%s" to %i table(s)' % (includes, len(expanded)), verbose)
    ...
    def normalize_backend_structure(self, owner, table_name):
        try:
            ...
            self._messages.log('Exception retrieving details for backend table %s.%s:\n%s'
                               % (owner, table_name, traceback.format_exc()), detail=VERBOSE)
    ...
    def check_valid_rdbms_columns(self, owner, table_name, columns):
        ...
        for column in columns:
            if column.is_hidden():
                self._messages.warning('Hidden column "%s" for RDBMS table "%s.%s" not supported. Skipping table' % (column.name, owner, table_name))
                is_valid = False

            if column.data_default:
                self._messages.warning('Column "%s" for RDBMS table "%s.%s" has default value "%s". Existing column data will not be synchronized' % (column.name, owner, table_name, column.data_default.strip()))

            if not column.is_nullable():
                self._messages.warning('Column "%s" for RDBMS table "%s.%s" is not nullable. Existing column data will not be synchronized' % (column.name, owner, table_name))

            if column.data_type not in offload_source_table.supported_data_types():
                self._messages.warning('Column "%s" has unsupported datatype "%s" for RDBMS table "%s.%s". Skipping table' % (column.name, column.data_type, owner, table_name))
                is_valid = False
    ...
    def compare_table_columns(self, first_table, second_table):
        ...
        assert len(second_table) == 3, 'second_table parameter tuple must contain 3 elements: (owner, table_name, source)'

        f_owner, f_table_name, f_source = first_table
        s_owner, s_table_name, s_source = second_table

        assert f_source in [schema_sync_constants.SOURCE_TYPE_RDBMS, schema_sync_constants.SOURCE_TYPE_BACKEND],\
            'first_table tuple "source" element must be one of: "%s", "%s"'\
            % (schema_sync_constants.SOURCE_TYPE_RDBMS, schema_sync_constants.SOURCE_TYPE_BACKEND)
        assert s_source in [schema_sync_constants.SOURCE_TYPE_RDBMS, schema_sync_constants.SOURCE_TYPE_BACKEND],\
    ...
    def compare_structures(self, include):
        ...
        offload_source_table = OffloadSourceTable.create(rdbms_owner, rdbms_table_name, self._orchestration_options, self._messages)
        backend_table = backend_table_factory(backend_owner, backend_table_name, self._orchestration_options.target,
                                              self._orchestration_options, self._messages,
                                              dry_run=bool(not self._options.execute),
                                              hybrid_owner_override=hybrid_owner, hybrid_view_override=hybrid_view_name)

        if not backend_table.schema_evolution_supported():
            raise SchemaSyncAnalyzerException('%s: %s' % (schema_sync_constants.EXCEPTION_SCHEMA_EVOLUTION_NOT_SUPPORTED,
        ...
        log('')
        log('Comparing RDBMS with backend for %s.%s'
            % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name)))

        #TODO ss@2020-05-20 this check is in place due to GOE-1030
        if rdbms_owner.lower() + rdbms_table_name.lower() != backend_owner.lower().replace(self._orchestration_options.db_name_pattern.lower() % '', '') + backend_table_name.lower():
            source_name = '%s.%s' % (double_quote_sandwich(include['offloaded_owner']),
                                     double_quote_sandwich(include['offloaded_table']))
            self._messages.warning('%s for table %s' % (schema_sync_constants.EXCEPTION_ORACLE_HIVE_NAME_MISMATCH, source_name), ansi_code='red')
        else:
            ...
            #
            delta_base_oracle_cols = self.compare_table_columns((rdbms_owner, rdbms_table_name, schema_sync_constants.SOURCE_TYPE_RDBMS), (backend_owner, backend_table_name, schema_sync_constants.SOURCE_TYPE_BACKEND))

            if delta_base_oracle_cols:
                if self.check_valid_rdbms_columns(rdbms_owner, rdbms_table_name, delta_base_oracle_cols):
                    vector = {'type'              : schema_sync_constants.ADD_BACKEND_COLUMN,
                    ...
                    log('%s.%s columns not aligned with backend %s.%s: %s' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), backend_owner, backend_table_name, get_column_names(delta_base_oracle_cols)), vverbose)
                else:
                    log('%s.%s columns not aligned with backend %s.%s, but invalid columns detected' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), backend_owner, backend_table_name), vverbose)
            else:
                log('%s.%s columns aligned with backend %s.%s, skipping' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), backend_owner, backend_table_name), vverbose)
            ...
            #
            delta_hybrid_view_cols = self.compare_table_columns((rdbms_owner, rdbms_table_name, schema_sync_constants.SOURCE_TYPE_RDBMS), (hybrid_owner, hybrid_view_name, schema_sync_constants.SOURCE_TYPE_RDBMS))
            delta_hybrid_ext_cols = self.compare_table_columns((rdbms_owner, rdbms_table_name, schema_sync_constants.SOURCE_TYPE_RDBMS), (hybrid_owner, hybrid_ext_table_name, schema_sync_constants.SOURCE_TYPE_RDBMS))
            ...
            if delta_hybrid_ext_cols:
                if self.check_valid_rdbms_columns(rdbms_owner, rdbms_table_name, delta_hybrid_ext_cols):
                    ...
                    log('%s.%s columns not aligned with %s.%s: %s' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), double_quote_sandwich(hybrid_owner), double_quote_sandwich(hybrid_ext_table_name), get_column_names(delta_hybrid_ext_cols)), vverbose)
                else:
                    log('%s.%s columns not aligned with %s.%s, but invalid columns detected' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), double_quote_sandwich(hybrid_owner), double_quote_sandwich(hybrid_ext_table_name)), vverbose)
            else:
                log('%s.%s columns aligned with %s.%s, skipping' % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name), double_quote_sandwich(hybrid_owner), double_quote_sandwich(hybrid_ext_table_name)), vverbose)

            if delta_hybrid_view_cols:
                if self.check_valid_rdbms_columns(rdbms_owner, rdbms_table_name, delta_hybrid_view_cols):
                    if ('type' in vector and schema_sync_constants.PRESENT_TABLE not in vector['type']) or 'type' not in vector:
                        vector['type'] = schema_sync_constants.PRESENT_TABLE
                        vector['rdbms_owner'] = rdbms_owner
                        vector['rdbms_table_name'] = rdbms_table_name
                        vector['backend_owner'] = backend_owner
                        vector['backend_table_name'] = backend_table_name
                        vectors.append(vector)
                    log('%s.%s columns not aligned with %s.%s: %s'
                        % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name),
                           double_quote_sandwich(hybrid_owner), double_quote_sandwich(hybrid_view_name),
                           get_column_names(delta_hybrid_view_cols)), vverbose)
                else:
                    log('%s.%s columns not aligned with %s.%s, but invalid columns detected'
                        % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name),
                           double_quote_sandwich(hybrid_owner), double_quote_sandwich(hybrid_view_name)), vverbose)
            else:
                log('%s.%s columns aligned with %s.%s, skipping'
                    % (double_quote_sandwich(rdbms_owner), double_quote_sandwich(rdbms_table_name),
                       double_quote_sandwich(hybrid_owner), double_quote_sandwich(hybrid_view_name)), vverbose)
