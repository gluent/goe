...
from gluentlib.offload.backend_api import BackendApiInterface, BackendApiException, BackendStatsException, UdfDetails,\
...
from gluentlib.offload.column_metadata import is_safe_mapping, valid_column_list, CanonicalColumn, \
...
from gluentlib.offload.offload_constants import DBTYPE_SNOWFLAKE, SNOWFLAKE_BACKEND_CAPABILITIES,\
...
from gluentlib.offload.snowflake.snowflake_column import SnowflakeColumn,\
    SNOWFLAKE_TYPE_BOOLEAN, SNOWFLAKE_TYPE_BINARY,\
    SNOWFLAKE_TYPE_DATE, SNOWFLAKE_TYPE_FLOAT, SNOWFLAKE_TYPE_INTEGER, SNOWFLAKE_TYPE_NUMBER, SNOWFLAKE_TYPE_TEXT,\
...
from gluentlib.util.misc_functions import backtick_sandwich, format_list_for_logging, unsurround
...
class BackendSnowflakeApi(BackendApiInterface):
    def __init__(self, connection_options, backend_type, messages, dry_run=False, no_caching=False,
        ...
        super(BackendSnowflakeApi, self).__init__(connection_options, backend_type, messages, dry_run=dry_run,
        ...
        if not do_not_connect:
            ...
            if connection_options.snowflake_pem_file:
                if connection_options.snowflake_pem_passphrase:
                    self._log('Establishing Snowflake connection with PEM file and passphrase', detail=VVERBOSE)
                else:
                    self._log('Establishing Snowflake connection with PEM file', detail=VVERBOSE)
                snowflake_pass = None
                pem_passphrase = None
                ...
                self._log('Establishing Snowflake connection with password authentication', detail=VVERBOSE)
    ...
    def _format_query_options(query_options=None):
        ...
        return ['ALTER SESSION SET {}={}'.format(k, prep_param_value(v)) for k, v in query_options.items()]
    ...
    def _get_query_profile(self, query_identifier=None):
        ...
        def get_query_history_after_delay(wait_seconds):
            ...
            sql = dedent("""\
                 ...
                 WHERE query_id = ?""" % self.enclose_identifier(self._catalog_name()))
    ...
    def _get_snowflake_ddl(self, db_name, object_name, object_type,
                           as_list=False, terminate_sql=False, for_replace=False):
        assert db_name and object_name
        self._log('Fetching DDL: %s.%s' % (db_name, object_name), detail=VVERBOSE)
        sql = "SELECT get_ddl('%s', '%s')" % (object_type, self.enclose_object_reference(db_name, object_name))
    ...
    def _role_granted_to_user(self, role_name, user_name):
        ...
        assert role_name
        assert user_name
        sql = 'SELECT role_name FROM %s.information_schema.applicable_roles WHERE role_name = ? AND grantee = ?'\
    ...
    def add_columns(self, db_name, table_name, column_tuples, sync=None):
        ...
        for col_name, col_type in column_tuples:
            sql = 'ALTER TABLE %s.%s ADD COLUMN %s %s' % (self.enclose_identifier(db_name),
                                                          self.enclose_identifier(table_name),
                                                          self.enclose_identifier(col_name), col_type)
    ...
    def alter_sort_columns(self, db_name, table_name, sort_column_names, sync=None):
        ...
        if sort_column_names:
            ...
            sql = 'ALTER TABLE %s CLUSTER BY (%s)' % (self.enclose_object_reference(db_name, table_name), sort_csv)
        else:
    ...
    def check_backend_supporting_objects(self, orchestration_options):
        ...
        if self.role_exists(orchestration_options.snowflake_role):
            if self._role_granted_to_user(orchestration_options.snowflake_role, orchestration_options.snowflake_user):
                results.append(
                    {CONNECT_TEST: 'Role',
                     CONNECT_STATUS: True,
    ...
    def create_database(self, db_name, comment=None, properties=None):
        ...
        sql = 'CREATE%s SCHEMA IF NOT EXISTS %s%s%s' % (transient_clause,
    ...
    def create_table(self, db_name, table_name, column_list, partition_column_names, storage_format=None, location=None,
        ...
        %(col_projection)s
        )%(sort_by_clause)s""") % {'db_table': self.enclose_object_reference(db_name, table_name),
    ...
    def create_view(self, db_name, view_name, column_tuples, ansi_joined_tables, filter_clauses=None, sync=None):
        ...
        SELECT %(projection)s
        FROM   %(from_tables)s%(where_clause)s""") % {'db_view': self.enclose_object_reference(db_name, view_name),
    ...
    def detect_column_has_fractional_seconds(self, db_name, table_name, column):
        ...
        if column.data_type not in [SNOWFLAKE_TYPE_TIMESTAMP_NTZ, SNOWFLAKE_TYPE_TIMESTAMP_TZ]:
            return False
        sql = dedent("""\
        SELECT %(col)s
        FROM %(db_table)s
    ...
    def drop_table(self, db_name, table_name, purge=False, if_exists=True, sync=None):
        ...
        drop_sql = 'DROP TABLE %s%s' % (if_exists_clause, self.enclose_object_reference(db_name, table_name))
    ...
    def drop_view(self, db_name, view_name, if_exists=True, sync=None):
        ...
        if_exists_clause = 'IF EXISTS ' if if_exists else ''
        drop_sql = 'DROP VIEW %s%s' % (if_exists_clause, self.enclose_object_reference(db_name, view_name))
        return self.execute_ddl(drop_sql, sync=sync)
    ...
    def enclose_object_reference(self, db_name, object_name):
        ...
        if snowflake_db is None:
            ...
            return '.'.join([self.enclose_identifier(db_name),
                             self.enclose_identifier(object_name)])
        else:
            return '.'.join([self.enclose_identifier(snowflake_db),
                             self.enclose_identifier(db_name),
                             self.enclose_identifier(object_name)])
    ...
    def gen_copy_into_sql_text(self, db_name, table_name, from_object_clause, select_expr_tuples, filter_clauses=None):
        ...
        self._gen_insert_select_sql_assertions(db_name, table_name, None, None, select_expr_tuples,
                                               None, filter_clauses, from_object_clause)
    ...
    def gen_default_numeric_column(self, column_name, data_scale=18):
        ...
        """
        return SnowflakeColumn(column_name, data_type=SNOWFLAKE_TYPE_NUMBER,
    ...
    def get_columns(self, db_name, table_name):
        def get_binary_lengths_from_desc(db_name, table_name):
            ...
            byte_lengths = {}
            for row in [_ for _ in self.execute_query_fetch_all(sql, log_level=VVERBOSE)
        ...
        sql = dedent("""\
            ...
            ORDER BY ordinal_position""" % self.enclose_identifier(self._catalog_name()))
        ...
        for col in col_rows:
            col_name, data_type, is_nullable, char_length, byte_length, num_precision, num_scale, time_scale = col
            ...
            backend_columns.append(SnowflakeColumn(col_name, data_type, data_length=byte_length,
                                                   char_length=char_length, data_precision=num_precision,
                                                   data_scale=data_scale, nullable=is_nullable))
    ...
    def get_session_query_profile(self, session_id, query_id):
        ...
        sql = dedent("""\
             ...
             WHERE query_id = ?""" % (self.enclose_identifier(self._catalog_name()), session_id))
        try:
    ...
    def get_table_ddl(self, db_name, table_name, as_list=False, terminate_sql=False, for_replace=False):
        return self._get_snowflake_ddl(db_name, table_name, 'TABLE', as_list=as_list, terminate_sql=terminate_sql,
    ...
    def get_table_sort_columns(self, db_name, table_name, as_csv=True):
        ...
        if not row or not row[0]:
            return []
        cluster_key_expressions = [unsurround(_.upper().strip(), self.enclosure_character())
    ...
    def get_view_ddl(self, db_name, view_name, as_list=False, terminate_sql=False, for_replace=False):
        try:
            return self._get_snowflake_ddl(db_name, view_name, 'VIEW', as_list=as_list, terminate_sql=terminate_sql,
                                           for_replace=for_replace)
        except snowflake.connector.ProgrammingError as exc:
    ...
    def insert_literal_values(self, db_name, table_name, literal_list, column_list=None, max_rows_per_insert=250,
        ...
        def add_cast(literal, formatted_data_type):
            if any(_ in formatted_data_type for _ in [SNOWFLAKE_TYPE_TEXT, SNOWFLAKE_TYPE_TIMESTAMP_NTZ,
        ...
        while remaining_rows:
            ...
            for row in this_chunk:
                formatted_rows.append(','.join(add_cast(gen_literal(py_val, col.data_type), col.format_data_type())
    ...
    def snowflake_file_format_exists(self, db_name, file_format_name, format_type=None):
        ...
        if row and format_type:
            ...
            if format_type.upper() != row[0].upper():
                raise BackendApiException('File format %s has file type mismatch: %s != %s'
                                          % (self.enclose_object_reference(db_name, file_format_name),
                                             format_type.upper(), row[0].upper()))
        return bool(row)
    ...
    def snowflake_stage_exists(self, db_name, stage_name, url_prefix=None):
        ...
        if row and url_prefix:
            ...
            if not row[0].lower().startswith(url_prefix.lower()):
                raise BackendApiException('Snowflake stage %s has URL mismatch: %s does not start with %s'
                                          % (self.enclose_object_reference(db_name, stage_name),
                                             row[0].lower(), url_prefix.lower()))
        return bool(row)
    ...
    def list_databases(self, db_name_filter=None, case_sensitive=True):
        sql = """SELECT schema_name FROM %s.information_schema.schemata WHERE catalog_name = ?"""\
              % self.enclose_identifier(self._catalog_name())
    ...
    def list_tables(self, db_name, table_name_filter=None, case_sensitive=True):
        sql = """SELECT table_name FROM %s.information_schema.tables WHERE table_catalog = ? AND table_schema = ?"""\
              % self.enclose_identifier(self._catalog_name())
    ...
    def list_udfs(self, db_name, udf_name_filter=None, case_sensitive=True):
        sql = dedent("""\
                ...
                AND   procedure_schema = ?""") % self.enclose_identifier(self._catalog_name())
        query_params = [self._client.database, db_name]
        if udf_name_filter:
    ...
    def list_views(self, db_name, view_name_filter=None, case_sensitive=True):
        sql = """SELECT table_name FROM %s.information_schema.views WHERE table_catalog = ? AND table_schema = ?"""\
              % self.enclose_identifier(self._catalog_name())
        query_params = [self._client.database, db_name]
        if view_name_filter:
            sql += (' AND table_name %s ?' % ('LIKE' if case_sensitive else 'ILIKE'))
    ...
    def rename_table(self, from_db_name, from_table_name, to_db_name, to_table_name, sync=None):
        ...
        rename_sql = 'ALTER TABLE {}.{} RENAME TO {}.{}'.format(self.enclose_identifier(from_db_name),
    ...
    def supported_backend_data_types(self):
        return [SNOWFLAKE_TYPE_BINARY, SNOWFLAKE_TYPE_DATE, SNOWFLAKE_TYPE_FLOAT, SNOWFLAKE_TYPE_NUMBER,
                SNOWFLAKE_TYPE_TEXT, SNOWFLAKE_TYPE_TIME, SNOWFLAKE_TYPE_TIMESTAMP_NTZ, SNOWFLAKE_TYPE_TIMESTAMP_TZ]
    ...
    def table_exists(self, db_name, table_name):
        ...
        sql = "SHOW TERSE TABLES LIKE '%s' IN SCHEMA %s.%s STARTS WITH '%s'"\
              % (table_name, self.enclose_identifier(self._catalog_name()),
    ...
    def udf_details(self, db_name, udf_name):
        ...
        sql = dedent("""\
                ...
                AND   procedure_name = ?""") % self.enclose_identifier(self._catalog_name())
    ...
    def view_exists(self, db_name, view_name):
        ...
        sql = "SHOW TERSE VIEWS LIKE '%s' IN SCHEMA %s.%s STARTS WITH '%s'" \
              % (view_name, self.enclose_identifier(self._catalog_name()),
                 self.enclose_identifier(db_name), view_name)
        try:
            cursor = self.execute_query_get_cursor(sql, log_level=None)
            headers = [_[0] for _ in cursor.description]
            assert headers[1] == 'name'
    ...
    def to_canonical_column(self, column):
        ...
        def new_column(col, data_type, data_precision=None, data_scale=None, safe_mapping=None):
            ...
            return CanonicalColumn(name=col.name, data_type=data_type, data_length=col.data_length,
                                   ...
                                   char_length=col.char_length, char_semantics=CANONICAL_CHAR_SEMANTICS_CHAR)
    ...
    def from_canonical_column(self, column, decimal_padding_digits=0):
        ...
        def new_column(col, data_type, data_length=None, data_precision=None, data_scale=None, char_length=None,
                       safe_mapping=None):
            safe_mapping = is_safe_mapping(col.safe_mapping, safe_mapping)
            return SnowflakeColumn(col.name, data_type=data_type, data_length=data_length, char_length=char_length,
                                   data_precision=data_precision, data_scale=data_scale, nullable=col.nullable,
                                   data_default=col.data_default, safe_mapping=safe_mapping)
